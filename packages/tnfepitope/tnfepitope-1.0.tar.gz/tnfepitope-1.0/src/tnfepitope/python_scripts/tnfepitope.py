######################################################################################
# TNFepitope is developed for predicting, desigining and scanning of TNF inducing epitopes #
# It is developed by Prof G. P. S. Raghava's group.                #
# Please cite: https://webs.iiitd.edu.in/raghava/tnfepitope/                        #
######################################################################################
import argparse
import warnings
import subprocess
...
def BLAST_processor(blast_result,name1,ml_results,thresh):
    if os.stat(blast_result).st_size != 0:
        ...
        df6['Prediction'] = ['TNF-inducer' if df6['Total_Score'][i]>thresh else 'TNF non-inducer' for i in range(0,len(df6))]
    else:
        df__2 = name1
        df3 = ml_results
        df2 = pd.DataFrame()
...
def main():
    ...
    parser.add_argument("-s", "--source",type=int, choices = [1,2], help="Source: 1:Human, 2:Mouse, by default 1")
    parser.add_argument("-j", "--job",type=int, choices = [1,2,3], help="Job Type: 1:Predict, 2: Design, 3:Scan, by default 1")
    parser.add_argument("-t","--threshold", type=float, help="Threshold: Value between 0 to 1 by default 0.45 for human, and 0.50 for Mouse")
    parser.add_argument("-w","--winleng", type=int, choices =range(9, 21), help="Window Length: 9 to 20 (scan mode only), by default 9")
    parser.add_argument("-d","--display", type=int, choices = [1,2], help="Display: 1:TNF-inducer only, 2: All peptides, by default 1")
    args = parser.parse_args()

    ('############################################################################################')
    print('# This program TNFepitope is developed for predicting, desigining and scanning          #')
    print('# TNF-inducing peptides, developed by Prof G. P. S. Raghava group.               #')
    print('# Please cite: TNFepitope; available at https://webs.iiitd.edu.in/raghava/tnfepitope/  #')
    print('############################################################################################')
    ...
    blastdb1 = nf_path + "/../blast_db/humandb"
    ...
    if Job==2:
        ...
        print('Input File: ',Sequence,'; Threshold: ', Threshold,'; Job Type: ',Job,';Host: ',host)
        ...
        print('Output File: ',result_filename,'; Display: ',dplay,';Host: ',host)
        print('# ############################################################################')
    #========================================Extracting Model====================================
    if os.path.isdir(nf_path+'/../model') == False:
        with zipfile.ZipFile(nf_path+ '/../model/model.zip', 'r') as zip_ref:
    ...
    if Job == 1:
        print('\n======= Thanks for using Predict module of TNFepitope. Your results will be stored in file :',result_filename,' =====\n')
        df_2,dfseq = readseq(Sequence)
        df1 = lenchk(dfseq)
        X = feature_gen(df1)
        if sorce == 1:
            mlres = model_run(X,nf_path+'/../model/main_human_model.pkl')
        else:
            mlres = model_run(X,nf_path+'/../model/alt_mouse_model.pkl')
        filename = str(uuid.uuid4())
        df11 = pd.concat([df_2,df1],axis=1)
        df11.to_csv(filename,index=None,header=False,sep="\n")
        mlres = mlres.round(3)
        if sorce == 1:
            os.system(blastp_ + " -task blastp-short -db " + blastdb1 + " -query " + filename  + " -out RES_1_6_6.out -outfmt 6 -evalue 1")
        else:
            os.system(blastp + " -task blastp-short -db " + blastdb2 + " -query " + filename  + " -out RES_1_6_6.out -outfmt 6 -evalue 0.1")
        ...
        if dplay == 1:
            df44 = df44.loc[df44.Prediction=="TNF-inducer"]
        else:
            df44 = df44
        df44 = round(df44,3)
    ...
    #===================== Design Model Start from Here ======================
        ...
        print('\n======= Thanks for using Design module of TNFepitope. Your results will be stored in file :',result_filename,' =====\n')
        ...
        mlres = mlres.round(3)
        if sorce == 1:
            os.system(blastp + " -task blastp-short -db " + blastdb1 + " -query " + filename  + " -out RES_1_6_6.out -outfmt 6 -evalue 1")
        else:
            os.system(blastp + " -task blastp-short -db " + blastdb2 + " -query " + filename  + " -out RES_1_6_6.out -outfmt 6 -evalue 0.1")
    ...
    #=============== Scan Model start from here ==================
        ...
        print('\n======= Thanks for using Scan module of TNFepitope. Your results will be stored in file :',result_filename,' =====\n')
        ...
        os.remove(filename)
        print("\n=========Process Completed. Have an awesome day ahead.=============\n")
    print('\n======= Thanks for using TNFepitope. Your results are stored in file :',result_filename,' =====\n\n')
