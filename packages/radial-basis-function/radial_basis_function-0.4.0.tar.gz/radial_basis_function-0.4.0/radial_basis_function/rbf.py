...
from .funcoes_auxiliares import check_Matrix_X
...
class RadialBasisFunction:
    ...
    def __init__(
        ...
        bias=1,
        Qtd_Polos=0,
        Polos_iniciais_fixos=False,
        Polos_Otimizados=None,
    ):
        self.funcao = funcao
        self.bias = bias
    ...
    def definir_numero_de_polos(self):
        ...
        if self.n_feature_in_ <= 2:
            ...
            if aux_polos_desejados > self.n_feature_in_ * 8:
                ...
                aux_calculo = (
                    aux_polos_desejados + self.n_feature_in_ * 8
                ) / 9
                self.numero_polos_definidos = int(round(aux_calculo))
    ...
    def Gerar_Polos(self, variaveis):
        ...
        if self.C is None:
            C = np.zeros(
                (self.numero_polos_definidos, self.n_feature_in_), dtype=float
            )
            ...
            C = np.array(C)
            dist_entre_os_polos = np.zeros(
                (self.numero_polos_definidos, self.numero_polos_definidos)
            )
            ...
            if self.Polos_iniciais_fixos:
                # TODO: Testar Mediana(Q2), Moda
                # TODO: Quartil 25% (Q1) e 75%(Q3), 1 e 99 Percentiu
                # TODO: Testar Variancia, Coeficiente de Variação
                # TODO: Amplitude, Intervalo Interquartil
                C[0] = variaveis.mean()
                C[1] = variaveis.std()
    ...
    def FuncoesBase(self, X, C, t, n_PoloAtual, N_TotalPolos):
        ...
        if self.funcao == 'Gaussiana':
            ...
            calculo = np.sqrt(2) * np.sin(
                -Gama * 2 * np.pi * t * (Radial**2) * n_PoloAtual
            )
        return calculo
    ...
    def Radial(self, X):
        ...
        for n in range(0, self.n_linhas_in_, 1):
            ...
            for i in range(0, self.numero_polos_definidos, 1):
                # Hidden Layer
                R[n, i] = self.FuncoesBase(
                    variaveis[n], self.C[i], n, (i + 1), len(self.C)
                )
        R = pd.DataFrame(R)
    ...
    def fit(self, X, Matriz_Y):
        feature_summary = check_Matrix_X(X)
        self.feature_names_in_ = feature_summary[0]
        self.n_feature_in_ = feature_summary[1]
        self.n_linhas_in_ = feature_summary[2]
    ...
    def predict(self, X):
        ...
        for i in range(0, len(variaveis_X), 1):
            ...
            for j in range(0, len(W) - 1, 1):
                ...
                Predicao_Y[i] = Predicao_Y[i] + W[j] * self.FuncoesBase(
                    variaveis_X[i], self.C[j], i, (j + 1), len(self.C)
                )
