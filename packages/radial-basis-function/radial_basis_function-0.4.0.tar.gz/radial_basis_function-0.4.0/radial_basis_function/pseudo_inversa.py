"""
Classe e métodos
para implementar a Regressão por multiplicação de Matriz Pseudo-Inversa.
    ...
    def fit(self, X, y):
        ...
        try:
            ...
            if 'Singular matrix' in str(err):
                Matriz_Regulada = Matriz_Quadrada + np.identity(
                    len(Matriz_Quadrada)
                )
                Matriz_Inversa = np.linalg.inv(Matriz_Regulada)
        # Inversa * Base_Dados_T
        Matriz_Pseudo_Inversa = pd.DataFrame(
            np.dot(Matriz_Inversa, Matriz_X.T)
        )
        # Isola o peso final
