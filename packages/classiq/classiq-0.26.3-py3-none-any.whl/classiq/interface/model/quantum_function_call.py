...
from classiq.interface.generator.quantum_function_call import (
    ...
    WireNamesOrInoutPortBinding,
    _generate_single_io_err,
    get_non_zero_wires,
    randomize_suffix,
    split_wire_names_or_inout_slice,
)
...
class QuantumFunctionCall(FunctionCall, Statement):
    ...
    @property
    def operand_lambda_definitions_dict(
        self,
    ) -> Dict[str, List["QuantumLambdaFunction"]]:
        return {
            name: get_lambda_defs(operand) for name, operand in self.operands.items()
        }
...
def get_lambda_defs(operand: LambdaFunctionList) -> List[QuantumLambdaFunction]:
