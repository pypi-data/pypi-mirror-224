...
def is_standard_cmain(model: SynthesisModel) -> bool:
    try:
...
def extract_sample_params(model: SynthesisModel) -> Dict[str, float]:
    classical_main = _get_classical_main(model)
...
def has_classical_main(model: SynthesisModel) -> bool:
    return CLASSICAL_ENTRY_FUNCTION_NAME in model.classical_function_dict


def _get_classical_main(model: SynthesisModel) -> ClassicalFunctionDefinition:
