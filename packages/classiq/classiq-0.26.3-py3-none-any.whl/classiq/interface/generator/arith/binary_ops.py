...
class BinaryOpParams(
    ...
    def _set_left_arg_name(cls, left_arg: LeftDataT) -> LeftDataT:
        if isinstance(left_arg, RegisterUserInput):
            return left_arg.revalued(name=cls.left_arg_name)  # type: ignore[return-value]
        return left_arg
    ...
    def _set_right_arg_name(cls, right_arg: RightDataT) -> RightDataT:
        if isinstance(right_arg, RegisterUserInput):
            return right_arg.revalued(name=cls.right_arg_name)  # type: ignore[return-value]
        return right_arg
...
class EffectiveUnaryOpParams(
    InplacableBinaryOpParams[RegisterUserInput, RightDataT], Generic[RightDataT]
):
    left_arg_name = "arg"
...
class LShift(EffectiveUnaryOpParams[pydantic.NonNegativeInt]):
    ...
    def _validate_legal_modulo(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(arg, RegisterUserInput):
            raise ValueError("left arg must be a RegisterUserInput")
...
class RShift(EffectiveUnaryOpParams[pydantic.NonNegativeInt]):
    ...
    @staticmethod
    def _shifted_fraction_places(*, arg: RegisterUserInput, shift: int) -> int:
...
class Modulo(EffectiveUnaryOpParams[int]):
    ...
    def _validate_left_arg_is_integer(
        cls, left_arg: RegisterUserInput
    ) -> RegisterUserInput:
