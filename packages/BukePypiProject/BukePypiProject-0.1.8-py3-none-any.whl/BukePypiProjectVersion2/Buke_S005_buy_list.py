...
from BukePypiProjectVersion2.indicator import sma, ibs, pivot_standard, 순거래대금, pdi
from BukePypiProjectVersion2.function import rank, ts_max
...
def read_index_day_price():
    ...
    df = pd.read_csv(f"data_file/Kosdaq_Price.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
    return df
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['거래대금_최댓값_5'] = ts_max(group['거래대금'], 5)
        ...
        group['고가_최댓값_10'] = ts_max(group['고가'], 10)
    ...
    grouped = day_price.groupby('종목코드')
    for key, group in grouped:
        increase_condition1 = group['ibs'] < 0.5
        increase_condition2 = group['종가'] / group['종가_이평선_20'] > 1.05
        increase_condition3 = group['pdi_이평선_10'] > 0.3
        increase_condition4 = group['종가'] / group['종가_이평선_20'] < 1.4
        increase_condition5 = group['거래대금'] < group['거래대금_최댓값_5']
        increase_condition6 = group['고가'] < group['고가_최댓값_10']
        increase_condition7 = group['기관순거래대금_이평선_10'] > group['기관순거래대금_이평선_10'].shift(10)
        ...
        group['#priority'] = -1 * group['종가'] / group['종가_이평선_3']

        group = group.set_index('날짜')
        day_price_dict[key] = group

    index_day_price['이평선_2'] = sma(index_day_price['종가'], 2)
    index_day_price['이평선_4'] = sma(index_day_price['종가'], 4)
    index_day_price['이평선_6'] = sma(index_day_price['종가'], 6)
    # index_day_price['pct_change_21'] = pct_change(index_day_price['close'], 21)
    index_day_price['#market_timing'] = (index_day_price['종가'] > index_day_price['이평선_2']) | \
                                        (index_day_price['종가'] > index_day_price['이평선_4']) | \
                                        (index_day_price['종가'] > index_day_price['이평선_6'])
    ...
    for priority_value, ticker in pre_buy_list:
        ...
        try:
            ...
            if ticker in kospi_ticker_list:
                buy_price1 = get_ask_price(yesterday_low * 0.975, Market.kospi)

            else:
                buy_price1 = get_ask_price(yesterday_low * 0.975, Market.kosdaq)
