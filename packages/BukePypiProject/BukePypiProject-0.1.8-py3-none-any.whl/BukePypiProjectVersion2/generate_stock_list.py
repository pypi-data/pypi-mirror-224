...
def get_day_price(end_date):
    ...
    day_price = pd.read_csv(f"data_file/수급데이터_2022_to_{end_date.year}.csv")

    return day_price


def generate_stock_list(day):

    day_price = get_day_price(day)
    ...
    df.to_csv(f"data_file/코스피_종목리스트_2022_to_{day.year}.csv")

    df = pd.DataFrame(kosdaq_date_groupby_df)
    df.to_csv(f"data_file/코스닥_종목리스트_2022_to_{day.year}.csv")
