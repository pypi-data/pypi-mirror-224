...
from BukePypiProjectVersion2.indicator import sma, ibs, pivot_standard, 순거래대금
...
def read_trading_days(end_date: date):
...
def read_day_price(end_date: date):
...
def read_index_history(end_date: date):
...
def Buke_S001_buy_list(day):

    path = f"trade_list/{day}"

    # 1. 거래일 가져오기
    trading_days_list = read_trading_days(day)

    # 2. 일봉 가져오기
    day_price = read_day_price(day)
    index_day_price = read_index_day_price()

    # 3. 지수 종목 구성 내역 가져오기
    kospi_history, kosdaq_history = read_index_history(day)
