...
from BukePypiProjectVersion2.indicator import sma, ibs, pivot_standard, 순거래대금, pivot_standard_for_sellprice
from BukePypiProjectVersion2.function import rank, ts_min
from BukePypiProjectVersion2.parameter import Market
...
def read_day_price(start_date: date, end_date: date):
    df = pd.read_csv(f"data_file/수급데이터_{start_date.year}_to_{end_date.year}.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
...
def read_index_day_price(start_date: date, end_date: date):
    # df = pd.read_csv(f"data/Kosdaq_Price.csv", index_col=0)
    df = pd.read_csv(f"data_file/VIX_지수.csv", index_col=0)
    # df = pd.read_csv(f"data/MSCI_신흥국_ETF.csv", index_col=0)
    # df = pd.read_csv(f"data/미국_5년_BEI.csv", index_col=0)
    # df = pd.read_csv(f"data/미국_NDF_달러_원_환율.csv", index_col=0)
    # df = pd.read_csv(f"data/미국_국채선물_10년_2년_금리차.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
    return df


def read_index_history(start_date: date, end_date: date):
    ks_history = pd.read_csv(f"data_file/코스피_종목리스트_{start_date.year}_to_{end_date.year}.csv", index_col=0)
    ks_history['날짜'] = ks_history['날짜'].apply(lambda day: str_to_date(day))
    ks_history = ks_history.set_index('날짜')
    kq_history = pd.read_csv(f"data_file/코스닥_종목리스트_{start_date.year}_to_{end_date.year}.csv")
...
def add_indicator(day_price: DataFrame, index_day_price: DataFrame) -> DataFrame:
    ...
    for key, group in grouped:
        ...
        group['외국인순거래대금_이평선_10'] = sma(group['외국인순거래대금'], 10)
        group['순거래대금합_이평선_10'] = group['기관순거래대금_이평선_10'] + group['외국인순거래대금_이평선_10']

        group['거래대금_최솟값_3'] = ts_min(group['거래대금'], 3)
    ...
    for key, group in grouped:
        ...
        increase_condition1 = group['ibs'] < 0.8
        increase_condition2 = group['순거래대금합_이평선_10'] > 0
        increase_condition3 = group['거래대금_최솟값_3'] == group['거래대금']
        increase_condition4 = group['종가'] / group['종가_최솟값_10'] > 1.1
        increase_condition5 = group['종가'] / group['종가_최솟값_10'] < 1.5
        ...
        liquidity_condition1 = group['순거래대금_이평선_5_순위'] > 0.5
        liquidity_condition2 = group['시가총액_순위'] > 0.3
        liquidity_condition3 = group['거래대금_이평선_5'] > 1000000000.
        ...
        group['#priority'] = group['종가_이평선_5'] / group['종가_이평선_20']
    ...
    # Market Timing
    index_day_price['종가비율'] = index_day_price['종가'] / index_day_price['종가'].shift(1)
    index_day_price['종가비율_이평선_3'] = sma(index_day_price['종가비율'], 3)
    index_day_price['종가비율_이평선_5'] = sma(index_day_price['종가비율'], 5)
    index_day_price['종가비율_이평선_10'] = sma(index_day_price['종가비율'], 10)

    index_day_price['#market_timing'] = (index_day_price['종가비율'] > index_day_price['종가비율_이평선_3']) | \
                                        (index_day_price['종가비율'] > index_day_price['종가비율_이평선_5']) | \
                                        (index_day_price['종가비율'] > index_day_price['종가비율_이평선_10'])
    index_day_price = index_day_price.set_index('날짜')
