from typing import List, Set, Union, Type, Any
...
class EntityList(List[T]):
    ...
    def __init__(
            self,
            initial_entities: Union[T, List[T], Set[T]] = None
        ):
        super().__init__()
    ...
    def filter_by_attr(self, attr: str, value: Any) -> 'EntityList[T]':
        ...
        """
        return EntityList([x for x in self if hasattr(x, attr) and getattr(x, attr) == value])
