...
class Model:
    ...
    def assign_spconstraint(self, xlim=[], ylim=[], zlim=[], dofs=[]):
        ...
        self.SpConstraints.append([xlim, ylim, zlim, dofs])
    ...
    def assign_equaldof(self, xyzr, xclim=[], yclim=[], zclim=[], dofs=[]):
        ...
        self.EqualDofs.append([xyzr, xclim, yclim, zclim, dofs])
    ...
    def __assignspconstraint_model(self, print_command):
        ...
        for spconsts in self.SpConstraints:
            ...
            file_fix = open(_name + "\\" + "file_fix.txt", "a")
            node_s = find_nodes_dict(self.getNodes(), xlim, ylim, zlim)
            if node_s is False:
                return
...
def __plotdeformed(anname, resp, name_model, modenumber, direction='mag', dir_name='', parts=[], scale=10, lineelements='y',
    ...
    file_re = name_model + "\\output_" + anname + ".pkl"
    fresps = open(file_re, 'rb')
    resps = dill.load(fresps)
    db = resps[resp]
    data_ = db.loc[modenumber]
...
def plot_modeshape(name_model, modenumber, direction='mag', dir_name='', parts=[], scale=10, lineelements='y',
                   quadtrielements='y', brickelements='y', linewidth=0.7, edgelinewidth=0.3, xlim=[], ylim=[], zlim=[],
                   alpha=0.70, colormap="rainbow", numseg_line=2, numseg_surf=2):
    __plotdeformed('eigen', 'eigen', name_model, modenumber, direction=direction, dir_name=dir_name, parts=parts, scale=scale, lineelements=lineelements,
...
def eigen(ops, num_Modes, solver='-genBandArpack'):
    ...
    otputs['eigen'] = pd.DataFrame(columns=nodes_)
    for i in range(num_Modes):
        vectors = []
        for nod in ops.getNodeTags():
        ...
        otputs['eigen'].loc[i + 1] = vectors

    file_re = _name + "\\output_" + 'eigen' + ".pkl"
    fres = open(file_re, 'wb')
    dill.dump(otputs, fres)
    fres.close()
...
def analyze_push_cyclic(ops, analysis_option, filename_protocole, cnodeTag, cdof, Dy, du_min, du_max, numIter = 10, duDiv=2):
    print('#################################')
    print('### Push Over Cyclic Analysis ###')
    print('#################################')
...
def analyze_static(ops,name, analysis_option, resp_nodes=['Disp', 'Reaction'], resp_elements=['force', 'stresses', 'strains'],
                   num_steps=10, loadConst='yes', time=0.0):
    print('##########################')
    ...
    global _ndf
    global _parts
    # logfilename = name_model + '\\' + 'opslogfile.txt'
    # ops.logFile(logfilename, '-noEcho')
    ...
    resp_node_ID = {'Disp': 1, 'Vel': 2, 'Accel': 3, 'IncrDisp': 4, 'IncrDeltaDisp': 5, 'Reaction': 6, 'Unbalance': 7,
                    'RayleighForces': 8}
    otputs['Disp'] = pd.DataFrame(columns=nodes_)

    for re_n in resp_nodes:
        otputs[re_n] = pd.DataFrame(columns=nodes_)

    for re_n in resp_elements:
        otputs[re_n] = pd.DataFrame(columns=elements_)
    ...
    for step in range(num_steps):
        ...
        if ok != 0:
            ...
            file_re = _name + "\\output_" + name + ".pkl"
            fres = open(file_re, 'wb')
            dill.dump(otputs, fres)
            fres.close()
            exit()
            ...
            for re_n in resp_elements:
                otputs[re_n].loc[step + 1] = [ops.eleResponse(n, re_n) for n in elements_]
    ...
    print('Analysis successful for ' + name)
...
def analyze_push_mono(ops, name, analysis_option, TargetDisp, cnodeTag, cdof, du_min, du_max,
                 resp_nodes=['Disp', 'Reaction'], resp_elements=['force', 'stresses', 'strains'], numIter=10, duDiv=2):
    ...
    while round(abs(node_location - TargetDisp), 7) > 0:
        ...
        if ok != 0:
            ...
            for re_n in resp_elements:
                otputs[re_n].loc[Nstep] = [ops.eleResponse(n, re_n) for n in elements_]
