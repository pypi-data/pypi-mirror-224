...
@logwrap
def loop_find(query, timeout=ST.FIND_TIMEOUT, threshold=None, interval=0.5, intervalfunc=None,platform='android'):
    """
    ...
    while True:
        if platform == 'qnx':
            bmp2 = G.BASEDIR[0] + os.sep + 'bmp2'
            G.DEVICE.cmd(f'pull /data/nfs/nfs_log/qnx_log/QNX.bmp {bmp2}')
            screen = aircv.imread(bmp2)
        else:
