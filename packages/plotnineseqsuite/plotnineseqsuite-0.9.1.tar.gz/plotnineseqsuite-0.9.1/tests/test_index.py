...
def test_plot_an_aligned_sequences():
...
def test_plot_a_sequence_logo():
    ggplot() + geom_logo(seqs_dna['MA0001.1']) + theme_seq()


def test_accepted_input_formats():
...
def test_multiple_grouped_sequences():
...
def test_fonts():
    ...
    for i in range(0, 15, 3):
        g1 = pw.load_ggplot(
            ggplot() + geom_logo(data=seqs_dna['MA0001.1'], font=fonts[i]) + theme_seq() + ggtitle(fonts[i]))
        g2 = pw.load_ggplot(
            ggplot() + geom_logo(data=seqs_dna['MA0001.1'], font=fonts[i + 1]) + theme_seq() + ggtitle(fonts[i + 1]))
        g3 = pw.load_ggplot(
            ggplot() + geom_logo(data=seqs_dna['MA0001.1'], font=fonts[i + 2]) + theme_seq() + ggtitle(fonts[i + 2]))
        if i == 0:
            allgg = g1 | g2 | g3
        else:
            temp = g1 | g2 | g3
...
def test_modify_the_starting_position():
    names = ['seq-a', 'seq-b', 'seq-c', 'seq-d', 'seq-e', 'seq-f']
    seqs = geom_alignedSeq(seqs_dna['MA0013.1'], seq_names=names)
