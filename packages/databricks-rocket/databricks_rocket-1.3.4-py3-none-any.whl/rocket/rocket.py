...
setuptools.setup(
    ...
    def launch(
        ...
        return execute_shell_command(
            f'rocket launch --project_location={project_location} --watch={watch} --dbfs_path={dbfs_path} _deploy=False')

    def _build_and_deploy(self, watch, modified_files=None):
        self._build()
        self._deploy(watch=watch, modified_files=modified_files)

    def _deploy(self, watch, modified_files=None):
        """
        ...
        if os.path.exists(f"{self.project_location}/pyproject.toml"):
