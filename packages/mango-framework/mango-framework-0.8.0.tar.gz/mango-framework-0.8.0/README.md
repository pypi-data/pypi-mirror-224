...
7. Render the HTML to the user as a template with Shake (Legacy method will be removed in future versions.):
...
def form(form_data):
    ...
    return f"Hello, {name}!"
```
9. Get file upload (only takes single file uploads, with no other form data.):
