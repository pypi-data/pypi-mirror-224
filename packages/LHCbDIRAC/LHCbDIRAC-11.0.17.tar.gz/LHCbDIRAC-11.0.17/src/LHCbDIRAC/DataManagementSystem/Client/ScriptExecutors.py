        ...
        if bkQuery:
            ...
            if os.path.exists(bkFile):
                ...
                if "*" in processingPass:
                    ...
                    progressBar = ProgressBar(1, title=f"Getting list of processing passes (depth={depth})")
                    processingPasses = getProcessingPasses(bkQuery, depth=depth)
                    progressBar.endLoop(message=f"obtained {len(processingPasses)} processing passes")
                    if len(processingPasses) > 1:
                        gLogger.notice("\n".join(processingPasses))
                    bkQuery.setProcessingPass(None)
                    bkQueries = [
                        BKQuery(bkQuery.setProcessingPass(processingPass)) for processingPass in processingPasses
                    ]
                else:
                    bkQueries = [bkQuery]
            ...
            progressBar = ProgressBar(
                nbQueries,
                title=("Executing %d BK queries:" % nbQueries) if nbQueries > 1 else "Executing BK query",
                step=1,
            )
            for bkQuery in bkQueries:
                progressBar.loop()
                lfnList += bkQuery.getLFNs(printOutput=progressBar is None and printOutput)
            progressBar.endLoop(message=f"Got {len(lfnList)} LFNs")
...
def removeReplicas(lfnList, seList, minReplicas=1, checkFC=True, allDisk=False, force=False, verbose=True):
    ...
    if fullyRemoved or allDisk:
        ...
        for lfns in [
            lfns
            for reason, siteLFNs in errorReasons.items()  # can be an iterator
            for lfns in siteLFNs.values()
            if reason == "Only ARCHIVE replicas"
        ]:
    ...
    if verbose:
        ...
        for reason, seDict in errorReasons.items():  # can be an iterator
            for se, lfns in seDict.items():  # can be an iterator
                gLogger.error(f"Failed to remove {len(lfns)} replicas from {se} with reason: {reason}")
        if not successfullyRemoved and not errorReasons and not checkFC:
            gLogger.notice(f"Replicas were found at no SE in {str(seList)}")
...
def removeReplicasWithFC(lfnList, seList, minReplicas=1, allDisk=False, force=False):
        ...
        for lfn in res["Value"]["Successful"]:
            ...
            if not seList & existingReps:
                if allDisk:
                    ...
                    reason = f"No replicas at requested SEs ({len(existingReps)} existing)"
                ...
                if force and not existingReps - seList:
                    ...
                    errorReasons.setdefault(
                        "No replicas to remove (%d existing/%d requested)" % (len(existingReps), minReplicas), {}
                    ).setdefault(seString, []).append(lfn)
            else:
                ...
                if remaining < minReplicas:
                    ...
                    errorReasons.setdefault(
                        "Not all replicas could be removed in order to keep at least %d" % minReplicas, {}
                    ).setdefault(seString, []).append(lfn)
    ...
    if notExisting:
        ...
        if not res["OK"]:
            gLogger.error(f"Error getting replicas of {len(notExisting)} non-existing files", res["Message"])
...
def removeReplicasNoFC(lfnList, seList):
    ...
    ##################################
    gLogger.notice(f"Removing {len(lfnList)} physical replica from {str(seList)}, for replicas not in the FC")
    ...
    for reason, lfns in notInBK.items():  # can be an iterator
        gLogger.notice(f"Failed to remove replica flag in BK for {len(lfns)} files with error: {reason}")

    inFC = {}
    for seName in seList:
        se = StorageElement(seName)
        progressBar = ProgressBar(len(lfnList), title=f"Checking and removing files from {seName}", chunk=chunkSize)
        for lfnChunk in breakListIntoChunks(lfnList, chunkSize):
            progressBar.loop()
            ...
            lfns += [
                lfn for lfn, reason in res["Value"]["Failed"].items() if "SRM_FILE_BUSY" in reason
            ]  # can be an iterator
            if not lfns:
                continue
            gLogger.setLevel("FATAL")
    ...
    if inFC:
        for se, lfns in inFC.items():  # can be an iterator
            gLogger.notice(f"{len(lfns)} files have replica in FC at {se}, not removed")
...
def executeAccessURL(dmScript):
    ...
    if not lfnList:
        ...
        results = getAccessURL(
            lfnList, seList, protocol=protocol, active=active, diskOnly=diskOnly, preferDisk=preferDisk, forJobs=forJobs
        )
...
def getAccessURL(lfnList, seList, protocol=None, active=True, diskOnly=False, preferDisk=False, forJobs=False):
    ...
    if isinstance(seList, str):
        seList = seList.split(",")
    if not seList:
        seList = sorted({se for lfn in lfnList for se in replicas.get(lfn, {})})
        if len(seList) > 1:
            gLogger.notice(f"Using the following list of SEs: {str(seList)}")
    bk = BookkeepingClient()
    ...
    mdfFiles = {lfn for lfn, fileType in bkRes.get("Value", {}).items() if fileType == "MDF"}  # can be an iterator
    for se in seList:
...
def removeFiles(lfnList, setProcessed=False):
    ...
    progressBar = ProgressBar(len(lfnList), title=f"Removing {len(lfnList)} files", chunk=chunkSize)
    for lfnChunk in breakListIntoChunks(lfnList, chunkSize):
        progressBar.loop()
        # gLogger.setLevel( 'FATAL' )
    ...
    if notExisting:
        ...
        progressBar = ProgressBar(
            len(notExisting), title=f"Removing {len(notExisting)} non-existing files from FC ", chunk=chunkSize
        )
        notExistingRemoved = []
        for lfnChunk in breakListIntoChunks(notExisting, chunkSize):
            ...
            if not res["OK"]:
                gLogger.error(f"\nError getting replicas of {len(lfnChunk)} non-existing files", res["Message"])
                ...
                if lfnChunk:
                    ...
                    if not res["OK"]:
                        gLogger.error(
                            f"\nError removing {len(lfnChunk)} non-existing files from the FC", res["Message"]
                        )
                        errorReasons.setdefault(str(res["Message"]), []).extend(lfnChunk)
                    else:
                        for lfn, reason in res["Value"]["Failed"].items():  # can be an iterator
        ...
        if notExistingRemoved:
            ...
            gLogger.notice(f"Removed from FC {len(notExistingRemoved)} non-existing files")

    if successfullyRemoved:
        gLogger.notice(f"Successfully removed {len(successfullyRemoved)} files")
    maxLfns = 20
    for reason, lfns in errorReasons.items():  # can be an iterator
        nbLfns = len(lfns)
        gLogger.notice(
            "Failed to remove %d files with error: %s%s"
            % (nbLfns, reason, " (first %d)" % maxLfns if nbLfns > maxLfns else "")
        )
        gLogger.notice("\n".join(lfns[:maxLfns]))
    return 0
...
def removeFilesInTransformations(lfns, setProcessed=False):
    ...
    if not res["OK"]:
        ...
        if setProcessed:
            ...
            if ignoredFiles:
                for transID, lfns in ignoredFiles.items():  # can be an iterator
                    gLogger.notice(
                        "%d files in status Processed in transformation %d: status unchanged" % (len(lfns), transID)
                    )
        ...
        for transID, lfns in lfnsToSet.items():  # can be an iterator
            ...
            if not res["OK"]:
                gLogger.error(f"Error setting {len(lfns)} files to Removed", res["Message"])
            else:
...
def printLfnReplicas(lfnList, active=True, diskOnly=False, preferDisk=False, forJobs=False):
    ...
    if res["OK"] and not active:
        replicas = res["Value"]["Successful"]
        seSet = {se for ses in replicas.values() for se in ses}
        seStatus = {se: {True: "Active", False: "Banned"}[StorageElement(se).status()["Read"]] for se in seSet}
        ...
        for lfn in sorted(replicas):
            ...
            for se in sorted(replicas[lfn]):
                ...
                if not res["OK"]:
                    ...
                    key = f"{se} ({seStatus[se]})"
                    value["Successful"][lfn][key] = f"{replicas[lfn][se]} ({res['Value']['Successful'][lfn]})"
        res = S_OK(value)
    return printDMResult(
        res,
        empty=f"No {'active disk ' if diskOnly else 'allowed ' if active else ''}replica found",
...
def printPfnMetadata(lfnList, seList, check=False, exists=False, summary=False):
    ...
    chunkSize = 20
    progressBar = ProgressBar(len(lfnList), title=f"Getting replicas for {len(lfnList)} files", chunk=chunkSize)
    for lfnChunk in breakListIntoChunks(lfnList, chunkSize):
        progressBar.loop()
        res = fc.getReplicas(lfnChunk, allStatus=True)
        if not res["OK"]:
            gLogger.fatal(f"Error getting replicas for {len(lfnChunk)} files", res["Message"])
            return 2
        else:
    ...
    for lfn in sorted(replicas):
        if seList and not [se for se in replicas[lfn] if se in seList]:
            metadata["Failed"][lfn] = {"FC": f"No such file at {' '.join(seList)}"}
            replicas.pop(lfn)
            lfnList.remove(lfn)
    metadata["Failed"].update(
        dict.fromkeys(
            (url for url in lfnList if url not in replicas and url not in metadata["Failed"]),
            {"FC": "No active replicas"},
        )
    )
    if not seList:
        # take all seList in replicas and add a fake '' to printout the SE name
        seList = [""] + sorted({se for lfn in replicas for se in replicas[lfn]})
    if replicas:
        if check:
            ...
            for fileChunk in breakListIntoChunks(fileList, chunkSize):
                ...
                if res["OK"]:
                    ...
                    for url in seMetadata["Successful"]:
                        ...
                        if isinstance(pfnMetadata.get("Mode"), int):
                            pfnMetadata["Mode"] = f"{pfnMetadata['Mode']:o}"
                        metadata["Successful"].setdefault(url, {})[se] = (
                            pfnMetadata
                            if not exists
                            else {
                                "Exists": "True (%sCached%s)"
                                % (
...
def executeReplicaStats(dmScript):
    ...
    return printReplicaStats(
        directories,
        lfnList,
        getSize=getSize,
        prNoReplicas=prNoReplicas,
        prWithReplicas=prWithReplicas,
        prWithArchives=prWithArchives,
        prFailover=prFailover,
        prSEList=prSEList,
        notAtSE=dumpNotAtSE,
        summary=summary,
    )
...
def printReplicaStats(
    ...
    if directories:
        ...
        chunkSize = 1000
        lfnReplicas = {}
        progressBar = ProgressBar(len(lfnList), title=f"Getting replicas for {len(lfnList)} LFNs", chunk=chunkSize)
        for lfnChunk in breakListIntoChunks(lfnList, chunkSize):
            progressBar.loop()
            res = dm.getReplicas(lfnChunk, getUrl=False)
            if not res["OK"]:
    ...
    if getSize:
        ...
        progressBar = ProgressBar(len(lfnReplicas), title=f"Getting size for {len(lfnReplicas)} LFNs", chunk=chunkSize)
    ...
    for lfn, replicas in lfnReplicas.items():  # can be an iterator
        ...
        if (
            (not isinstance(prWithReplicas, list) and not prFailover)
            or (isinstance(prWithReplicas, list) and len(replicas) in prWithReplicas)
            or (prFailover and prFailover in {dmsHelper.isSEFailover(se) for se in replicas})
        ):
    ...
    if prSEList:
        ...
        if not dumpFromSE:
            prStr = f"\nNo files found {atOrNot}at {','.join(sorted(prSEList))}"
        else:
            prStr = f"\nFiles {atOrNot}present at {','.join(sorted(dumpFromSE))}"
        if prWithReplicas:
            if len(prWithReplicas) == 1:
                prStr += " with %d non-archive replicas" % prWithReplicas[0]
            else:
                prStr += f" with # of non-archive replicas in {str(prWithReplicas)}"
        elif prFailover:
            prStr += " with replicas in Failover"
        gLogger.notice(prStr)
        for se in dumpFromSE:
            if not notAtSE:
                gLogger.notice(f"At {se}")
...
def replicateLfn(lfnList, sourceSE, destList, localCache=None, removeSource=False, verbose=False):
    ...
    if len(lfnList) > 1 or len(destList) > 1 or verbose:
        gLogger.notice(
            f"Replicating {len(lfnList)} files to {','.join(destList)}" + (f" from {sourceSE}" if sourceSE else "")
        )
    ...
    for ses, lfnList in replicasToRemove.items():  # can be an iterator
        gLogger.info(f"Removing {len(lfnList)} files from {ses}")
        seList = ses.split(",")
        ...
        if not code:
            for reason, seDict in errorReasons.items():  # can be an iterator
                for se, lfns in seDict.items():  # can be an iterator
                    for lfn in lfns:
                        finalResult["Value"]["Failed"].setdefault(f"Remove from {se}", {}).update({lfn: reason})
                    if se in seList:
                        seList.remove(se)
...
def setProblematicFiles(lfnList, targetSEs, reset=False, fullInfo=False, action=True):
    ...
    gLogger.notice(f"Now processing {len(lfnList)} files")
    chunkSize = max(50, min(500, len(lfnList) // 10))
    ...
    for lfn in lfnList:
        if lfn in replicas["Failed"]:
            ...
            repsDict[lfn] = {se: reps[se] for se in overlapSEs}
    ...
    if bkToggle:
        ...
        transStatusOK = {
            True: ("Problematic", "MissingLFC", "MissingInFC", "ProbInFC", "MaxReset"),
    ...
    if notFound:
        gLogger.notice(f"\n{len(notFound)} files not found in FC")
        for lfn in notFound:
            gLogger.info(f"\t{lfn}")

    if notFoundAtSE:
        gLogger.notice(f"{len(notFoundAtSE)} files not found in FC at any of {targetSEs}")
    ...
    if repsDict:
        ...
        progressBar = ProgressBar(toSet, title="Setting replicas %s for %d files" % (status, toSet), chunk=chunkSize)
        errors = {}
        for lfnChunk in breakListIntoChunks(repsDict, chunkSize):
            progressBar.loop()
            chunkDict = {lfn: repsDict[lfn] for lfn in lfnChunk}
    ...
    if bkToggle and reset:
        ...
        chunkSize = max(10, min(100, toSet // 10))
        progressBar = ProgressBar(toSet, title="Replica flag being %s for %d files" % (status, toSet), chunk=chunkSize)
        errors = {}
        success = 0
    ...
    if transDict:
        ...
        for transID in sorted(transDict):
            ...
            if not res["OK"]:
                gLogger.error(
                    f"\tError setting {len(lfns)} files {status} for transformation {transID}", res["Message"]
                )
            else:
                gLogger.notice(f"\t{len(lfns)} files set {status} for transformation {transID}")
            for lfn in lfns:
                gLogger.info(f"\t\t{lfn}")
    ...
    if transNotSet:
        ...
        for status in sorted(transNotSet):
            ...
            for transID in transDict:
                gLogger.notice(
                    "\t%d files were in status %s in transformation %s"
                    % (len(transDict[transID]), status, str(transID))
                )
                for lfn in transDict[transID]:
                    gLogger.verbose(f"\t\t{lfn}")

    gLogger.notice(f"Execution completed in {time.time() - startTime:.2f} seconds")
    return 0
...
def executeLfnMetadata(dmScript):
    ...
    savedLevel = gLogger.getLevel()
    gLogger.setLevel("FATAL")
    filesList = [lfn for lfn in lfnList if catalog.isFile(lfn).get("Value", {}).get("Successful", {}).get(lfn)]
    ...
    for metadata in success.values():
        if "Mode" in metadata:
            metadata["Mode"] = f"{metadata['Mode']:o}"
    gLogger.setLevel(savedLevel)
    return printDMResult(
        S_OK({"Successful": success, "Failed": failed}), empty="File not in FC", script="dirac-dms-lfn-metadata"
    )
...
def executeGetFile(dmScript):
    ...
    gLogger.notice(
        ...
        % (("%d files" % nLfns) if nLfns > 1 else "file", dirList[0], " from %s" % sourceSE if sourceSE else "")
    )
    ...
    if result["OK"]:
        ...
        if popReport:
            ...
            if not res["OK"]:
                ...
                gLogger.info("Successfully reported data usage", f"(site: {localSite}, usage: {str(popReport)})")
    ...
    for lfnDict in lfnList:
        ...
        if localFile.startswith("/castor/cern.ch/user") or localFile.startswith("/eos/lhcb/user"):
            ...
            if not os.path.isfile(localFile):
                gLogger.error(f"{localFile} is not a file")
                continue
        else:
            gLogger.error(f"File {localFile} doesn't exist locally")
            continue
        ...
        if remoteFile:
            ...
            localFile = os.path.join(
                os.environ.get("TMPDIR", os.environ.get("TMP", "/tmp")), os.path.basename(remoteFile)
            )
            gLogger.notice("Attempting to download file to", localFile)
            from subprocess import Popen, PIPE

            process = Popen([eos, "cp", f"{prefix}{remoteFile}", localFile], stdout=PIPE, stderr=PIPE)
        ...
        gLogger.notice(f"\nUploading {localFile} as {lfn}")
        gLogger.setLevel("FATAL")
        res = dm.putAndRegister(lfn, localFile, lfnDict["SE"], lfnDict["guid"])
        gLogger.setLevel(logLevel)
        if not res["OK"]:
            exitCode = 3
            gLogger.error(f"Error: failed to upload {localFile} to {lfnDict['SE']}", res["Message"])
        else:
...
def executeListDirectory(dmScript, days=0, months=0, years=0, wildcard=None, depth=0):
    ...
    progressBar = ProgressBar(len(baseDirs), title=f"Getting files from {len(baseDirs)} directories", chunk=1)
    filesInDirs = {}
    for baseDir in baseDirs:
        progressBar.loop()
        if baseDir[-1] == "/":
            baseDir = baseDir[:-1]
        gLogger.info(f"Will search for files in {baseDir}")
        activeDirs = [baseDir]
        ...
        while len(activeDirs) > 0:
            ...
            if not res["OK"]:
                gLogger.error(
                    ...
                    f"{res['Message'].replace(currentDir, '')} {currentDir}/",
                )
            elif currentDir in res["Value"]["Failed"]:
                gLogger.error(
                    "Error retrieving directory contents -",
                    f"{res['Value']['Failed'][currentDir].replace(currentDir, '')} {currentDir}/",
                )
            else:
                ...
                if not onlyFiles:
                    gLogger.notice(
                        ...
                        % (currentDir, len(dirContents["Files"]), len(dirContents["SubDirs"]))
                    )
                if empty:
    ...
    for baseDir, allFiles in filesInDirs.items():  # can be an iterator
        if outputFlag:
            outputFileName = f"{baseDir[1:].replace('/', '-')}.lfns"
        ...
        if emptyDirsFlag:
            outputFileName = f"{baseDir[1:].replace('/', '-')}.emptydirs"
            outputFile = open(outputFileName, "w")
...
def registerBK2FC(lfnList, seList, printResult=False):
    ...
    replicas = res["Value"]["Successful"]
    if replicas:
        gLogger.notice(f"{len(replicas)} files already registered in FC")
    ...
    if failed:
        ...
        gLogger.notice(f"{len(failed)} files are not in the BK")
    ...
    if lfnList:
            ...
            for lfn, metadata in success.items():  # can be an iterator
                if metadata.get("Cached", metadata["Accessible"]):
                    ...
                    if size != bkMetadata[lfn]["FileSize"]:
                        ...
                        result["Failed"][lfn] = f"Size mismatch between BK and SE {se}"
                        continue
        ...
        if lfnList:
            result["Failed"].update(
                dict.fromkeys(
                    (lfn for lfn in lfnList if lfn not in result["Failed"]), f"Not found at any of {seListString}"
                )
            )
