    ...
    def getMetadataFromTSorBK(self, lfns, param):
        ...
        if lfnsLeft:
            ...
            for fileDict in self.transFiles:
                lfn = fileDict["LFN"]
                if lfn in success:
                    for par in self.tsParams:
                        if par == "FileSize":
                            par = "Size"
                        if fileDict.get(par) is None:
                            lfnDict.setdefault(lfn, {}).update({par: success[lfn][par]})
            if lfnDict:
...
get from BK"
    ...
    def checkRun3CondDB(self, runID: int) -> bool:
        ...
        if not self.onlineCondDB:
            self.onlineCondDB = Path(
                self.getPluginParam(
                    "OnlineCondDB", "/cvmfs/lhcb-condb.cern.ch/git-conddb/lhcb-conditions-database.git/"
                )
            )

        if not self.onlineCondDB.exists():
            self.logWarn("OnlineCondDB not found", self.onlineCondDB)
            return None
        ...
        # --exit-code works the opposit of what you'd think
        # i.e in this case it returns 1 if the file exists
        # and 0 otherwise
