...
def initializeRunDBInterfaceHandler(serviceInfo):
    ...
    sys.path.append(f"{str(ORACLE_HOME)}")
        ...
        for selectParam in allFileFields:
            if selectParam in selectDict:
                ...
                if isinstance(selectValue, str):
                    paramString = f"{paramString},{selectParam}='{selectValue}'"
                else:
                    paramString = f"{paramString},{selectParam}={selectValue}"
        decending = False
        if sortList:
            paramString = f"{paramString},orderBy='{sortList[0][0]}'"
            if sortList[0][1] == "DESC":
                decending = True
        paramString = f"{paramString},no={sys.maxsize}"
        if paramString:
            filesQueryString = f"success,result = server.getFilesDirac(fields=allFileFields{paramString})"
        else:
  ...
  """
    ...
    def export_getRunsSummaryWeb(self, selectDict, sortList, startItem, maxItems):
        ...
        for runID in runCounters:
            ...
            size = f"{runCounters[runID]['Size'] / (1000 * 1000 * 1000.0):.2f}"
            runCounters[runID]["Size"] = size
        resultDict["Counters"] = {
            "Files": totalFiles,
            "Events": totalEvents,
            "Size": f"{totalSize / (1000 * 1000 * 1000.0):.2f}",
        }
    ...
    def export_getRunSelections(self):
        ...
        try:
            ...
            queries = [
                ("PartitionName", "getPartitionNames"),
                ("RunType", "getRunTypes"),
                ("FillID", "getFillIDs"),
            ...
            for key, query in queries:
                startTime = time.time()
                execString = f"success,result = server.{query}()"
                print(execString)
                ...
                if not success:  # using exec statement above -> pylint: disable=used-before-assignment
                    errStr = f"RunDBInterfaceHandler.getSelections: Failed to get distinct {key}."
