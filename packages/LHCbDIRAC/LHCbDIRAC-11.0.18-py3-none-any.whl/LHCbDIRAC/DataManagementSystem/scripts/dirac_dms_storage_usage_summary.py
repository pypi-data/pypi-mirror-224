...
def printSEUsage(totalUsage, grandTotal, scaleFactor):
    ...
    dashes = "-" * 48
    print(dashes)
    print(f"{'DIRAC SE'.ljust(20)} {('Size (%s)' % unit).ljust(20)} {'Files'.ljust(20)}")
    form = "%.1f"
    orderedSEs = orderSEs(totalUsage)
    for se in orderedSEs:
        if totalUsage[se]["Size"] / scaleFactor < 1.0:
            form = "%.3f"
            break
    svcClass = ""
    sumFiles = 0
    ...
    for se in orderedSEs:
        ...
        if newSvcClass != svcClass:
            if svcClass:
                print(
                    "%s %s %s"
                    % (("Total (%s)" % svcClass).ljust(20), (form % (sumSize)).ljust(20), str(sumFiles).ljust(20))
                )
                sumFiles = 0
                sumSize = 0.0
            print(dashes)
        ...
        sumFiles += files
        sumSize += size
        print(f"{se.ljust(20)} {(form % size).ljust(20)} {str(files).ljust(20)}")
    if grandTotal:
        size = grandTotal["Size"] / scaleFactor
        print(f"{'Total (disk)'.ljust(20)} {(form % size).ljust(20)} {str(grandTotal['Files']).ljust(20)}")
    print(dashes)
...
def printBigTable(siteList, bigTable):
        ...
        for processingPass in sorted(bigTable[cond]):
            ...
            for site in siteList:
                if site in bigTableUsage:
                    ...
                    prStr += f"{bigTableUsage[site]:.3f}".ljust(just[2])
                elif site == "Total":
    ...
    for site in siteList:
        if site in grandTotal:
            prStr += f"{grandTotal[site]:.3f}".ljust(just[2])
        else:
            prStr += "-".ljust(just[2])
    print(prStr)
...
def browseBK(bkQuery, ses, scaleFactor):
    ...
    if not bkQuery.getConfiguration():
        print(f"The Configuration should be specified in the --BKQuery option: {bkPath}")
        return None
    conditions = bkQuery.getBKConditions()
    if not conditions:
        print(f"No Conditions found for this Configuration {bkPath}")
        return None
    ...
    for cond in conditions:
        strCond = f"Browsing conditions {cond}"
        writeInfo(strCond)
        ...
        for processingPass in [pp for pp in eventTypesForPP if eventTypesForPP[pp]]:
            ...
            strPP = f" - ProcessingPass {processingPass}"
            ...
            for eventType in eventTypes:
                strEvtType = f" - EventType {str(eventType)}"
                ...
                # print prods
                strProds = f" - FileTypes {str(fileTypes)} - Prods {str(prods)}"
...
def execute(unit, minimum, depth):
    ...
    for site in sites:
        res = gConfig.getOptionsDict(f"/Resources/Sites/LCG/{site}")
        if not res["OK"]:
            print(f"Site {site} not known")
            Script.showHelp()
    ...
    if users:
        if users != "all":
            dirs += [f"/lhcb/user/{user[0]}/{user}" for user in users]
            minimum = 0
        else:
            ...
            if not res["OK"]:
                ...
                DIRAC.exit(2)
            dirs += sorted({"/".join(d.split("/")[0:5]) for d in res["Value"]})
    ...
    if not dirs:
        ...
        if bkQuery:
            ...
            if fileTypes == ["RAW"]:
                ...
                prods = sorted(bkQuery.getBKProductions())
                if not prods:
                    print(f"No productions found for bkQuery {str(bkQuery)}")
                    DIRAC.exit(0)
                ...
                if fileTypes and "FULL.DST" not in fileTypes and "DST" not in fileTypes:
                    fileTypes.append("DST")
                print(f"Looking for {len(prods)} productions:", prods)
    ...
    if sites:
        prString += f"Sites {str(sites)} "
    elif ses:
        prString += f"SEs {str(ses)} "
    if prods[0] != "":
        prString += f"productions {str(prods)} "
    if fileTypes[0] not in ("", None):
        prString += f"file types {str(fileTypes)} "
    if dirs[0] != "":
        if fileTypes == ["RAW"]:
            prString += f"in {len(dirs)} directories"
        else:
            prString += f"directories {str(dirs)}"
    if not users:
        ...
        for fileType in fileTypes:
            for dirName in dirs:
                if users:
                    ...
                    userRegistry = gConfig.getOptions(f"/Registry/Users/{user}")
                    ...
                    if summary:
                        ...
                        print(
                            "Storage usage for user %s (quota: %.1f %s)%s"
                            % (
                                user,
                                quota,
                                unit,
                                ...
                                if not quota
                                else (" <== Over quota" if spaceUsed > quota else ""),
                            )
                        )
                        printSEUsage(totalUsage, grandTotal, scaleFactor)
                else:
        ...
        for se in sorted(totalUsage):
            ...
            if seStatus["OK"]:
                ...
                if seStatus["DiskSE"]:
                    ...
                    diskTotalSize += size
        print(f"{'Storage Type'.ljust(20)} {('Size (%s)' % unit).ljust(20)} {'Files'.ljust(20)}")
        print("-" * 50)
        print(
            "%s %s %s"
            % ("T1D*".ljust(20), ("%.1f" % (tapeTotalSize / scaleFactor)).ljust(20), str(tapeTotalFiles).ljust(20))
        )
        print(
            "%s %s %s"
            % ("T*D1".ljust(20), ("%.1f" % (diskTotalSize / scaleFactor)).ljust(20), str(diskTotalFiles).ljust(20))
        )
        DIRAC.exit(0)
    ...
    if not users:
        ...
        if rank:
            users = sorted(usersUsage, key=lambda x: float(usersUsage[x][0]), reverse=True)
        for user in users:
            spaceUsed, quota = usersUsage[user]
            if spaceUsed > minimum:
                print(
                    "Storage usage for user %8s: %6.3f %s (quota: %4.1f %s)%s"
                    % (
                        user,
                        spaceUsed,
                        unit,
...
def main():
    ...
    minimum = 0.1
    depth = 4
    Script.registerSwitch("u:", "Unit=", f"   Unit to use [{unit}] (MB,GB,TB,PB)")
    ...
    Script.registerSwitch(
        "", "Minimum=", f"   Don't print usage for users below that usage (same units, default {minimum:.1f}"
    )
