...
class BookkeepingDBClient(FileCatalogClientBase):
    ...
    def getFileSize(self, path):
        ...
        # Always returns OK
        successful = {lfn: metadata["FileSize"] for lfn, metadata in res["Value"]["Successful"].items()}
    ...
    def __getFileMetadata(self, lfns):
        ...
        for lfnList in breakListIntoChunks(lfns, self.splitSize):
            ...
            if not res["OK"]:
                ...
                failed.update({lfn: val for lfn, val in success.items() if isinstance(val, str)})
                successful.update({lfn: val for lfn, val in success.items() if not isinstance(val, str)})
        return S_OK({"Successful": successful, "Failed": failed})
