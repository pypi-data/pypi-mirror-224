...
@LastEditTime: 2023-08-09 17:32:08
...
class UserAvatarHandler(ClientBaseHandler):
    ...
    def post_async(self):
        ...
        result = self.upload(file_name=file_name, data=files_data["image"][0]['body'])
        if not result:
    ...
    def upload(self, file_name, data=None):
        ...
        end_point = oss_config.get("end_point", "")
        domain = oss_config.get("domain", "")
        is_auto_file_name = oss_config.get("is_auto_file_name", 1)
        folder = oss_config.get("folder", "")

        if is_auto_file_name == 1:
            file_extension = os.path.splitext(file_name)[1]
            file_name = UUIDHelper.get_uuid().replace("-", "") + file_extension
        object_name = folder + "/" + file_name
        result = COSHelper(access_key, secret_key, end_point).put_file(bucket, object_name, data)
        if result == True:
            return domain + "/" + object_name
        else:
            return ""