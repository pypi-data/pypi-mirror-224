...
from fringe_utils.utils import Helper, MimeTypes, GSheetsApiDefinedValues
...
class GSuite():
    ...
    def __init__(self, secrets_path='../creds/oauth-creds-gsuite-api.json', pickle_name='token'):
        dirname = os.path.dirname(os.path.abspath(__file__))
        self.secretsPath = os.path.join(dirname, secrets_path)
        self.mimetypes = MimeTypes()
        self.pickle_name = pickle_name

    def connect(self, service='sheets', version='v4', scopes=scopes, 
        pathToCreds=None, pickleName=None):
        if not pickleName:
            pickleName = self.pickle_name if self.pickle_name else 'token'
...
class GSheets():
    ...
    def __init__(self, scopes=None, secretsPath=None, pickle_name=None):
        gsuite = partial(GSuite)
        if secretsPath: gsuite = partial(gsuite, secrets_path=secretsPath)
        if pickle_name: gsuite = partial(gsuite, pickle_name=pickle_name)
        gsuite_connect = partial(gsuite().connect, service='sheets', version='v4')
        if scopes: gsuite_connect = partial(gsuite_connect, scopes=scopes)
        self.service = gsuite_connect()

    def testGet(self, ssid=test_ssid, range=test_get_range, 
        valueRenderOption=enums.value_render_option.FORMATTED_VALUE):
    ...
    def testUpdate(self, ssid=test_ssid, range=test_update_range):
        ...
        value_range_body = {
            "range": range,
            "majorDimension": self.enums.major_dimension.ROWS,
            "values": [
                [1234, 34123],
                [1234, 17634, 1234]
            ]
    ...
    def get(self, ssid, range, valueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        majorDimension=enums.major_dimension.ROWS):
        '''
        Get values from a spreadsheets with id `ssid` for the specified `range`.
        - `ssid`: id of the Google Sheets spreadhseet
        - `range`: alphanumeric range
        - `valueRenderOption`: use `self.enums.value_render_option` for options
        - `majorDimensionn`: use `self.enums.major_dimension` for options
        '''
    ...
    def update(self, ssid, _range, valuesDf, 
        majorDimension=enums.major_dimension.ROWS, valueInputOption=enums.value_input_option.RAW, 
        includeValuesInResponse=False, responseValueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        responseDateTimeRenderOption=enums.date_time_render_option.SERIAL_NUMBER, fillnaWithBlank=True):
        '''
        Updates values from a spreadsheets with id `ssid` for the specified `range`.
        # Parameters:
        - `ssid` (str): id of the Google Sheets spreadhseet
        - `_range` (str): alphanumeric range
        - `valuesDf` (DataFrame): a pandas dataframe of values
        - `majorDimension`: use `self.enums.major_dimension` for 'options'
        - `valueRenderOption`: use `self.enums.value_render_option` for options
        - `includeValuesInResponse` (bool): default `False`
        - `responseValueRenderOption`: use `enums.value_render_option` for options
        - `responseDateTimeRenderOption`: use `enums.date_time_render_option` for options
        - `fillnaWithBlank` (bool): default `True`
        '''
    ...
    def batchGet(self, ssid, range, valueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        majorDimension=enums.major_dimension.ROWS):
        ''
    
    def batchGetByDataFilter(self, ssid, range, valueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        majorDimension=enums.major_dimension.ROWS):
        ''
    ...
    def batchUpdateByDataFilter(self, ssid, _ranges, valuesDf, 
        majorDimension=enums.major_dimension.ROWS, valueInputOption=enums.value_input_option.RAW, 
        includeValuesInResponse=False, responseValueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        responseDateTimeRenderOption=enums.date_time_render_option.SERIAL_NUMBER, fillnaWithBlank=True):
        ''
    ...
    def batchClearByDataFilter(self, ssid, _ranges):
        ''
    
    def append(self, ssid, _range, valuesDf, 
        majorDimension=enums.major_dimension.ROWS, valueInputOption=enums.value_input_option.RAW, 
        includeValuesInResponse=False, responseValueRenderOption=enums.value_render_option.FORMATTED_VALUE, 
        responseDateTimeRenderOption=enums.date_time_render_option.SERIAL_NUMBER, fillnaWithBlank=True):
        '''
        Appends values to spreadsheets with id `ssid` at the end of the specified `range`.
        # Parameters:
        - `ssid` (str): id of the Google Sheets spreadhseet
        - `_range` (str): alphanumeric range
        - `valuesDf` (DataFrame): a pandas dataframe of values
        - `majorDimension`: use `self.enums.major_dimension` for 'options'
        - `valueInputOption`: use `self.enums.value_input_option` for options
        - `includeValuesInResponse` (bool): default `False`
        - `responseValueRenderOption`: use `enums.value_render_option` for options
        - `responseDateTimeRenderOption`: use `enums.date_time_render_option` for options
        - `fillnaWithBlank` (bool): default `True`
    ...
    def getColumnNumber(self, col):
        ...
        num = ord(c) - first + 1
        if len(col) > 1:
            num = num + 26*self.getColumnNumber(col[:-1])
        return num
    ...
    def getIndexesFromRange(self, _range):
        ...
        c1 = s.group(1)
        if c1:
            c1 = self.getColumnNumber(c1)
        c2 = s.group(4)
        if c2:
            c2 = self.getColumnNumber(c2)
        r1 = s.group(2)
        if not r1:
            r1 = 1
...
class GSlides():
    ...
    def __init__(self, scopes=None, secretsPath=None, pickle_name=None):
        ...
        if secretsPath: gsuite = partial(gsuite, secrets_path=secretsPath)
        if pickle_name: gsuite = partial(gsuite, pickle_name=pickle_name)
        gsuite_connect = partial(gsuite().connect, service='slides', version='v1')
        if scopes: gsuite_connect = partial(gsuite_connect, scopes=scopes)
        self.service = gsuite_connect()
        self.__helper = Helper()
    
    def test_enumerate_slides(self, slides_id):
        presentation = self.service.presentations().get(presentationId = slides_id).execute()
...
class GMail():
    ...
    def __init__(self, scopes=None, secretsPath=None, pickle_name=None):
        ...
        if secretsPath: gsuite = partial(gsuite, secrets_path=secretsPath)
        if pickle_name: gsuite = partial(gsuite, pickle_name=pickle_name)
        gsuite_connect = partial(gsuite().connect, service='gmail', version='v1')
        if scopes: gsuite_connect = partial(gsuite_connect, scopes=scopes)
        self.service = gsuite_connect()
        self.__helper = Helper()
    ...
    def fetchEmails(self, searchTerm, directory, userId='me', subject=None, label=None, hasAttachment=False, maxResults=None, pageToken=None, includeSpamTrash=False, getAttachments=False):
        ...
        for emailId in emailIds:
            ...
            elif (msg_raw['payload']['parts'][0]['mimeType'] == 'text/html'):
                ix = None
                for i, part in enumerate(msg_raw['payload']['parts']):
                    if part['mimeType'] == 'text/csv':
                        ix = i
                        break
                if ix is None: continue
                attachmentId = msg_raw['payload']['parts'][ix]['body']['attachmentId']
                attachments = self.service.users().messages().attachments()
                attachment = attachments.get(id=attachmentId, userId="me", messageId=emailId['id']).execute()
                msg_data = attachment['data']
            else:
...
class GDrive():
    ...
    def __init__(self, scopes=None, secretsPath=None, pickle_name=None):
        ...
        if secretsPath: gsuite = partial(gsuite, secrets_path=secretsPath)
        if pickle_name: gsuite = partial(gsuite, pickle_name=pickle_name)
        gsuite_connect = partial(gsuite().connect, service='drive', version='v3')
        if scopes: gsuite_connect = partial(gsuite_connect, scopes=scopes)
        self.service = gsuite_connect()
    ...
    def createGDriveFolder(self, folderPrefix, parentID=None):
        # Create a folder in Drive, returns the newely created folders ID
        # Then create a spreadsheet in that folder
        folder_metadata = {
            'name': folderPrefix,
