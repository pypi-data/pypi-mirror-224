...
class Aggregator:
    ...
    @staticmethod
    def get_all_seq_statistic(start_end_data, model_data, act_dict_processed_swapped):
        ...
        for w1, w2 in pairs:
            ind1 = start_end_data.loc[(start_end_data["work_name"] == w1)]
            ind2 = start_end_data.loc[(start_end_data["work_name"] == w2)]
            merged_data = pd.merge(
                ind1, ind2, how="inner", on="upper works", suffixes=("_x", "_y")
            )
            counts = pd.Series(
    ...
    def get_res_ved_stat(
        self, brave, ksg_for_val_data, act_dict_processed_swapped, res_dict_swapped
    ):
        df_stat = pd.DataFrame(columns=["Работа", "Ресурс", "Метка ресурса"])
        brave_res = brave.index
        brave_act = brave.columns
        new_index = []
        new_columns = []
        for r in brave_res:
