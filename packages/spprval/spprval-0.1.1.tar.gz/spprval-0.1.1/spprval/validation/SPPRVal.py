...
class PlanValidator:
    def __init__(self, plan, url):
        self.ksg_data = plan
        self.plan_dataset = PlanDataset(ksg_data=plan)
    ...
    def validate(self):
        ...
        df_vedom, not_perc_vedom, norm_perc_vedom = self.aggregator.get_res_ved_stat(
            brave, self.ksg_data, act_dict_granulary_swapped, res_dict_granulary_swapped
        )
        print("Нарезка данных на окна")
        df_wind_val = self._window_zero(validation_df)

        df_wind_model = self._window_zero(plan_df)
        ...
        #         )
        # df_wind_val.to_csv('df_wind_val.csv', index=False)
        ...
        seq_history = pd.read_csv("../data/links_history_messoyaha_new.csv")
        (
            df_seq_stat,
            df_color_seq,
            norm_perc_seq,
            ...
            seq_history, self.ksg_data, act_dict_granulary_swapped
        )
    ...
    @staticmethod
    def _window_zero(df: pd.DataFrame, min_window: int = 5, max_window: int = 14):
        df_new = pd.DataFrame(columns=list(df.columns) + ["Start day", "Window length"])
        window_list = list(range(min_window, max_window + 1))
        for window_len in window_list:
            for start in range(0, len(df) - window_len + 1):
                new_row = df.iloc[start:(start + window_len)].sum(axis=0, skipna=True)
                new_row["Start day"] = datetime.datetime.strptime(
                    df.index[start], "%d.%m.%Y"
                )
                new_row["Window length"] = window_len
                df_new = pd.concat([df_new, new_row], ignore_index=True)
        return df_new

    # def _window_zero(df: pd.DataFrame, min_window: int = 2, max_window: int = 21):
