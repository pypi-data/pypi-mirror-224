$(document)
  .ready(function() {
    ...
    for (var i = 0; i< lTextAreaSpecificationArray.length; i++) {
        ...
        delete lTextAreaSpecificationArray[i]['attributes']
        delete lTextAreaSpecificationArray[i]['current_value']
        delete lTextAreaSpecificationArray[i]['localized_role_name']
        delete lTextAreaSpecificationArray[i]['child_count']
        delete lTextAreaSpecificationArray[i]['alpha']
        delete lTextAreaSpecificationArray[i]['rectangle']
        delete lTextAreaSpecificationArray[i]['geometry']
        delete lTextAreaSpecificationArray[i]['icon_name']
        delete lTextAreaSpecificationArray[i]['group_leader']
        delete lTextAreaSpecificationArray[i]['pid']
        delete lTextAreaSpecificationArray[i]['ref_count']
        if (i!=0) {
    ...
    in_data.forEach((item, index) => {
        ...
        var description_str = ""
        if (item.backend=="at-spi") {
            description_str=`process_id: ${item.process_id}; id: ${item.id}; role_name: ${item.role_name}; RECT: L${item.rectangle.x} T${item.rectangle.y} W${item.rectangle.width} H${item.rectangle.height}`
        } else {
            description_str=`pid: ${item.pid}; xid: ${item.xid}; role: ${item.role}; class_group_name: ${item.class_group_name}; RECT: L${item.geometry.x} T${item.geometry.y} W${item.geometry.width} H${item.geometry.height}`
        }
        var l_data_item = {
        "title_str": l_title,
    ...
    in_data.forEach((item, index) => {           
        ...
        {
            "icon_str": "dot circle",
            "title_str": "Уровень "+index+": "+item.name,
            "description_str":description_str,
            "action_list":[],
    ...
    if (key in const_ignore) {
        ...
        str_render = "Отсутствует"
            ...
            str_render = JSON.stringify(str_render)
        }
    ...
    slot_dict["backend"]=orpa_range_value_get("orpa-desktop-framework")
    ...
    if (in_slot_key in orpa_desktop_memory_slot_dict) {
        ...
        orpa_range_value_set("orpa-desktop-framework", slot_dict["backend"])
