...
import time
import secrets # random integers
from pyOpenRPA.Tools import CrossOS
if CrossOS.IS_WINDOWS_BOOL: from win32api import GetSystemMetrics # Get Screen rect
...
def SessionIsFullScreen(inSessionHexStr):
    ...
    if CrossOS.IS_LINUX_BOOL: 
        lSize = pyautogui.size()
        lWeight = lSize.width
        lHeight = lSize.height
...
def SystemCMDRun(inSessionHexStr, inCMDCommandStr = "echo 1", inModeStr="CROSSCHECK", inClipboardTimeoutSec = 5, inLogger = None):
    ...
    lClipboardTextOld = str(secrets.randbelow(9999000)+999)  # Set random text to clipboard (for check purposes that clipboard text has been changed)
    Clipboard.TextSet(lClipboardTextOld)
    lCrosscheckKeyStr = str(secrets.randbelow(9999000)+999)
