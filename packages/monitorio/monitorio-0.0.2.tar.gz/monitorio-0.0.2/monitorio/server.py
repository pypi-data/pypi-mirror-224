...
class MonitorioMonitor(App[None]):
    ...
    async def task(data, command, timeout, times, update, cleanup):
        async def listen(stream):
            def cleaned(raw: bytes) -> str:
                line = raw.decode().strip("\n")
                log().info(line)
                return line

            return [cleaned(raw_line) async for raw_line in stream]

        try:
            log().info("task %r started", command)
            for i in range(times):
                p = await create_subprocess_exec(command, stdout=PIPE, stderr=PIPE)
                stdout, stderr, return_code = await asyncio.gather(
                    asyncio.ensure_future(listen(p.stdout)),
                    asyncio.ensure_future(listen(p.stderr)),
                    p.wait(),
                )
                log().info("task %r: %d, returned %d", command, i, return_code)
                data[command] = i
                update()
...
def serve(log_level=None) -> None:
    ...
    asyncio.ensure_future(MonitorioMonitor().exe(lambda: terminate(terminator)))
