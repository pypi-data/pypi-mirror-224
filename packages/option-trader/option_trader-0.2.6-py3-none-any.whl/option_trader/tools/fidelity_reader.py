...
def fidelity_reader(account, summary_file, position_file):
    ...
    for pid in pair_list:            
        ...
        if fidelity_strategy == 'Unpaired':                     
            ...
            share =  srow[srow['Expiration & Strike'] == 'shares'] 
            share_quantity = abs(share['Quantity'].min())
            share_cost = share['Averaged Cost'].astype(float).min() / (share_quantity)        
        
            call = srow[srow['Expiration & Strike'] != 'shares']    
            call_quantity = abs(call['Quantity'].min())
            call_price = call['Averaged Cost'].astype(float).min() / (quantity * 100) 
            ...
            call_leg  =  optionLegDesc(exp_date, strike, asset.CALL, asset.SELL_TO_OPEN, quantity)            
            ...
            '''
            if srow.shape[0] == 1:
                strike = srow.head(1)[position.STRIKE].values[0]
                if option_trader_strategy == strategy.LONG_CALL:
                    breakeven_l =  strike+open_price
                    breakeven_h =  np.nan
                    if days_to_expire > 0:
                        win_prob  = calc_prob_higher_than(predList, breakeven_l)                    
                elif option_trader_strategy == strategy.LONG_PUT:
                    breakeven_h =  strike-open_price
                    breakeven_l =  np.nan
                    if days_to_expire > 0:                    
                        win_prob = calc_prob_lower_than(predList, breakeven_h)
                elif option_trader_strategy == strategy.SHORT_PUT:        
                    breakeven_l =  strike-open_price
                    breakeven_h =  np.nan
                    if days_to_expire > 0:                    
                        win_prob = calc_prob_higher_than(predList, breakeven_l)
                else:
                    logger.error('Unhandlered 1 leg strategy %s' % option_trader_strategy)
            '''
            if srow.shape[0] == 2:
...
def import_fidelity_pofolio(user, profolio_file_name):
    ...
    for account_number in account_number_list:
        ...
        summary_file_path = jihuang.user_home_dir+'/dataset/fidelity/Option_Summary_for_Account_'+account_number+'.csv'
...
if __name__ == '__main__':
    ...
    position_file = 'C:\\Users\\jimhu\\option_trader\\data\\dataset\\fidelity\\jihuang_tranditionalIRA_position.csv'
    summary_file = 'C:\\Users\\jimhu\\option_trader\\data\\dataset\\fidelity\\jihuang_tranditionalIRA_summary.csv'
