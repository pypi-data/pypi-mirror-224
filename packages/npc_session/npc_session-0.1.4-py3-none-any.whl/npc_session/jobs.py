...
class JobQueue(Protocol):
    ...
    def update(self, key: npc_session.SessionRecord, **kwargs: JobArgs) -> None:
        """Add update the fields on an existing entry."""

    @abc.abstractmethod
    def set_finished(self, key: npc_session.SessionRecord) -> None:
        """Mark a job as finished. May be irreversible, so be sure."""

    @abc.abstractmethod
    def set_started(self, key: npc_session.SessionRecord) -> None:
        """Mark a job as being processed. Reversible"""

    @abc.abstractmethod
    def set_queued(self, key: npc_session.SessionRecord) -> None:
        """Mark a job as requiring processing, undoing `set_started`."""

    @abc.abstractmethod
    def set_errored(
        self, key: npc_session.SessionRecord, error: str | Exception
    ) -> None:
        """Mark a job as having errored."""

    @abc.abstractmethod
    def is_started(self, key: npc_session.SessionRecord) -> bool:
