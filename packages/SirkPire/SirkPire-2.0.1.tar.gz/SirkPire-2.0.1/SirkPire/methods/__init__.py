...
class methods:
    ...
    def create_session(self, session_name, phone_number:str):
        ...
        if sign_in_data[0]['status'] == 'OK':
            ...
            methods(session_name).register_device(
                "Windows 10",
                f"SirkPire Library | {session_name}",
    ...
    def send_image(self, chat_id, file, caption = None, message_id = None, thumbnail = None):
        ...
        data = {
            'file_inline': {
                ...
                'type':'Image',
                'file_name': f'SirkPire {randint(1, 100)}.png' if url_data or str(type(file)) == "<class 'bytes'>" else file,
    ...
    def send_video(self, chat_id, file, caption = None, message_id = None):
        ...
        data = {
            'file_inline': {
                ...
                'dc_id': file_data['dc_id'],
                'access_hash_rec': req[2],
                'file_name': f'SirkPire {randint(1, 100)}.mp4' if url_data or str(type(file)) == "<class 'bytes'>" else file,
    ...
    def send_voice(self, chat_id, file, caption = None, message_id = None, time = None):
        ...
        data = {
            'file_inline': {
                ...
                'file_id': file_data['id'],
                'type': 'Voice',
                'file_name': f'SirkPire {randint(1, 100)}.ogg' if url_data or str(type(file)) == "<class 'bytes'>" else file,
    ...
    def send_music(self, chat_id, file, caption = None, message_id = None):
        ...
        data = {
            'file_inline': {
                ...
                'dc_id': file_data['dc_id'],
                'access_hash_rec': req[2],
                'file_name': f'SirkPire {randint(1, 100)}.mp3' if url_data or str(type(file)) == "<class 'bytes'>" else file,
                'width': 0,
                'height': 0,
    ...
    def send_poll(self, chat_id, text_question = None, options = [], message_id = None, multiple_answers = False, anonymous = True,  quiz = False):
        return self.method(
            'createPoll',
            {
                ...
                'object_guid': chat_id,
                'options': options if len(options) == 2 else ['This poll was created with "SirkPire Library"', 'حداقل باید دو گزینه برای نظر سنجی بگزارید !'],
    ...
    def request_file(self, file):
        ...
        return self.method(
            'requestSendFile',
            {
                'file_name': f'SirkPire library {randint(1, 100)}',
                'mime': tools.get_mime_from_url(file) if url_data else 'SirkPire' if str(type(file)) == "<class 'bytes'>" else file.split('.')[-1],
    ...
    def download_file(self, chat_id=None, message_id=None, save=False, file_inline=None):
        ...
        file_name = message_data.get('file_name', f'SirkPire {time()}.{message_data.get("mime", "SirkPire")}')
        header = {
            'auth': self.session_data['auth'],
