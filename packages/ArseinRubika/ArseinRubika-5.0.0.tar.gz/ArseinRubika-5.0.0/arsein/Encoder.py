...
class encoderjson:
    ...
    def decryptRsaOaep(self,private:str,data_enc:str):
        keyPair = RSA.import_key(private.encode("utf-8"))
        return PKCS1_OAEP.new(keyPair).decrypt(base64.b64decode(data_enc)).decode("utf-8")

    def rsaKeyGenerate(self):
    	keyPair = RSA.generate(1024)
    	public = self.changeAuthType(base64.b64encode(keyPair.publickey().export_key()).decode("utf-8"))
