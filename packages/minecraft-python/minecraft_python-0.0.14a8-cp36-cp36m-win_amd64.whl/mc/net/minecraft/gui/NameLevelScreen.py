...
class NameLevelScreen(Screen):
    ...
    def _keyPressed(self, key, modifiers, state):
        if key == window.key.BACKSPACE and len(self.__name) > 0:
            self.__name = self.__name[:-1]

        char = window.key.symbol_string(key)
        if not modifiers & window.key.MOD_SHIFT and not modifiers & window.key.MOD_CAPSLOCK:
            char = char.lower()
