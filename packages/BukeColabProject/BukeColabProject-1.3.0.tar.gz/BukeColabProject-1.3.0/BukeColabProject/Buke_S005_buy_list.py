...
from BukeColabProject.indicator import sma, ibs, pivot_standard, 순거래대금, pdi
from BukeColabProject.function import rank, ts_max
...
def read_index_day_price(dir):
    ...
    df = pd.read_csv(f"{dir}/data_file/Kosdaq_Price.csv", index_col=0)
    df['날짜'] = df['날짜'].apply(lambda day: str_to_date(day))
    return df
...
def Buke_S005_buy_list(dir, day):
    ...
    try:
        ...
        df.to_csv(f"{path}/buy_5_{str(day)[-5:]}.txt")
        return False
