...
def create_run(project_id = None, sweep = False):
    if not project_id:
      print("Please provide project ID")
      return
    
    data = {'project_id': project_id, 'username': username, 'key': key, 'sweep': sweep}
...
def start(base_folder = "", project_name = "", save_files = False, sweep = False, sweep_name = ""):
    ...
    if sweep == True:
      ...
      if len(str(sweep_name)) > 0:
        r_id = create_run(int(p_id), sweep = True, sweep_name = sweep_name)
      else:
...
def init(sweep = False, sweep_name = ""):
    global sweep_setup_global
    global sweep_global
        
    if sweep = True:
      if sweep_setup_global == False:
        if len(str(sweep_name)) > 0:
          r_id = create_run(int(p_id), sweep = True, sweep_name = sweep_name)
        else:
          r_id = create_run(int(p_id), sweep = True, sweep_name = "[S] Best Run")
