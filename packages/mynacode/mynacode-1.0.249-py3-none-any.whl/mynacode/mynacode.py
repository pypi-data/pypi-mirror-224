...
def start(base_folder = "", project_name = "", save_files = False, sweep = False):
...
def torch_save_best():
    ...
    # Delete old checkpoint if sweep == True. Only saving best.
    if sweep_global == True:
      files = os.listdir( run_dir_global+'/model' )
      for file in files:
        if item.endswith("_myna_chkpt.pt"):
            os.remove( os.path.join( run_dir_global+'/model', item ) )
