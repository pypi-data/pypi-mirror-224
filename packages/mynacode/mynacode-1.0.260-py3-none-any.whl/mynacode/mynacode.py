...
def torch_save_best():    
    ...
    config_dict.update({'best_model_path': run_dir_global+'/model/best_model_chkpt.pt', 'best_model_logs': run_dir_global+'/model/best_model_logs.txt'})    
    results(results_dict = config_dict, run_id = None, node_name = 'Results')
  ...
  class MynacodeCallback(keras.callbacks.Callback):
      ...
      def on_train_end(self, logs=None):
          ...
          if self.goal == 'maximize':
            if self.best_metric_value > prev_max:
              prev_max = self.best_metric_value 
              if not os.path.exists(run_dir_global+'/model'):
                os.mkdir(run_dir_global+'/model')
            ...
            if self.best_metric_value < prev_min:
              prev_min = self.best_metric_value 
              if not os.path.exists(run_dir_global+'/model'):
                os.mkdir(run_dir_global+'/model')
