...
def start(base_folder = "", project_name = "", save_files = False):
    ...
    run_dir_global = base_folder+'/mynacode/'+project_name+'/'+str(r_id)+'_'+str(datetime.now())[:10]
    ...
    if save_files == True:
      ...
      data = {'run_id' : run_id_global, 'config_dict': str({"python_files":run_dir_global+'/files/'}), 'node_name': "Datasets", 'username': username, 'key': key}
...
def torch_save_best():
    config_dict = {}
    torch.save(model_global.state_dict(), run_dir_global+'/model/best_'+str(metric_name_global)+'_'+str('%.4f' % metric_value_global)+'_chkpt.pt')
    config_dict.update({'best_weights_path': run_dir_global+'/model/best_'+str(metric_name_global)+'_'+str('%.4f' % metric_value_global)+'_chkpt.pt'})
    config_dict.update({'metric_name': metric_name_global})
    
    results(results_dict = config_dict, run_id = None, node_name = 'Results')
    
    

def data(train_set=[], train_labels=[], test_set=[], test_labels=[], val_set=[], val_labels=[], save_once=True,
  ...
  class MynacodeCallback(keras.callbacks.Callback):
      ...
      def on_train_end(self, logs=None):
          ...
          self.best_model.save(run_dir_global+'/model/best_'+str(self.metric_name)+
                               '_'+str('%.4f' % logs.get(self.metric_name))+'_epoch_'+str(self.best_metric_epoch)+'_chkpt.hdf5')
      ...
      def on_epoch_end(self, epoch, logs=None):
          ...
          if self.metric_name not in logs.keys():
            ...
            if self.goal == 'maximize':
              print(self.metric_name, self.prev_max)
              if logs.get(self.metric_name) > self.prev_max:
                ...
                config_dict.update({'metric_goal': 'maximize'})
                print(logs.get(self.metric_name))
