...
if not _RELEASE:
    ...
    _tracks = (
        process_tracks(
            st.session_state.track_data[st.session_state.data["date"].split(" ")[0]]
        )
        if st.session_state.data["date"].split(" ")[0] in keys
        else []
    )
    ...
    _tracks2 = (
        process_tracks(
            st.session_state.track_data2[st.session_state.data["date"].split(" ")[0]]
        )
        if st.session_state.data in keys2
        else []
    )
    ...
    print(key)
    if key != 0 and not st.session_state.data["date"] == key["date"]:
