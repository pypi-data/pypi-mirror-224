...
class Solver:
    def __init__(
        self,
        session: Session,
        service: Literal["CAPSOVLER", "ANTICAPTCHA", "CAPBYPASS", "CAPMONSTER"] = "CAPSOLVER", # will add more in the future
        capKey: str = None,
    ...
    def solveCaptcha(self) -> str:
        if self.service in self.valid_services:
            ...
            raise ValueError("Invalid captcha service  |> [CAPSOLVER, ANTICAPTCHA, CAPBYPASS, HCOPTCHA, CUSTOM]")
    ...
    def solveHCOP(self, domain: str) -> str:
        ...
        if not resp1.json().get("error"):
            ...
            resp = post(f"{domain}/getTaskData", json=data)
