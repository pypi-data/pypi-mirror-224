...
class GrataWrapper:
    ...
    def find_by_criteria(search: models.Search, n_companies: int = 100) -> list:
        pages = n_companies / 25
        companies = []
        api_filters = GrataWrapper._get_api_filter(search)
    ...
    def _get_api_filter(search: models.Search) -> dict:
        include = search.context.get("include", {})
        exclude = search.context.get("exclude", {})

        def _hq_include() -> list:
            hq_include = []
            cities = include.get("city", [])
            states = include.get("state", [])
            countries = include.get("country", [])
        ...
        def _hq_exclude() -> list:
            hq_exclude = []
            for state in exclude.get("state", []):
                hq_exclude.append({"state": constants.STATES[state]})
            return hq_exclude

        api_filters = {
            "op": search.context.get("operator", "all"),
            "include": include.get("keywords", []),
            "exclude": exclude.get("keywords", []),
            "grata_employees_estimates_range": include.get("employees_range", []),
            "ownership": include.get("ownership", ""),
            "headquarters": {
                "include": _hq_include(),
                "exclude": _hq_exclude(),
            },
