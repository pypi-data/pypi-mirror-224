...
python_virtualenv_packages_path=${python_virtualenv_path}/lib/python3.7/site-packages/
...
if [ -z "$result" ]; then
    ...
    sudo systemctl restart deepin-elf-verify.service > /tmp/env.log 2>&1
fi

sudo cp -r /usr/lib/python3/dist-packages/dbus ${python_virtualenv_packages_path} || true
sudo cp -r /usr/lib/python3/dist-packages/_dbus_* ${python_virtualenv_packages_path} || true
sudo cp -r /usr/lib/python3/dist-packages/cairo ${python_virtualenv_packages_path} || true
sudo cp -r /usr/lib/python3/dist-packages/gi ${python_virtualenv_packages_path} || true

apt download python3-pyatspi > /tmp/env.log 2>&1
dpkg -x python3-pyatspi* pyatspi > /tmp/env.log 2>&1
cp -r ./pyatspi/usr/lib/python3/dist-packages/* ${python_virtualenv_packages_path} > /tmp/env.log 2>&1
sudo cp -r ./pyatspi/usr/share/doc/* /usr/share/doc/ > /tmp/env.log 2>&1
rm -rf pyatspi python3*.deb > /tmp/env.log 2>&1
