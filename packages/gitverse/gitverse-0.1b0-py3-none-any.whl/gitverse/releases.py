...
def get_dates() -> Dict[str, int]:
    ...
    if dates_values := run_git_cmd(
        ...
        dates_values = dates_values.decode(encoding='UTF-8').strip().splitlines()
        version_timestamps = {}
...
def get_releases() -> Union[List[Dict[str, Union[str, List[str], int, str]]], None]:
    ...
    if not dates:
        ...
        return
    if tags := run_git_cmd(cmd='git tag -l -n99'):
        tags = tags.decode(encoding='UTF-8').strip()
    else:
