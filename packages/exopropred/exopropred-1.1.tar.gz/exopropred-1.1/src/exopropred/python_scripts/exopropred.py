...
def get_psiblast_path(def_file_path):
    ...
    if not os.path.exists(psiblast_path):
        print(f"PSIBLAST binary not found: {psiblast_path}")
        return None
...
def pssm_gen(pspath,sppath):
    listdir = glob.glob('fasta/*.fasta')
    os.makedirs('pssm_raw', exist_ok=True)
    os.makedirs('pssm_raw1', exist_ok=True)
    for i in listdir:
        #print(i.split("\\")[-1])
        if (platform.system() == 'Windows'):
            ...
            filename = i.split('/')[-1].split('.')[0]
        cmd = pspath+" -out pssm_raw1/"+filename+".homologs -outfmt 7 -query fasta/"+filename+".fasta -db "+sppath+" -evalue 0.0001 -word_size 3 -max_target_seqs 6000 -num_threads 4 -gapopen 11 -gapextend 1 -matrix BLOSUM62 -comp_based_stats T -num_iterations 3 -out_pssm pssm_raw1/"+filename+".cptpssm -out_ascii_pssm pssm_raw/"+filename+".pssm"
        os.system(cmd)
    filelist = [f for f in os.listdir("pssm_raw1/") if f.endswith(".homologs") or f.endswith(".cptpssm")]
    for f in filelist:
        os.remove("pssm_raw1/"+f)
    os.rmdir("pssm_raw1")
...
def main():
    ...
    MERCI_Locator = nf_path + "/../perl_scripts/MERCI_motif_locator.pl"
    Motif_Files = nf_path + "/../motifs"

    #========================================Extracting Model====================================
    if os.path.isdir(nf_path+'/../model/RF_model.pkl') == False:
        with zipfile.ZipFile(nf_path+ '/../model/RF_model.zip', 'r') as zip_ref:
            zip_ref.extractall(nf_path+ '/../model')
    else:
        pass
    model = nf_path+'/../model/RF_model.pkl'

    #=======================================SwissProt Database Download===========================
    if os.path.isdir(nf_path+ '/../swissprot') == False:
        urllib.request.urlretrieve("https://webs.iiitd.edu.in/raghava/exopropred/swissprot.zip", nf_path + "/../swissprot.zip")
        ...
        pass
    PSI_BLAST_database = nf_path+ '/../swissprot/swissprot'
    ###########################################################################################
    ...
    if Model == 1:
        ...
        file_split(Sequence)
        pssm_gen(PSIBLAST,PSI_BLAST_database)
        os.system("python "+os.path.dirname(__file__)+"/../extra/possum.py -i "+filename+" -o "+filename+"_example_aac_pssm_no_header -t aac_pssm -p "+os.getcwd()+"/pssm_raw -a 0 -b 0");
        os.system("python "+os.path.dirname(__file__)+"/../extra/headerHandler.py -i "+filename+"_example_aac_pssm_no_header -p aac_pssm -n 20 -o "+filename+"_aac_pssm.csv");
        os.system("python "+os.path.dirname(__file__)+"/../extra/possum.py -i "+filename+" -o "+filename+"_example_dpc_pssm_no_header -t dpc_pssm -p "+os.getcwd()+"/pssm_raw -a 5 -b 0");
        os.system("python "+os.path.dirname(__file__)+"/../extra/headerHandler.py -i "+filename+"_example_dpc_pssm_no_header -p dpc_pssm -n 400 -o "+filename+"_dpc_pssm.csv");
        os.system("python "+os.path.dirname(__file__)+"/../extra/possum.py -i "+filename+" -o "+filename+"_example_pssm_comp_no_header -t pssm_composition -p "+os.getcwd()+"/pssm_raw -a 5 -b 0");
        os.system("python "+os.path.dirname(__file__)+"/../extra/headerHandler.py -i "+filename+"_example_pssm_comp_no_header -p pssm_composition -n 400 -o "+filename+"_pssm_comp.csv");
        X = top_feat(feat,filename+"_aac_pssm.csv",filename+"_dpc_pssm.csv",filename+"_pssm_comp.csv")
        mlres = model_run(X,model)
        mlres = mlres.round(3)
    ...
    #===================== Design Model Start from Here ======================
        ...
        mlres = model_run(X,model)
        mlres = mlres.round(3)
        os.system("perl "+MERCI_Locator+" -p "+filename+" -c KOOLMAN-ROHM -gl 2 -i "+Motif_Files+"/pos_motifs.txt -o "+filename+"_pos_hits.txt")
        os.system("perl "+MERCI_Locator+" -p "+filename+" -c KOOLMAN-ROHM -gl 2 -i "+Motif_Files+"/neg_motifs.txt -o "+filename+"_neg_hits.txt")
