...
class Bert(Model):
    ...
    def __init__(self, inference_adapter, configuration, preload=False):
        ...
        if self.inputs.keys() != set(self.input_names):
            self.raise_error(
                "The Wrapper expects input names: {}, actual network input names: {}".format(
    ...
    def parameters(cls):
        ...
        parameters.update(
            {
                "vocab": DictValue(),
                "input_names": StringValue(
                    description="Comma-separated names of input layers"
                ),
                "enable_padding": BooleanValue(
                    description="Should be input sequence padded to max sequence len or not",
                    default_value=True,
                ),
            }
        )
    ...
    def preprocess(self, inputs):
        ...
        pad_len = (
            self.pad_input(input_ids, attention_mask, token_type_ids)
    ...
    def pad_input(self, input_ids, attention_mask, token_type_ids):
        ...
        if pad_len < 0:
            self.raise_error(
                "The input request is longer than max number of tokens ({})"
    ...
    def create_input_dict(self, input_ids, attention_mask, token_type_ids):
        ...
        if len(self.input_names) > 3:
            inputs[self.input_names[3]] = np.arange(len(input_ids), dtype=np.int32)[
                None, :
            ]
    ...
    def reshape(self, new_length):
        ...
        self.logger.debug(
            "\tReshape model from {} to {}".format(
                default_input_shape, new_shapes[input_name]
            )
        )
        self.max_length = (
            new_length if not isinstance(new_length, tuple) else new_length[1]
        )
...
class BertNamedEntityRecognition(Bert):
    ...
    def postprocess(self, outputs, meta):
        ...
        filtered_labels_id = [
            (i, label_i)
            for i, label_i in enumerate(labels_id)
...
class BertQuestionAnswering(Bert):
    ...
    def __init__(self, inference_adapter, configuration, preload=False):
        ...
        if self.outputs.keys() != set(self.output_names):
            self.raise_error(
                "The Wrapper expects output names: {}, actual network output names: {}".format(
    ...
    def form_request(self, inputs):
        ...
        attention_mask = [1] * len(input_ids)
        token_type_ids = [0] * (len(q_tokens_id) + 2) + [1] * (
            len(c_data.c_tokens_id) + 1
        )
        return input_ids, attention_mask, token_type_ids
    ...
    def postprocess(self, outputs, meta):
        ...
        max_score, max_s, max_e = self.find_best_answer_window(
            score_s, score_e, c_s_idx, c_e_idx
        )
    ...
    def find_best_answer_window(
        self, start_score, end_score, context_start_idx, context_end_idx
    ):
