...
class NanoDet(DetectionModel):
    ...
    def _get_outputs(self):
        ...
        if len(output_size) != 3:
            self.raise_error(
                "Unexpected output blob shape {}. Only 3D output blob is supported".format(
                    output_size
                )
            )
    ...
    def _parse_outputs(self, outputs, meta):
        ...
        input_height, input_width = (
            meta["padded_shape"][:2]
            if meta.get("padded_shape")
            else meta["resized_shape"][:2]
        )
        ...
        for label, score in enumerate(np.transpose(cls_scores)):
            ...
            dets += [
                Detection(*det)
                for det in zip(x_mins, y_mins, x_maxs, y_maxs, score, labels)
            ]
        return dets
    ...
    def get_single_level_center_point(self, featmap_size, stride):
        h, w = featmap_size
        x_range, y_range = (np.arange(w) + self.ad) * stride, (
            np.arange(h) + self.ad
        ) * stride
    ...
    def get_bboxes(self, reg_preds, input_height, input_width):
        ...
        x = np.dot(
            softmax(
                np.reshape(reg_preds, (*reg_preds.shape[:-1], 4, self.reg_max + 1)),
                -1,
                True,
            ),
            dist_project,
        ...
        return self.distance2bbox(
            center_priors[:, :2], dis_preds, (input_height, input_width)
        )
