...
class FaceBoxes(DetectionModel):
    ...
    def _parse_outputs(self, outputs, meta):
        ...
        feature_maps = [
            [math.ceil(self.h / step), math.ceil(self.w / step)] for step in self.steps
        ]
        ...
        if filtered_score.size != 0:
            ...
            detections = [
                Detection(*det, 0)
                for det in zip(x_mins, y_mins, x_maxs, y_maxs, filtered_score)
            ]
        return detections
    ...
    def calculate_anchors_zero_level(self, f_x, f_y, min_sizes, image_size, step):
        ...
        for min_size in min_sizes:
            if min_size == 32:
                ...
                list_y = [f_y + 0.5]
            anchors.extend(
                self.calculate_anchors(list_x, list_y, min_size, image_size, step)
            )
        return anchors
    ...
    def prior_boxes(self, feature_maps, image_size):
        ...
        for k, f in enumerate(feature_maps):
            for i, j in itertools.product(range(f[0]), range(f[1])):
                if k == 0:
                    anchors.extend(
                        self.calculate_anchors_zero_level(
                            j, i, self.min_sizes[k], image_size, self.steps[k]
                        )
                    )
                else:
                    anchors.extend(
