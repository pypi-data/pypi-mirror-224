...
def test_keys_handle_add(mocker, keys_json, config, response, monkeypatch, capsys):
    ...
    output, error = capsys.readouterr()
    assert (
        "'unknown-type' (choose from 'pat', 'variables:env', 'variables:file')" in error
    )
    ...
    output, error = capsys.readouterr()
    assert (
        "invalid choice: 'variables' (choose from 'pat', 'variables:env', 'variables:file')"
        in error
    )
