...
class PulsarQrm(Ieee488_2):
    ...
    def __init__(self, transport: Transport, debug: int=0):
        ...
        if self._debug == 0:
            ...
            build_ref = "fwVersion=0.11.0 fwBuild=25/07/2023-20:56:45 fwHash=0xB125FF47 fwDirty=0 kmodVersion=0.11.0 kmodBuild=27/07/2023-12:55:51 kmodHash=0x27FC9847 kmodDirty=0 swVersion=0.11.0 swBuild=27/07/2023-12:55:51 swHash=0x27FC9847 swDirty=0 cfgManVersion=0.3.0 cfgManBuild=27/07/2023-12:55:51 cfgManHash=0x27FC9847 cfgManDirty=0"
            if build != build_ref:
    ...
    # -------------------------------------------------------------------------
    @scpi_error_check
    def _set_in_offset_0(self, offset: float) -> None:
        """
        ...
        ----------
        offset : float
            Offset in volts in a range of -0.09 to 0.09.
        ...
        # SCPI call
        self._write('AFE:OFFSet:IN0 {}'.format(offset))

    # -------------------------------------------------------------------------
    @scpi_error_check
    def _get_in_offset_0(self) -> float:
        """
        ...
        # SCPI call
        var0 = self._read('AFE:OFFSet:IN0?')
    ...
    # -------------------------------------------------------------------------
    @scpi_error_check
    def _set_in_offset_1(self, offset: float) -> None:
        """
        ...
        # SCPI call
        self._write('AFE:OFFSet:IN1 {}'.format(offset))

    # -------------------------------------------------------------------------
    @scpi_error_check
    def _get_in_offset_1(self) -> float:
        """
        ...
        # SCPI call
        var0 = self._read('AFE:OFFSet:IN1?')
    ...
    # -------------------------------------------------------------------------
    @scpi_error_check
    def _set_acq_scope_config(self, config: Any) -> None:
        """
        ...
        # SCPI call
        self._write_bin('ACQ:SCOpe:CONFiguration '.format(), json.dumps(config).encode('utf-8'))

    # -------------------------------------------------------------------------
    @scpi_error_check
    def _get_acq_scope_config(self) -> Any:
        """
    ...
    # -------------------------------------------------------------------------
    @scpi_error_check
    def _set_sequencer_acq_channel_map(self, sequencer: int, channel_map: Any) -> None:
        """
        ...
        sequencer : int
            Sequencer index.
        channel_map : any
            ADC/loopback channels mapped to the I and Q path.
        ...
        # SCPI call
        self._write_bin('SEQuencer{}:ACQ:CHANnelmap '.format(sequencer), json.dumps(channel_map).encode('utf-8'))

    # -------------------------------------------------------------------------
    @scpi_error_check
    def _get_sequencer_acq_channel_map(self, sequencer: int) -> Any:
        """
        Get input channel map of the indexed sequencer. The channel map consists of a list with two elements representing the components of the complex signal to be acquired, each mapping to a list of ADC or loopback indices. The hardware only supports one channel per complex number component, so the inner list must have 0 or 1 elements. 0 and 1 are used to select the ADCs for input 1 and input 2, while 2 and 3 are used to select outputs 1 and 2 in loopback mode. If an invalid sequencer index is given, an error is set in system  error.
        ...
        # SCPI call
        var0 = self._read_bin('SEQuencer{}:ACQ:CHANnelmap?'.format(sequencer))
    ...
    # -------------------------------------------------------------------------
    @scpi_error_check
    def _delete_acq_acquisition_data(self, sequencer: int, name: str) -> None:
        """
        ...
        # SCPI call
        self._write('SEQuencer{}:ACQ:ALISt:ACQuisition:DATA:DELete "{}"'.format(sequencer, name))
