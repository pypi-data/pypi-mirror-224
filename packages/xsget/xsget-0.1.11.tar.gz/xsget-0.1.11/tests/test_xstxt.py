...
def test_raise_error_for_duplicate_output_file(script_runner, tmp_path):
    txt = tmp_path / "book.txt"
    txt.write_text("")
    ret = script_runner.run(["xstxt", "-o", str(txt)])
    assert (
        "xstxt: error: argument -o/--output: output file: "
        f"{str(txt)} already exists" in ret.stderr
    )
