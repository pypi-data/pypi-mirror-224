import warnings, math, time, copy, random
import numpy as np
import pandas as pd
...
def get_cnv_threshold_useref( obs, ref_ind, ref_ind_org, score_key = 'tumor_score', 
    ...
    if ref_ind_org is not None:
        ...
        b = (df_tmp['b_inc'] == False) & (df_tmp['cmean'] > normal_max) & (df_tmp['dec'] == 'Normal')
        if np.sum(b) > 0:
            name_set = 'Tumor'
...
def clustering_subsample( X_vec, clust_algo = 'lv', N_clusters = 25, resolution = 1, N_neighbors = 10, 
                   mode='connectivity', n_cores = 4, Ns = 10000 ):

    method = clust_algo
    Ns = min(Ns, X_vec.shape[0])
...
def identify_tumor_cells(X_cnv, ref_ind, pca = False, use_cnv_score = False, clust = None, 
    ...
    if ref_ind is None: 
        ...
        y_clust, cobj, adj = clustering_subsample( X_vec, clust_algo = Clustering_algo, N_clusters = N_clusters, 
                                            resolution = Clustering_resolution, N_neighbors = n_neighbors, 
                                            mode='connectivity', n_cores = n_cores, Ns = N_cells_max_for_clustering )        
        if adj is None:
        ...
        resolution = Clustering_resolution # *(np.sum(~ref_ind)/len(ref_ind))
        N_cs = find_num_clusters( np.sum(~ref_ind), resolution )
        # print('Res/Nclust A: ', resolution, N_clusters)
        y_clust_sel, cobj, adj = clustering_subsample( X_vec_sel, clust_algo = Clustering_algo, N_clusters = N_cs, 
                                             resolution = resolution, N_neighbors = n_neighbors, 
                                             mode = 'connectivity', n_cores = n_cores, Ns = N_cells_max_for_clustering)
            
        X_vec_ref = X_vec[ref_ind, :]
        resolution = Clustering_resolution # *(np.sum(ref_ind)/len(ref_ind))
        N_cr = find_num_clusters( np.sum(ref_ind), resolution )
        # print('Res/Nclust B: ', resolution, N_clusters)
        y_clust_ref, cobj, adj = clustering_subsample( X_vec_ref, clust_algo = Clustering_algo, N_clusters = N_cr, 
                                             resolution = resolution, N_neighbors = n_neighbors, 
                                             mode = 'connectivity', n_cores = n_cores, Ns = N_cells_max_for_clustering)
