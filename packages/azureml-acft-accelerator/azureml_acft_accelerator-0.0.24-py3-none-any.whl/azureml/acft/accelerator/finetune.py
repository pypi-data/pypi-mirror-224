...
from .utils.callbacks import AzuremlPyTorchSaveCallback, FinetuneCallback
...
class AzuremlFinetuneArgs:
    ...
    def __post_init__(self):
        ...
        # force peft algorithm for lora in case of 8 bit training
        if self.optimization_args.apply_lora and self.optimization_args.finetune_in_8bit:
            logger.info("Finetuning with 8bit quantization is enabled with lora. Forcing the lora algorithm to peft")
...
class AzuremlTrainer:
    ...
    def train(self):
        ...
        pytorch_save_callback = None
        if self.optimization_args.apply_lora:
            if self.optimization_args.finetune_in_8bit or self.optimization_args.finetune_in_4bit:
                ...
                # add lora callback
                pytorch_save_callback = AzuremlPyTorchSaveCallback(
        ...
        trainer_callbacks = []
        if pytorch_save_callback is not None:
            trainer_callbacks.append(pytorch_save_callback)
        ...
        self.hf_trainer = self.ft_with_trainer(
            model, trainer_callbacks=trainer_callbacks, load_lora_weights=is_lora_weights_path_exist)
