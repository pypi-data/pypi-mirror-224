...
from arthurai.common.constants import OutputType, InputType, TextDelimiter, Stage
from arthurai.common.exceptions import UserValueError, MethodNotApplicableError, ArthurUserError, InternalValueError, \
    UserTypeError
from arthurai.explainability.validation import validate_predicted_attribute_order_matches_dataframe
...
class EmptyLimeExplanation:
    ...
    def __init__(self, class_int_labels: List[int], num_features: int):
        nans: List[Tuple[int, float]] = [(feat_idx, float("nan")) for feat_idx in range(num_features)]
        self.nan_dict: Dict[int, List[Tuple[int, float]]] = {label: nans for label in class_int_labels}
...
class ArthurExplainer:
    ...
    def __init__(self,
        ...
        for attr in predicted_attributes:
            if attr.stage != Stage.PredictedValue:
                raise InternalValueError(f"Attribute {attr.name} in predicted_attributes parameter is of stage "
        ...
        if len(self.predicted_attributes) == 2:
            positive_positions = [attr.position for attr in self.predicted_attributes
                                  if attr.is_positive_predicted_attribute]
            if len(positive_positions) != 1:
                raise ArthurUserError('Must have exactly one positive predicted attribute')
        ...
        if model_input_type != InputType.Image:
            if data is None:
                raise UserTypeError(f"Sample data must be provided for non-image models")
            if predictions_df is None:
                raise UserTypeError(f"Sample prediction data must be provided for non-image models")

        # filter nans from data
        if data is not None:
            if predictions_df is None:
                raise UserTypeError(f"Must provide prediction data if providing sample input data")
            if len(data) != len(predictions_df):
                raise InternalValueError(f"Length of input data and predictions do not match")
            ...
            if len(data) != length_with_nans:
                logger.warning(f"Dropped {length_with_nans - len(data)} rows with NaN values from explainer dataset")
        ...
        if model_input_type != InputType.Image:
            ...
            try:
                ...
                raise UserValueError(f"Supplied predict() function failed for provided input, see exception "
                                     f"above") from e
            # ensure that the prediction columns are in the correct order
            validate_predicted_attribute_order_matches_dataframe(actual_preds,
                                                                 predictions_df, self.predicted_attributes)
    ...
    def build_tabular_explainers(self, data: pd.DataFrame, **kwargs) -> None:
        ...
        try:
            ...
            raise ArthurUserError(f"Failed to build explainer. "
    ...
    def predict_wrapper(self, encoded_feature_vectors: Iterable) -> np.ndarray:
        ...
        if self.model_type == OutputType.Multiclass and self.num_predicted_attributes == 2:  # binary classifier
            ...
            if output.shape[1] == 1:  # check if single predict output
                if self.positive_predicted_attr_position == 0:
                    ...
                    raise ArthurUserError('Positive predicted attr position must be 0 or 1')
        return output
    ...
    def predict_tabular(self, input_feature_vectors: List[List[Any]]) -> np.ndarray:
        ...
        if self.model_input_type != InputType.Tabular:
            raise MethodNotApplicableError(f"Cannot run a {InputType.Tabular} prediction on a {self.model_input_type} model")

        try:
            # encode input features to pass into predict_wrapper()
            encoded_features = np.array([self.encode_labels(fv) for fv in input_feature_vectors], dtype=np.float64)

            # use predict wrapper to handle decoding and additional processing
            predictions = self.predict_wrapper(encoded_features)
        except Exception:
            raise UserValueError("Could not make prediction(s) based on input - please ensure data is properly "
    ...
    def explain_tabular_shap(self, raw_feature_vectors: List, **kwargs) -> List[List[List[float]]]:
        ...
        feature_vectors = np.array([self.encode_labels(fv) for fv in raw_feature_vectors], dtype=np.float64)
    ...
    def explain_tabular_lime(self, raw_feature_vectors: List, **kwargs) -> List[List[List[float]]]:
        ...
        for feature_vector in feature_vectors:
            ...
            if np.any(np.isnan(feature_vector)):
                exp = EmptyLimeExplanation(class_int_labels=labels, num_features=num_features)
            else:
    ...
    def format_lime_results(self, results: List[Union[Explanation, EmptyLimeExplanation]]) -> List[List[List[float]]]:
        """
