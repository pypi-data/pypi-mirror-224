...
from tests.fixtures.models.nlp import initial_nlp_classification_model, initial_nlp_regression_model, initial_nlp_sequence_model
...
def test__str__(model_filename, expected_string):
    model_json = load_json_string(os.path.join(os.path.dirname(__file__), 'test_request_models', 'resources', model_filename))
...
def test_build_nlp_sing_col(initial_nlp_classification_model):
    ...
    pd.testing.assert_frame_equal(initial_nlp_classification_model.reference_dataframe, nlp.NLP_CLASSIFICATION_DATAFRAME)
...
def test_save_model_provisioning_exception(reg_unsaved_batch_model):
    ...
    with pytest.raises(ArthurInternalError), \
            patch('arthurai.core.model_status_waiter.ModelStatusWaiter.wait_for_valid_model_status',
                  side_effect=ArthurInternalError) as model_status_waiter_mock:
        reg_unsaved_batch_model.save()
