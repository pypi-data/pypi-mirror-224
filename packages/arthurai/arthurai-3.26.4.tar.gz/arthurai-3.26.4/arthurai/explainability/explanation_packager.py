...
from arthurai.explainability.validation import validate_predicted_attribute_order_matches_remote
...
class ExplanationPackager(object):
        ...
        for i, attr in enumerate(sorted_attr):
            if attr.categorical and attr.value_type == ValueType.String:
                ...
                if attr.categories is None or len(attr.categories) == 0:
                    ...
                    label_mapping[i] = {val: j for j, val in enumerate(self.df.iloc[:, i].unique())}
                else:
    ...
    def create(self) -> None:
        ...
        if self.df is not None:
            ...
            pred_cols_in_df = [pred_attr.name for pred_attr in predicted_attrs if pred_attr.name in self.df.columns]
        ...
        try:
            ...
            try:
                ...
                raise ArthurUserError(f"Failed to import user predict function "
