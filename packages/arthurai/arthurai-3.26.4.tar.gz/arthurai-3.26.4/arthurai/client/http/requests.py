...
def _validate_base_url(base_url, allow_insecure=True) -> None:
    ...
    if parsed.scheme == "http" and not allow_insecure:
        raise UserValueError("Must set allow_secure flag to be True in order to use an http url.")
...
class HTTPClient(AbstractHTTPClient):
    ...
    def __init__(self,
                 ...
                 header_refresh_func: Optional[Callable[[], Tuple[Dict[str, str], timedelta]]] = None):
        """
        :param base_url:
        :param path_prefix:
    ...
    def _update_headers(self):
        ...
        thread = threading.Timer(next_refresh_wait.total_seconds(), self._update_headers)
        thread.daemon = True
        thread.start()
                ...
                # if dict, ensure in tuple format like ("fname", data, [encoding]) or reformat if not
                    ...
                    for fname in files.keys():
                        ...
                        if isinstance(file_obj, tuple):
                            ...
                            data[fname] = (file_obj.name, file_obj) if hasattr(file_obj, 'name') else (fname, file_obj)
                        else:
