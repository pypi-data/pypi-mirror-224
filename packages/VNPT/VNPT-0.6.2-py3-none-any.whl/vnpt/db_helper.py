...
class Field:
    ...
    @classmethod
    def decimalField(cls, name, remark, cannull=False, default_val=''):
        return cls(name, 'decimal', remark, cannull, default_val)
...
class SqlTable:
    def __init__(self,name,key,fields):
        self.name=name
        self.key=key
        self.fields=fields
    ...
    def getCreateSql(self,savepath=''):
        ...
        result="use EPFund\nGo\nCreate Table [dbo].[{TableName}](\n\t{Key} int primary key identity(1,1),\t--ID,主键\n{Fields}) ON [PRIMARY]\nGO".format(TableName=self.name,Key=self.key,Fields=unitfield)
        if savepath != "":
            try:
    ...
    def getCsharpClass(self,savepath=NULL):
        cs='using System;\nusing System.Collections.Generic;\nusing System.Linq;\nusing System.Text;\nusing EPFundUpgrade.Models;\nnamespace EPFundUpgrade.Models\n{s}\npublic partial class {name}: EntityBase\n{s}\n#region 构造方法\n  /// <summary>\n  /// 初始化实体\n  /// <summary>\npublic {name}(): this(0){s}{e}\n  /// <summary>\n  /// 初始化实体\n  /// <summary>\npublic {name}(int id): base(id){s}{e}\n  #endregion\n{members}{e}\n{e}'.format(name=self.name,members=self.__getCsharpClass(),s="{",e="}")
        if savepath != NULL:
    ...
    def getIRepository(self,savepath=NULL):
        ir='using EPFundUpgrade.Models;\nnamespace EPFundUpgrade.IRepository{s}\n/// <summary>\n///实体数据仓储接口\n/// <summary>\npublic partial interface I{name}Repository : IBaseRepository<{name}>{s}{e}{e}'.format(name=self.name,s="{",e="}")
        if savepath != NULL:
