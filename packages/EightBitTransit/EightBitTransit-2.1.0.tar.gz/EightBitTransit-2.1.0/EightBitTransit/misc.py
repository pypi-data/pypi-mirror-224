...
__all__ = ['lowres_grid_ti', 'change_res_grid', 'sigmoid_opacities',
           'continuous_opacities', 'RMS', 'RMS_penalty', 'b_penalty',
           'toBinaryGrid', 'fromBinaryGrid', 'calculateLCdecrements',
           'ternary', 'perimeter', 'compactness']
...
def lowres_grid_ti(ti, nside, method='mode', rounding=True):
    ...
    Outputs:
    lowres_tau = (nside, (M*nside)/N) matrix of opacities, where each entry
        is either 0 (transparent) or 1 (opaque)
    """
    ...
    for (i, j), value in np.ndenumerate(lowres_tau):
        ...
        thisneighborhoodmask = ((tau_orig_pos[:, :, 0] > leftedge)
                                & (tau_orig_pos[:, :, 0] < rightedge)
                                & (tau_orig_pos[:, :, 1] > bottomedge)
                                & (tau_orig_pos[:, :, 1] < topedge))

        if method == 'mode':
            lowres_tau[i, j] = np.round(stats.mode(
                ti.opacitymat[thisneighborhoodmask], axis=None)[0][0])
        elif method == 'mean':
            if rounding:
                lowres_tau[i, j] = np.round(
                    np.mean(ti.opacitymat[thisneighborhoodmask]))
            else:
...
def change_res_grid(grid, newdim, rounding=True):
    """
    Takes a grid and outputs a new grid of the same opacity content but
    different shape, where opacities in the output grid are assigned by
    subdividing or grouping the original grid pixels, then taking the mean of
    the resulting opacities.
    ...
    # positions of centers of pixels in original grid in "physical units"
    # at t=tref
    ...
    if Nnew < N:
        for (i, j), value in np.ndenumerate(newGrid):
            ...
            thisneighborhoodmask = ((origpos[:, :, 0] > leftedge)
                                    & (origpos[:, :, 0] < rightedge)
                                    & (origpos[:, :, 1] > bottomedge)
                                    & (origpos[:, :, 1] < topedge))
    ...
    # low res to high res
        ...
        for (i, j), value in np.ndenumerate(newGrid):
            ...
            # for each old pixel, calculate overlapping area with new pixel
            ...
            for a in range(0, N):
                for b in range(0, M):
                    ...
                    # if old bottom edge > new top edge, overlap = 0
                        ...
                        # if new pixel's center is left of old's, overlap width
                        # is new right minus old left
                        if center_x < origpos[a, b, 0]:
                            overlap_width = rightedge - \
                                (origpos[a, b, 0] - w/2.)
                        # else, overlap width is old right minus new left
                        else:
                            overlap_width = (
                                origpos[a, b, 0] + w/2.) - leftedge

                        # if new pixel's center is below old's, overlap height
                        # is new top minus old bottom
                        if center_y < origpos[a, b, 1]:
                            overlap_height = topedge - \
                                (origpos[a, b, 1] - w/2.)
                        # else, overlap height is old top minus new bottom
                        else:
                            overlap_height = (
                                origpos[a, b, 1] + w/2.) - bottomedge

                        overlapareas[a, b, i, j] = overlap_width * \
                            overlap_height
            ...
            # in going from low to higher res, we want to weight the opacities
            # by the area of pixel overlap to "dilute" them appropriately
            if rounding:
                newGrid[i, j] = np.round(np.average(
                    grid[thisneighborhoodmask],
                    weights=overlapareas[:, :, i, j][thisneighborhoodmask]
                ))
            else:
                newGrid[i, j] = np.average(
                    grid[thisneighborhoodmask],
                    weights=overlapareas[:, :, i, j][thisneighborhoodmask]
                )
    return newGrid
...
def sigmoid_opacities(q_ravel, output_shape):
    """
    Takes a vector of continuous opacities and returns a matrix of opacity
...
def continuous_opacities(tau):
    ...
    Inputs:
    tau = N x M matrix of discrete opacity values,
        where 0 = transparent and 1 = opaque

    Outputs:
    q = NM x 1 matrix of transformed opacity values:
        tau_ij = 1./(1. + e^(-q_ij))

    Note: If element tau_ij is 0, then element q_ij is a negative number,
          and if element tau_ij is 1, then element q_ij is a positive number.
    """
    ...
    q[continuousmask] = np.log(
        (tau[continuousmask])
        / (np.ones_like(tau[continuousmask]) - tau[continuousmask])
    )
    ...
    # q[opmask] = 1.0 # reverse: opacity_transform(1.) = 0.73. choose s.t.
    #                 # amoeba has a chance of walking in either direction
    # q[~opmask] = -1.0 # reverse: opacity_transform(-1.) = 0.27. choose s.t.
    #                   # amoeba has a chance of walking in either direction
...
def RMS(LC_obs, LC_obs_err, LC_model):
    ...
    LC_obs_err = uncertainty on each observed light curve data point
        (to calculate RMS, make this a vector of ones!)
...
def RMS_penalty(grid, LC_obs, LC_model, temperature):
    ...
    RMS = (1./np.sqrt(N)) * \
        np.sqrt(np.sum((LC_model - LC_obs)**2)/temperature**2)
...
def toBinaryGrid(base10number, N, M):
    ...
    # reverse it so that it starts from the upper left
    # rather than lower right corner
    binary = binary[::-1]
...
def fromBinaryGrid(binarygrid):
    """
    Takes a pixel grid of 0s and 1s and outputs a base-10 number describing
    the arrangement.
    """
...
def calculateLCdecrements(N, M, LDlaw, LDCs, v, t_ref, times):
    ti = TransitingImage(opacitymat=np.zeros(
        (N, M)), LDlaw=LDlaw, LDCs=LDCs, v=v, t_ref=t_ref, t_arr=times)
    ...
    for i in range(N):
        for j in range(M):
            ...
            onepixgrid[i, j] = 1
            onepix_ti = TransitingImage(
                opacitymat=onepixgrid,
                LDlaw=LDlaw,
                LDCs=LDCs,
                v=v,
                t_ref=t_ref,
...
def compactness(grid):
    ...
    # first, step through the grid vertically, column by column, incrementing
    # the contact perimeter every time there is a
    # pixel with a vertical neighbor
    # second, step through the grid horizontally, row by row, incrementing the
    # contact perimeter every time there is a
