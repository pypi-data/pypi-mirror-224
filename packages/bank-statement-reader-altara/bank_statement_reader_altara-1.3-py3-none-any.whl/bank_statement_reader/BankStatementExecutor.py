    ...
    def execute(self, choice, pdf_file='', password='', min_salary=10000,
        ...
        try:
            ...
            if salary_excel_file_path:
                result.update({'salary_excel_file': excel_file_path})
            response = BankStatementFinalResultResponse(
                period=result.get('period'),
                account_name=result.get('account_name'),
                account_number=result.get('account_number'),
                total_turn_over_credit=result.get('total_turn_over_credit'),
                total_turn_over_debits=result.get('total_turn_over_debits'),
                ...
                salary_excel_file_path=result.get('salary_excel_file'),
            )
            return response
