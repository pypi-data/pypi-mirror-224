...
from data_aggregator_sdk.integration_message import IntegrationV0MessageData, \
    IntegrationV0MessageProfile, ProfileKind, ProfileGranulation, GRANULATION_TO_SECONDS_DELTA_MAP
...
# PACKET (128 bits)   smpm_ul_device_energy_16b_profile_8h2_energy
#
...
# -----------------------  -----------------------------------------  ----  ----------  --------------------------------------------------------------------------------------------------------------------------------    # noqa: E501
# packet_type_id.0.VAL     u7                                            7         107                                                                                                                           1101011    # noqa: E501
# packet_type_id.0.DFF     bool                                          1           0                                                                                                                          0           # noqa: E501
# type                     SmpmUlDeviceEnergy16bProfile8h2EnergyType     2           2                                                                                                                        10            # noqa: E501
...
class SmpmUlDeviceEnergy16BProfile8H2EnergyData(Packet):
    ...
    def serialize(self) -> bytes:
        ...
        result |= ((0) & (2 ** (1) - 1)) << size
        size += 1
        assert isinstance(data.type, SmpmUlDeviceEnergy16bProfile8h2EnergyType)
    ...
    @classmethod
    def parse(cls, buf: BufRef) -> 'SmpmUlDeviceEnergy16BProfile8H2EnergyData':
        result__el_tmp2: Dict[str, Any] = dict()
        if 107 != buf.shift(7):
            raise ValueError("packet_type_id: buffer doesn't match value")
        if 0 != buf.shift(1):
            raise ValueError("packet_type_id: buffer doesn't match value")
        result__el_tmp2["type"] = SmpmUlDeviceEnergy16bProfile8h2EnergyType(buf.shift(2))
        ...
        result = SmpmUlDeviceEnergy16BProfile8H2EnergyData(**result__el_tmp2)
        return result
