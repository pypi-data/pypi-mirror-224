...
class SmpmUlDeviceEnergy16BProfile8HPqsData(Packet):
    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        ...
        profiles = (
            true_round(self.profile[0] * self.point_factor),
            true_round(self.profile[1] * self.point_factor),
            true_round(self.profile[2] * self.point_factor),
            true_round(self.profile[3] * self.point_factor),
            true_round(self.profile[4] * self.point_factor),
            true_round(self.profile[5] * self.point_factor),
            true_round(self.profile[6] * self.point_factor),
            true_round(self.profile[7] * self.point_factor),
        )
