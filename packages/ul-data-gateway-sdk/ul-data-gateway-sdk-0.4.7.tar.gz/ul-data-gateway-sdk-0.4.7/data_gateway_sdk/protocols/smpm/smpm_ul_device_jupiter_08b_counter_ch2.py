...
from data_aggregator_sdk.integration_message import IntegrationV0MessageData, IntegrationV0MessageCurrentBatteryLevel, \
    IntegrationV0MessageConsumption, IntegrationV0MessageCurrentTemperature, CounterType, ResourceType

from data_gateway_sdk.utils.buf_ref import BufRef
from data_gateway_sdk.utils.packet import Packet


# PACKET (64 bits)   smpm_ul_device_jupiter_08b_counter_ch2
#
# RESULT int:        31139818566057831
# RESULT bin:  MSB   00000000 01101110 10100001 01111111 11111111 11111111 11111111 01100111   LSB
# RESULT hex:  LE    67 ff ff ff 7f a1 6e 00
#
# name                     type   size  value(int)                                                        data(bits)
# -----------------------  -----  ----  ----------  ----------------------------------------------------------------
# packet_type_id.0.VAL     u7        7         103                                                           1100111
...
class SmpmUlDeviceJupiter08BCounterCh2Data(Packet):
    value: int
    battery_volts: float
    temperature: int
    event_reset: bool
    ...
    @classmethod
    def parse(cls, buf: BufRef) -> 'SmpmUlDeviceJupiter08BCounterCh2Data':
        ...
        result = SmpmUlDeviceJupiter08BCounterCh2Data(**result__el_tmp1)
        buf.shift(9)
        return result
    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        return [
            IntegrationV0MessageData(
                dt=received_at,
                battery=[
                    IntegrationV0MessageCurrentBatteryLevel(voltage=self.battery_volts),
                ],
                consumption=[
                    IntegrationV0MessageConsumption(
                        ...
                        overloading_value=Decimal(1099511627775.0),
                    ),
                ],
                events=[
                ...
                temperature=[
                    IntegrationV0MessageCurrentTemperature(value=self.temperature),
                ],
            ),
        ]
