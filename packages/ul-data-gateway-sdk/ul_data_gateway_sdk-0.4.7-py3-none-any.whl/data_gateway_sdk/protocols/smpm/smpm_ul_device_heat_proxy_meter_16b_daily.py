    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        return [
            IntegrationV0MessageData(
                dt=received_at,
                battery=[
                    IntegrationV0MessageCurrentBatteryLevel(voltage=self.meter_battery_volts),
                    IntegrationV0MessageCurrentBatteryLevel(battery_id=BatteryId.CAPACITOR, voltage=self.capacitor_volts),
                    IntegrationV0MessageCurrentBatteryLevel(battery_id=BatteryId.RADIO_MODULE, voltage=self.radio_proxy_battery_volts),
                ],
                consumption=[
