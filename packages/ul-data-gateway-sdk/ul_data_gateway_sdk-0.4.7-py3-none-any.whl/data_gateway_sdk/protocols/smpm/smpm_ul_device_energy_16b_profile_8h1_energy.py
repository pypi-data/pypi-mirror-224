...
from data_aggregator_sdk.integration_message import IntegrationV0MessageData, ProfileKind, IntegrationV0MessageProfile, \
    ProfileGranulation, GRANULATION_TO_SECONDS_DELTA_MAP
...
# PACKET (128 bits)   smpm_ul_device_energy_16b_profile_8h1_energy
#
...
# -----------------------  -----------------------------------------  ----  ----------  --------------------------------------------------------------------------------------------------------------------------------    # noqa: E501
# packet_type_id.0.VAL     u7                                            7         106                                                                                                                           1101010    # noqa: E501
# packet_type_id.0.DFF     bool                                          1           0                                                                                                                          0           # noqa: E501
# type                     SmpmUlDeviceEnergy16bProfile8h1EnergyType     2           2                                                                                                                        10            # noqa: E501
...
class SmpmUlDeviceEnergy16BProfile8H1EnergyData(Packet):
    ...
    def serialize(self) -> bytes:
        ...
        result |= ((0) & (2 ** (1) - 1)) << size
        size += 1
        assert isinstance(data.type, SmpmUlDeviceEnergy16bProfile8h1EnergyType)
    ...
    @classmethod
    def parse(cls, buf: BufRef) -> 'SmpmUlDeviceEnergy16BProfile8H1EnergyData':
        result__el_tmp2: Dict[str, Any] = dict()
        if 106 != buf.shift(7):
            raise ValueError("packet_type_id: buffer doesn't match value")
        if 0 != buf.shift(1):
            raise ValueError("packet_type_id: buffer doesn't match value")
        result__el_tmp2["type"] = SmpmUlDeviceEnergy16bProfile8h1EnergyType(buf.shift(2))
        ...
        result__el_tmp2["point_factor"] = round(buf.shift(6) / 10.0, 1)
        result = SmpmUlDeviceEnergy16BProfile8H1EnergyData(**result__el_tmp2)
        return result
        ...
        if DeviceHack.electricity_profile_packets_days_ago_is_zero in hacks:
            return [
                IntegrationV0MessageData(
                    dt=(
                        round_dt(
                            received_at.replace(tzinfo=pytz.UTC).astimezone(device_tz),
                            timedelta(
                    ...
                    profiles=[
                        IntegrationV0MessageProfile(
                            ...
                            values=(
                                true_round(self.profile[0] * point_factor, 3),
                                true_round(self.profile[1] * point_factor, 3),
                                true_round(self.profile[2] * point_factor, 3),
                                true_round(self.profile[3] * point_factor, 3),
                                true_round(self.profile[4] * point_factor, 3),
                                true_round(self.profile[5] * point_factor, 3),
                                true_round(self.profile[6] * point_factor, 3),
                                true_round(self.profile[7] * point_factor, 3),
            ...
            IntegrationV0MessageData(
                ...
                profiles=[
                    IntegrationV0MessageProfile(
                        ...
                        granulation=ProfileGranulation.MINUTE_60,
                        values=(
                            true_round(self.profile[0] * point_factor),
                            true_round(self.profile[1] * point_factor),
                            true_round(self.profile[2] * point_factor),
                            true_round(self.profile[3] * point_factor),
                            true_round(self.profile[4] * point_factor),
                            true_round(self.profile[5] * point_factor),
                            true_round(self.profile[6] * point_factor),
                            true_round(self.profile[7] * point_factor),
                            None, None, None, None, None, None, None, None,
                            None, None, None, None, None, None, None, None,
                        ),
                    ),
