...
class RepoDirectory:
    ...
    def copyDocStruct(self, lvl, root , root_index_text = "" ):
        ...
        for mod in self.mods.values():
            ...
            if os.path.isdir(mod.path + "/doc"):
                shutil.copytree(mod.path + "/doc", path+"/"+mod.name+"/doc")
    ...
    def getKConfigString(self, lvl, title = "Config"):
        ...
        #Add Unit test option to main menu
        if lvl == 0:
            fileTxt += "config ENABLE_UNITTEST\n"
            fileTxt += "\t bool \"UnitTest\"\n"
            fileTxt += "\t default n\n"
...
class Repo:
    def __init__(self, path :str, remote = False, createdir=True):
        ...
        if not self.name == "" and not self.isGroup:
    ...
    def getDefaultBranch(self):
        ...
        try:
            ...
            if self.isRemote:
                git_ls_remote_cmd = ['git', 'ls-remote', '--symref', self.url, 'HEAD']
                awk_cmd = ["awk", "-F[/\\t]", "NR == 1 {print $3}"]
                git_ls_remote_output = subprocess.check_output(git_ls_remote_cmd)

                result = subprocess.check_output(awk_cmd, input=git_ls_remote_output)

                branch = result.decode().strip()
                self.default_branch = branch
            else:
                branch = subprocess.check_output(['git remote show', self.path ,"| sed -n '/HEAD branch/s/.*: //p'", '-f'])
                self.default_branch = branch
        except:

            if(self.isRemote):
                print("Couldnt get default branch for " + self.name + " from " + self.url)
            else:
                print("Couldnt get default branch for local repo: " + self.name )
