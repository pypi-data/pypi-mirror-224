...
class CloudInterface:
    ...
    def copy_directory(
        ...
        self.identify_path_type(source_dir)

        filtered_file_list = self._current_storage.list_directory(
            source_dir,
            exclude,
            continue_token
        )
        ...
        for file in filtered_file_list:
            logger.info(f'Copying: {file} to {dest_dir}.')
            self._copy_new(source_dir, dest_dir, file)
            logger.info(f'Copied: {file} to {dest_dir}.')

    def _copy_new(self, source_dir, dest_dir, file):
        relative_path = PurePath(file.path()).relative_to(source_dir)

        try:
            with self.open_new(file) as file:
                file_data = file.read()
        except CredentialsExpiredError:
            self._current_storage.relogin()
            file = self._current_storage.reload_file(file)
            with self.open_new(file) as file:
                file_data = file.read()

        self.save(path=str(dest_dir / relative_path), content=file_data)

    def open_new(self, path_or_file: Union[str, File]):
        ...
        self.identify_path_type(path)
        return self._current_storage.open_new(path_or_file)
