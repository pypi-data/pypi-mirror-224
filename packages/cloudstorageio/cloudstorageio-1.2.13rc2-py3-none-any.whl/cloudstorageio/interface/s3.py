...
from cloudstorageio.continue_token.s3_continue_token import S3ContinueToken
from cloudstorageio.enums.enums import PrefixEnums
from cloudstorageio.file.s3_file import S3File
from cloudstorageio.hooks import hook_registry
...
class S3Interface:
    ...
    def __init__(self, **kwargs):
        ...
        self._encoding = 'utf8'
        self._config = Config(retries={
            'max_attempts': 5,
            'mode': 'standard'
        })

        self._s3 = self._session.resource('s3', config=self._config)
        self._client = self._session.client('s3', config=self._config)
        self._mode = None
    ...
    def relogin(self):
        logger.info('Relogin called')
        self._acc_key, self._acc_secret_key, self._acc_session_token = (
            hook_registry.execute('relogin_s3'))
    ...
    def reload_file(self, file: S3File):
        s3_object = self._s3.Object(file.object.bucket_name, file.object.key)
    ...
    def listdir(self, path: str, recursive: Optional[bool] = False,
        ...
        for f in self._object_key_list:
            should_exclude = self._should_exclude(f,
                                                  exclude)
            if should_exclude:
                continue
            if f.endswith('/'):
    ...
    def _get_file(self, path):
        bucket_name, path_without_bucket = self._get_parts_from_path(path)

        s3_object = self._s3.Object(bucket_name, str(path_without_bucket))

        return S3File(s3_object)

    @contextmanager
    def open_new(self, path_or_file: Union[str, S3File], mode: str = None):
        if isinstance(path_or_file, S3File):
            yield path_or_file
            return

        yield self._get_file(path_or_file)

    def list_directory(
            self,
            source_dir,
            exclude,
            continue_token: S3ContinueToken = None
    ):
        ...
        while True:
            try:
                ...
                yield from self.get_objects_from_page(
                    bucket_name,
                    exclude,
                    page
                )
            except ClientError:
                page_iterator = self.get_relogined_page_iterator(
    ...
    def _get_page_iterator(
        ...
        if continue_token:
            pagination_config['StartingToken'] = continue_token.get()
    ...
    def get_relogined_page_iterator(
            self,
            bucket_name, next_token, pagination_config, paginator,
            path_without_bucket
    ):
        self.relogin()
        if next_token:
    ...
    def get_objects_from_page(self, bucket_name, exclude, page):
        for obj in page['Contents']:
            ...
            yield S3File(s3_object)

    def _get_parts_from_path(self, source_dir):
        path_without_prefix = PurePath(
            source_dir.split(S3Interface.PREFIX, 1)[-1]
        )
