...
async def create_replymarkup3(chat_id, post_id, POST_BUTTON, type_='pst', is_counter=False):
    result = None
    try:
        ...
        buttons = []
        post_id = int(post_id)
        dic_btns = await check_buttons2(POST_BUTTON, is_counter)
        result = InlineKeyboardBuilder()
        cnt_k = 0
        for k, v in dic_btns.items():
            try:
                if v[0]:
                    if len(tmp) > 0 and tmp[-1] is None:
                        ...
                        if 'ᴵ' in v[0]:
                            ...
                            buttons = [types.InlineKeyboardButton(text=str(v[0]),
                                                                  callback_data=f"btn_{type_}_{chat_id}_{post_id}_{cnt_k}")]
                            cnt_k += 1
                        else:
                            buttons = [types.InlineKeyboardButton(text=str(v[0]), url=v[1])]
                            cnt_k += 1
