...
class 分隔线模板:
    ...
    def 副本(self):
        ...
        副本.分隔线模板__修饰符 = self.__修饰符
        副本.分隔线模板__提示内容 = self.__提示内容
        副本.分隔线模板__总长度 = self.__总长度
        副本.分隔线模板__提示对齐 = self.__提示对齐
        副本.分隔线模板__修饰方法 = self.__修饰方法
        副本.分隔线模板__打印方法 = self.__打印方法
...
class 语义日期模板:
    ...
    def 副本(self):
        ...
        副本.语义日期模板__目标日期 = self.__目标日期
        副本.语义日期模板__打印方法 = self.__打印方法
...
class 调试模板:
    ...
    def 副本(self):
        ...
        副本.调试模板__调试状态 = self.__调试状态
        副本.调试模板__打印头 = self.__打印头
        副本.调试模板__缩进字符 = self.__缩进字符
        副本.调试模板__位置提示符 = self.__位置提示符
    ...
    def 添加多行(self, 行列表: list or tuple, 拆分列数: int = -1, 拆分行数: int = -1) -> None:
    ...
    def 添加一调试行(self, *元素列表) -> None:
        if self.__调试状态:
            self.添加一行(*元素列表)

    def 添加多调试行(self, 元素列表: list[list]) -> None:
        if self.__调试状态:
            self.添加多行(元素列表)
    ...
    def __表格展开(self) -> list[list[str]]:
        ...
        if self.__表格:
            for 行元素 in self.__表格:
                ...
                if sum([1 if str(元素).__contains__('\n') else 0 for 元素 in 行元素]) == 0:
                    # 列表中的元素字符串中不包括换行符
                    这一行 = []
                    for 元素 in 行元素:
                        这一行.append(元素)
                    ...
                    行列表: list[list[str]] = [str(元素).split('\n') for 元素 in 行元素]
