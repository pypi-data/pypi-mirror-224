...
class XAI:
    ...
    def __init__(
        ...
        Args:
            model_input (str, optional): "default", "custom", determines if using a sklearn GradientBoosting model with "default"
    ...
    def fit(self, db, output_model, manage_groups=False, groups=None):
        ...
        if self.use_cv:
            if self.model_input == "default":
                self.grid = default_grid["grid_model"]
        ...
        if self.use_cv:
            ...
            if self.manage_groups:
                ...
                if not (str(self.cv_type.__class__()).startswith(("GroupKFold", "StratifiedGroupKFold"))):
                    ...
                    self.model.cv = GroupKFold(number_splits).split(db_tsf, self.output_model, self.groups)

        # Fit
        self.model_fitted = self.model.fit(db_tsf, self.output_model)
        if self.use_cv:
    ...
    def get_feature_importance(
        ...
        self.dict_report_feat_imp = {
            "type": self.feat_imp_mode,
            "feat_importance": self.report_feat_imp.set_index("feature")["feat_importance"].to_dict(),
        }
