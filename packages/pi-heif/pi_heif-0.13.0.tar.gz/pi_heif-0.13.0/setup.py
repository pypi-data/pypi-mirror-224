...
def get_version():
    version_file = "pi_heif/_version.py"
    ...
    def build_extensions(self):  # pylint: disable=too-many-branches disable=too-many-statements
        ...
        if sys.platform.lower() == "win32":
            ...
            self._update_extension(
                "_pi_heif", ["libheif"], extra_compile_args=["/d2FH4-", "/WX"], extra_link_args=["/WX"]
            )
            ...
            self._update_extension("_pi_heif", ["heif"], extra_compile_args=["-Ofast", "-Werror"])
...
if getenv("READTHEDOCS", "False") == "True":
    ...
    setup(
        ...
        ext_modules=[Extension("_pi_heif", ["pi_heif/_pi_heif.c"])],
    )
