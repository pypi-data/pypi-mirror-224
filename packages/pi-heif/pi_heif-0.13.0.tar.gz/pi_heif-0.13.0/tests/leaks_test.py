...
def test_open_to_numpy_mem_leaks():
    ...
    for i in range(1000):
        heif_file = pi_heif.open_heif(image_file_data, convert_hdr_to_8bit=False)
...
def test_color_profile_leaks(im, cp_type):
    mem_limit = None
    heif_file = pi_heif.open_heif(Path(im), convert_hdr_to_8bit=False)
    for i in range(1000):
...
def test_metadata_leaks():
    mem_limit = None
    heif_file = pi_heif.open_heif(Path("images/heif_other/exif_xmp_iptc.heic"))
    for i in range(1000):
