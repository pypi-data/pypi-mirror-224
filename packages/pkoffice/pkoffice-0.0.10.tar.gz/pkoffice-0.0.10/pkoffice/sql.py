...
class SqlDB:
    ...
    def upload_data_mass_single(self, df: pd.DataFrame, table_name: str) -> None:
        """
        Method to upload large pandas dataframe to database in mass as single chunk.
        :param df: pandas dataframe with data to upload
        :param table_name: name of table without []
        :return: None
        """
        ...
        for val in df.values:
            sql_query = f"Insert into dbo.[{table_name}] Values ({','.join(val)})".replace('nan', 'null')
            sql_query_batch = f'{sql_query_batch} {sql_query}'
        with self.engine.begin() as conn:
            conn.execute(sql.text(sql_query_batch))

    def upload_data_mass(self, df: pd.DataFrame, table_name: str, chunksize: int,
                         flag_delete_data: bool = True) -> None:
        """
        ...
        for i in range(0, len(df), chunksize):
            if i != 0:
                self.upload_data_mass_single(df[beg:i], table_name)
                beg = i
        if len(df) >= beg:
            self.upload_data_mass_single(df[beg:], table_name)
    ...
    def upload_parameters(self):
        ...
        if self.df is not None:
            ...
            return [self.process_time_beg.strftime("%Y-%m-%d"),
                    ...
                    (self.process_time_end - self.process_time_beg).seconds,
                    df_max, 'Commit' if df_max == df_db_max else 'Fault']
        else:
            return [self.process_time_beg.strftime("%Y-%m-%d"),
                    self.process_time_beg.strftime("%H:%M:%S"),
                    (self.process_time_end - self.process_time_beg).seconds]
