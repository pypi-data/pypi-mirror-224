            ...
            for event in events:
                ...
                if area:
                    ...
                    if user:
                        ...
                        add_error_log(message, ErrorLog.ErrorType.NO_MATCHING_USER, data=cardholder_sync_value)
...
def login(user: User, area: Area, start: datetime.datetime = None):
    ...
    if project:
        ...
        add_error_log(message, ErrorLog.ErrorType.NO_ACTIVE_PROJECTS, data=user.id)
...
def add_error_log(description: Union[str,Exception], error_type: ErrorLog.ErrorType = ErrorLog.ErrorType.GENERAL, data=None):
    rs2_settings = get_rs2_settings()
    date_filter = timezone.now() - datetime.timedelta(hours=rs2_settings.duplicate_error_hours)
    last_error_same_type = ErrorLog.objects.filter(created__gt=date_filter, error_type=error_type, data=data, data__isempty=False).order_by("-created").first()
    # Only create the error with the same type and data if there is not already one during the last x hours
    if not last_error_same_type:
        # Force description to str
        ErrorLog.objects.create(description=str(description), error_type=error_type, data=data)
