...
from hestia_earth.aggregation.utils.quality_score import (
    ...
    calculate_score, _calculate_score_emissions_system_boundary, _calculate_score_global_production
)
...
def test_calculate_score_global_production():
    cycle = {
        'endDate': '2000',
        'products': [{'primary': True, 'term': {'termType': 'crop', '@id': 'wheatGrain'}}]
    }
    ...
    assert not _calculate_score_global_production(cycle, countries)
    ...
    assert _calculate_score_global_production(cycle, countries) is True
