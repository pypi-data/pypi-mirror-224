...
YIELD_THRESHOLD = 20
GLOBAL_PRODUCTION_THRESHOLD = 75
...
class ScoreKeys(Enum):
    ...
    GLOBAL_PRODUCTION_RATIO = 'global_production_ratio'
...
def _production_delta(cycle: dict, countries: list, lookup, lookup_column: str):
    ...
    delta = (total_value / world_value) * 100 if world_value and total_value else 0

    debugRequirements(id=cycle.get('id'),
                      world_production_quantity=world_value,
                      countries_production_quantity=total_value,
                      production_delta=delta,
                      production_delta_min=GLOBAL_PRODUCTION_THRESHOLD)

    return delta


def _calculate_score_global_production(cycle: dict, countries: list):
    product = find_primary_product(cycle)
    ...
    return delta >= GLOBAL_PRODUCTION_THRESHOLD
...
_FUNC_SCORE_KEY = {
    ...
    ScoreKeys.GLOBAL_PRODUCTION_RATIO: _calculate_score_global_production,
}
...
_RUN_SCORE_KEY = {
    ...
    ScoreKeys.GLOBAL_PRODUCTION_RATIO: lambda cycle, countries: len(countries) > 0 and _should_run_crop(cycle)
}
