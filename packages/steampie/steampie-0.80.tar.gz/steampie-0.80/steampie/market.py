...
class SteamMarket:
    ...
    def fetch_price(
        self, item_hash_name: str, game: GameOptions, currency: str = Currency.USD
    ) -> dict:
    ...
    def create_sell_order(
        ...
        headers = {
            "Referer": "%s/profiles/%s/inventory"
            % (SteamUrl.COMMUNITY_URL, self._steam_guard["steamid"])
        }
    ...
    def create_buy_order(
        ...
        response = self._session.post(
            SteamUrl.COMMUNITY_URL + "/market/createbuyorder/", data, headers=headers
        ).json()
    ...
    def buy_item(
        ...
        response = self._session.post(
            SteamUrl.COMMUNITY_URL + "/market/buylisting/" + market_id,
            data,
            headers=headers,
        ).json()
        try:
