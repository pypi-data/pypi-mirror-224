...
class LoginExecutor:
    ...
    def _send_login_request(self) -> requests.Response:
        ...
        return self.session.post(
            SteamUrl.COMMUNITY_URL + "/login/dologin", data=request_data
        )
    ...
    def _fetch_rsa_params(self, current_number_of_repetitions: int = 0) -> dict:
        ...
        key_response = self.session.post(
            SteamUrl.COMMUNITY_URL + "/login/getrsakey/",
    ...
    def _prepare_login_request_data(
        self, encrypted_password: str, rsa_timestamp: str
    ) -> dict:
        return {
    ...
    def _enter_steam_guard_if_necessary(
        self, login_response: requests.Response
    ) -> requests.Response:
