"""The Jupyter Run Server application."""
...
class DatalayerExampleExtensionApp(ExtensionAppJinjaMixin, ExtensionApp):
    """The Jupyter Run Server extension."""

    name = "jupyter_run"

    extension_url = "/jupyter_run"
    ...
    def initialize_settings(self):
        self.log.info("Jupyter Run Config {}".format(self.config))
    ...
    def initialize_templates(self):
        ...
        env = Environment(loader=PackageLoader('jupyter_run', 'templates'))
        ...
        """
        self.serverapp.jinja_template_vars.update({"jupyter_run_version" : __version__})

    def initialize_handlers(self):
        self.log.info("Jupyter Run Config {}".format(self.settings['jupyter_run_jinja2_env']))
#        host_pattern = ".*$"
...
#        ]
        handlers = [
            ("jupyter_run", IndexHandler),
            (url_path_join("jupyter_run", "get_config"), ConfigHandler),
            (url_path_join("jupyter_run", "echo"), WsEchoHandler),
            (url_path_join("jupyter_run", "relay"), WsRelayHandler),
            (url_path_join("jupyter_run", "proxy"), WsProxyHandler),
            (url_path_join("jupyter_run", "ping"), WsPingHandler),
        ]
