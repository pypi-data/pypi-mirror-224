...
class ShapeObject(LoggingMixin):
    ...
    def is_valid(self) -> bool:
        return (
            ...
            and self.contour_area > 0.0
            and self.mask_area < self.canvas_area
        )
    ...
    def match_center(
        ...
        *,
        use_bbox: bool = False,
        return_dxy: bool = False,
    ) -> Union["ShapeObject", Tuple[float, float]]:
        if not use_bbox:
            current_center = self.contour_center
    ...
    def separate(
        self, *, min_area_threshold: float = 0.01, **kwargs
    ) -> List["ShapeObject"]:
        fg = self.gray >= 127
    ...
    def get_scaled_img(self, scale: float, align_bottom: bool):
        return self.scale_img(
            ...
            self.contour_center,
            scale,
            bbox=self.bbox_simplified,
            align_bottom=align_bottom,
        )

    def get_scaled_contour(
        self,
        w_scale,
        h_scale=None,
        *,
    ...
    def _fetch_match_shape_scores(src, tgt):
        ...
        for method_num in [1, 2, 3]:
            ...
            scores[str(method_num)] = max(0.0, 10.0 - score)
        return scores

    def get_match_scores(
        self,
        template: "ShapeObject",
        matching_method: Union[
            str, Callable, List[Union[str, Callable]]
        ] = "contour_all",
        **kwargs,
    ) -> Dict[str, float]:
        ...
        def _raise():
            raise NotImplementedError(
                f"matching_method '{matching_method}' not defined"
            )
        ...
        for matching_method in matching_methods:
            with timing_context(self, matching_method):
                ...
                if callable(matching_method):
                    local_scores["mean"] = matching_method(
                        self.contour, template.contour
                    )
                elif "contour" in matching_method:
                    if matching_method == "contour_all":
                        local_scores.update(
                            self._fetch_match_shape_scores(
                                self.contour, template.contour
                            )
                        )
                        ...
                        with timing_context(self, "get contour"):
                            self_contour, resized_contour = (
                                self.contour,
                                template_resized.contour,
                            )
                        with timing_context(self, "diff matrix"):
                            resized_transpose = resized_contour.transpose([1, 0, 2])
                            diff_matrix = self_contour - resized_transpose
                        ...
                        local_scores["mean"] = 1.0 - diff
                    else:
                        _raise()
                elif "filled" in matching_method:
                    resized, template_resized = map(
                        ShapeObject.resize, [self, template]
                    )
                    gray, template_gray = resized.gray, template_resized.gray
                    if matching_method == "filled_shape_all":
                        local_scores.update(
                            self._fetch_match_shape_scores(gray, template_gray)
                        )
                    elif matching_method == "filled_iou":
                        local_scores["mean"] = 1.0 - self.iou_distance(
                            gray, template_gray
                        )
                    else:
                        _raise()
    ...
    def get_min_distance(self, template: "ShapeObject") -> float:
        return np.min(
            np.linalg.norm(
                self.contour - template.contour[..., 0, :][None, ...],
                axis=-1,
            )
        )
    ...
    def resize(
        ...
        *,
        ...
        if not keep_aspect_ratio:
            ...
            resized_contour = target.get_scaled_contour(
                w_scale,
                h_scale,
                canvas_changed=True,
            )
            ...
            target_size = tuple(
                map(int, map(round, [w * target_scale, h * target_scale]))
            )
    ...
    def resize_to(
        ...
        *,
        ...
        new_obj = ShapeObject(
            resized, self.get_scaled_contour(w_scale, h_scale, canvas_changed=True)
        )
        if area_definition == "contour":
            ...
            raise NotImplementedError(
                f"area_definition '{area_definition}' is not defined"
            )
        ...
        if tight:
            if match_center:
                print("Warning : match_center will not take effect because tight=True")
            scaled_obj = scaled_obj.to_tight()
        elif match_center:
    ...
    def rotate_to(
        ...
        resize_result = self.resize_to(
            template, match_center, scale_ceiling=scale_ceiling
        )
        ...
        res = {
            "theta": min_theta,
            "loss": min_loss,
            "resize_scale": resize_res["resize_scale"],
        }
