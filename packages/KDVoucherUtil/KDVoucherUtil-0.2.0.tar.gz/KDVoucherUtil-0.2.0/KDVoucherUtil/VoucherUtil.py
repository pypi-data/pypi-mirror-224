...
class TB:
    def __init__(self):
        """
        Tested the size of one year end tb, with all asst accounts info, the size was approximately 24.4M
        which should be considered acceptable.
        However, user should be very careful about loading more than one month's trial balance, and all its
        asst ledger, the object may be very large, since sometimes user may want to look up information over several years,
        especially used in web server.
        :param tb: the original result from kd webservice
        """
        self.record_list = []
        self.asst_tb_dict = {}

    def add_tb(self, tb: list):
        if tb:
            for line in tb:
    ...
    def is_leaf_account(self, account_code: str) -> bool:
        for each in self.record_list:
            if re.match(account_code + '.+', each.account_code):
                return False
        return True

    def add_asst_tb(self, asst_tb_obj: [AsstTB, list]):
        if type(asst_tb_obj) == AsstTB:
            if not asst_tb_obj.asst_list:
                return -1
            self.asst_tb_dict[asst_tb_obj.asst_list[0].account_code] = asst_tb_obj
            return 0
        elif type(asst_tb_obj) == list:
            if not asst_tb_obj:
                return -1
            try:
                o = AsstTB(asst_tb_obj)
                return self.add_asst_tb(o)
            except Exception:
                raise ObjInitException


def separate_voucher_list(voucher_list: list, renumber=False) -> dict:
    """
    ...
    :param renumber: WARNING! if this argument is True,
     the function will comb the list and renumber the sequence of entry, which may lead unknown error.
     Especially in finance booking term. Developer who has NO finance background, please use this argument carefully.
...
def _get_standard_voucher(assit=0):
    """
    this function is in order to help the developer remember the field name of voucher object.
    :param assit:
    :return:
    """
    standard_voucher = {
