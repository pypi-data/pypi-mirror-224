...
from check_multithread import MultiThreadsTelnetJob


@click.command(context_settings=dict(help_option_names=['-h', '--help']))
@click.option('-i', '--input', default="check_port_list.txt", type=str, help='input file Default : check_port_list.txt.')
@click.option('-o', '--output', default ="check_port_result.json", type=str, help='output file Default : check_port_result.json.')
@click.option('-n', '--number-thread', default=100, show_default=True, type=int, help='End block/ISO date/Unix time.')
