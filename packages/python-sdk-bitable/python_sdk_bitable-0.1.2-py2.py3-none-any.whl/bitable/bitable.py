...
class BITable:
    ...
    def field_url(self, field_id):
        """ Builds URL with field id """
        return posixpath.join(self.url_table, "fields", field_id)
    ...
    def add_field(self, field_name, type, property=None,description=None,ui_type=None):
        json_data = {"field_name": field_name,
                     "type": type}
        if property:
            json_data["property"] = property
        if description:
            json_data["description"] = description
        if ui_type:
            json_data["ui_type"] = ui_type
        return self._post(
            self.url_table + "/fields", json_data=json_data,
        )

    def update_field(self, field_id, field_name, type, property=None ,description=None ,ui_type=None):
        field_url = self.field_url(field_id)
        json_data = {"field_name": field_name,
                     "type": type}
        if property:
            json_data["property"] = property
    ...
    def delete_field(self, field_id):
        ...
        >>> bitable.delete_field(record['id'])
...
def main():
    ...
    # result = table.add_field(field_name='测试新增', type=1)
    # result = table.update_field(field_id='fldzjwQMB1', field_name='测试新增', type=2)
    # result = table.delete_field(field_id='fldi76mPZV')
    # print(result)
