...
Refer to the :any:`Bitable` class to verify which kwargs can be
...
class BitableParams(object):
    class MaxRecordsParam(_BaseParam):
        ...
        >>> bitable.get_all(max_records=10)
    ...
    class FormulaParam(_BaseParam):
        ...
        `bitable Docs on formulas. `_

        Usage - Text Column is not empty:

        >>> bitable.get_all(formula="NOT({COLUMN_A}='')")

        Usage - Text Column contains:

        >>> bitable.get_all(formula="FIND('SomeSubText', {COLUMN_STR})=1")

        Args:
            formula (``str``): A valid bitable formula.
        ...
        def from_name_and_value(field_name, field_value):
            ...
            formula = 'CurrentValue.[{name}]={value}'.format(name=field_name, value=field_value)
            print(formula)
            return formula
    ...
    class _OffsetParam(_BaseParam):
        ...
        >>> bitable.get_iter(offset='recjAle5lryYOpMKk')
    ...
    class FieldsParam(_BaseStringArrayParam):
        ...
        >>> bitable.get(fields='ColumnA')

        Multiple Columns:

        >>> bitable.get(fields=['ColumnA', 'ColumnB'])
    ...
    class SortParam(_BaseObjectArrayParam):
        ...
        >>> bitable.get(sort='ColumnA')

        Multiple Columns:

        >>> bitable.get(sort=['ColumnA', '-ColumnB'])

        Explicit Directions:

        >>> bitable.get(sort=[('ColumnA', 'asc'), ('ColumnB', 'desc')])
