import pytest
from test_tc.utility.configuration_utils import *
...
def test_get_configuration():
    ...
    expected_config =     {'logging': {'logging_folder': 'logs'},
                           'data_paths': {'data_folder': 'tests/data', 'experiment_folder': 'tests/experiments/mlruns', 'predict_save_folder': 'data/output/output_predictions', 'predict_real_folder': 'tests/data/models'}, 
...
def test_check_missing_data_strategy():
    ...
    with pytest.raises(InputTypeError, match=re.escape(f"test_tc.utility.configuration_utils.check_missing_data_strategy encountered an error: Invalid value for {invalid_string_strategy}. Value not found in {valid_possible_strategies} - ")):
    ...
    with pytest.raises(InputTypeError, match=re.escape(f"test_tc.utility.configuration_utils.check_missing_data_strategy encountered an error: Invalid value for not_interpolation_or_order. Value not found in {valid_possible_dictionary_keys} - ")):
        check_missing_data_strategy(invalid_dict_strategy1, valid_possible_strategies, 
                                    valid_possible_dictionary_keys, valid_possible_interpolation_methods)

    with pytest.raises(InputTypeError, match=re.escape(f"test_tc.utility.configuration_utils.check_missing_data_strategy encountered an error: Invalid value for invalid_method. Value not found in {valid_possible_interpolation_methods} - ")):
