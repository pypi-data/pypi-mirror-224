...
def DEiso_anal_single( df, gene_names, groups, ref_group, norm = True, log = False, 
...
def mc_core_DEiso_anal_per_gene_old( rs_tuple ):

    dfs, groups, gr, norm, log, n_pca_comp, rho, nth, g = rs_tuple
...
def DEiso_anal_m_old( df, gene_names, groups, ref_group, norm = True, log = False, 
                n_pca_comp = 0, rho = 0.1, nth = 0.8, verbose = True, n_cores = 8 ):
    ...
    for k, g in enumerate(glst):
        ...
        if len(idx) >= 2:
            ...
            if bt.sum() >= dfs.shape[1]: 
                ...
                rs_tuple.append( (dfs, groups, gr, norm, log, n_pca_comp, rho, nth, g ) )

        if verbose: 
            if k%100 == 0: 
                print('DEiso preparing: %i/%i(%i)   ' % (k, len(glst), cnt), end = '\r', flush=True)

    num_core = min( n_cores, (cpu_count() - 1) )
    pool = Pool(num_core)
    cnt = 0
    for v, g in tqdm(pool.imap_unordered(mc_core_DEiso_anal_per_gene, rs_tuple), total=len(rs_tuple)):
        if v is not None:
            if isinstance(v, dict):
                keys = v.keys()
                if cnt == 0:
...
def mc_core_DEiso_anal_per_gene( rs_tuple ):
    ...
    df, groups, gr, norm, log, n_pca_comp, rho, nth, g = rs_tuple
    
    df_res = DEiso_anal_single( df, g, groups, ref_group = gr,
        ...
        for k, g in enumerate(glst):
            ...
            if len(idx) >= 2:
                ...
                if bt.sum() >= dfs.shape[1]: 
                    ...
                    if cnt == chunk_size:
                        b = gene_names.isin(glst_tmp)
                        gns = gene_names_tmp # gene_names[b]
                        rs_tuple.append( (dfx, groups, gr, norm, log, n_pca_comp, rho, nth, gns ) )
                        cnt = 0
