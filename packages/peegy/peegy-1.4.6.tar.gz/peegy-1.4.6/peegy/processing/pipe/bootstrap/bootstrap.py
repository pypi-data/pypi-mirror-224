...
class Bootstrap(InputOutputProcess):
    ...
    def transform_data(self):
        ...
        if self.origin_input_process is not None:
            ...
            self._origin_input_process = copy.deepcopy(list(self.at_each_bootstrap_do.values())[0].input_process)
        ...
        for _key, _process in list(self.at_each_bootstrap_do.items()):
            if _process.input_process.name == self._origin_input_process.name:
                self.at_each_bootstrap_do[_key].input_process = self._origin_input_process
        ...
        # compute statistics
        print('Generating statistical table {:1.3} seconds'.format(end_time - start_time))
...
@ray.remote
def ray_worker_bootstrap_pool(ref_process: InputOutputProcess = None,
                              ref_data: np.array = None,
                              pipe_pool: PipePool = None,
                              bootstrap_targets: [BootstrapTarget] = None,
                              epoch_length: u.Quantity = None,
                              n_events: int = None,
                              event_code: float = 1,
...
def worker_bootstrap_pool(ref_process: InputOutputProcess = None,
                          ref_data: np.array = None,
                          pipe_pool: PipePool = None,
                          bootstrap_targets: [BootstrapTarget] = None,
                          epoch_length: u.Quantity = None,
...
def catch_tables(pipeline: PipePool = None,
                 targets: [BootstrapTarget] = None):
    output = []
    for _source, _value in pipeline.items():
        for _target in targets:
...
def table_list_to_pd(tables: [pd.DataFrame] = None,
                     targets: [BootstrapTarget] = None):
    output = pd.DataFrame()
    for _target in targets:
