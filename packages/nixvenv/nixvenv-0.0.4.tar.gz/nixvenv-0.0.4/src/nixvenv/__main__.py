...
def main(config_file="./config.json", new=False, delete=False):
    ...
    p = subprocess.run("whoami", shell=True,
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    username = p.stdout.decode('utf-8').strip()
    p = subprocess.run("hostname", shell=True,
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    hostname = p.stdout.decode('utf-8').strip()
    p = subprocess.run("echo $SHELL", shell=True,
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    local_bin_folder = os.getcwd() + "/" + nvenv_name + \
        "/" + config["local_bin"]
...
def load_config():
    ...
    cmds += 'export PS1="' + username + \
        ' [' + config["nvenv_name"] + '@' + hostname + '] :> "\n'
...
def new_environment(new_flag=False):
    ...
    for cmd in init_cmds_list:
        ...
        if (cmd == "here"):
            ...
            if not os.path.exists(running_dir):
                raise Exception(
                    "Running directory does not exist: " + running_dir)
    ...
    for file in files:
        ...
        if os.path.exists(destination):
            raise Exception(
                "[x] Destination file already exists: " + destination)
...
if __name__ == '__main__':
    ...
    # Sanity checks
    if not os.path.exists(params["config_file"]) and not params["operation"] == "init":
        raise Exception("[x] Config file does not exist: " +
                        params["config_file"])
    ...
    if params["operation"] == "activate" or params["operation"] == "run":
        ...
        main(config_file=params["config_file"], new=True, delete=False)
