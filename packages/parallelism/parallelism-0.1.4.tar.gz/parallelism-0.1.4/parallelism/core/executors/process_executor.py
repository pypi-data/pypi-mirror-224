from __future__ import annotations

from multiprocessing import Process
from typing import TYPE_CHECKING

if TYPE_CHECKING:
    from multiprocessing.managers import DictProxy
    from typing import Any

__all__ = ('ProcessExecutor',)


class ProcessExecutor(Process):
    def __init__(self, proxy: DictProxy, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        proxy['run'] = False
        proxy['start'] = False
        proxy['join'] = False
        proxy['terminate'] = False
        proxy['kill'] = False
        proxy['close'] = False
        self.proxy = proxy

    def __getstate__(self):
        state = self.__dict__.copy()
        if configuration := state.get('_config'):
            if authentication_key := configuration.get('authkey'):
                state['_config']['authkey'] = bytes(authentication_key)
        return state

    def run(self) -> None:
        self.proxy['run'] = True
        super().run()

    def start(self) -> None:
        self.proxy['start'] = True
        super().start()

    def join(self, timeout: float = None) -> None:
        self.proxy['join'] = True
        super().join(timeout)

    def terminate(self) -> None:
        self.proxy['terminate'] = True
        super().terminate()

    def kill(self) -> None:
        self.proxy['kill'] = True
        super().kill()

    def close(self) -> None:
        self.proxy['close'] = True
        super().close()
