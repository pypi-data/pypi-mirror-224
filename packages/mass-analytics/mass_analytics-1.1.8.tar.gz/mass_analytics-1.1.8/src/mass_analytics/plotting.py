...
def plot_AVM(dates, y_true, y_pred, dates_test=None, y_true_test=None, y_pred_test=None):
    ...
    df = df.groupby(by='date').agg('sum').reset_index()
    ...
    if dates_test is not None and y_true_test is not None and y_pred_test is not None:
        ...
        df_test = df_test.groupby(by='dates_test').agg('sum').reset_index()
...
def plot_stacked_vs_lines(df,date_col, stacked_cols, lines_cols, two_axis=False, agg=None):
    """
    Create a plot with stacked bars and line plots to compare data trends over time.
    ...
    stacked_cols : list-like
        List of column names to be represented as stacked bars in the plot.
    ...
    with stacked bars for some columns and line plots for others. It is useful for visualizing
    and comparing multiple data series with different scales and formats.
    ...
    for col in stacked_cols:
        fig.add_trace(go.Bar(name=col, x=new_df[date_col], y=new_df[col]))
