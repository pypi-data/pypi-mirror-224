...
def extract_ctype(
    ...
    # ? Regex
    extracted_data.loc[extracted_data[name_col].notna(), "customer_type"] = None
    ...
    for ctype in level_name_type_regex.keys():
        regex_ctype = "|".join(
            [
                f"(^|[^a-z0-9]){term}($|[^a-z0-9])"
                for term in level_name_type_regex[ctype]
            ]
        )
        # Only work with the non-predict type
        ctype_mask = (extracted_data[f"de_{name_col}"].str.contains(regex_ctype)) & (
    ...
    # * Fill customer
    extracted_data["customer_type"] = extracted_data["customer_type"].fillna("customer")
