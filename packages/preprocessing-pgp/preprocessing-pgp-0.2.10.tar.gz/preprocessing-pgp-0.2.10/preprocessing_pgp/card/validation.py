...
class PersonalIDValidator(CardValidator):
    ...
    def is_valid_old_card(card_id: str) -> bool:
        ...
        if not PersonalIDValidator.is_old_card(card_id):
            return False
        return any(
            (card in OLD_PID_REGION_CODE_NUMS for card in [card_id[:2], card_id[:3]])
        )
...
class DriverLicenseValidator(CardValidator):
    ...
    def is_real_driver_license(card_id: str) -> bool:
        ...
        if PersonalIDValidator.is_valid_new_card(card_id):
            ...
            return (first_year_char not in INVALID_DRIVER_LICENSE_FIRST_YEAR_CHAR) and (
                second_year_char in VALID_DRIVER_LICENSE_LAST_YEAR_CHAR
            )
...
def parse_gender_from_card(
    ...
    profile.loc[new_pid_mask & semi_card_mask, card_col] = "0" + profile[card_col]

    # * Map gender by dict
    profile["gender"] = None
    profile.loc[new_pid_mask, "gender"] = profile[card_col].str[3].map(gender_dict)

    # * Gender from driver license
    driver_license_mask = (profile["is_driver_license"]) & (profile["gender"].notna())
...
def parse_city_from_card(
    ...
    # * Driver License
    driver_license_mask = (profile["is_driver_license"]) & (~profile["is_personal_id"])
    profile.loc[driver_license_mask, "code"] = "0" + profile[card_col].str[0:2]
...
def extract_card_info(profile: pd.DataFrame, card_col: str = "card_id") -> pd.DataFrame:
    """
...
def verify_card(
    ...
    data["is_personal_id"] = data[card_col].apply(PersonalIDValidator.is_valid_card)
    ...
    data["is_passport"] = data[card_col].apply(PassportValidator.is_valid_card)
    ...
    data["is_driver_license"] = data[card_col].apply(
...
def process_verify_card(
    ...
    card_data = data
    clean_card_col = f"clean_{card_col}"
    ...
    # ? VALIDATE CARD ID
    if logging_info:
        print(">>> Verifying card id...")
    start_time = time()
    validated_data = parallelize_dataframe(
        clean_data,
        verify_card,
        n_cores=n_cores,
        card_col=clean_card_col,
        print_info=False,
    )
    ...
    # ? FIX MISSING 0 CARD
    validated_data.loc[
        validated_data["is_personal_id"]
        & validated_data[clean_card_col].apply(
            PersonalIDValidator.is_semi_correct_length
        ),
        clean_card_col,
    ] = (
        "0" + validated_data[clean_card_col]
    )
