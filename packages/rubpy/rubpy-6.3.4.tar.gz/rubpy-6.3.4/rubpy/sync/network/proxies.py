...
class BaseSocketWrapper(object):
    ...
    def _resolve_addr(self, host, port):
        addresses = socket.getaddrinfo(host=host, port=port,
    ...
    def connect(self, address):
        ...
        try:
            self._socket.connect((self._socks_host, self._socks_port))
        except OSError as x:
            self.close()
            raise exceptions.SocksConnectionError(
                x.errno,
                f'Can not connect to proxy {self._socks_host}:{self._socks_port} [{x.strerror}]'
            ) from x
    ...
    def _socks_connect(self):
        ...
        try:
            ...
            if self._resolver:
                ...
                _, host = self._resolve_addr(self._dest_host,
                                             self._dest_port)
                host_bytes = socket.inet_aton(self._dest_host)
...
class Socks5SocketWrapper(BaseSocketWrapper):
    ...
    def _socks_auth(self):
        ...
        self._send([0x05, len(auth_methods)] + auth_methods)
        respond = self._receive(2)
        if respond[0] != 0x05:
    ...
    def _socks_connect(self):
        req_addr, resolved_addr = self._build_dest_address()
        self._send([0x05, 0x01, 0x00] + req_addr)
        respond = self._receive(3)
        if respond[0] != 0x05:
            raise exceptions.InvalidServerVersion(
                f'Unexpected SOCKS version number: {respond[0]}')
        ...
        return resolved_addr, self._read_binded_address()

    def _build_dest_address(self):
        ...
        family, _ = self._resolve_addr(host=self._dest_host,
    ...
    def _read_binded_address(self):
        atype = (self._receive(1))[0]
        if atype == 0x01:
            addr = self._receive(4)
            addr = socket.inet_ntoa(addr)
        elif atype == 0x03:
            length = self._receive(1)
            addr = self._receive(ord(length))
        elif atype == 0x04:
            addr = self._receive(16)
            addr = socket.inet_ntop(socket.AF_INET6, addr)
        else:
            raise exceptions.InvalidServerReply(
                'SOCKS5 proxy server sent invalid data')
        port = self._receive(2)
