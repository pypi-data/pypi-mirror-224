...
ROOT_MACROS = \
{
    ...
    namespace RFUtils{
            ...
            if (pdfList.getSize() == 1){
                ...
                while ((component = (RooAbsArg *)iter->Next())){
                    if ((std::string) component->ClassName() == "RooProdPdf"){
                        RooProdPdf *thisPdf = dynamic_cast<RooProdPdf *>(component);
                        if (thisPdf)
                            GetProdPdfBaseComponents(thisPdf, components);
                    }
                    else{
        ...
        RooArgSet SelectConstantArgs(RooArgSet* args, bool isConstant=true){
            RooArgSet result;
            const size_t n_args = args->size();
            for (size_t i = 0; i < n_args; i++)
                if (((RooRealVar*)(*args)[i])->isConstant() == isConstant)
                    result.add(*((*args)[i]));
            return result;
        }
