...
from qualys_etl.etld_policy_compliance import policy_compliance_04_extract_from_qualys


def begin_policy_compliance_03_extract_controller(message=""):
    etld_lib_functions.logger.info(f"start {message}")


def end_policy_compliance_03_extract_controller(message=""):
    etld_lib_functions.logger.info(f"end  {message}")


def get_next_batch(json_file=None):
    policy_compliance_batch_status = {}
    with etld_lib_config.policy_compliance_open_file_compression_method(json_file, "rt", encoding='utf-8') as read_file:
        policy_compliance_dict = json.load(read_file)
        if 'hasMore' in policy_compliance_dict.keys():
            policy_compliance_batch_status = {
                'responseCode': policy_compliance_dict['responseCode'],
                'count': policy_compliance_dict['count'],
                'hasMore': policy_compliance_dict['hasMore'],
                'lastSeenAssetId': policy_compliance_dict['lastSeenAssetId'],
                          }

        if 'hasMore' not in policy_compliance_batch_status.keys():
            etld_lib_functions.logger.error("Error downloading records")
            etld_lib_functions.logger.error(f"policy_compliance_batch_status: {policy_compliance_batch_status}")
            exit(1)

    return policy_compliance_batch_status


def start_multiprocessing_transform_json_to_sqlite():

    if etld_lib_config.policy_compliance_json_to_sqlite_via_multiprocessing is True:
        batch_queue_of_file_paths, batch_queue_process = \
            policy_compliance_05_transform_load_json_to_sqlite.\
...
def get_policy_compliance_data():

    begin_policy_compliance_03_extract_controller()
    last_seen_asset_id = 0
    if etld_lib_config.policy_compliance_last_seen_asset_id_for_restart != '0':
        last_seen_asset_id = etld_lib_config.policy_compliance_last_seen_asset_id_for_restart
    ...
    etld_lib_config.remove_old_files(
        dir_path=etld_lib_config.policy_compliance_extract_dir,
        dir_search_glob=etld_lib_config.policy_compliance_extract_dir_file_search_blob,
        other_files_list=etld_lib_config.policy_compliance_data_files
    )
    etld_lib_config.remove_old_files(
        dir_path=etld_lib_config.policy_compliance_extract_dir,
        dir_search_glob=etld_lib_config.policy_compliance_extract_dir_file_search_blob_two,
    )
    etld_lib_config.remove_old_files(
        dir_path=etld_lib_config.policy_compliance_distribution_dir,
        dir_search_glob=etld_lib_config.policy_compliance_distribution_dir_file_search_blob
    )
    batch_queue_of_file_paths, batch_queue_process = \
    ...
    file_info_dict = \
        etld_lib_config.prepare_extract_batch_file_name(
            ...
            extract_dir=etld_lib_config.policy_compliance_extract_dir,
            file_name_type="policy_compliance_count",
            file_name_option="assetLastUpdated",
            file_name_option_date=etld_lib_config.policy_compliance_asset_last_updated,
            file_extension="json",
            compression_method=etld_lib_config.policy_compliance_open_file_compression_method)

    policy_compliance_04_extract_from_qualys.policy_compliance_extract_count(
        asset_last_updated=etld_lib_config.policy_compliance_asset_last_updated,
    ...
    while has_more_records == '1':
        ...
        file_info_dict = \
            etld_lib_config.prepare_extract_batch_file_name(
                ...
                next_batch_date=utc_datetime,
                extract_dir=etld_lib_config.policy_compliance_extract_dir,
                file_name_type="policy_compliance",
                file_name_option="assetLastUpdated",
                file_name_option_date=etld_lib_config.policy_compliance_asset_last_updated,
                file_extension="json",
                compression_method=etld_lib_config.policy_compliance_open_file_compression_method)
        ...
        cred_dict = policy_compliance_04_extract_from_qualys.policy_compliance_extract(
            asset_last_updated=etld_lib_config.policy_compliance_asset_last_updated,
            last_seen_assetid=str(batch_info['lastSeenAssetId']),
            batch_number_str=batch_number_str,
            qualys_headers_dict=qualys_headers_dict,
        ...
        has_more_records = test_system_option(
            ...
            number_of_files_to_extract=etld_lib_config.policy_compliance_test_number_of_files_to_extract,
            test_system_flag=etld_lib_config.policy_compliance_test_system_flag
        )
    ...
    end_policy_compliance_03_extract_controller()
...
def main():
    get_policy_compliance_data()


if __name__ == "__main__":
    etld_lib_functions.main(my_logger_prog_name='policy_compliance_03_extract_controller')
