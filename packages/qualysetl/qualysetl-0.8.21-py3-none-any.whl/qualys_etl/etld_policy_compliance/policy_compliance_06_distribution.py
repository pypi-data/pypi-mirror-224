...
def begin_policy_compliance_06_distribution():
    etld_lib_functions.logger.info(f"start")


def end_policy_compliance_06_distribution():
...
def extract_csv_from_database(test_extract=False):
    ...
    if test_extract or etld_lib_config.test_system_do_not_test_intermediary_extracts_flag:
        etld_lib_csv_distribution.distribute_csv_data_for_one_workflow(
            distribution_csv_flag=etld_lib_config.policy_compliance_distribution_csv_flag,
            etl_workflow='policy_compliance_06_distribution',
            distribution_csv_flag_name='policy_compliance_distribution_csv_flag'
        )
    else:
        etld_lib_csv_distribution.distribute_csv_data_for_one_workflow(
            distribution_csv_flag=etld_lib_config.policy_compliance_distribution_csv_flag,
            etl_workflow='policy_compliance_etl_workflow',
            distribution_csv_flag_name='policy_compliance_distribution_csv_flag',
...
def main(test_extract=False):
    begin_policy_compliance_06_distribution()
    extract_csv_from_database(test_extract)
    end_policy_compliance_06_distribution()


if __name__ == "__main__":
    etld_lib_functions.main(my_logger_prog_name='policy_compliance_06_distribution')
