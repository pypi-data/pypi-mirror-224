...
################################################################
@ftracer
def build_HolidaysCSV_V1(file='C:\pypjts\TradeDatetime\Holidays.xlsx'):
...
def build_HolidaysText(data=None):
    if data is None:
        data = krx.fetch_holidays()
    else:
        pass 

    df = pd.DataFrame(data)
    dates = list(df.calnd_dd)
    text = "|".join(dates)

    _dir = os.path.dirname(__file__)
    file = os.path.join(_dir, 'Holidays.txt')
    with open(file, 'w', encoding='utf-8') as f:
        f.write(text)
        f.close()
    logger.info(['휴장일 텍스트파일 생성완료.', file])
...
def get_Holidays(): 
    ...
    dates = text.split('|')
    return [datetime.strptime(t, "%Y-%m-%d").astimezone() for t in dates]
