...
class ParaSpacePlanner(engines.Engine, engines.mixins.OneshotPlannerMixin):
        ...
        for combination in all_combinations:
            ...
            if len(combination) > 1:
                for c in combination[1:]:
                    fluent_name += ", " + c
            fluent_name += ")"
            grounded_fluents.append(Fluent(name=fluent_name,typename=fluent.type,environment=fluent.environment))
        ...
        if fnode.type.is_bool_type():
            if fnode.is_equals():
                ...
                if fnode.fluent().type.is_bool_type():
                    ...
                    raise "Not supported type"+ str(fnode.node_type) + str(fnode.fluent().name)
            elif fnode.is_not():
                if len(fnode.args) > 1:
                ...
                raise Exception("Not supported precondition operator kind" + str(fnode.node_type))
        else: 
    ...
    def _solve(
        ...
        domains = {ut.name: [obj.name for obj in problem.objects(ut)]
        ...
        for key in inits:
            static_tokens[str(key)].append(pps.StaticToken(value=str(inits[key]),
                        const_time=pps.fact(0,None),capacity=0,conditions=[]))
        for goal in problem.goals:
            if goal.is_equals():
                ...
                static_tokens[timeline].append(pps.StaticToken(value=value,
                                            const_time=pps.goal(),capacity=0,conditions=[]))
            elif goal.is_fluent_exp():
                static_tokens[str(goal)].append(pps.StaticToken(value="true",
                                            const_time=pps.goal(),capacity=0,conditions=[]))
            elif goal.is_not() and goal.args[0].is_fluent_exp():
                static_tokens[str(goal.args[0])].append(pps.StaticToken(value="false",
                                            const_time=pps.goal(),capacity=0,conditions=[]))
            else:
                    ...
                    if fluent.name in frame_cond_dict.keys():
                        ...
                        if value in frame_cond_dict[fluent.name].keys():
                            ...
                            if len(possible_previous_values) == 0:
                                token_types.append(pps.TokenType(
                                    value=value,
                                    conditions=[pps.OrCond(actions_with_this_effect)],
                                    duration_limits=(1,None),capacity=0))
                            else:
                                token_types.append(pps.TokenType(
                                    value=value,
                                    conditions=[pps.OrCond(actions_with_this_effect),pps.OrCond(possible_previous_values)],
                                    duration_limits=(1,None),capacity=0))

                timelines += [pps.Timeline(
                    name=fluent.name,
                ...
                for cii in vii.conditions:
                    if isinstance(cii,pps.OrCond):
                        ...
                        for dii in cii.disjuncts:
                            print("      ","OR ", str(dii.temporal_relation)[17:], " : ", dii.timeline, " == ", dii.value)
                    else:
                        print("      ", str(cii.temporal_relation)[17:], " : ", cii.timeline, " == ", cii.value)
        ...
        for tl in solution.timelines:    
            for token in tl.tokens:
                print(tl.name," = ", token.value, "[",token.start_time,",",token.end_time,"]")
        
        plan = self._decompile(solution)
        return plan
