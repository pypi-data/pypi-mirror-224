...
def download_attachments(response, folder_name):
    ...
    folder_name : str
        The folder path in which to save the attachment.
...
def readmails(username, password, tag, folder_name, sender, n_mails=None, min_date=None):
    """
    ...
    for p in range(messages, 0, -1):
        ...
        if min_date is not None and str(datetime_obj.date()) < min_date: continue
        datetimeorder.update({(email_message['Subject'], datetime_obj) : p})
        if n_mails is not None and len(datetimeorder) >= n_mails: break
