...
class ProjectString(ProjectInIt):
    ...
    def get_api_fun_str(self, name, row):
        ...
        api_str = '@allure-1.0.step(title="调接口：%s")\n' % row.get("path").split('{')[0]
        api_str += "def " + name + "("
    ...
    def get_assert_header_str(self, class_name, sql_path):
        header_str = f"import allure-1.0\n\n"
    ...
    def get_api_fun_fun_str(self, fun_name):
        ...
        api_fun_fun_str = f"    @allure-1.0.step(title=\"{api.title or api.url}\")\n"
    ...
    def get_api_testcase_header_str(self, fun_name):
        ...
        header_str += "@allure-1.0.epic(\"接口测试\")\n"
        header_str += f"@allure-1.0.feature(\"场景：接口（{api.url}）\")\n"
    ...
    def get_api_testcase_str(self, fun_name):
        ...
        def get_str(n1="", n2="", n3="", severity="normal", is_smoke=False, is_success=True, is_get=True):
            ...
            # _str += f'    @allure-1.0.severity("{severity}")  # 优先级，包含blocker, critical, normal, minor, trivial\n'
            ...
            # _str += f"    @allure-1.0.title(\"{api.title or api.url}{'__' + n1 if n1 else ''}\")\n"
