...
def records(nda:str, rename:bool = False,step_cycle:bool = False):
    '''
    ## Parameters
    ...
    if (nda.split('.')[-1] != 'nda'):
        raise ValueError("File passed in function is not an nda file")
    df = nda_version_8_0.nda_in_df_out(nda,step_cycle)
    rec_columns = {
        'record_ID': 'DataPoint',
        'cycle': 'Cycle Index',
        'step_ID': 'Step Index',
...
def recipe(df):
    ...
    dict_recipe_range={}
    for x in dict_temp:
        dict_recipe_range[iter]=dict_temp[x]
        iter+=1
...
def nda_to_excel(nda:str,save_path:str):
    ...
    ----------
    ----------
    Returns cycle,step,record sheets same as generated by the BTS software. 
    '''
    writer = pd.ExcelWriter(save_path)
    
    cycle(nda).to_excel(writer,sheet_name='cycle',index=False)
    step(nda).to_excel(writer,sheet_name='step',index=False)
    records(nda).to_excel(writer,sheet_name='records',index=False)
    writer.close()
