...
def records(nda:str, rename:bool = False,step_cycle:bool = False,include_aux:bool = False):
    '''
    ## Parameters
...
def cycle(df,last_cycle_skip:bool = False):  #! Function to group the data cycle-wise
    ...
    try:
        ...
        raise ValueError('Argument pushed into the function is neither an appropiate DataFrame nor a path to an nda file')
    rec_columns = {
        'DataPoint':'record_ID',
        'Cycle Index':'cycle',
        'Step Index':'step_ID',
    ...
    if keycheck == -1:
        raise ValueError('Wrong dataframe entered. Kindly check the coloumn names. Or pass the nda file.')
    ...
    if('DCIR (mOhm)' not in df.keys()):
        ...
        df['DCIR (mOhm)']=-1
        df.loc[((df['prev_cur']==0)&(df['current_mA']!=0)),'DCIR (mOhm)']=abs((df['voltage_V']-df['prev_vol'])/(df['current_mA']-df['prev_cur']))*1000000
    ...
    lastcycle_counter = 0
    if (df[df['cycle']==max(df['cycle'])].step_ID.nunique() == df.groupby(['cycle']).step_ID.nunique().mode()[0]):
        lastcycle_counter = 1
    ...
    for i in range(min(df['cycle']),max(df['cycle']+lastcycle_counter)):                         
        ...
        dchg_starting_volt=df_chg.at[0,'voltage_V']
        ...
        charging_time='{:02}:{:02}:{:02}'.format(chgtime//3600, chgtime%3600//60, chgtime%60)
        dchgtime = df_dchg.iloc[-1].at['time_in_step']
        discharging_time='{:02}:{:02}:{:02}'.format(dchgtime//3600, dchgtime%3600//60, dchgtime%60)
    ...
    df3=pd.DataFrame(complete_list,columns=['Cycle Index',
                                            'Onset Date',
                                            'End Date',
                                            'Chg. Cap.(Ah)',
                                            'DChg. Cap.(Ah)',
...
def step(df):  #! Function to group the data step-wise
    ...
    try:
        ...
        raise ValueError('Argument pushed into the function is neither an appropriate DataFrame nor a path to an nda file')
...
def recipe(df):
    ...
    df_recipe=pd.DataFrame(zip(np.array(stepid).reshape(-1,1),
                            ...
                            np.array(cutoff_vol).reshape(-1,1)),columns=['Step_Id','Cycle','Step_Name','Voltage','Current','Rest_time','Cutoff_current','Cutoff_voltage'])                      
    ...
    for i in range(len(recipe_cycle)-1):
        ...
        if((df_temp1.reset_index(drop=True).shape)==(df_temp2.reset_index(drop=True).shape) and _df_diff(df_temp1.reset_index(drop=True),df_temp2.reset_index(drop=True))==1):
            continue
        else:
...
def nda_to_excel(nda:str,save_path:str):
    ...
    cycle(nda).to_excel(writer,sheet_name='cycle',engine='xlsxwriter',index=False)
    step(nda).to_excel(writer,sheet_name='step',engine='xlsxwriter',index=False)
    records(nda).to_excel(writer,sheet_name='records',engine='xlsxwriter',index=False)
    writer.close()
