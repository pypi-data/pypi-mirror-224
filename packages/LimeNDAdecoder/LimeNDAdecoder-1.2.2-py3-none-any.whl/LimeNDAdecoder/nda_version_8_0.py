...
# #!IMPORTRANT TEMPORARY CODE FOR TESTING PUPOSES PLS REMOVE
    ...
    if(List[0]<1 or List[1]<1 or List[2]<1 or List[4]<0 or List[5]<1.5 or List[7]<0 or List[8]<0): 
...
def nda_in_df_out(file,include_step_number:bool =False,include_aux:bool = False):
    '''
    #### Params:
    file is the path to the nda file.\n
    Set include_step_number to True if you want the repeating step-id.\n
    Set include_aux to True if you want to have the Auxiliary data. 
    ...
    if(include_step_number == True):
        df['step_cyclic_id'] = df['step_ID']
