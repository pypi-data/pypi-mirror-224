...
class CommonRegistryConfig(scfg.DataConfig):
    name = scfg.Value(None, help='specify a name to query or store or remove', position=1)
...
class DVC_RegisteryCLI(scfg.ModalCLI):
    ...
    class List(CommonRegistryConfig):
        __command__ = 'list'
        must_exist = scfg.Value(False, help='if True, filter to only directories that exist. Defaults to false except on "find", which is True.')
    ...
    class Find(CommonRegistryConfig):
        __command__ = 'find'
        must_exist = scfg.Value(True, help='if True, filter to only directories that exist. Defaults to false except on "find", which is True.')
...
if __name__ == '__main__':
    """
    CommandLine:
        python ~/code/simple_dvc/simple_dvc/registery.py list
    """
    DVC_RegisteryCLI.main()
