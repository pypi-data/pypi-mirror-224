...
class PIH:
    ...
    class EXECUTOR:
        ...
        def python_path(host: str | None = None) -> str:
            ...
            variable_list: list[str] = if_else(DataTool.is_none(host),
                lambda: PIH.OS.variable(WINDOWS.ENVIROMENT_VARIABLES.PATH),
        ...
        def create_command_for_psexec(command: list[str], host: str | None = None, login: str | None = None, password: str | None = None, interactive: bool | None = False, run_from_system_account: bool = False, run_with_elevetion: bool = False) -> list[str]:
            result_command: list[str] = DataTool.check_not_none(interactive, lambda: [["-d", "-i"][interactive]], [])
            host_start: str = r"\\"
            local: bool = PIH.OS.host_is_local(host)
            if not local and not DataTool.is_empty(host):
                result_command.append(("" if host.startswith(host_start) else host_start) + host)
            if run_from_system_account:
                result_command.append("-s")
            if run_with_elevetion:
                result_command.append("-h")
            if not local:
                login = login or PIH.DATA.variable_value(LINK.ADMINISTRATOR_LOGIN)
                password = password or PIH.DATA.variable_value(LINK.ADMINISTRATOR_PASSOWORD)
            return PIH.EXECUTOR.create_command_for_executor(CONST.PSTOOLS.PS_EXECUTOR, result_command + command, login, password)
        ...
        def create_command_for_start_service(role_or_information: ServiceRoles | ServiceInformationBase, interactive: bool | None = False) -> list[str]:
            ...
            local: bool = not PIH.OS.host_is_local(description.host)
        ...
        def process_is_exists(pid: int, host: str | None = None, login: str | None = None, password: str | None = None) -> str:
            ...
            login = "\\".join(
                [AD.DOMAIN_NAME, login or PIH.DATA.variable_value(LINK.ADMINISTRATOR_LOGIN)])
            password = password or PIH.DATA.variable_value(LINK.ADMINISTRATOR_PASSOWORD)
    ...
    class SERVICE:
        ...
        class ADMIN:
            ...
            def start(role_or_information: ServiceRoles | ServiceInformationBase, check_if_started: bool = True, show_output: bool = True) -> bool | None:
                ...
                if DataTool.is_empty(description.service_path):
                    service_file_path = PIH.PATH.join(
                        PATH_FACADE.VALUE, j((description.name, CONST.FACADE.SERVICE_FOLDER_SUFFIX)), PathTool.add_extension(CONST.SERVICE.NAME, FILE.EXTENSION.PYTHON))
                else:
                    service_file_path = PIH.PATH.join(
            ...
            def stop(role_or_information: ServiceRoles | ServiceInformationBase, check_if_started: bool = True, direct_call: bool = False) -> bool:
                description: ServiceDescription = ServiceRoles.description(role_or_information)
                if not check_if_started or PIH.SERVICE.check_accessibility(description):
    ...
    class PATH(PATHS, PathTool):
        ...
        def get_command_for_mount_facade_storage_on_linux_host() -> str:
            return PIH.PATH.get_command_for_mount_storage_on_linux_host(PATHS.FACADE.STORAGE_PATH(), 
                                                                        ...
                                                                        PIH.DATA.variable_value(LINK.ADMINISTRATOR_LOGIN),
                                                                        PIH.DATA.variable_value(LINK.ADMINISTRATOR_PASSOWORD))
        @staticmethod
        def get_file_list_by_directory_info(path: str) -> list[str] | None:
            return PIH.PATH.get_file_list(path, DataTool.if_not_empty(PIH.RESULT.DATA_STORAGE.value(PIH.PATH.get_directory_info_name(
                None, PIH.PATH.path(path)), DirectoryInfo()).data, lambda info: info.last_created_file_timestamp))
    ...
    class DATA(DataTool, StringTool, ListTool, DateTimeTool, EnumTool, FullNameTool):
        ...
        @staticmethod
        def variable_value(variable_name: str, only_local: bool = False) -> Any | None:
            variable_handler: Callable[[str], Any] = PIH.OS.variable
            if only_local:
        ...
        class FORMAT:
            ...
            @staticmethod
            def variable(value: str | None, parse: bool = True) -> str | None:
                if DataTool.is_none(value):
                    return value
                ...
                return PIH.DATA.variable_value(variable_name)
        ...
        class TELEPHONE_NUMBER:
            ...
            def it_administrator() -> str:
                return PIH.DATA.TELEPHONE_NUMBER.by_login(PIH.DATA.variable_value(LINK.ADMINISTRATOR_LOGIN))
    ...
    class OS:
        ...
        @staticmethod
        def os_name() -> str:
            return PIH.OS.variable("OS")

        @staticmethod
        def domain_dns() -> str:
            return PIH.OS.variable("USERDNSDOMAIN")
        
        @staticmethod
        def domain() -> str:
            return PIH.OS.variable("USERDOMAIN")
        ...
        class RESOURCE:
            ...
            def accessibility_by_ping(address_or_ip: str, host: str | None = None, count: int | None = None, check_for_all: bool = True) -> bool:
                ...
                process_result: CompletedProcess = PIH.EXECUTOR.execute(DataTool.check_not_none(host,
                    ...
                    PIH.DATA.variable_value(LINK.ADMINISTRATOR_LOGIN),
                    PIH.DATA.variable_value(LINK.ADMINISTRATOR_PASSOWORD), True), ping_command), True, True)
