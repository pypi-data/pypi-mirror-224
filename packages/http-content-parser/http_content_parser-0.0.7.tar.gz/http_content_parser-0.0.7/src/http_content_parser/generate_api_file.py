...
class GenerateApiFile():
    ...
    def convert_curl_data_to_model(self, curl_file_path, curl_filter=None) -> list[ReqData]:
        ...
        with open(curl_file_path, 'rt') as f:
            ...
            for r in line_num_array:
                ...
                req_model.temp_api_label = self.replace_api_label_chars(
                    url_content['path'][1:]) + '_' + req_model.method
    ...
    def convert_curl_data_to_model_old(self, curl_file_path, curl_filter=None) -> list[ReqData]:
        curl_parser = CurlParser()
        payload_list = []
        with open(curl_file_path, 'rt') as f:
            lines = f.readlines()
