...
def get_price_ranges(price_range: str) -> Tuple[float, float, float]:
    # first, split
...
def get_price_with_unit(price_with_unit: str) -> Tuple[float, str]:
    # sanitize
...
def get_financial_value(lang: str, value_str: str, percent_str: str) -> FinancialValue:
    ...
    if contains_unit(value_str):
        ...
        value = clean_currency(value_str[:-1])
    else:
        value_unit = ""
        value = clean_currency(value_str)
