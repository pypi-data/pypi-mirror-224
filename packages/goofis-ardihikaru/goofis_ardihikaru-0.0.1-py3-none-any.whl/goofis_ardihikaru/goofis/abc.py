...
from src.goofis_ardihikaru.utils.async_class import AsyncClass
from src.goofis_ardihikaru.utils.currency import clean_currency, get_percent_value
...
class AbcParser(AsyncClass, ABC):
    ...
    @staticmethod
    async def fetch_current_price(selector: Selector) -> float:
        """
        fetches current price
        """
        try:
            current_price_str = selector.css(".YMlKec.fxKbKc::text").get()
            return clean_currency(current_price_str)
