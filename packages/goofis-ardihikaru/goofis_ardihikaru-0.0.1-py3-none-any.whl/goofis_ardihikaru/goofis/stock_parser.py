...
from src.goofis_ardihikaru.dto.stock_data import StockData
from src.goofis_ardihikaru.goofis.abc import AbcParser
from src.goofis_ardihikaru.utils.currency import clean_currency, get_price_ranges, get_price_with_unit, \
    get_percent_value, \
    get_financial_value
from src.goofis_ardihikaru.utils.user_agent import build_fake_ua
...
class StockParser(AbcParser, ABC):
    ...
    async def update_general_info(self, general_info_list: List):
        ...
        self.info.general.prev_price_close = clean_currency(general_info_list[self.incr()])
        ...
        self.info.general.pe_ratio = clean_currency(general_info_list[self.incr()])
        ...
        self.info.general.employees = clean_currency(general_info_list[self.incr()])
    ...
    async def parser(self, html: Response):
        ...
        self.info.current_price = await self.fetch_current_price(selector)
    ...
    async def fetch_general_info(selector: Selector):
        ...
        for index, data_results in enumerate(selector.css(".gyFHrc"), start=1):
            ...
            # for website
            if val is None:
                website_selector = data_results.css(".P6K39c")
                val = website_selector.css(".tBHE4e::text").get()
