...
class UTestCase(unittest.TestCase):
    ...
    #     # This will check if a remote version of the data files exist; in this case it will try to sync.
    #     if not self._run_in_report_mode():
    #         # Only check remote when not run in grade mode. When run in grade mode, remote checking is a job for the report.
    #         import inspect
    #         source_code = inspect.getsourcefile(self.__class__)
    #         print(source_code)
    #         # Compare the version string in the source with the remote source version?
    #         # You could implant the report version in the source for the .pkl files.
    #         # you could also not care about the remote test version at all.
    #         self.get
    #
    #
    #         a = 2345
