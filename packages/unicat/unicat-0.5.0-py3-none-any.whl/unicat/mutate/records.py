...
def create_record(unicat, parent, ordering):
    success, result = unicat.api.call(
        "/records/create",
        {"parent": parent.gid, "ordering": ordering},
    )
    if not success:
...
def set_record_channels(unicat, record, channels, enabled):
    success, result = unicat.api.call(
        "/records/channels/set",
        {"record": record.gid, "channels": channels, "enabled": enabled},
    )
    if not success:
...
def copy_record_channels_from_parent(unicat, record, channels=None):
    data = {"record": record.gid}
    if channels:
        data["channels"] = channels
    success, result = unicat.api.call("/records/channels/copy_from_parent", data)
    if not success:
        raise UnicatError("copy_record_channels_from_parent", result)
    return UnicatRecord(unicat, result["record"])


@_job
def copy_record_channels_down(unicat, record, channels=None):
    # becomes copy_record_channels_down(unicat, record, channels=None, *, return_job=False): UnicatRecord | UnicatJob
    data = {"record": record.gid}
    if channels:
        data["channels"] = channels
    success, result = unicat.api.call("/records/channels/copy_down", data)
    if not success:
        raise UnicatError("copy_record_channels_down", result)
    return result["job.token"], record  # unchanged, only descendants are changed


def copy_record_channels_up(unicat, record, channels=None):
    data = {"record": record.gid}
    if channels:
        data["channels"] = channels
    success, result = unicat.api.call("/records/channels/copy_up", data)
    if not success:
        raise UnicatError("copy_record_channels_up", result)
    return record  # unchanged, only ancestors are changed


def set_record_orderings(unicat, record, orderings):
    success, result = unicat.api.call(
        "/records/orderings/set",
        {"record": record.gid, "orderings": orderings},
    )
    if not success:
...
def link_record(unicat, parent, record, ordering):
    success, result = unicat.api.call(
        "/records/link",
        {"parent": parent.gid, "record": record.gid, "ordering": ordering},
    )
    if not success:
...
def permanent_delete_record(unicat, record):
    # becomes permanent_delete_record(unicat, record, *, return_job=False): UnicatRecord | UnicatJob
    success, result = unicat.api.call(
        "/records/permanent_delete", {"record": record.gid}
    )
    if not success:
    ...
    return result["job.token"], UnicatRecord(unicat, result["parent"])
