...
def test_sequences_with_same_length(tmp_datasets_dir):
    ds = deeplake.dataset(tmp_datasets_dir / "sequence_with_same_length", overwrite=True)
    with ds:
        ds.create_tensor("images", htype="sequence[image]", dtype=np.uint8, sample_compression=None)
...
def test_non_standard_jpegs(tmp_datasets_dir):
    ...
    with ds:
        ds.create_tensor("images", htype="link[image]", dtype=np.uint8, sample_compression=None)
        image = Image.fromarray(np.random.randint(0, 255, (20, 20, 3), np.uint8))
        stream = io.BytesIO()
        ...
        f.close()
        ds.images.append(deeplake.link(str(tmp_datasets_dir / "non_standard_jpeg_0.jpg")))
        stream.seek(3)
        stream.write(b'\xe1')
        f = open(tmp_datasets_dir / "non_standard_jpeg_1.jpg", "wb")
        f.write(stream.getbuffer())
        f.close()
        ds.images.append(deeplake.link(str(tmp_datasets_dir / "non_standard_jpeg_1.jpg")))
        stream.seek(3)
        stream.write(b'\xe2')
        f = open(tmp_datasets_dir / "non_standard_jpeg_2.jpg", "wb")
        f.write(stream.getbuffer())
        f.close()
        ds.images.append(deeplake.link(str(tmp_datasets_dir / "non_standard_jpeg_2.jpg")))
        stream.seek(3)
        stream.write(b'\xfe')
        f = open(tmp_datasets_dir / "non_standard_jpeg_3.jpg", "wb")
        f.write(stream.getbuffer())
        f.close()
        ds.images.append(deeplake.link(str(tmp_datasets_dir / "non_standard_jpeg_3.jpg")))
    
    ids = api.dataset(str(tmp_datasets_dir / "non_standard_jpegs"))
    assert ids.tensors[0][0].numpy().shape == (20, 20, 3)
    assert ids.tensors[0][1].numpy().shape == (20, 20, 3)
    assert ids.tensors[0][2].numpy().shape == (20, 20, 3)
    assert ids.tensors[0][3].numpy().shape == (20, 20, 3)
...
def test_multidimensional_tiles_bug(tmp_datasets_dir):
    ...
    with ds:
        ...
        random_tensor = np.random.randint(0, 65535, size=(5, 3679, 5502, 3), dtype=np.uint16)
        entry = {
            'data': random_tensor
        }
        ds.append(entry)
