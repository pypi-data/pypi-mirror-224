...
class TableRenderer:
    ...
    def __init__(self, headers: List[str], data: Iterable, caption: str | None = None):
        ...
        h_len, c_len = len(self._headers), len(min(data, key=len))
        check_argument(h_len >= 1, "Headers are required")
        if data:
            check_argument(c_len == h_len, "Headers and Columns must have the same size: {} vs {}", c_len, h_len)
        self._column_sizes = [0 for _ in range(h_len)]
    ...
    def adjust_cells_auto(self) -> None:
        ...
        for cell, c_size in zip(self._data, sizes):
            sizes = [
                max(len(self._headers[idx]), max(sizes[idx], max(c_size, len(cell[idx]))))
                for idx, _ in enumerate(self._headers)
            ]
        self.set_cell_sizes(*(size for size in sizes))
