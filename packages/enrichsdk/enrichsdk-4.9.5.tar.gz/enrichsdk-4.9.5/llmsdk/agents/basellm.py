...
class BaseLLMAgent(RedisMixin):
    ...
    def _load_search_agent(self, cred, searchapi, llm):
        ...
        tools = [
            Tool(
                ...
                description="Useful for when you need to answer questions about current events. Input must be a search query."
            )
        ]
        # create the agent
    ...
    def _get_token_count(self, content):
        """
        get token count for a given piece of content
        """
        n_tokens = -1
        if self.platform == "openai":
            n_tokens = self.llm.get_num_tokens_from_messages([ChatMessage(content=content, role="human")])
        else:
            msg = f"get_num_tokens_from_messages() is not presently implemented for platform={self.platform}." + \
