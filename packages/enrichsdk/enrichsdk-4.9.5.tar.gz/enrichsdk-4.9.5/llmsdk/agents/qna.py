...
class LLMIndexQuerier(BaseLLMAgent):
    """
...
If the question is framed in active voice, first convert it to passive voice and use that to answer the question.
    ...
    def run_query_internal(self, query):
        ...
        p = self.llm_chain_int.llm_chain.prompt.format(input=query, context=context, chat_history=chat_history)
    ...
    def query(self, query, mode="internal", policy={}):
        ...
        if mode == 'internal':
            ...
            result = self.run_query_external(query)
        elif mode == 'search':
            result = self.run_query_search(query)
        elif mode == 'suggest':
            result = self.run_query_suggest(query)
        else:
