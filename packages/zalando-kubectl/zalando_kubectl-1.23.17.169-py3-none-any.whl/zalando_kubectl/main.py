...
@click_cli.command(
    ...
    - fabric # generate fabricgateway resource with input from API repository
""",
)
...
@click.option(
    "--application",
    "-a",
    help="Application ID to lookup required data by internal APIs",
    required=True,
)
@click.option(
    "--component",
...
@click.option(
    "--team",
    "-t",
    help="Radical Agility Team name that is used to find team members to generate fabric admin access",
    required=False,
)
@click.option(
    "--file",
    "-f",
    help="File to OpenAPI Spec",
    required=False,
)
@click.option(
    "--debug",
...
@click.pass_obj
def generate(env: Environment, typ, application, component, team, file, debug):
    try:
        if typ is None:
        ...
        if typ == "fabric":
            gen.fabric(token, application, component, team, file, debug)
        else:
