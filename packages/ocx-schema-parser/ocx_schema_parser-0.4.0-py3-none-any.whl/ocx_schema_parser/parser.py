...
# Application imports
from ocx_schema_parser import DEFAULT_SCHEMA, PROCESS_SCHEMA_TYPES, SCHEMA_FOLDER, W3C_SCHEMA_BUILT_IN_TYPES
from ocx_schema_parser.data_classes import SchemaSummary, SchemaType, OcxEnumerator
from ocx_schema_parser.elements import OcxSchemaAttribute, OcxChildElement, OcxGlobalElement
...
#        uris = sorted(self.resolve_source(schema_url, recursive=True))
#        self._process_schemas(uris)
        if self._process_schemas(schema_url):
    ...
    def _add_element_to_lookup_table(self, element: Element, target_ns) -> None:
        ...
        if name is not None:
            ...
            if LxmlElement.is_substitution_group(element):
                ...
                self._add_member_to_substitution_group(group, SchemaHelper.unique_tag(LxmlElement.get_name(element),
                                                                                      target_ns))


    def _lookup_tables(self) -> None:
        """Create the global lookup tables of Schema data classes with the tag as key."""

        if self._parser is None:
            raise OcxParserError('No schema has been parsed.')
        ...
        for e in LxmlElement.find_all_children_with_name_and_attribute(root, 'element', "name"):
            self._add_element_to_lookup_table(e,self._parser.get_target_namespace() )
        ...
        for name in names:
            element = LxmlElement.find_all_children_with_attribute_value(root, 'attribute', 'name', name)
            if len(element) == 1:
                self._add_element_to_lookup_table(element[0], self._parser.get_target_namespace())
    ...
    def _parse_schema(self, schema_url: str = DEFAULT_SCHEMA) -> bool:
        ...
        if "http" in schema_url:
            try:
                ...
                return False
        elif not Path(schema_url).exists():
    ...
    def _process_ocx_elements(self):
        ...
        # Enumeration types
        for tag in self._get_schema_types('enumeration'):
            e = self._get_element(tag)
            name = LxmlElement.get_name(e)
            prefix = self._get_prefix_from_namespace(QName(e).namespace)
        ...
        # Simple types
        for tag in self._get_schema_types('simpleType'):
            e = self._get_element(tag)
            name = LxmlElement.get_name(e)
            prefix = self._get_prefix_from_namespace(QName(e).namespace)
            simple_type = self._process_attribute(e, prefix)
            self._simple_types.append(simple_type)
        return
    ...
    def _process_attribute(self, xs_attribute: Element, ns_prefix: str) -> OcxSchemaAttribute:
        ...
        """
        attribute = OcxSchemaAttribute(xs_attribute, ns_prefix)
    ...
    def get_ocx_children_data(self, schema_type: str) -> Dict:
        ...
        """
        if element := self.get_ocx_element_from_type(schema_type):
    ...
    def tbl_attribute_groups(self) -> Dict:
        ...
        return {
            tag: self.get_schema_type_data_class(tag).to_dict()
            for tag in elements
        }
    ...
    def tbl_attribute_types(self) -> Dict:
        ...
        return {
            tag: self.get_schema_type_data_class(tag).to_dict() for tag in elements
        }
    ...
    def get_schema_type_data_class(self, tag: str) -> SchemaType:
    ...
    def _process_schemas(self, schema_url: str) -> bool:
        if "http" in schema_url:
            remote_file = Path(schema_url).name
            file = Path(self._local_folder) / remote_file
            r = requests.get(schema_url)
        ...
        if self._is_parsed:
            ...
            for ns in references:
                url = references[ns]
                self._is_parsed = self._process_schemas(url)
                if not self._is_parsed:
                    break
                if ns not in list(self.get_namespaces().values()):
                    logger.error(f'Mismatched _namespace "{ns}" in xsd with url: "{url}"')
