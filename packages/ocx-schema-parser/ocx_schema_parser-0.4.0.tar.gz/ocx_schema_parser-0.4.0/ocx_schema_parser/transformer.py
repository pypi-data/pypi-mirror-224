...
from typing import Any, Iterator, Dict, List, Union, DefaultDict
# Third party imports
...
# Application imports
from ocx_schema_parser import DEFAULT_SCHEMA, WORKING_DRAFT, TMP_FOLDER, W3C_SCHEMA_BUILT_IN_TYPES, SCHEMA_FOLDER
...
@staticmethod
def download_schema_from_url(url: str , folder: str ) -> bool:
    """"Download the schemas from an url before processing.

    Args:
        url: The location of the schema
        folder: The download folder. Will be created if not existing. If existing,
                    any ``.xsd`` files will be deleted.

    Returns:
        True if the was downloaded , false otherwise:
    """
    schema_folder = Path(folder)
...
class Transformer:
    ...
    def transform_schema_from_folder(self, folder: str = SCHEMA_FOLDER) -> bool:
    ...
    def _transform_schema_from_url(self, url: str, folder:str) -> bool:
