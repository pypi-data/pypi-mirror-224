...
class StrategySunburst:
    ...
    def plot(self, select_strategy=None, path=None, color_continuous_scale='RdBu_r'):
        ...
        if position is None:
            return
        position = position[(position['return'] != np.inf) | (position['weight'] != np.inf)]
        if path is None:
...
class StrategyReturnStats:
    ...
    def _get_returns_set(self):
        ...
        """
        returns_set = {}
        strategy_names = self.s_data.keys() if len(self.strategy_names) == 0 else self.strategy_names
        for s_name in strategy_names:
            try:
                ...
                return_value = round(((returns.iloc[-1] / returns.iloc[0]) - 1) * 100, 2)
    ...
    def plot_strategy_last_return(self):
        ...
        benchmark_last_return = round(((benchmark_return.iloc[-1] / benchmark_return.iloc[0]) - 1) * 100, 2)
    ...
    def plot_strategy_creturn(self):
        ...
        benchmark_creturn = round(((benchmark_return / benchmark_return.iloc[0]) - 1) * 100, 2)
        ...
        fig.update_layout(yaxis_range=(returns_df["creturns"].min() - 0.1, returns_df["creturns"].max() + 0.1))
