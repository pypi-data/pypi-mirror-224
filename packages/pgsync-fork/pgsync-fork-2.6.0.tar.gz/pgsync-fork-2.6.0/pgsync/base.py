    ...
    def parse_logical_slot(self, row: str) -> Payload:
        ...
        if "old-key" and "new-tuple" in suffix:
            ...
            if i > -1:
                ...
                for key, value in _parse_logical_slot(s):
                    sys.stdout.write(f"UPDATE: {key} = {value}\n")
                    sys.stdout.flush()
            ...
            for key, value in _parse_logical_slot(suffix):
                sys.stdout.write(f"NOT UPDATE: {key} = {value}\n")
                sys.stdout.flush()
                payload.new[key] = value

        sys.stdout.write(f"Old: {json.dumps(payload.old)}")
        sys.stdout.flush()
        sys.stdout.write(f"New: {json.dumps(payload.old)}")
        sys.stdout.flush()
        return payload
