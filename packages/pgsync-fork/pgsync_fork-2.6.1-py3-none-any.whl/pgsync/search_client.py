...
def filter_actions(iterable: Iterable[Union[bytes, str, Dict[str, Any]]]) -> None:
    for idx, item in enumerate(iterable):
        if isinstance(item, dict):
            iterable[idx] = {k: v for k, v in item.items() if v != ""}
            sys.stdout.write(f"Filtered: {json.dumps(iterable[idx])}")
            sys.stdout.flush()
