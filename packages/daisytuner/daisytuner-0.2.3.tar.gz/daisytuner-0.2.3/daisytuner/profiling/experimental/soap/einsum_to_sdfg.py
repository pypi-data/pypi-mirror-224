...
def sdfg_gen(
    subscripts: str, arrays: List[np.ndarray] = None, inp_dim: int = 10
) -> dace.SDFG:
    """
    ...
    for cidx, contraction in enumerate(path_info[1].contraction_list):
        ...
        in_memlets["__in0"] = dace.Memlet(
            data=first_inp, subset=",".join(c for c in tokens[0].strip())
        )
        in_memlets["__in1"] = dace.Memlet(
            data=second_inp, subset=",".join(c for c in tokens[1].strip())
        )
        out_memlets = {}
        out_memlets["__out"] = dace.Memlet(
            data=out_name, subset=",".join(c for c in tokens[-1].strip()), wcr=wcr
        )
        state.add_mapped_tasklet(
            state.label,
            map_ranges,
...
def sdfg_multigen(
    subscripts: List[str],
    out_match: Dict[int, Dict[int, Union[int, List[int]]]],
    arrays: List[np.ndarray] = None,
    inp_match: Dict[int, List[int]] = None,
    inp_dim: int = 10,
) -> dace.SDFG:
    """
    ...
    for i, sd in enumerate(sdfgs):
        ...
        for j, inp in enumerate(filtered_inputs[i]):
            if isinstance(inp, tuple):
                ...
                mapping.update(
                    {str(s1): str(s0) for s0, s1 in zip(iarr.shape, oarr.shape)}
                )
        ...
        nsd = state.add_nested_sdfg(
            sd, sdfg, set(inpaccess.values()), set(outaccess.values()), mapping
        )
        for acc, conn in inpaccess.items():
            state.add_edge(
                ...
                nsd,
                conn,
                dace.Memlet.from_array(acc.data, sdfg.arrays[acc.data]),
            )
...
if __name__ == "__main__":
    ...
    sdfg2 = sdfg_multigen(
        ["ik,kj->ij", "il,lj->ij", "ij,jm->im"], {0: {1: -1}, 1: {2: 0}}
    )
    sdfg2.view()
