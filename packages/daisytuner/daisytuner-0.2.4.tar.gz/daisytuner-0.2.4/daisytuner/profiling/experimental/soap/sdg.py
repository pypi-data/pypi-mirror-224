...
from daisytuner.profiling.soap.soap import AccessParams, SoapStatement
from daisytuner.profiling.soap.utils import *
...
class SDG:
    ...
    def add_edge(
        ...
        phi_str = (
            "["
            + _base_access.replace("*", ",")
            + "] "
            + str(access_params.offsets).replace(",)", ")")
        )
        if is_wcr:
            phi_str += "*"
        self.graph.add_edge(
    ...
    def from_SDFG(self, sdfg: dace.SDFG) -> None:
        ...
        sdfg_.apply_transformations_repeated(TaskletFusion)

        sdg_scope = SdgScope()
        dace.propagate_memlets_sdfg(sdfg_)
        sdfg_.save("tmp.sdfg", hash=False)
        # get all output arrays within a given STATE (for SSA purposes)
        sdg_scope.output_arrays = self._get_output_arrays_sdfg(sdfg_, sdg_scope)
        self._from_SDFG_sdfg(sdfg_, sdg_scope)
        ...
        for control_node in control_tree.children:
            if isinstance(control_node, control_flow.SingleState):
                ...
                loop_ranges = {
                    str(k): v
                    for k, v in zip(
                        control_node.guard.ranges.keys(),
    ...
    def _from_SDFG_loop(self, loop: control_flow.ForScope, sdg_scope: SdgScope) -> None:
    ...
    def _from_SDFG_scope(
        ...
        sdg_scope.input_SDFG_arrays += [
            src_node.data
            for src_node in state.source_nodes()
            if isinstance(src_node, AccessNode)
        ]

        snodes = [
            n
            for n in nx.topological_sort(state.nx)
            if n in state.scope_children()[scope]
        ]
        for node in snodes:
            if isinstance(node, (dace.nodes.Tasklet, dace.nodes.LibraryNode)):
                ...
                if len(in_edges) > 0:
                    if isinstance(sdg_scope.SDFG_arrays[node.label], dace.data.View):
                        ...
                        self.add_projection(
                            src_node, state.out_edges(node)[0].data, sdg_scope
                        )
                    ...
                    if sdg_scope.SDFG_arrays[node.data].transient:
                        ...
                        if isinstance(in_node, dace.nodes.Tasklet):
                            ...
                            if len(in_edges) > 0:
                                ...
                                if isinstance(in_in_node, dace.nodes.AccessNode):
                                    ...
                                    if node.data == "_x":
                                        a = 1
                                    tmp = SDG._fuse_mapping(
                                        {node.data: in_edges[0].data},
                                        sdg_scope.sdfg_mapping,
                                    )
                                    if any(
                                        [
                                            len(sp.sympify(i).free_symbols) > 0
                                            for (i, j, k) in list(tmp.values())[
                                                0
                                            ].subset.ranges
                                        ]
                                    ):
                        ...
                        if isinstance(in_node, dace.nodes.MapEntry):
                            if node.data == "_x":
                                a = 1
                            tmp = SDG._fuse_mapping(
                                {node.data: in_edges[0].data}, sdg_scope.sdfg_mapping
                            )
                            if any(
                                [
                                    len(sp.sympify(i).free_symbols) > 0
                                    for (i, j, k) in list(tmp.values())[0].subset.ranges
                                ]
                            ):
                                sdg_scope.sdfg_mapping = {
                                    **sdg_scope.sdfg_mapping,
                        ...
                        if (
                            ...
                            if len(composed_memlet.subset.ranges) == 0 or (
                                len(composed_memlet.subset.ranges) == 1
                                and composed_memlet.subset.ranges[0][0] == 0
                            ):
                                composed_memlet.subset.ranges += (
                                    SDG._dict_ranges_to_list(sdg_scope.ranges)
                                )
                            else:
                                composed_memlet.subset.ranges += (
                                    SDG._dict_ranges_to_list(sdg_scope.loop_ranges)
                                )
                            composed_memlet.subset.ranges = [
                                rng
                                for rng in composed_memlet.subset.ranges
                                if len(sp.sympify(rng[0]).free_symbols) > 0
                            ]
                            sdg_scope.sdfg_mapping[
                                composed_memlet.data
                            ] = composed_memlet
    ...
    def _from_SDFG_node(
        self, node: dace.nodes, state: dace.SDFGState, sdg_scope: SdgScope
    ) -> None:
        ...
        for e in input_edges:
            ...
            if (
                not outer_memlet
                or len(outer_memlet.subset.ranges) == 0
            ...
            if e.data.wcr:
                if (
                    e.data.src_subset is None
                    or e.data.dst_subset.ranges != e.data.src_subset.ranges
                ):
            ...
            if e.data.wcr:
                ...
                if e.data.src_subset is not None and (
                    e.data.dst_subset.ranges != e.data.src_subset.ranges
                ):
                    SSA_dim = e.data.dst_subset
                    a = 1
                else:
    ...
    def _add_ranges_and_mappings(
        self, state: dace.SDFGState, sdg_scope: SdgScope, node: dace.nodes
    ) -> SdgScope:
        inner_scope = copy.deepcopy(sdg_scope)
        inner_scope.sdfg_path.append(node)
        ...
        if len(sdg_scope.ranges) > 0:
            vars_to_add = list(
                map(dace.symbol, [v for v in sdg_scope.map_ranges.keys()])
            )
            ...
            for arr, mem in dst_sdfg_mapping.items():
                ...
                if tmp_mem.data not in sdg_scope.input_SDFG_arrays:
                    vars_to_add_in = vars_to_add + list(
                        map(dace.symbol, [v for v in sdg_scope.loop_ranges.keys()])
                    )
                else:
        ...
        inner_scope.sdfg_mapping = SDG._fuse_mapping(
            cur_scope_mapping, inner_scope.sdfg_mapping
        )
    ...
    def _find_transient_assignement(
        self, state: dace.SDFGState, sdg_scope: SdgScope, node: dace.nodes
    ):
        a = 1
    ...
    def _add_statement_to_SDG(self, S: SoapStatement, sdg_scope: SdgScope) -> None:
        ...
        for output_array in S.output_accesses.keys():
            ...
            output_arrays_vers.append(
                output_array + "_" + str(self.array_version_counter[output_array])
            )

            # check if it's transient
            if (
                output_array in sdg_scope.SDFG_arrays.keys()
                and sdg_scope.SDFG_arrays[output_array].transient
            ):
                out_transient = True
            else:
                out_transient = False
        ...
        for array_name, array_accesses in S.phis.items():
            if array_name in S.output_accesses.keys():
                input_array_ver = (
                    array_name + "_" + str(self.array_version_counter[array_name] - 1)
                )
            else:
                input_array_ver = (
                    array_name + "_" + str(self.array_version_counter[array_name])
                )

            # check if it's transient
            if (
                array_name in sdg_scope.SDFG_arrays.keys()
                and sdg_scope.SDFG_arrays[array_name].transient
            ):
                in_transient = True
            else:
                in_transient = False
            ...
            for base_access, access_params in array_accesses.items():
                for output_array in output_arrays_vers:
                    ...
                    # it is a wcr
                    out_dim = len(
                        S.output_accesses[strip(output_array)].baseAccess.split("*")
                    )
    ...
    def add_projection(
        self, project_from: dace.memlet, project_to: dace.memlet, sdg_scope: SdgScope
    ) -> None:
        """
        ...
        if len(from_ranges) < len(to_ranges):
            ...
            src_memlet.subset.ranges += [(0, 0, 1)] * (
                len(to_ranges) - len(from_ranges)
            )
    ...
    def _get_output_arrays_sdfg(
        self, sdfg: dace.SDFG, sdg_scope: SdgScope
        ...
        for control_node in control_tree.children:
            if isinstance(control_node, control_flow.SingleState):
                ...
                output_arrays = {
                    **output_arrays,
                    **self._get_output_arrays_state(state, None, inner_scope),
                }
                ...
                output_arrays = {
                    **output_arrays,
                    **self._get_output_arrays_loop(control_node, inner_scope),
                }

        return output_arrays

    def _get_output_arrays_loop(
        self, loop: control_flow.ForScope, sdg_scope: SdgScope
    ) -> Dict[str, Tuple[str, Tuple[int]]]:
        output_arrays = {}
    ...
    def _get_output_arrays_state(
        ...
        for node in snodes:
            if isinstance(node, (dace.nodes.Tasklet, dace.nodes.LibraryNode)):
                ...
                output_arrays = {
                    **output_arrays,
                    **self._get_output_arrays_state(state, node, inner_scope),
                }
                ...
                output_arrays = {
                    **output_arrays,
                    **self._get_output_arrays_sdfg(node.sdfg, inner_scope),
                }
                ...
                if len(state.in_edges(node)) > 0 and len(state.out_edges(node)) == 0:
                    ...
                    (arrayName, baseAccess, offsets) = get_access_from_memlet(
                        SDG._memlet_ranges_to_iters(state.in_edges(node)[0].data),
                        iter_vars,
                    )
    ...
    def _get_outer_memlet(
        self,
        inner_memlet: dace.Memlet,
        sdg_scope: SdgScope,
        output: bool,
        single_in_transient_mapped: dace.Memlet = None,
    ) -> dace.Memlet:
        """
        ...
        if new_memlet.data in sdg_scope.sdfg_mapping:
            ...
            if new_memlet.data in [
                "_".join(node.split("_")[:-1])
                for node in self.graph.nodes._nodes.keys()
            ]:
            ...
            offset_variables = set.union(
                *[
                    sp.sympify(x).free_symbols
                    for (x, y, z) in outer_memlet.subset.ranges
                    if (x != y)
                ]
                + [set()]
            )
            outer_memlet.subset.ranges = [
                (0, y, z) if (x != y) else (x, y, z)
                for (x, y, z) in outer_memlet.subset.ranges
            ]
            ...
            if len(offset_variables) > 0:
                ...
                if new_memlet.src_subset is not None:
                    new_memlet.src_subset.ranges = [
                        (rng[0].subs(subs_list), rng[1].subs(subs_list), rng[2])
                        for rng in new_memlet.src_subset
                    ]
                if new_memlet.dst_subset is not None:
                    new_memlet.dst_subset.ranges = [
                        (rng[0].subs(subs_list), rng[1].subs(subs_list), rng[2])
            ...
            if (
                output
                and sdg_scope.SDFG_arrays[new_memlet.data].transient
                and single_in_transient_mapped
            ):
        ...
        if output:
            ...
            try:
                ...
                try:
                    outer_memlet.subset = outer_memlet.dst_subset.compose(
                        new_memlet.src_subset
                    )
                except:
                    pass
        ...
        outer_memlet.subset.ranges = [
            rng
            for rng in outer_memlet.subset.ranges
            if len(sp.sympify(rng[0]).free_symbols) > 0
        ]
        ...
        outer_maps = [
            map_node
            for map_node in sdg_scope.sdfg_path[:mem_scope_ind]
        ...
        if outer_memlet._state is not None:
            ...
            mem_acc_node = [
                n
                for n in state.nx.nodes
                if isinstance(n, AccessNode) and n.data == outer_memlet.data
            ][0]
            scope = 1
        ...
        for outer_map in outer_maps:
            map_ranges = {
                k: v
                for k, v in zip(outer_map.params, outer_map.range.ranges)
                if isinstance(k, str)
            }
            outer_memlet.subset.ranges += rng_to_subset(map_ranges)

        if (
            outer_memlet.subset is None
            or len(outer_memlet.subset.ranges) == 0
            or len(outer_memlet.subset.ranges[0][0].free_symbols) == 0
        ):
            ...
            if (
                outer_memlet.data in sdg_scope.SDFG_arrays.keys()
                and sdg_scope.SDFG_arrays[outer_memlet.data].transient
            ):
                ...
                warn(
                    "Unable to parse memlet "
                    + str(outer_memlet)
                    + " in state "
                    + str(sdg_scope.sdfg_path.path[-1])
                    + "\n"
                )
                # I SUPER dislike this
                return None
            if (
                len(outer_memlet.subset.ranges) > 0
                and len(sp.sympify(outer_memlet.subset.ranges[0][0]).free_symbols) > 0
            ):
                a = 1
    ...
    @staticmethod
    def _get_SSA_dim(
        memlet: dace.Memlet, sdg_scope: SdgScope, preliminary_check=False
    ) -> List[dace.symbol]:
        SSA_dim = []
        if len(sdg_scope.ranges) == 0:
            iter_vars = []
        ...
        if preliminary_check or memlet.data in sdg_scope.output_arrays.keys():
            ...
            (array_name, base_access, offsets) = get_access_from_memlet(
                memlet, iter_vars
            )

            if preliminary_check or eq_accesses(
                base_access, sdg_scope.output_arrays[memlet.data][0]
            ):
                ...
                if len(memlet_iter_vars) == 0 and not hasattr(
                    sdg_scope, "inntermost_ranges"
                ):
                    return SSA_dim
                ...
                potential_ssa_pool = set(
                    sdg_scope.innermost_ranges
                )  # set(sdg_scope.ranges) - set(sdg_scope.innermost_ranges)
                SSA_dim = [
                    [dace.symbol(k), dace.symbol(k), 1]
    ...
    # SDG PARTITIONING
    # ------------------------------------
    def perform_loop_swapping(
        self, node: str, swaplist: Dict[str, str], visited: Set[str], first: bool
    ) -> bool:
        """
        ...
        for e in list(self.out_edges(node)) + list(self.in_edges(node)):
            ...
            if any(
                ((iter in swaplist.keys()) or (iter in inv_swaplist.keys()))
                for iter in _base_access.split("*")
            ):

                e[2]["base_access"] = swap_in_string(
                    e[2]["base_access"], swaplist, inv_swaplist
                )
                e[2]["label"] = (
                    "["
                    + ",".join(e[2]["base_access"].split("*"))
                    + "]"
                    + e[2]["label"].split("]")[1]
                )

            valid_swapping = valid_swapping and (
                self.perform_loop_swapping(in_node, swaplist, visited, False)
            ...
            if len(self.in_edges(node)) == 1 and len(self.out_edges(node)) == 1:
                if self.graph.nodes[node]["transient"] == True:
                    ...
                    if out_base_access != _base_access:
                        if len(out_base_access.split("*")) != len(
                            _base_access.split("*")
                        ):
                            ...
                            visited_nodes = set(
                                nx.single_source_shortest_path(
                                    self.graph.reverse(), node
                                ).keys()
                            )
                            visited_nodes.remove(node)
                            if node == "fc2_1":
                                a = 1
                            if not self.perform_loop_swapping(
                                node, swaplist, visited_nodes, first=True
                            ):
                                invalid_swapping = True
                            self.plot_SDG()
    ...
    def recursive_SDG_subgraphing(
        self, node: str, nodes_so_far: Set[str], checked_subgraphs: set
    ) -> List[SoapStatement]:
        ...
        for pred in self.graph.predecessors(node):
            ...
            for sibling in self.graph.successors(pred):
                if sibling != node:
                    ...
                    if is_divergent:
                        ...
                        if (
                            len(list(self.graph.predecessors(sibling))) == 1
                            and len(self.in_edges(sibling)) == 1
                        ):
                            ...
                            for sib_stat in sibling_statements:
                                ...
                                if (
                                    status == 0
                                    and S.subgraph
                                    not in checked_subgraphs.union(
                                        set(
                                            [
                                                frozenset(sg.subgraph)
                                                for sg in sdg_statements
                                            ]
                                        )
                                    )
                                ):
            ...
            if (
                pred_arr_name not in base_st.phis.keys()
                or len(base_st.phis[pred_arr_name]) > 1
            ):
                continue
            ...
            if len(self.in_edges(pred)) > 0:
                ...
                if len(pred_st.rhoOpts.free_symbols) > 0 and any(
                    [pred_arr_name in in_arr for in_arr in pred_st.phis.keys()]
                ):
                    continue
                if any(
                    [len(base_accesses) > 1 for base_accesses in pred_st.phis.values()]
                ):
                    continue
                ...
                if self.is_complete_tree(pred, inner_nodes):
                    pred_statements = self.recursive_SDG_subgraphing(
                        pred, inner_nodes, checked_subgraphs
                    )
    ...
    def calculate_IO_of_SDG(self) -> Tuple[sp.core.Expr, List[SoapStatement]]:
        ...
        for node in all_nodes:  # final_sinks: #all_nodes:
            ...
            subgraph_opt.Q = 0
            Q_opt_val = 0
            sdg_subgraphs_statements = self.recursive_SDG_subgraphing(
                node, set(), checked_subgraphs
            )
            # append checked sugraphs from this node to the global set
            checked_subgraphs = checked_subgraphs.union(
                set([frozenset(sg.subgraph) for sg in sdg_subgraphs_statements])
            )
            ...
            for subgraph_st in sdg_subgraphs_statements:
                ...
                if subgraph_st.parent_subgraph:
                    [better_subgraph, Q_val] = compare_st(
                        subgraph_st, subgraph_st.parent_subgraph
                    )
                ...
                [subgraph_opt, Q_opt_val] = compare_st(
                    subgraph_st, subgraph_opt, Q_opt_val
                )
