    ...
    def compute(self, counters: pd.DataFrame) -> pd.DataFrame:
        ...
        if "L3" in self._groups:
            ...
            metrics["L3_mem_evict_bandwidth"] = (
                metrics["l3_mem_evict_data_volume"] / runtime
            )
        ...
        if "MEM" in self._groups:
            metrics["memory_read_data_volume"] = 1.0e-06 * cs["DRAM_READS"] * 64.0
            metrics["memory_read_bandwidth"] = (
                metrics["memory_read_data_volume"] / runtime
            )
            metrics["memory_write_data_volume"] = 1.0e-06 * cs["DRAM_WRITES"] * 64.0
            metrics["memory_write_bandwidth"] = (
                metrics["memory_write_data_volume"] / runtime
            )
            metrics["memory_data_volume"] = (
                1.0e-06 * (cs["DRAM_READS"] + cs["DRAM_WRITES"]) * 64.0
            )
            metrics["memory_bandwidth"] = metrics["memory_data_volume"] / runtime
            metrics["memory_data_volume_0"] = (
                1.0e-06 * (cs_first["DRAM_READS"] + cs_first["DRAM_WRITES"]) * 64.0
            )
