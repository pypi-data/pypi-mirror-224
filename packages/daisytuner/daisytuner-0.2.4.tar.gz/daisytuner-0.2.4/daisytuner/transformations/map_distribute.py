...
class MapDistribute(transformation.SingleStateTransformation):
    ...
    def can_be_applied(
        ...
        # TODO: Relax by NestedSDFG solution
        for access_node in first_state.data_nodes():
            ...
            if first_state.out_degree(access_node) == 0:
                for inedge in state.in_edges(self.map_entry):
                    if inedge.data.data == access_node.data:
                        return False
    ...
    def apply(self, state: SDFGState, sdfg: SDFG):
        nsdfg = self.nested_sdfg_node.sdfg
        first_state = nsdfg.start_state
        outer_state = sdfg.add_state_before(state, first_state.label + "_init")
        ...
        outer_state.add_node(first_map_entry)
        ...
        outer_state.add_node(first_map_exit)
        ...
        first_nsdfg_node: nodes.NestedSDFG = outer_state.add_nested_sdfg(
            first_nsdfg, parent=sdfg, inputs=inputs, outputs=outputs
        )

        if not inputs:
            outer_state.add_edge(
                first_map_entry, None, first_nsdfg_node, None, dace.Memlet()
            )
        ...
        for output in mapped_outputs:
            intermediate_output_nodes[output] = outer_state.add_access(output)
