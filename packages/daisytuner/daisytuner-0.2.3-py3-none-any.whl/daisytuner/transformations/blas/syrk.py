...
class SYRK(SingleStateTransformation):
    ...
    def can_be_applied(
        ...
        a1_edge = list(state.in_edges_by_connector(tasklet, in_conns[0]))
        a2_edge = list(state.in_edges_by_connector(tasklet, in_conns[1]))
        ...
        if b1 != 0 or e1 != A_desc.shape[0] - 1 or s1 != 1:
            return False
        if b2 != 0 or e2 != A_desc.shape[1] - 1 or s2 != 1:
            return False
    ...
    def apply(self, state: SDFGState, sdfg: dace.SDFG):
        ...
        a1_edge = list(state.in_edges_by_connector(tasklet, in_conns[0]))[0]
        A, A_desc = (a1_edge.data.data, sdfg.arrays[a1_edge.data.data])

        a2_edge = list(state.in_edges_by_connector(tasklet, in_conns[1]))[0]
        ...
        A_node = list(state.in_edges(self.outer_map_entry))[0].src
        C_node = list(state.out_edges(outer_map_exit))[0].dst

        state.remove_node(tasklet)
        state.remove_node(self.inner_map_entry)
        state.remove_node(inner_map_exit)
        state.remove_node(self.outer_map_entry)
        state.remove_node(outer_map_exit)

        libnode = Syrk("_Syrk_", uplo="L", trans="N", alpha=1.0, beta=0.0)
