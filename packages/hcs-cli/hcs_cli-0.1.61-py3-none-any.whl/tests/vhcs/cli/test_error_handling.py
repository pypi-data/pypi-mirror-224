...
class TestErrorHandling(CliTest):
    def test001_error_simple(self):
        ...
        self.verify("hcs cli-test echo-error --reason r1 --code 123 --tuple", '', 123, False, verify_stderr=verify_stderr)

    def test002_error_ctxp(self):
        def verify_stderr(text):
            self.assertTrue(text.find("r1") >= 0, "Root cause message must be printed")
        self.verify("hcs cli-test echo-error --reason r1 --code 123 --ctxp-error", '', 123, False, verify_stderr=verify_stderr)
    ...
    def test010_exception_ctxp(self):
        ...
        self.verify("hcs cli-test echo-exception --reason r1 --type=CtxpException", '', 1, False, verify_stderr=verify_stderr)
    ...
    def test011_exception_critical(self):
        ...
        self.verify("hcs cli-test echo-exception --reason r1 --type=TypeError", '', 1, False, verify_stderr=verify_stderr)
    ...
    def test012_exception_http_status(self):
        ...
        self.verify("hcs cli-test echo-exception --reason r1 --type=httpx.HTTPStatusError", '', 1, False, verify_stderr=verify_stderr)
    ...
    def test013_exception_default(self):
        def verify_stderr(text):
            ...
            self.assertTrue(text.find("KeyError") >= 0, "Root cause type must be printed")
            self.assertTrue(text.find("Traceback") >= 0, "For critical exception, stack trace must be printed.")
        self.verify("hcs cli-test echo-exception --reason r1 --type=Exception", '', 1, False, verify_stderr=verify_stderr)
