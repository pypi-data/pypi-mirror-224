...
from block import drawBlock

from stylesheet import loadFromCSSStyleSheet
...
def bsdm():
    ...
    dwg = svgwrite.Drawing(str(output_svg_path), debug=True)
    
    styleSheet = None
    cssStyleSheet = getInputCSS(args.css_file)    
    if(cssStyleSheet is not None) :
        dwg.embed_stylesheet(cssStyleSheet)
        loadFromCSSStyleSheet(cssStyleSheet)        

    
    drawBlock(50, 50, 500, 300, 'first_block', dwg)
