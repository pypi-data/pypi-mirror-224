...
class BaseSLM(AbstractSLM):
    ...
    def discuss(self, user_input: list[dict], conversation_id: str = None) -> dict:
        """
        Send user input to our language model and return the reply
        """
        ...
        replies = Utils.canonicalize_discuss_result(replies)

        # Save response to the conversation
        conversation.append(replies)
    ...
    def _make_completion_prompt(self, conversation: list[dict]) -> str:
        ...
        system = Prompts.get_prompt(__name__, "completion")
...
class PassthroughSLM(BaseSLM):
    ...
    def discuss(self, user_input: list[dict], conversation_id: str = None) -> dict:
        ...
        """
        responses = self.adapter.query_all(user_input, conversation_id)
        return Utils.canonicalize_discuss_result(responses)
