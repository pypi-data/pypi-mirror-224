...
class ThreedOptixAPI(object):
    ...
    def run_batch(self, setup: Union[str, Setup], config_json: dict):
        ...
        endpoint = f"setups/{setup_id}/batch_changes"

        response = put(endpoint, config_json, self.api_key).json()
        print(response)
        if response.get('status') == 'SUCCESS':
            job_num_of_changes = config_json['number_of_changes']
            job_url, job_id = prepare_job(response)

            job = Job.new(job_url, job_id, job_num_of_changes, config_json['simulation_file_prefix'])
            self.jobs.append(job)

            print(f"Job {job_id} created with {job_num_of_changes} changes")
...
class Job(object):
    ...
    @classmethod
    def new(cls, url, id, number_of_changes, prefix):
        obj = object.__new__(cls)
        obj.url = url
        obj.id = id
        obj.prefix = prefix
        obj.number_of_changes = number_of_changes
...
def prepare_job(batch_job_response):

    full_job_url = batch_job_response['data']['url']\
        .replace('$','')

    job_url_splitted = full_job_url\
        .split('/')\

    job_url = '/'.join(job_url_splitted)
    job_id = job_url_splitted[-2]
