...
def run_smb_server(create_fs_params, mount_signal,
    ...
    sock.bind((smb_listen_address[0], smb_listen_address[1] or 0))
...
def _add_simple_cli_arguments(parser):
...
def simple_main_from_argv(display_name, create_fs_params, argv=None):
    if argv is None:
        argv = sys.argv
    ...
    if args.mount_point is None and not args.smb_no_mount:
        print("error: the 'mount_point' argument is required unless --smb-no-mount is specified",
              file=sys.stderr)
        return -1
