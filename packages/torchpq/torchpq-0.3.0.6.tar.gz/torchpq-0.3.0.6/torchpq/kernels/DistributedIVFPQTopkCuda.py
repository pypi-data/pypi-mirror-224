...
class DistributedIVFPQTopkCuda(CustomKernel):
  def __init__(
      self,
      m=8,
      k=256,
    ...
    with open(get_absolute_path("kernels","cuda","distributed_ivfpq_topk.cu"), "r") as f:
      self.code = f.read()
  ...
  def topk(
    ...
    assert cell_capacity.shape[1] == n_probe
    ...
    self._topk_fn(
      ...
      args=[
        ...
        values.data_ptr(),
        indices.data_ptr(),
        n_query, n_probe, n_candidates_pow_of_2
        ],
      stream=self.stream
    )
  ...
  def topk_residual_precomputed(
    ...
    assert cell_size.shape == (n_query, n_probe)
    assert base_sims.shape == (n_query, n_probe)
    assert cell_ptr.dtype == cell_size.dtype == torch.int64
