...
#define get_precomputed_from_smem(arr, y, z) arr[(y) * _K_ + (z)]
#define get_precomputed_from_regf(val, x) __shfl_sync(0xffffffff, val, x, 32)
...
__device__ __forceinline__ void load_precomputed_to_regf(
  ...
  for (int i = 0; i < HALFM; i++){
    int adr = ((i + HALFM * hwy) * nQuery * _K_) + (qid * _K_) + (hwx);
...
__device__ __forceinline__ void prefetch_precomputed(
  ...
  if (tid < 16){
    #pragma unroll
    for (int i = 0; i < _M_; i++){
      const float* adr = precomputed + (i * nQuery * _K_) + (qid * _K_) + (tid);
      __prefetch_l1(adr);
    }
  }
}
...
__device__ __forceinline__ void consume_data_from_smem(
  _VOLATILE_ float sMem[],
  ...
  for (int i = 0; i < HALFM / _NCS_; i++){
    ...
    for (int j = 0; j < _NCS_; j++){
      float pre = get_precomputed_from_smem(sMem, i * _NCS_ + j, threadData.val[j] >> 4);
      value += pre;
    }
    #pragma unroll
    for (int j = 0; j < _NCS_; j++){
      float pre = get_precomputed_from_smem(sMem, i * _NCS_ + j, threadData.val[j] & 0x0f);
      value += pre;
    }
    // float pre0 = sMem[(i * _NCS_ + 0) * _K_ + int(threadData.val[0]) ];
    // float pre1 = sMem[(i * _NCS_ + 1) * _K_ + int(threadData.val[1]) ];
...
__device__ __forceinline__ void consume_data_from_regf(
  float precomputedREGF[HALFM],
  uint8n_t dataCache[_M_ / _NCS_],
  float &value
){
  ...
  for (int i = 0; i < HALFM / _NCS_; i++){
    ...
    #pragma unroll
    for (int j = 0; j < _NCS_; j++){
      float low = get_precomputed_from_regf(precomputedREGF[i * _NCS_ + j], threadData.val[j] >> 4);
      value += low;

      float high = get_precomputed_from_regf(precomputedREGF[i * _NCS_ + j], 16 + threadData.val[j] & 0x0f);
      value += high;
    }
...
__global__ void ivfpq4_top1_v1(
  ...
  extern __shared__ _VOLATILE_ float precomputedSMEM[]; // M * K
  prefetch_precomputed(precomputed, nQuery);
  ...
  // load_precomputed_to_smem(precomputed, precomputedSMEM, nQuery);
  float precomputedREGF[HALFM];
  load_precomputed_to_regf(precomputed, precomputedREGF, nQuery);
  ...
  for (int i = 0; i < nIter + 1; i++){
    ...
    if (i > 0){
      ...
      consume_data_from_regf(precomputedREGF, dataCache, value);

      // if (prevCellStart <= prevIN && prevIN < prevCellEnd){
      //   cIsEmpty = isEmpty[prevIN];
      //   uint8n_t dataCache[HALFM / _NCS_];
      //   // consume_data_from_smem(precomputedSMEM, dataCache, value);
      //   consume_data_from_regf(precomputedREGF, dataCache, value);
      // } else {
      //   value = -INFINITY;
      // }
  ...
  block_max(finalValue, finalIndex, precomputedSMEM);
