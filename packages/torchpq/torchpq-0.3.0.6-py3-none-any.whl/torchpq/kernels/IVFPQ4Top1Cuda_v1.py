...
class IVFPQ4Top1Cuda_v1(CustomKernel):
  """
    What's new:
      bunch of stuff
  """
  def __init__(
    ...
    with open(get_absolute_path("kernels", "cuda", f"ivfpq4_top1_v{vnum}.cu"), "r") as f:
      self.kernel = f.read()
    ...
    self._topk_fn.max_dynamic_shared_size_bytes = self.sm_size
    print(self._topk_fn.attributes)

    # self._topk_residual_fn = cp.RawKernel(
    #   code = kernel,
    #   name = f'ivfpq4_top1_residual_v{vnum}',
    #   options = (
    #     '--maxrregcount=255',
    #     '--use_fast_math',
    #     '-lineinfo'
    ...
    #   backend='nvrtc',
    # )
    # self._topk_residual_fn.max_dynamic_shared_size_bytes = self.sm_size
    
    # self._topk_residual_precomputed_fn = cp.RawKernel(
    #   code = kernel,
    #   name = f'ivfpq4_top1_residual_precomputed_v{vnum}',
    #   options = (
    #     '--maxrregcount=255',
    #     '--use_fast_math',
    #     '-lineinfo'
    ...
    #   backend='nvrtc',
    # )
    # self._topk_residual_precomputed_fn.max_dynamic_shared_size_bytes = self.sm_size
  ...
  def topk(
    ...
    assert data.shape[0] == (self.m // 2) // self.n_cs
  ...
  def topk_residual(
    ...
    assert data.shape == ((self.m // 2) // self.n_cs, n_data, self.n_cs)
