...
class Output():
    ...
    def __init__(self, content: Union[bytes, dict], content_type: str="application/json") -> None:
...
class ImageCaptioningInference(Inference, Input):
    ...
    def __call__(self, img: ImageType, 
        ...
        if isinstance(result, list):
            return [ ImageCaptioningOutput(item) for item in result ]

        return [ ImageCaptioningOutput(result) ]
