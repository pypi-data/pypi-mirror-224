...
class DemandExtraDuration(BaseModel):
    ...
    """
    demand_key: Optional[constr(strict=True, max_length=1024, min_length=1)] = Field(..., description="The key of the ticket whose events need to increase the execution time.")
    performer_key: constr(strict=True, max_length=1024, min_length=1) = Field(..., description="Performer's key.")
    additional_duration: constr(strict=True, max_length=16, min_length=3) = Field(..., description="Time duration according to [ISO 8601 duration](https://en.wikipedia.org/wiki/ISO_8601#Durations). ")
    __properties = ["demand_key", "performer_key", "additional_duration"]

    @validator('additional_duration')
    def additional_duration_validate_regular_expression(cls, v):
