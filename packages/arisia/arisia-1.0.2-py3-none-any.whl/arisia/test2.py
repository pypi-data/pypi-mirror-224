...
def connect(ex, type):
    ...
    print(f"loading example_loadder : {ex}", f"type : {type}")
    connect = pymysql.connect(host='testdb.arisia.space', user='view', password='example', db='example', charset='utf8mb4', cursorclass=pymysql.cursors.DictCursor)
    cur = connect.cursor()
    # trying querry
    try:
