...
def connect(ex:str, type:str):
    ...
    if type == "dataframe" or type == "df":
        ...
        df = pd.DataFrame(cur.fetchall())
        return np.array(df)
    # elif type == "list" or type == "ls":
    #     print("리스트 타입으로 반환합니다.")
    #     df = pd.DataFrame(cur.fetchall())
    #     return list(df)
    else:
...
def connectdb(ip:str, port:int, userid:str, userpw:str, database:str, type:str, querry:str):
...
def mean(list_n:list):
    ac = sum(list_n) / len(list_n)
    return ac
