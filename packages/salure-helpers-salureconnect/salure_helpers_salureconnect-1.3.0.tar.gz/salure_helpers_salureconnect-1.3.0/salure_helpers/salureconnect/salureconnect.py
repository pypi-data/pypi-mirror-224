...
class SalureConnect:
    ...
    def get_system_credential(self, system: str, label: Union[str, list], test_environment: bool = False) -> json:
        ...
        """
        response = requests.get(url=f'{self.url}connector/{system}', headers=self._get_headers())
    ...
    def refresh_system_credential(self, system: str, system_id: int) -> json:
        ...
        :return json response from salureconnect
        """
        response = requests.post(url=f'{self.url}connector/{system}/{system_id}/refresh', headers=self._get_headers())
        response.raise_for_status()
        credentials = response.json()
    ...
    def get_mappings(self, task_id: int) -> dict:
        ...
        """
        response = requests.get(url=f'{self.url}connectors/{task_id}/data-mapping', headers=self._get_headers())
