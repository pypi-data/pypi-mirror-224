...
def validate_prover_args(value: str) -> str:
    ...
    for arg in ContextAttribute:
        ...
        for string in strings:
            if string[0] == '-' and ('=' in string or ',' in string):
                raise Util.CertoraUserInputError(f"the flag {string} should not contain '=' or ','\n"
                                                 "If this is a separator please replace it with a space")

            if string == arg.value.jar_flag:
                raise Util.CertoraUserInputError(f"the flag {string} should be set using {arg.get_flag()} "
...
def validate_solc_args(value: str) -> str:
    ...
    for string in strings:
        if string in ['--path', '--allow-paths', '--allow-path', '--solc_allow_path']:
            raise Util.CertoraUserInputError(f"the flag {string} should be set using the flag --solc_allow_path")
        if string in ['--optimize', '--solc_optimize', '--optimize-runs']:
            raise Util.CertoraUserInputError(f"the flag {string} should be set using the flag --solc_optimize")
        if string in ['--via_ir', '--solc_via_ir', '--via-ir']:
            raise Util.CertoraUserInputError(f"the flag {string} should be set using the flag --solc_via_ir")
