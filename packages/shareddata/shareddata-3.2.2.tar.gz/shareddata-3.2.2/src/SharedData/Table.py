...
class Table:
            ...
            if self.create_map == 'create':
                if (not self.exists):
                    if (not records is None):
                        ...
                        raise Exception('%s/%s/%s/%s not found create first!' %\
                            (self.database,self.period,self.source,self.tablename))
                
                elif (self.exists):
                    if not overwrite:
                        if not self.read(size):
                            raise Exception('%s/%s/%s/%s failed to read!' %\
                                (self.database,self.period,self.source,self.tablename))
                    else:
                        if (not records is None):
                            # create new shared memory set value
                            ...
                            raise Exception('%s/%s/%s/%s failed to overwrite!' %\
                                (self.database,self.period,self.source,self.tablename))
    ...
    def malloc_create(self):
        ...
        [self.shm, ismalloc] = \
            self.shareddata.malloc(self.shm_name,create=True,size=total_size,overwrite=True)
        if not ismalloc:
    ...
    def download_head(self):
        ...
        [head_io_gzip, head_local_mtime, head_remote_mtime] = \
            S3Download(str(self.headpath),str(self.headpath)+'.gzip',force_download)            
    ...
    def download_tail(self):
        ...
        [tail_io_gzip, tail_local_mtime, tail_remote_mtime] = \
            S3Download(str(self.tailpath),str(self.tailpath)+'.gzip',force_download)
