...
class Table:
    ...
    #TODO: create preallocate progress bar
    ...
    def read(self,size=None):        
        ...
        if self.shareddata.s3read:                                     
            [head_io,head_remote_mtime] = self.download_head(headpath)            
        ...
        if not head_io is None:
            ...
            if self.shareddata.s3read:
                if self.hdr['hastail']==1:
                    [tail_io,tail_remote_mtime] = self.download_tail(tailpath)
            ...
            if self.hdr['md5hash'] != head_md5.digest():
                Logger.log.error('Head corrupted %s!' % (headpath))
                raise Exception('Head corrupted %s!' % (headpath))
            elif isinstance(head_io, gzip.GzipFile) \
                    & (self.shareddata.save_local):
                self.write_head(path,head_remote_mtime)
            ...
            if not tail_io is None:
                ...
                if self.tailhdr['md5hash'] != tail_md5.digest():
                    Logger.log.error('Tail corrupted %s!' % (tailpath))
                    raise Exception('Tail corrupted %s!' % (tailpath))
                elif (isinstance(tail_io, gzip.GzipFile)) \
                    & (self.shareddata.save_local):
                    self.write_tail(path,tail_remote_mtime)
    ...
    def download_head(self,headpath):
        head_io = None
    ...
    def download_tail(self,tailpath):
        tail_io = None
        force_download= (not self.shareddata.save_local)        
        tini = time.time()
    ...
    def write(self):
        ...
        try:
            ...
            if self.shareddata.s3write:
                if write_head:
                    self.upload_head(path,mtime)

                if self.hdr['hastail']==1:        
                    self.upload_tail(path,mtime)
                    
            if self.shareddata.save_local:
                if write_head:
                    self.write_head(path,mtime)
                
                if self.hdr['hastail']==1:
                    self.write_tail(path,mtime)
    ...
    def fill_header(self):
        ...
        if self.hdr['hastail']!=1:
            m = hashlib.md5(self.shm.buf[nb_header+nb_head:nb_header+nb_head+nb_tail])
            self.tailhdr['md5hash'] = m.digest()

        return write_head
    
    def upload_head(self,path,mtime):
        nb_header = int(self.hdrdtype.itemsize)
        nb_head = int(self.hdr['headsize']*self.hdr['itemsize'])
        # zip head
        gzip_io = io.BytesIO()
    ...
    def upload_tail(self,path,mtime):
        nb_header = int(self.hdrdtype.itemsize)
        nb_head = int(self.hdr['headsize']*self.hdr['itemsize'])
        nb_tail = int(self.tailhdr['tailsize']*self.hdr['itemsize'])
        gzip_io = io.BytesIO()
    ...
    def write_head(self,path,mtime):
        nb_header = int(self.hdrdtype.itemsize)
        nb_head = int(self.hdr['headsize']*self.hdr['itemsize'])
    ...
    def write_tail(self,path,mtime):
        nb_header = int(self.hdrdtype.itemsize)
        nb_head = int(self.hdr['headsize']*self.hdr['itemsize'])
        nb_tail = int(self.tailhdr['tailsize']*self.hdr['itemsize'])
