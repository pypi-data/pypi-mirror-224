...
for idx, x1 in enumerate(imgs):
    if idx % 10 == 0:
        print(idx)
    Cx1 = len(gzip.compress(bytes(np.array2string(x1), encoding='utf8')))
    distance_from_x1 = []
    for idy, x2 in enumerate(imgs) :
        Cx2 = len(gzip.compress(bytes(np.array2string(x2), encoding='utf8')))
        x1x2 = np.array2string(x1) + np.array2string(x2)
        print(x1x2)
        Cx1x2 = len(gzip.compress(bytes(x1x2, encoding='utf8')))
