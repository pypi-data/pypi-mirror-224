...
class DataTools(object):
    ...
    def __init__(self, stub=None, es_hosts='http://es.sanywind.net:9200/'):
        ...
        self.log = SanyLog(project_name=self.PROGRAMME, project_version=self.VERSION, author="SANY")
        if stub:
            with grpc.insecure_channel(stub, options=options) as channel:
                stub = model_data_message_pb2_grpc.ModelDataMessageStub(channel)
    ...
    def return_result(self, stub, project_name, wind_farm, data_start_time, data_end_time,
        ...
        extra = {'project_name': project_name, 'description':"模型return_result结果{}".format(int(json.loads(res.code))),
                    'author': "SANY", 'func_status': "Info", "data_start_time": data_start_time, 
                    'data_end_time': data_end_time}
        # 将结果打到reduce_log 
        self.log.info(log_str="模型return_result结果{}".format(int(json.loads(res.code))),
                      farm=wind_farm, turbine=turbine_num, func_name="return_result", extra=extra)
    ...
    def get_first_faultV2(self, stub, farm=None, turbine=None, fault_code=None,
        ...
        try:
            ...
            if result_dataframe != '':
                return pd.DataFrame(json.loads(result_dataframe))
            else:
                return None
