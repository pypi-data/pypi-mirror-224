...
def speed_test(assert_download: tuple or None = None, assert_upload: tuple or None = None, unit: str = 'MB'):
    """进行网络测速, 并返回下载|上传速率
    ...
    def _get_sentence(speed: str, param: tuple):
        ...
        if param[0] and param[1] is None:
            str_list = [speed, '>=', param[0]]
        elif param[1] and param[0] is None:
            str_list = [param[1], '>=', speed]
        elif param[0] and param[1]:
            str_list = [param[1], '>=', speed, '>=', param[0]]
        return eval(''.join(str_list))

    unit = unit.upper()
    unit_dict = {'B': 1, 'KB': 1024, 'MB': 1024 * 1024, 'GB': 1024 * 1024 * 1024}
    ...
    logging.info("download speed:" + str(download_speed) + f"{unit}", "upload speed:" + str(upload_speed) + f"{unit}")
    if isinstance(assert_download, tuple):
        assert _get_sentence(download_speed, assert_download), 'download speed assert error'
    elif isinstance(assert_upload, tuple):
        assert _get_sentence(upload_speed, assert_upload), 'upload speed assert error'
    elif assert_download is None and assert_upload is None:
...
if __name__ == '__main__':
    ...
    # print(speed_test(assert_download='>2', assert_upload='>1'))
    print(speed_test(unit='MB'))
