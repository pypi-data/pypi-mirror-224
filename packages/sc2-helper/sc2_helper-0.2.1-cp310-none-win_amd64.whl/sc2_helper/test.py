...
class basic_bot(BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if iteration == 0:
            ...
            await self.client.debug_create_unit([[UnitTypeId.MARINE, 10, self.start_location, 1]])
            await self.client.debug_create_unit([[UnitTypeId.BATTLECRUISER, 10, self.start_location, 2]])
        ...
        if self.units(UnitTypeId.MARINE):
            ...
            # print(t)
            # self.double += t
            print(self.combat_predictor.predict_engage(self.units, self.enemy_units, 1, self.combat_settings))
        if iteration > 10:
