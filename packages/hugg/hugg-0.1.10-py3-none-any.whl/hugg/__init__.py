...
try:
    ...
    class ghub(mem):
        ...
        def timeout(self, setValue:int=None):
            if setValue != None:
                self.internal_timeout = setValue
            return self.internal_timeout

        def __init__(self,repo,access_token,branch='master',create=False,wraplambda=lambda foil:False, timeout:int=15):
            super().__init__(wraplambda)
            self.internal_timeout = 15
            self.github_access = Github(access_token, timeout=self.timeout())
            if create:
                try:
