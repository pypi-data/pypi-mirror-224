...
class Portfolio:
    ...
    def sharpe_ratio(self, weights=[], risk_free_rate=0.0, period="daily", trading_days=252, hours_per_day=8):
        ...
        annualized_volatility = self.annualized_volatility(strategy="custom", weights=weights,  period=period, trading_days=trading_days, hours_per_day=hours_per_day)
    ...
    def optimize_portfolio(self, strategy="sharpe", risk_free_rate=0.0, level=5):
        if strategy == "sharpe":
