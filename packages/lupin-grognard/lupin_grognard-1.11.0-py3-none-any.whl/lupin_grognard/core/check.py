...
def check_commit(
    ...
    Args:
        ...
        is_ci_mr_target_branch_main (bool): If True, active checks for jama references
    """
    ...
    if check_mode == CommitCheckModes.CHECK_ALL_COMMITS:
        disable_permissive_mode_on_main = (
            True  # disable permissive mode to main and master branches
        )
    else:
        disable_permissive_mode_on_main = False

    error_counter.error_report(
        permissive_mode=permissive_mode,
        disable_permissive_mode_on_main=disable_permissive_mode_on_main,
    )
