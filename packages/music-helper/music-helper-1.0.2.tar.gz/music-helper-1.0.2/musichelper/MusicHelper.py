...
class MusicHelper:
    def __init__(self, deezer_arl:str=None, ytm_oauth:str=None, sc_data:tuple=(None, None),
                 loop:asyncio.BaseEventLoop=None, logger:logging.Logger=None) -> None:
        ...
        if deezer_arl:
            ...
            self.deezer = Deezer(arl = deezer_arl, loop = self.loop, logger = self.logger)
        else: self.deezer = DummyClass(_type = 'deezer')


        if ytm_oauth:
            self.ytm = YTM(ytm_oauth = ytm_oauth, loop = self.loop, logger = self.logger)
