...
import shutil
import filetype, mimetypes, linecache, os, urllib, time, math, zipfile,\
    tarfile, qrcode, imghdr, uuid
import pandas as pd
...
class FileZIPOperation(object):
    ...
    @staticmethod
    def zip_download(urllist, namelist):
        count = 0
        dir = os.path.abspath('.')
        for item in urllist:
    ...
    # 打包目录为zip文件（未压缩）
    @staticmethod
    def zip_make(source_dir, output_filename):
        """
    ...
    # zip解压缩
    @staticmethod
    def zip_unpack(zip_path, dst_dir, ):
    ...
    # zip解加密压缩包
    @staticmethod
    def zip_extract(zip_path, dst_dir, pwd):
        """
        p = 'di201805'
    ...
    @staticmethod
    def tar_gz_make(source_dir, output_filename, ):
        """
    ...
    # 如果只打包不压缩，将"w:gz"参数改为"w:"或"w"即可。
    @staticmethod  # 多个不同目录下的文件打包成一个tar
    def tar_gz_files_make(source_dir, output_filename, ):
        """
    ...
    # 多个不同目录下的文件打包成一个tar
    @staticmethod
    def tar_gz_any_files_make(output_filename, source_dir_lis):
        """
    ...
    @staticmethod
    def tar_gz_unpack(tar_gz_path, dst_dir):
    ...
    @staticmethod
    def rar_unpack(rar_path, dst_dir):
...
class FileBasicOperation(object):
    ...
    @staticmethod
    def glob(endswith, root_dir, dir_fd=None, recursive=False):
        # glob.glob("*.ipynb")
        return glob.glob(f"*.{endswith}", root_dir=root_dir, dir_fd=dir_fd, recursive=recursive)
    # </editor-fold>
