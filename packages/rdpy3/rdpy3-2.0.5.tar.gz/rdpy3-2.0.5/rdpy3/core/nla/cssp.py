...
from rdpy3.model.message import Stream
from rdpy3.model import error
from rdpy3.security.x509 import X509Certificate
...
def encode_der_trequest(nego_types=[], auth_info=None, pub_key_auth=None):
    ...
    if not auth_info is None:
        request.setComponentByName(
            "authInfo",
            univ.OctetString(auth_info).subtype(
                explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 2)
            ),
        )
    ...
    if not pub_key_auth is None:
        request.setComponentByName(
            "pubKeyAuth",
            univ.OctetString(pub_key_auth).subtype(
                explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 3)
            ),
        )
...
def getNegoTokens(tRequest):
    ...
    return [
        Stream(negoData.getComponentByPosition(i).getComponentByPosition(0).asOctets())
        for i in range(len(negoData))
    ]
...
async def connect(
    reader, writer, authentication_protocol: sspi.IAuthenticationProtocol
):
    """
    ...
    writer.write(
        encode_der_trequest(nego_types=[authentication_protocol.getNegotiateMessage()])
    )
    await writer.drain()

    trequest = decode_der_trequest(await reader.read(1500))
    message, interface = authentication_protocol.getAuthenticateMessage(
        getNegoTokens(trequest)[0]
    )

    # get binary certificate
    # There is no other way to get certificate that have net been validated
    peer_certificate = (
        writer.transport._ssl_protocol._sslpipe.ssl_object._sslobj.getpeercert(True)
    )
    x509_certificate = der_decoder.decode(peer_certificate, asn1Spec=X509Certificate())[
        0
    ]
    public_key = (
        x509_certificate.getComponentByName("tbsCertificate")
    ...
    writer.write(
        encode_der_trequest(
            nego_types=[message], pub_key_auth=interface.GSS_WrapEx(public_key)
        )
    )
    await writer.drain()
    ...
    trequest = decode_der_trequest(await reader.read(1500))
    public_key_inc = interface.GSS_UnWrapEx(
        trequest.getComponentByName("pubKeyAuth").asOctets()
    )

    if int.from_bytes(public_key, "little") + 1 != int.from_bytes(
        public_key_inc, "little"
    ):
    ...
    writer.write(
        encode_der_trequest(
            auth_info=interface.GSS_WrapEx(
                encode_der_tcredentials(domain, user, password)
            )
        )
    )
    await writer.drain()
...
def encodeDERTRequest(negoTypes=[], authInfo=None, pubKeyAuth=None):
    ...
    if not authInfo is None:
        request.setComponentByName(
            "authInfo",
            univ.OctetString(authInfo).subtype(
                explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 2)
            ),
        )
    ...
    if not pubKeyAuth is None:
        request.setComponentByName(
            "pubKeyAuth",
            univ.OctetString(pubKeyAuth).subtype(
                explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 3)
            ),
        )
...
def encodeDERTCredentials(domain, username, password):
    passwordCred = TSPasswordCreds()
    passwordCred.setComponentByName(
        "domainName",
        univ.OctetString(domain).subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)
        ),
    )
    passwordCred.setComponentByName(
        "userName",
        univ.OctetString(username).subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 1)
        ),
    )
    passwordCred.setComponentByName(
        "password",
        univ.OctetString(password).subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 2)
        ),
    )
    ...
    credentials.setComponentByName(
        "credType",
        univ.Integer(1).subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 0)
        ),
    )
    credentials.setComponentByName(
        "credentials",
        univ.OctetString(der_encoder.encode(passwordCred)).subtype(
            explicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatConstructed, 1)
        ),
    )
...
class CSSP(protocol.Protocol):
    ...
    def startNLA(self, sslContext, callback=None):
        ...
        self.transport.write(
            encodeDERTRequest(
                negoTypes=[self._authenticationProtocol.getNegotiateMessage()]
            )
        )
    ...
    def recvChallenge(self, data):
        ...
        message, self._interface = self._authenticationProtocol.getAuthenticateMessage(
            getNegoTokens(request)[0]
        )
        # get back public key
        # convert from der to ber...
        pubKeyDer = crypto.dump_privatekey(
            crypto.FILETYPE_ASN1,
            self.transport.protocol._tlsConnection.get_peer_certificate().get_pubkey(),
        )
        pubKey = der_decoder.decode(pubKeyDer, asn1Spec=OpenSSLRSAPublicKey())[0]

        rsa = x509.RSAPublicKey()
        rsa.setComponentByName(
            "modulus", univ.Integer(pubKey.getComponentByName("modulus")._value)
        )
        rsa.setComponentByName(
            "publicExponent",
            univ.Integer(pubKey.getComponentByName("publicExponent")._value),
        )
        ...
        self.transport.write(
            encodeDERTRequest(
                negoTypes=[message],
                pubKeyAuth=self._interface.GSS_WrapEx(self._pubKeyBer),
            )
        )
    ...
    def recvPubKeyInc(self, data):
        ...
        self.transport.write(
            encodeDERTRequest(
                authInfo=self._interface.GSS_WrapEx(
