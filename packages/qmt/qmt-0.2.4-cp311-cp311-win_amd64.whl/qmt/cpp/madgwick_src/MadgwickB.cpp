...
#include "MadgwickB.hpp"
#include <math.h>

MadgwickB::MadgwickB(float deltat, float beta, float zeta)
    : deltat(deltat), beta(beta), zeta(zeta), SEq_1(1), SEq_2(0), SEq_3(0), SEq_4(0), b_x(1), b_z(0), w_bx(0), w_by(0), w_bz(0)
{

}

void MadgwickB::update(float w_x, float w_y, float w_z, float a_x, float a_y, float a_z, float m_x, float m_y, float m_z, bool use_mag)
{
    ...
    // CHANGED by Daniel Laidig:
    // Unlike the original code, this is calculated after normalizing the magnetometer measurement.
    // Otherwise, the b_x and b_z will be estimated in original units and substracted from the normalized
    // measurements.
