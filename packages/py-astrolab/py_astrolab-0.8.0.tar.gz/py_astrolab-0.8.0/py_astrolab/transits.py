...
class Transits():
    ...
    def refined_search(self, start_time, end_time, planet1, planet2, aspect, target_orb, natal_planet_orb):
        ...
        while current_jd <= end_jd:
            ...
            if objective_function(current_jd) * objective_function(current_jd + interval) < 0 or is_near_opposition:
                ...
                if result_jd:
                    return self.jd_to_datetime(result_jd), objective_function(result_jd) + target_orb
    ...
    def prev_new_moon(self, date):
        ...
        jd_start = self.datetime_to_jd(date)
        precision = 1.0
        ...
        while precision > (1.0 / (24 * 60)):
            ...
            if delta_long < delta_long_threshold:
                ...
                jd_start -= precision
        new_moon_dt = self.__get_local_time(self.jd_to_datetime(jd_start))
        return create_new_moon_obj(new_moon_dt, delta_long)
    ...
    def lunar_phase(self, new_moon_date, new_moon_position, start=None, end=None):
        ...
        """
        def create_lunar_phase_obj(time, orb, aspect):
            moon_longitude = self.calc_long(self.datetime_to_jd(time), swe.MOON)
            days_since_new_moon = (time - new_moon_date).days
            if days_since_new_moon > 14 and aspect != 180:
                aspect = 360 - aspect  # Fase lunare speculare
            return {
                'when': time,
                'orb': orb,
                'aspect': aspect,
                'phase': angle_to_phase(aspect),
                'sign': self.get_zodiac_sign(moon_longitude),
                'house': self.point_in_house(moon_longitude),
                'abs_pos': moon_longitude
            }
        ...
        for aspect in self.aspects:
            ...
            if exact_time:
                results.append(create_lunar_phase_obj(exact_time, orb, aspect))
        return results
