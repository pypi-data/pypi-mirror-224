...
class Transits():
    ...
    def __find_aspects(self, times, positions, natal_positions, look_for_planets, waxing_only, waning_only):
        ...
        for planet1 in self.planets:
            points_of_interest = list(natal_positions.items())
            if planet1 == swe.MOON and planet1 in look_for_planets:
                new_moon = self.new_moon()
                new_moon_position = new_moon['abs_pos']
                points_of_interest.append(("New_Moon", new_moon_position))
                self.user.new_moon = {'signs': [new_moon['sign']], 'house': new_moon['house']}
            for planet2_name, natal_position in points_of_interest:
                ...
                for time1, time2, lon1_1, lon1_2 in zip(times, times[1:], positions[planet1], positions[planet1][1:]):
                    for a in self.aspects:
                        ...
                        if not ((diff1 - a) * (diff2 - a) <= 0) and not (abs(diff1 - a) <= orbit_tolerance) and not (abs(diff2 - a) <= orbit_tolerance):
                            current_aspects.discard((self.planet_names[planet1]['name'], planet2_name, self.aspect_names[a]))
                            continue
    ...
    def new_moon(self):
        def create_new_moon_obj(time, orb):
        ...
        orbit_tolerance = self.planet_names[swe.MOON]['orbit']
        found_new_moon_time, orb = self.binary_search(self.start, self.end, swe.MOON, swe.SUN, 0, orbit_tolerance, None)
        if abs(orb - orbit_tolerance) <= 0.1:
            return create_new_moon_obj(found_new_moon_time, orb)
        found_new_moon_time, orb = self.backward_binary_search(swe.MOON, swe.SUN, 0, orbit_tolerance, None, self.planet_names[swe.MOON]['interval'])
        return create_new_moon_obj(found_new_moon_time, orb)
