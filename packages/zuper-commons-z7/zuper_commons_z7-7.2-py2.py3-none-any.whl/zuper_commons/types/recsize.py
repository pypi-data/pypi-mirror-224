...
from .exceptions import ZException
...
def visualize(rs: RecSize, percentile: float = 0.95, min_rows: int = 5) -> str:
    types = list(rs.sizes)
    ...
    def key(ii: int) -> int:
        if sizes[ii] is object:  # type: ignore # XXX
            return -1
        return sizes[ii].nbytes
    ...
    res = format_table(cells, style="lefts", draw_grid_v=False, col_style=col_style)
    return res
...
def get_rec_size(ob: object) -> RecSize:
    ...
    seen: Set[int] = set()
    sizes: Dict[type, RSize] = defaultdict(RSize)
...
def _get_rec_size(rs: RecSize, obj: object, seen: Set[int], prefix: Tuple[str, ...]) -> None:
    ...
    def rec(x: object, p: Tuple[str, ...]) -> None:
