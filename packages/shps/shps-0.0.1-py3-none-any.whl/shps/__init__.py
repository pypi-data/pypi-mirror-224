...
#       x,y = sp.symbols("xi eta")
        ...
        coef, expr = sp.factor(exp).as_coeff_Mul()

        if coef == 1:
            coef = ""
        else:
            coef = sp.latex(coef) if latex else ccode(coef)+"*"
        ...
        if latex:
            ...
            output = "\n".join((output, f"    shp[{i}] = {coef}{ccode((expr))};"))
...
def shapes(A, terms, keys=None, sym=False):
    import sympy as sp
    if keys is None:
        keys = list(range(1, len(A)+1))
    b = np.zeros(len(A))
...
class Quadrilateral:
    ...
    def shapes(self, symbols=False):
        if self._shapes is None:
            A = self.vandermonde()
            f = lambda x,y: [eval(t,dict(x=x,y=y)) for t in self.terms]
            self._shapes = shapes(A, f, keys=list(self.nodes.keys()))
        return self._shapes
...
if __name__ == "__main__":
    ...
    if "-P" in sys.argv:
        ...
        evaluate shapes at coordinates
            python shp.py Lagrange 2 -x '[0.2, 0.2]'
        """
        import json
