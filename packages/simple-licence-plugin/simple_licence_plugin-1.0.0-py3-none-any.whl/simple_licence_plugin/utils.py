""" functions for performing licence checks """
...
def get_licence_data(package: str):
    """return data embedded in licence file"""
    try:
...
def _call_home(package: str, audit_url: str, data, silent_audit_fail: bool = True):
    ...
    try:
        ...
        post(audit_url, json=data)
    except Exception as err:
        if not silent_audit_fail:  # Don't interrupt the user for an optional audit
            raise RuntimeError(f"Failed to contact licence audit server! ({audit_url})") from err
