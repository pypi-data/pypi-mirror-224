...
def get_limit(event):
    return json.loads(open(_divination_cachepath, "r").read())[get_group_id(event)][get_user_id(event)]["limit"]
...
def get_users():
    users = []
    for group in json.loads(open(_divination_cachepath, "r").read()):
        users += [user for user in group.keys()]

    return users

def get_groups():
    groups = json.loads(open(_divination_cachepath, "r").read()).keys()
    if len(groups) == 1:
        if groups[0] == "0":
            return False
