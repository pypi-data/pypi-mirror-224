...
def cdf(x_j, prior: dirichlet.MixedDirichlet, pre_samples: NDArray[np.uint32]) -> NDArray[np.float64]:
    ...
    bad_samples = np.where(~ posterior_pi.any(axis=1))  # samples with no compatible classes
    if bad_samples:
        bad_full = ONE == pre_samples[bad_samples].sum(axis=1)
        assert bad_full.all(), 'samples with no compatible classes should have full mass'
        warnings.warn(f'every sample should have at least one compatible class, but samples {bad_samples} do not')
