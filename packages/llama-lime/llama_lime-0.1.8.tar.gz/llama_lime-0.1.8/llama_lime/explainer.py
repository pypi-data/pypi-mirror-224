...
class Explainer:
    ...
    def shap(self, sample = None, check_additivity=False):
        ...
        return explainer(self.features)
...
### Results
{self.shap(sample=sample).values}
"""
    ...
    def feature_importances(self):
        importances = self.shap.values
        feature_importances = np.mean(np.abs(self.shap.values), axis=(0, 2))
...
### Results
    ...
    def class_importances(self):
        class_importances = np.mean(np.abs(self.shap.values), axis=(0, 1))
...
"""
    ...
    def instance_importances(self):
        instance_importances = np.sum(np.abs(self.shap.values), axis=(1, 2))
...
"""
    ...
    def feature_class_interactions(self, normalize=True):
        feature_class_importances = np.mean(np.abs(self.shap.values), axis=0)
        if normalize:
