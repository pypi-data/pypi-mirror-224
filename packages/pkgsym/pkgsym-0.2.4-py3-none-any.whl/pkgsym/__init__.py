...
def link_or_unlink(args: argparse.Namespace, pkgdir: str):
...
def npm_proxy(args):
    npm_env = {
        "NPM_CONFIG_PREFIX": args.prefix,
    }

    npm_args = ["npm", *args.npm_args]
    print(f"> env NPM_CONFIG_PREFIX={args.prefix} {' '.join(npm_args)}")
    subprocess.check_call(npm_args, env=npm_env)
...
def main():
    ...
    link_parser = subparsers.add_parser('link',
        help="Link a package under the self-contained directory to the prefix",
    )
    link_parser.add_argument('package', action='store', type=str,
        help="The package name to link -- should be the directory under $prefix/opt that contains the package",
    )
    unlink_parser = subparsers.add_parser('unlink',
        help="Unlink a package under the self-contained directory to the prefix",
    )
    unlink_parser.add_argument('package', action='store', type=str,
        help="The package name to unlink -- should match the directory under $prefix/opt that contains the package",
    )

    install_parser = subparsers.add_parser('install',
        help="Wrap a package manager install with the correct prefix",
    )

    npm_parser = subparsers.add_parser('npm',
        help="Convenience wrapper around npm that sets its prefix for you",
    )
    npm_parser.add_argument('npm_args', nargs=argparse.REMAINDER,
        help="Arguments to be passed to npm verbatim",
    )
