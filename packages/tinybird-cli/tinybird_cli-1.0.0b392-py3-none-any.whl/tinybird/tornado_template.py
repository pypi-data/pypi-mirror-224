...
class UnClosedIfError(Exception):
    ...
    def __str__(self):
        ...
        if self.node != "<string>":
            return 'Missing {%% end %%} block for if in the node %s at line %d of the SQL: %s' % (self.node, self.lineno, self.sql)
...
VALID_FUNCTION_NAMES = {'Boolean', 'DateTime', 'DateTime64', 'Date',
                        'Float32', 'Float64', 'Int8', 'Int16', 'Int32', 'Int64', 'Int128', 'Int256',
                        'UInt8', 'UInt16', 'UInt32', 'UInt64', 'UInt128', 'UInt256', 'String', 'Array',
                        'defined', 'column', 'columns', 'day_diff', 'date_diff_in_seconds', 'date_diff_in_minutes', 'date_diff_in_hours', 'date_diff_in_days', 'split_to_array',
                        'enumerate_with_last', 'symbol', 'sql_and', 'cache_ttl', 'error', 'custom_error',
                        'backend_hint', 'activate', 'sql_unescape', 'max_threads', 'Int', 'table', 'TABLE',
                        'len', 'list', 'min', 'max',
                        'Integer',
                        'int', 'float'}
