    ...
    def getrecords(self, no_of_records=100, limit=30, offset=0):
        ...
        while no_of_records!=0:
            if no_of_records<limit:
                response = requests.get(self.base_url+self.endpoint+f'?offset={offset}&limit={no_of_records}', headers=headers)
                response_data = response.json()
                final_data.extend(response_data['data'])
                return final_data
            
            response = requests.get(self.base_url+self.endpoint+f'?offset={offset}&limit={limit}', headers=headers)
            response_data = response.json()
...
# Initialize the globalmartAPI object
api_obj = globalmartAPI(base_url=base_url, endpoint=endpoint, header=headers)

# Call the API
data = api_obj.getrecords(no_of_records=500, limit=100)
