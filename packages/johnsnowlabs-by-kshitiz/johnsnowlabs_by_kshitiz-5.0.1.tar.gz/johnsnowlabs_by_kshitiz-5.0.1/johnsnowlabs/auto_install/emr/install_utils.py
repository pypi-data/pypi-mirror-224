...
def create_bootstrap_script(
    ...
    installation_script = f"""python3 -m pip install 'urllib3<2.0'
python3 -m pip install {Software.jsl_lib.pypi_name}=={settings.raw_version_jsl_lib}
"""
    if license:
        installation_script += f"python3 -c \"from johnsnowlabs import nlp;nlp.install(med_license='{secrets.HC_LICENSE or ''}',aws_access_key='{secrets.AWS_ACCESS_KEY_ID or ''}',aws_key_id='{secrets.AWS_SECRET_ACCESS_KEY or ''}', spark_nlp={spark_nlp}, nlp={nlp}, visual={visual})\""
    else:
        installation_script += 'python3 -c "from johnsnowlabs import nlp;nlp.install(browser_login=False, nlp=False)"'
    installation_script += '\nsudo bash -c "mkdir -p /usr/lib/spark/jars; cp $HOME/.johnsnowlabs/java_installs/*.jar /usr/lib/spark/jars/"'

    with open(path.join(here, "bootstrap_install.sh"), encoding="utf-8") as f:
        content = f.read()
        s3_client = boto3.client("s3")
        return upload_content(
            s3_client=s3_client,
            content=content.replace("{{installation_script}}", installation_script),
            bucket=bootstrap_bucket,
            file_name="jsl_emr_bootstrap.sh",
        )
