...
def register_server_with_middle_tier(server_ctx: ServerContext) -> None:
    ...
    server_config = server_ctx.config
    ...
    if address in ["0.0.0.0", "127.0.0.1", "localhost"]:
        raise ValueError('Externally accessible address must to be set for the server to use the 4D Viewer API.')

    four_d_viewer_url = f"http://{address}:{port}/4d_viewer"
