...
def run_experiment_prompting(config=None, experiment="ibmsc", offline=False, validate=True, splits=None, logger=None):
    ...
    print(hi)
    for batch_df in chunker(test_dataset, batch_size):
        prompts_to_predict = []
        for i, record in batch_df.iterrows():
            ...
            prompts_to_predict.append(prompt_to_predict)
            log_message(logger, prompt_to_predict, level=logging.INFO)
            labels.append(label)
        seq = tokenizer(prompts_to_predict, return_tensors="pt", padding=True)
        generated = model.generate(**seq, max_new_tokens=3)
        output_strings = tokenizer.batch_decode(generated, skip_special_tokens=True)
        if experiment == "vast":
            batch_predictions = [predict_three_labels(resulting_string) for resulting_string in output_strings]
        else:
            batch_predictions = [predict_two_labels(resulting_string) for resulting_string in output_strings]
        if output_strings:
            for result_string in output_strings:
                log_message(logger, result_string, level=logging.INFO)
