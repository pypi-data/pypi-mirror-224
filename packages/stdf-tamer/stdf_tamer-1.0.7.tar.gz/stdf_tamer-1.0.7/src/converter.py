...
def worker(si, ftype, add_sites, add_head):
    ...
    def _worker():
        with _opener[ftype](si, "rb") as f:
            try:
                while True:
                    q.put(ams_rw_stdf.parse_record(f))
    ...
    while True:
        ...
        if type_and_subtyp == (15, 10,):
            ...
            df = pl.LazyFrame(data[key], schema=schema).with_columns(pl.lit(c.PL.PART_ID).cast(pl.Utf8).alias("part_id"), pl.lit(c.PL.PART_TXT).cast(pl.Utf8).alias("part_txt"))
            if add_sites:
                df = df.with_columns(pl.lit(c.PL.SITE_NUM).cast(pl.Utf8).alias("site"))
            if add_head:
                df = df.with_columns(pl.lit(c.PL.HEAD_NUM).cast(pl.Utf8).alias("head"))
            yield df
            data[key] = []
...
def main():
    ...
    try:
        ...
        data = pl.concat(worker(si, ftype, add_sites=bool(arguments["--add-site"]), add_head=bool(arguments["--add-head"])), rechunk=False)
