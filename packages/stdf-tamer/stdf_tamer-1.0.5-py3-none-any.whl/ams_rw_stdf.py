...
_dict_of_payloads = {#Required
                     tuple((0x00, 0x0a)): FAR_payload,
                     tuple((0x01, 0x0a)): MIR_payload,
                     tuple((5,10)):       PIR_payload,
                     tuple((15,10)):      PTR_payload,
                     tuple((0x05,0x14)):  PRR_payload,
                     tuple((1,20)):       MRR_payload,
                     #optional
                     tuple((2, 30)):      WCR_payload,
                     tuple((0x01, 0x50)): SDR_payload,
                     tuple((0x01, 60)):   PMR_payload,
                     tuple((0x01, 62)):   PGR_payload,
                     tuple((1,50)):       SBR_payload,
                     tuple((1,30)):       PCR_payload,
                     tuple((10,30)):      TSR_payload,
                     tuple((2,10)):       WIR_payload,
                     tuple((5,10)):       PIR_payload,
                     tuple((1,40)):       HBR_payload}
...
def parse_record(stream):
    ...
    """
    raw_RECORD = construct.Struct("REC_LEN"  / construct.Int16ul  * "Bytes of data following header",
                                  "REC_TYP"  / construct.Byte     * "Record type (1)",
                                  "REC_SUB"  / construct.Byte     * "Record sub-type (10)",
                                  "PL_RAW"   / construct.Bytes(lambda this: this.REC_LEN))
    data =  raw_RECORD.parse_stream(stream)
    try:
        return RECORD.parse(raw_RECORD.build(data))
    except:
        print(str(data))
