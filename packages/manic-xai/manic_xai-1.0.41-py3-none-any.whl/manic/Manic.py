...
class Manic:
    def __init__(self, data_instance, base_counterfactuals, categorical_features, immutable_features, feature_ranges, data, predict_fn, predict_proba_fn, class_labels, population_size=100, num_generations=50, alpha=0.25, beta=0.25, crossover_method="uniform", mutation_method="random_resetting", perturbation_fraction=0.1, num_parents=2, seed=42, verbose=1, early_stopping=None, max_time=None, disagreement_method="euclidean_distance", theta=0.25, gamma = 0.25, parallel=False, labels=[], weights = [1/3, 1/3, 1/3], wachter=False):
    ...
    def get_feature_ranges(self, continuous_feature_ranges):
        ...
        for i in range(len(self.data_instance)):
            if i in self.immutable_features_set:
                ...
                # LOWER_BOUND = min(self.data[:, i]) - (min(self.data[:, i]))
                # UPPER_BOUND = max(self.data[:, i]) + (max(self.data[:, i]))

                LOWER_BOUND = self.data_instance[:, i] * 0.5
                UPPER_BOUND = self.data_instance[:, i] * 1.5
                feature_ranges.append((LOWER_BOUND, UPPER_BOUND))
