    ...
    def get_feature_ranges(self, continuous_feature_ranges):
        ...
        for i in range(len(self.data_instance)):
            if i in self.immutable_features_set:
                ...
                UPPER_BOUND = self.data_instance[i] + ((UPPER_BOUND - LOWER_BOUND )* self.feature_entropy)
                LOWER_BOUND = self.data_instance[i] - ((UPPER_BOUND - LOWER_BOUND) * self.feature_entropy)
