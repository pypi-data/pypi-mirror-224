    ...
    def get_feature_ranges(self, continuous_feature_ranges):
        ...
        for i in range(len(self.data_instance)):
            if i in self.immutable_features_set:
                ...
                UPPER_BOUND = self.data_instance[i] + min(UPPER_BOUND, ((UPPER_BOUND - LOWER_BOUND ) * self.feature_entropy))
                LOWER_BOUND = self.data_instance[i] - max(LOWER_BOUND, ((UPPER_BOUND - LOWER_BOUND) * self.feature_entropy))
    ...
    def nearest_unlike_neighbors(self):
        ...
        for i, instance in enumerate(self.data):
            if self.target_class != self.class_labels[i]:
                ...
                for j in range(len(self.data_instance)):
                    if(instance[j] > self.feature_ranges[j][1] or instance[j] < self.feature_ranges[j][0]):
                        print(f"NUN OUT OF BOUNDSL: {instance}")
