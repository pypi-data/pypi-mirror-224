    ...
    def most_rad_counterfactual(self):
        ...
        for i in range(len(self.base_counterfactuals)):
            for j in range(i + 1, len(self.base_counterfactuals)):
                ...
                manhattan_distance = self.disagreement.calculate_manhattan_distance(base_counterfactual[i], base_counterfactual[j], True)
        ...
        best_index = min(np.argsort(average_direction_overlap_scores))
