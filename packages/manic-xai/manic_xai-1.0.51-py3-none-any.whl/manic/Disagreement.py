...
class Disagreement:
    ...
    def calculate_direction_overlap(self, instance1, instance2):
        ...
        proximity_scores = np.zeros(len(self.base_counterfactuals) + 1)

        for i in range(len(self.base_counterfactuals + instance2)):
            for j in range(i + 1, len(self.base_counterfactuals)):
                manhattan_distance = self.calculate_manhattan_distance(self.base_counterfactuals[i], self.base_counterfactuals[j]) / len(instance2)
                proximity_scores[i] =+ manhattan_distance
                proximity_scores[j] =+ manhattan_distance

        average_proximity_scores = [item / (len(self.base_counterfactuals) - 1) for item in proximity_scores]

        direction_overlap_scores = np.zeros(len(self.base_counterfactuals) + 1)
        ...
        for i in range(len(self.base_counterfactuals)):
            ...
            manhattan_direction_overlap = (self.proxmity_weight * average_proximity_scores[i]) + (self.overlap_weight * direction_overlap)
            direction_overlap_scores[i] += manhattan_direction_overlap

        average_direction_overlap_score = np.mean(direction_overlap_scores)
        print(f'cf_actions[-1]: {cf_actions[-1]}')
        print(f'cf_actions[i]: {cf_actions[i]}')
        print(f'Matching count: {matching_count}')
        print(f'Manhattan: {average_proximity_scores[i]}\nDirection Overlap: {direction_overlap}\nManhattan Direction Overlap: {manhattan_direction_overlap}\nAverage Overlap: {average_direction_overlap_score}')
