...
def line_chart(xAxisvalue, predicatedLine, **optional_params):
    x = np.arange(len(xAxisvalue))
    x_smooth = np.linspace(0, len(xAxisvalue) - 1, 100)
    f_actual = interp1d(x, optional_params['actualLine'], kind='cubic')
...
# theme = "black"
# line_chart(xAxisvalue, predicatedLine, xlabel=xlabel, actualLine=actualLine, theme=theme)
...
# theme = 'white'
# xlabel='xlabel'
# horizontal_waterfall(categories, values, xlabel=xlabel, title=title, theme=theme)
...
# theme = 'black'
# vertical_waterfall(categories, values, title=title, theme=theme)
...
# title='Title'
# theme = 'white'
# baseline_graph(baseline, x_labels, colors, series1, series2, series3, title=title, theme=theme)
...
# title='Title'
# colors = ["#FC2964", "#0095FF", "#5C33F6"]
# scatter_plot_graph(marker_shapes, x_values, series_data, colors,title=title, theme='white')
...
# ylabel='Title'
# legend='legends'
# stack_bar(categories,below,above,title=title,ylabel=ylabel,legend=legend,theme = 'black')
...
# heatmap_graph(y_axis_data, x_axis_data, colors,title=title,theme='black')
...
# colors = ["#1D2026", "#393939", "#666666", "#BDBDBD", "#E0E0E0", "#F2F2F2", "#FD93B1",
#           "#FC2964", "#FEC9D8", "#FD5F8B", "#0095FF", "#E6F5FF", "#5C33F6", "#8566F8"]
# heatmap2_graph(y_axis_data, x_axis_data, colors,dataFrom,dataTo,title=title,theme='black')
...
# xAxisTitle='Title'
# yAxisTitle='Title'
# boxPlot_graph(data, titles, colors,xlabel=xAxisTitle,ylabel=yAxisTitle,theme='white')
...
# xAxisTitle = 'Title'
# yAxisTitle = 'Title'
# boxPlot2_graph(data, titles, colors, xlabel=xAxisTitle, ylabel=yAxisTitle,theme='black')
...
#       [20, 21, 22, 23, 24]
# ] 
# boxPlot3_graph(data, colors, legend=legend,title=title, xlabel=xAxisTitle, ylabel=yAxisTitle,theme='black')
...
def horizontal_waterfall(categories, values, **optional_params):
    ...
    bars = ax.barh(categories, values, align='center')
    ax.set_axisbelow(True)
    ax.set_yticklabels(categories, fontdict={'fontsize': 8})
    ...
    if ('xlabel' in optional_params):
        ax.set_xlabel(optional_params['xlabel'],fontdict={'fontsize': 12})   
    if ('ylabel' in optional_params):
        ax.set_ylabel(optional_params['ylabel'],fontdict={'fontsize': 8}) 
    if ('title' in optional_params):
        ax.set_title(optional_params['title'],fontdict={'fontsize': 15})
...
# horizontal_waterfall(categories, values, title=title,theme='white')

def nextItemPrediction(categories, values, **optional_params):
    fig, ax = plt.subplots()
...
# values = [-10, -5, -4, 4, 5, 10]
# title = 'Secondary text'
# nextItemPrediction(categories, values, title=title,theme='black')
...
def line_chart(xAxisvalue, predicatedLine, actualLine,**optional_params):
    ...
    ax.plot(x_smooth, actualLine_smooth, color="#FF8080", label='Actual')  
    ax.plot(x_smooth, predicatedLine_smooth, color="#80A0FF", label='Predicted')
    legend = ax.legend(bbox_to_anchor=(1.02, 1), facecolor='black' if theme == 'black' else 'white')
    ax.xaxis.set_major_locator(plt.IndexLocator(base=1, offset=0))

    if theme=='black':
        legend.get_texts()[0].set_color('white') 
        legend.get_texts()[1].set_color('white')
        ax.set_xticklabels(xAxisvalue, color='white')
    else:
        ax.set_xticklabels(xAxisvalue, color='black')
...
# predicatedLine = [30, 30, 33, 28, 34, 29, 33, 28, 34, 29, 28, 33, 32]
# actualLine = [30, 30, 29, 33, 32, 29, 29, 33, 32, 29, 30, 31, 37]
# line_chart(xAxisvalue, predicatedLine, actualLine, title=title, theme='white')


def table_count(xAxisvalue, predicatedLine, actualLine,**optional_params):
    fig, ax = plt.subplots(figsize=(16, 9))
...
# predicatedLine = [30, 30, 33, 28, 34, 29, 33, 28, 34, 29, 28, 33, 32]
# actualLine = [30, 30, 29, 33, 32, 29, 29, 33, 32, 29, 30, 31, 37]
# table_count(xAxisvalue, predicatedLine, actualLine,theme='white')
...
# dot_plot(categories,values,theme='black')
...
# dot_plot_grouped(categories, values,theme='white')
...
# ylabel='Title'
# handles=['Group 1']
# scatter_plot_with_dotted_line(data,title=title,xlabel=xlabel,ylabel=ylabel, theme='white')
...
# ]
# colors = ["#FC2964", "#0095FF", "#5C33F6"]
# scatter_plot_graph(marker_shapes, x_values, series_data, colors, theme='white')
...
def forest_plot_graph(yAxisdata, xAxisdata, effect_sizes, color,confidence_intervals):
    ...
    x_vals = np.full(len(effect_sizes), np.nan) 
