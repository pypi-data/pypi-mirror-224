...
class S3Bucket(object):
    ...
    def upload_normal(self, path_prefix, file_upload):
        ...
        try:
            self.s3.upload_file(
                file_upload, self.bucket_name, object_name, Config=config
            )
    ...
    def download_file(self, object_name, path_local):
        ...
        try:
            self.s3.download_file(
                self.bucket_name, object_name, file_name, Config=config
            )
        except Exception as exc:
            print("some wrong!")
            print("error occurred.", exc)
    ...
    def download_files(self, path_prefix, path_local):
        ...
        list = self.s3.list_objects_v2(Bucket=self.bucket_name, Prefix=path_prefix)[
            "Contents"
        ]
        for key in list:
...
if __name__ == "__main__":
    ...
    s3_buk = S3Bucket()
    file_list = s3_buk.get_list_s3("")
    print(file_list)

    # 上传
    s3_buk.upload_normal("", "src/package/boto3_test/updown_s3.py")

    # 下载
    s3_buk.download_file("520-happy_0.0.2_x64.dmg", "src/package/boto3_test/")
