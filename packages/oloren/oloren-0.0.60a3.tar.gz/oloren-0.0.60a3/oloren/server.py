...
def set_vars(dispatcher_url, token):
    config["DISPATCHER_URL"] = dispatcher_url
    config["TOKEN"] = token
...
def run_blue_node(
    ...
    try:
        ...
        def node(node_data):
            ...
            if node_data["data"]["id"].endswith(f"-graph"): print("On uid ", uid, "Received node data graph: ", node_data)
            if f"{uid}-graph" == node_data["data"]["id"]:
                print("Received node data graph: ", node_data)
...
def operator(FUNCTION_NAME):
    ...
    try:
        ...
        DISPATCHER_URL_ = body.get("dispatcherurl") or f"http://{os.environ['DISPATCHER_URL']}"

        p = Process(target=execute_function, args=(DISPATCHER_URL_, body, FUNCTION_NAME))
        p.start()
...
def upload_file(file_path, dispatcher_url=config["DISPATCHER_URL"], token=config["TOKEN"]):
    """
...
def upload_file_purl(file_path, dispatcher_url=config["DISPATCHER_URL"]):
    """
    This function uploads a file to Orchestrator and returns the file presigned url.
    """
...
__all__ = ["register", "get_log_message_function","config", "run", "handler", "upload_file", "upload_file_purl", "download_from_signed_url", "download_from_file_record", "download_from_registered_file", "map", "set_vars", "app", "ProgressBar"]
