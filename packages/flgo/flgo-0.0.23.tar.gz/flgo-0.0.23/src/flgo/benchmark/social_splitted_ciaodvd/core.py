...
class TaskGenerator(BasicTaskGenerator):
    ...
    def __init__(self, rawdata_path=os.path.join(flgo.benchmark.path,'RAW_DATA', 'CIAODVD'), scale = 1.0, order='max', min_items = 3):
        super(TaskGenerator, self).__init__(benchmark='social_splitted_ciaodvd', rawdata_path=rawdata_path)
        self.scene='vertical'
    ...
    def get_task_name(self):
        return 'social_splitted_ciaodvd_u{}_o{}_m{}'.format(self.num_users, self.order,self.min_items)
    ...
    def load_data(self, *args, **kwargs):
        ...
        # extract zip
        if not os.path.exists(os.path.join(self.rawdata_path, 'readme.txt')): _ = extract_from_zip(zippath, self.rawdata_path)
        # read data into memory
        ratings = pd.read_csv(os.path.join(self.rawdata_path, 'movie-ratings.txt'), header=None)[[0,1,4]]
        self.ratings = ratings.rename(columns={0:'userID', 1:'movieID', 2:'Rating'})
        links = pd.read_csv(os.path.join(self.rawdata_path, 'trusts.txt'), header=None)
        self.links = links.rename(columns={0: 'trustorID', 1: 'trusteeID', 2: 'Trust'})
        return
