...
class TaskGenerator(BasicTaskGenerator):
    _DATASET_INFO = {
        'url':"https://www.cse.msu.edu/~tangjili/datasetcode/epinions.zip",
        'link_type':'trust',
    ...
    def get_task_name(self):
        return 'social_splitted_epinions_u{}_o{}_m{}'.format(self.num_users, self.order,self.min_items)
    ...
    def load_data(self, *args, **kwargs):
        ...
        # extract zip
        if not os.path.exists(os.path.join(self.rawdata_path, 'epinions')): _ = extract_from_zip(zippath, self.rawdata_path)
        # read data into memory
        ratings = scio.loadmat(os.path.join(self.rawdata_path, 'epinions', 'rating.mat'))['rating'][:,[0,1,3]]
        ratings = ratings.T
        rating_dict = {'userID':ratings[0], 'movieID':ratings[1], 'Rating':ratings[2]}
        self.ratings = pd.DataFrame(rating_dict)
        links = scio.loadmat(os.path.join(self.rawdata_path, 'epinions', 'trustnetwork.mat'))['trustnetwork']
        links = links.T
