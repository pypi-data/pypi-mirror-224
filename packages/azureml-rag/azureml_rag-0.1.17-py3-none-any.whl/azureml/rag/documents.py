...
def crack_documents(sources: Iterator[DocumentSource], file_extension_loaders=file_extension_loaders) -> Iterator[ChunkedDocument]:
    ...
    for source in sources:
        ...
        if mlflow_enabled():
            mlflow.log_metric(source.path.suffix, files_by_extension[source.path.suffix], step=int(time.time() * 1000))
...
def split_documents(documents: Iterable[ChunkedDocument], splitter_args: dict, file_extension_splitters=file_extension_splitters) -> Iterator[ChunkedDocument]:
    ...
    for document in documents:
        ...
        if mlflow_enabled():
            mlflow.log_metric('total_source_documents', total_documents, step=int(time.time() * 1000))
        ...
        if mlflow_enabled():
            mlflow.log_metric('total_chunked_documents', total_splits, step=int(time.time() * 1000))
