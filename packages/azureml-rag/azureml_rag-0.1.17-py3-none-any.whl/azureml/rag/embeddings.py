    ...
    def _get_embeddings_internal(self, input_documents: Union[Iterator[Document], BaseLoader, DocumentChunksIterator]) -> OrderedDict:
        ...
        for document in input_documents:
            ...
            if mtime \
                ...
                if mlflow_enabled():
                    mlflow.log_metric('Documents Reused', len(documents_embedded.keys()), int(time.time() * 1000))
                continue
            ...
            if mlflow_enabled():
                mlflow.log_metric('Documents to embed', len(documents_to_embed), int(time.time() * 1000))
