...
from azureml.rag.documents import BaseDocumentLoader
from azureml.rag.documents import SUPPORTED_EXTENSIONS, DocumentChunksIterator, DocumentSource, Document, split_documents, crack_documents, file_extension_loaders, file_extension_splitters
...
def custom_loading(python_file_path: str, ext_loaders, ext_splitters):
    ...
    for _, obj in inspect.getmembers(module):
        logger.info(f"Found {obj} in {module_name}", extra={'print': True})
        if inspect.isclass(obj) and issubclass(obj, BaseDocumentLoader) and obj != BaseDocumentLoader:
            ...
            logger.info(f"Registering custom loader for {file_extensions}", extra={'print': True})
