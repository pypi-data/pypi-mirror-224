...
def load_embeddings_container(embeddings_model_uri: str, logger, embeddings_container_path: Optional[str] = None, worker_id=0):
...
def chunk_embedder(worker_id, chunk_queue, embeddings_model_uri, embeddings_container, output, logger_queue):
    ...
    try:
        ...
        embedder = load_embeddings_container(embeddings_model_uri, logger, embeddings_container, worker_id)
...
def create_embeddings(chunks: Iterator[Document],
    ...
    num_workers = num_workers if num_workers > 0 else mp.cpu_count() // 2
    ...
    with mp.Manager() as manager:
        try:
            ...
            with mp.Pool(processes=num_workers) as pool:
                ...
                embedder_futures = [pool.apply_async(chunk_embedder, (i, chunk_queue, embeddings_model_uri, embeddings_container, output, logger_queue)) for i in range(num_workers)]
                ...
                for chunk in chunks:
                    ...
                    if len(chunk_batch) == batch_size:
                        ...
                        chunk_queue.put((batch_id, chunk_batch))
                        batch_id += 1
                        chunk_batch = []
                ...
                logger.info(f'==== Waiting for embedders to finish, {num_workers}/{batch_id} batches remaining')
...
def main(args, logger, activity_logger):
    ...
    if args.embeddings_container is not None:
        with track_activity(logger, 'init.load_embeddings_container') as embeddings_container_activity_logger:
            ...
            try:
                mnt_location = f'{os.getcwd()}/embeddings_container'
