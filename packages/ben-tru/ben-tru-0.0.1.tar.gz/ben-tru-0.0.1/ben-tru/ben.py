...
with resources.open_binary('ben-tru.data', 'coeficientes_emissoes_gee_ajustado.csv') as f:
  data = f.read()
  bytes_io = io.BytesIO(data)
coef_tep_to_ghg = pd.read_csv(bytes_io)

with resources.open_binary('ben-tru.data', 'correspondencia_MIP56_TRU51_BEN.csv') as f:
  data = f.read()
  bytes_io = io.BytesIO(data)
ben_to_tru51_sectors = pd.read_csv(bytes_io)

with resources.open_binary('ben-tru.data', 'correspondencia_produtos_TRU51_BEN.csv') as f:
  data = f.read()
  bytes_io = io.BytesIO(data)
...
class system:
    ...
    def import_data(self):
        ...
        with resources.open_binary('ben-tru.data','Matrizes Consolidadas (em tep) 1970 - 2022.xls') as f:
          data = f.read()
          bytes_io = io.BytesIO(data)
