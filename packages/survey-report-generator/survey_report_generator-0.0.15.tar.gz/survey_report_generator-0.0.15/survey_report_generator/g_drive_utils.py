...
def generate_credentials():
    ...
    # Get the package path for 'survey_report_generator/storage.json'
    try:
        package_path = pkg_resources.files('survey_report_generator').joinpath('storage.json')
    except FileNotFoundError:
        package_path = None

    # Check if the file exists within the package
    if package_path is None or not package_path.exists():
        # Fallback to the home directory version
        home_path = os.path.expanduser('~/storage.json')
        if os.path.exists(home_path):
            store = file.Storage(home_path)
        else:
            raise FileNotFoundError("No valid 'storage.json' file found in either the package or home directory.")
    else:
        store = file.Storage(str(package_path))
    creds = store.get()
