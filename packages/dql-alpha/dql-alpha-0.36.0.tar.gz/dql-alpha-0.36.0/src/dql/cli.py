...
def apply_udf(udf: str, source: str, target: str, params: Optional[str] = None):
    get_catalog().apply_udf(udf, source, target, params)
...
def main(argv: Optional[List[str]] = None) -> int:  # noqa: C901
    ...
    try:
        if args.command == "cp":
            ...
            apply_udf(args.udf, args.source, args.target, args.udf_params)
        else:
