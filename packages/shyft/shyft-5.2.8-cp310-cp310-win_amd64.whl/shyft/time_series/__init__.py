...
if os.name == 'nt':
    ...
    if hasattr(os,'add_dll_directory'):  # try to even make it work on windows with new dll load limitations
        os.add_dll_directory(path.join(path.dirname(__file__),'..','lib'))
# first pull in type from the python extensions
from shyft.time_series._time_series import *

__doc__ = _time_series.__doc__
__version__ = _time_series.__version__
...
UtcTimeVector.__repr__ = lambda self: ShowUtcTime
...
GeoPoint.__repr__ = lambda self: "GeoPoint({0},{1},{2})".format(self.x, self.y, self.z)
