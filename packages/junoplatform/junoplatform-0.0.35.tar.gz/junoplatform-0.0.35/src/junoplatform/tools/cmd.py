...
@pass_base
@auth
def create(base, name, plant, module):
...
@pass_base
@auth
def new(base, conf_file,  tag, message, input):
    ''' make a new package
    '''
    try:
...
def list_current_deployed(base, plant:str, module=str):
    api = f"{api_url}/deploys/current"
    params = {}
    params["plant"] = plant
    params["module"] = module
...
def status(base, plant, module):
    ...
    '''
    res = list_current_deployed(base=base, plant=plant, module=module)
    if isinstance(res, str):
        logging.error(res)
    else:
        logging.info(f"results:\n{json.dumps(res, indent=2)}")

def list_pant(base, name:str):
    api = f"{api_url}/plants"
    params = {}
    params["name"] = name
...
def add_plant(base, name, location, description):
    api = f"{api_url}/plant"
    plant = {}
    plant["id"] = 0
    plant["name"] = name
