...
from src.volworld_aws_api_common.api.dom_id import dom_id
from src.volworld_aws_api_common.test.behave.selenium_utils import w__click_element_by_dom_id, get_element_by_dom_id
...
def click_to_open_list_nav_drawer(c, tar_id: str, prefix=None) -> WebElement:
    if prefix is None:
        prefix = list()
    elm = w__click_element_by_dom_id(c, prefix + [CA.List, CA.Navigator, CA.Button, tar_id])
...
def click_to_close_list_nav_drawer(c, tar_id: str, prefix=None) -> WebElement:
    if prefix is None:
        prefix = list()
    dom_id_list = prefix + [CA.Drawer, tar_id, CA.Header, CA.Close]
