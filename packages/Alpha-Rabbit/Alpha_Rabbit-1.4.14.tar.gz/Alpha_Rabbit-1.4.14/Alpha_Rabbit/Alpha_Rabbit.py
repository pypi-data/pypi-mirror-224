...
class multi_factor_test(object):
    ...
    def factors_abnormal_ret(self,factordf,return_col,factorlist,days,pricedf = None):
        ...
        ret_k = df.groupby(level = 'date',group_keys = False).apply(lambda x: sm.formula.ols(return_col+'~'+'+'.join(factorlist),data = x).fit().params)
        del ret_k['Intercept']
        return ret_k
    ...
    def in_some_pool(self,df,pool_components):
        temp1 = pd.concat([df,pool_components],axis = 1).dropna(subset='cap')
        # factordf = df.copy()
        ...
        # del testdf['inpool']
        return temp1.dropna(subset = list(temp1)[:-1],how = 'all')
