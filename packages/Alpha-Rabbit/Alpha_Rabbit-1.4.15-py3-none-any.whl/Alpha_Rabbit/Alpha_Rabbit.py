...
class multi_factor_test(object):
    ...
    def factors_abnormal_ret(self,factordf,return_col,factorlist,days,index_price = None,pricedf = None):
        df = factordf.copy()
        if pricedf is not None:
            ...
            if index_price is not None:
                ...
                df = ml.trans_ex_return(df,index_price,str(days)+'D').rename(columns = {str(days)+'D':return_col+str(days)+'D'})
            df.dropna(subset = return_col+str(days)+'D',inplace = True)
