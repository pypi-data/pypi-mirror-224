...
def cat_plot(df,target,title='',drop=[]):
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(17, 3*len(cols)),squeeze=False)
    ...
    if len(stt)>1:
        for i, var_name in enumerate(cols,start=1):
            ...
            if grouped[var_name].value_counts().shape[0]<=6:
                _, wedges, autotexts = axes[i,0].pie(grouped['values'], labels=grouped[var_name], autopct='%1.1f%%', startangle=10, colors=sns.color_palette('pastel'),
                ...
                sns.stripplot(x=var_name, y=target, data=df,ax=axes[i,0],palette='bright') 
                
            #axes[0,0].set_size_inches(15, (10*len(cols))/3)
            #axes[0,0].set_position([0.1, 0.1, 1, 1])
            #axes[0,1].set_position([0.1, 0.1, 0.4, 0.4])
            ...
            ax=sns.countplot(x=var_name,data=df, ax=axes[i, 1],palette=sns.color_palette('pastel'))
            
            ax.bar_label(ax.containers[0], fmt='%.1f',color='white',fontsize='7')
            ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right',size=15)
            ax.set_yticklabels(ax.get_yticklabels(),size=15)
            ax.set_xlabel('')
            print(grouped['values'].min(), grouped['values'].max())
            #axes[i,1].set_ylim((grouped['values'].min()), (grouped['values'].max()))
        ...
        for i, var_name in enumerate(cols,start=1):
            ...
            if grouped[var_name].value_counts().shape[0]<=6:
                ...
                sns.stripplot(x=var_name, y=target,hue=target, data=df,ax=axes[i,0],palette='bright')     
            ...
            #sns.set_style("whitegrid", {'grid.linestyle': '--'})
            ax=sns.barplot(x=var_name, y='values', data=grouped, ax=axes[i, 1],palette=sns.color_palette('pastel'))
            ax.bar_label(ax.containers[0], fmt='%.1f',color='white',fontsize='7')
            ax.set_xticklabels(ax.get_xticklabels(), rotation=45, ha='right',size=15)
            ax.set_yticklabels(ax.get_yticklabels(),size=15)
            ax.set_xlabel('')
