...
class Jira(object):
    ...
    def __init__(self, username=None, token=None, proxy=False):
        ...
        if not proxy:
            ...
            self.r = self._session.post(url, headers=self.headers, data=payload, proxies=proxies)
    ...
    def generated_issue(self, summary=None, description=None, assignee=None,
        ...
        url = f"{self.base_url}{self.base_path}"
        r = self._session.post(url, cookies=self.cookies, data=payload, headers=self.headers)
        response = r.json()
        return response
