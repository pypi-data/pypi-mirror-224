...
class BaseFrameCountDataGatherer(DataGatherer):
    ...
    def prepare(self) -> None:
        ...
        source_vlan_config = list(self._flow.source.vlan_config)
        self._rx_vlan_header_size = len(
            destination_vlan_config
        ) * VLAN_HEADER_LENGTH
        self._tx_vlan_header_size = len(
            source_vlan_config
        ) * VLAN_HEADER_LENGTH
    ...
    def summarize(self) -> None:
        ...
        self._framecount_data._total_rx_vlan_bytes = \
        ...
        total_tx_packets = tx.PacketCountGet()
        if total_tx_packets:
            ts_tx_first_ns = tx.TimestampFirstGet()
            ts_tx_last_ns = tx.TimestampLastGet()
        else:
            ts_tx_first_ns = None
            ts_tx_last_ns = None
        self._framecount_data._total_tx_bytes = tx.ByteCountGet()
        self._framecount_data._total_tx_vlan_bytes = \
             total_tx_packets * self._tx_vlan_header_size
        self._framecount_data._total_tx_packets = total_tx_packets
        self._framecount_data._timestamp_tx_first = \
            pandas.to_datetime(ts_tx_first_ns)
        self._framecount_data._timestamp_tx_last = \
            pandas.to_datetime(ts_tx_last_ns)
