...
from flask import redirect, send_file, request, render_template, Blueprint, jsonify, make_response, session
...
ALLOWED_ORIGINS = ["http://localhost:5000",
                   "https://login.microsoftonline.com", "https://rconnect.edav.cdc.gov", "https://rstudio.edav.cdc.gov"]
...
CORS(app, origins=ALLOWED_ORIGINS, allow_headers=[
    "Content-Type", "Authorization", "Access-Control-Allow-Credentials"], resources={r"/*": {"origins": "*", "methods": ["GET", "POST", "PUT", "DELETE"]}},
        ...
        try:
            ...
            if id_token is None:
                ...
                flask_response.set_cookie(
                    'user_id', user_id, secure=True, samesite='Strict')
                # Set the user_id as a secure cookie on the response
                # TODO:  secure=True, httponly=True,
                flask_response.set_cookie(
                    'id_token', id_token, secure=True, samesite='Strict')
...
def get_login_redirect_response():
    ...
    # TODO  secure=True,
    # response.set_cookie('id_token', httponly=True,  secure=True, samesite='Strict')
...
class AuthCallback(Resource):
    ...
    def get(self):
        ...
        if auth_code is None:
            msg = f"Missing authorization code in url {url_with_error}"
            print(msg)
            if not session.get("redirect_attempted"):
                ...
                return {"error": "Authorization code missing after redirect attempt."}, 400

        # If you reach here, reset the redirection attempt marker
        session["redirect_attempted"] = False
        ...
        if id_token and '.' in id_token and id_token.count('.') >= 2:
            try:
                ...
                print(ex)
                return {"message": "Error in decoding id_token: str(ex)"}
    ...
    def post(self):
        ...
        with tracer.start_as_current_span("metadata_excel_file_upload"):
            ...
            try:
                ...
                content_result, authorized_tables_count, unauthorized_table_count = schema.upload_schema_manifest_excel_file(
                    manifest_excel_file_path_temp, config, schema_json_file_path, authenticated_user_id)
    ...
    def post(self):
        ...
        with tracer.start_as_current_span("metadata_json_file_upload"):
            ...
            try:
                ...
                content_result, authorized_tables_count, unauthorized_table_count = schema.upload_schema_manifest_json_file(
