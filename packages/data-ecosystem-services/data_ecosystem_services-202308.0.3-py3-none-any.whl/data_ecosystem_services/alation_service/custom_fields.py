...
class CustomFieldsEndpoint():
    ...
    def update(self, edc_alation_api_token, edc_alation_base_url, object_type, alation_datasource_id, key, fields_dict, force_submit, valid_editable_fields, valid_date_fields):
        """
            ...
            try:
                ...
                if len(self.updates) >= batch_size or force_submit:
                    ...
                    array_of_json = [json.dumps(
                        u, cls=EnhancedJSONEncoder, separators=(',', ':')) for u in self.updates]
                    ...
                    data_str = '\n'.join(json.dumps(item, cls=EnhancedJSONEncoder)
