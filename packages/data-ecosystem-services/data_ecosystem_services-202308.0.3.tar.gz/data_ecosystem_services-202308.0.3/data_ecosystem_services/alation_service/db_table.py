...
class Table:
        ...
        with tracer.start_as_current_span("get_tables_for_schema"):
            ...
            for item in tables_result_json:
                ...
                for field in item['custom_fields']:
                    ...
                    # handle stewards
                    if field['field_name'] == "Steward":
                        table_steward_list = field['value']

                        # Create a list of unique otype and oid combinations from table_steward_list
                        existing_combinations = {
                            (item['otype'], item['oid']) for item in schema_steward_list + table_steward_list}

                        # Add items from schema_steward_list to table_steward_list if not present
                        for item in schema_steward_list:
                            combination = (item['otype'], item['oid'])
                            if combination not in existing_combinations:
                                item_copy = item.copy()
                                item_copy["is_inherited"] = "inherited"
                                table_steward_list.append(item_copy)
                            else:
                                if any(existing_item['otype'] == item['otype'] and existing_item['oid'] == item['oid'] for existing_item in table_steward_list):
                                    # The item exists in table_steward_list, set is_inherited to empty string
                                    for existing_item in table_steward_list:
                                        if existing_item['otype'] == item['otype'] and existing_item['oid'] == item['oid']:
                                            existing_item["is_inherited"] = ""

                        # Rename 'Steward' to 'Steward_Initial'
                        new_item['Steward_Initial'] = new_item.pop('Steward')

                        # Add new 'Steward' attribute with value "merged"
                        new_item['Steward'] = table_steward_list
    ...
    def get_tables_for_schema_for_excel(self, edc_alation_api_token, config, alation_datasource_id, alation_schema_id):
        """
