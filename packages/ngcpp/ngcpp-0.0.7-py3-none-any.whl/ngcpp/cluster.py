...
class Cluster:
    ...
    def report_hangs(self, verbose=True):
        ...
        for _it, row in df.iterrows():
            cur_id = row["Id"]
            print(f"Checking job {_it+1}/{len(df)} : {cur_id}", end="\r")
            dir_fp = f"/tmp/ngcpp/{cur_id}"
            if not os.path.exists(dir_fp):
                os.makedirs(dir_fp)
            _, stderr = run_simple_command(
                "ngc batch telemetry --interval-time 30 --interval-unit MINUTE --format_type csv"
                f" {cur_id} > {dir_fp}/telemetry.csv"
            )
            if stderr:
                logging.info(stderr)
                raise RuntimeError(f"ngc batch telemetry error {cur_id}")
