...
Originally posted by AlfredoCarella at https://github.com/alfredocarella/simplebendingpractice
"""
    ...
    def _plot_analytical(self, ax: plt.axes, sym_func, title: str = "", maxmin_hline: bool = True, xunits: str = "",
        ...
        if maxmin_hline:
            ...
            if abs(max(y_vec)) > tol:
                ...
                plt.annotate(
                    '${:0.1f}'.format(y_vec[max_idx] * (1 - 2 * inverted)).rstrip('0').rstrip('.') + " $ {}".format(
                        yunits),
            ...
            if abs(min(y_vec)) > tol:
                ...
                plt.annotate(
                    '${:0.1f}'.format(y_vec[min_idx] * (1 - 2 * inverted)).rstrip('0').rstrip('.') + " $ {}".format(
                        yunits),
