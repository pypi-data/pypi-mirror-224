...
class NodeSummary(QDialog):
    def __init__(self, nir: Node | None = None, parent: QWidget | None = None) -> None:
        ...
        self._ui.attr_tabel_widget.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self._ui.attr_tabel_widget.setSelectionMode(QAbstractItemView.SelectionMode.SingleSelection)
        self._ui.attr_tabel_widget.setAlternatingRowColors(True)
        self._ui.attr_tabel_widget.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self._ui.attr_tabel_widget.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.Stretch)
        self._ui.attr_tabel_widget.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.Stretch)

        self._ui.inputs_add.clicked.connect(lambda _: self.addItem(self._ui.inputs_list_widget, "-"))
        self._ui.inputs_del.clicked.connect(lambda _: self.delItem(self._ui.inputs_list_widget))
        self._ui.outputs_add.clicked.connect(lambda _: self.addItem(self._ui.outputs_list_widget, "-"))
        self._ui.outputs_del.clicked.connect(lambda _: self.delItem(self._ui.outputs_list_widget))
        self._ui.attr_add.clicked.connect(lambda _: self.addRow("-", '-', False))
    ...
    def delRow(self):
        row_id = [item.row() for item in self._ui.attr_tabel_widget.selectedItems()]
