...
class GraphEdge(QGraphicsObject):
    ...
    def needUpdate(self, nodes: List['GraphNode']):
        assert len(nodes) == 1
        node = nodes[0]
        src_obj = [n.read_ext('bind_gnode') for n in self._ir.src if n.read_ext('bind_gnode') is not None]
        dst_obj = [n.read_ext('bind_gnode') for n in self._ir.dst if n.read_ext('bind_gnode') is not None]
        if self._ir.isInput:
        ...
        if node not in src_obj + dst_obj:
            node.disconnect(self.needUpdate)
