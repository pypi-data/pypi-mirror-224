            ...
            if (arg is not None) and (arg is not inspect._empty):
                ...
                if param == "frequency":
                    if function.__name__ == "get_universe":
                        try:
                            arg_enum = UniverseFrequencyType(arg)
                        except Exception as e:
                            raise PrismValueError(f"{e}", valid_list=UniverseFrequencyType)
                    else:
                        try:
                            arg_enum = FrequencyType(arg)
                        except Exception as e:
                            raise PrismValueError(f"{e}", valid_list=FrequencyType)
                elif param == "datetype":
                    try:
                        arg_enum = DateType(arg)
                    except Exception as e:
                        raise PrismValueError(f"{e}", valid_list=DateType)
                elif param == "periodtype":
                    try:
                        arg_enum = PeriodType(arg)
                    except Exception as e:
                        raise PrismValueError(f"{e}", valid_list=PeriodType)
                elif param == "adjustment": # can be string or bool
                    if arg not in [a.value for a in AdjustmentType]:
                        raise PrismValueError(f"Invalid adjustment argument is given. ", valid_list=AdjustmentType)
                elif param == "shownid":
                    for idx, id in enumerate(arg):
                        arg[idx] = get_sm_attributevalue(id)
                elif param == "rank_method":
                    try:
                        arg_enum = RankType(arg)
                    except Exception as e:
                        raise PrismValueError(f"{e}", valid_list=RankType)
                elif param == "method":
                    try:
                        arg_enum = FillnaMethodType(arg)
                    except Exception as e:
                        raise PrismValueError(f"{e}", valid_list=FillnaMethodType)
                # elif param == "beyond":
                #     try:
                ...
                #         raise PrismValueError(f"{e}", valid_list=BeyondType)
                    ...
                    assert (date >= BEGINNING_DATE) & (date <= ACTIVE_DATE), "Not a valid date."
                    ...
                    try:
                        arg_enum = FinancialPreliminaryType(arg)
                    except Exception as e:
                        raise PrismValueError(f"{e}", valid_list=FinancialPreliminaryType)
                elif param == "setting":
...
def get_sm_attributevalue(attribute: str):
    ...
    smattributes_lower = {
        a.lower().replace(' ', '').replace('_', ''): SMValues[a]
        for a in SMValues.keys()
    }
    smattributes_lower.update({
        a.lower().replace(' ', '').replace('_', ''): a
        for a in SMValues.values()
    })
