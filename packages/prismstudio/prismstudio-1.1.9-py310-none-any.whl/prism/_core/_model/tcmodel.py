...
from ..._common.const import TCModelComponentType, PrismModelCategoryType
from ..._prismcomponent.prismcomponent import _PrismModelComponent, _PrismComponent
from ..._utils import _validate_args


class _PrismTCModelComponent(_PrismModelComponent):
    _component_category = PrismModelCategoryType.TC
...
class almgren(_PrismTCModelComponent):
    ...
    --------
        ...
        >>> prism.tcmodel.almgren(trade_amount, 10, 5).get_data("Korea_primary", "2020-01-01", shownid=["companyname"])
    ...
    _component_name = TCModelComponentType.ALMGREN

    @_validate_args
    def __init__(
        self,
        transaction: _PrismComponent,
        tradeduration: int,
        averageperiod: int,
    ):
        query = {}
        child_query = copy.deepcopy(transaction._query)
        child_query['nodeid'] = str(uuid.uuid4())
        query["transaction"] = child_query
        query["tradeduration"] = tradeduration
        query["averageperiod"] = averageperiod
...
class bidaskspread(_PrismTCModelComponent):
    ...
    _component_name = TCModelComponentType.BIDASKSPREAD
