...
class mongodb:
    ...
    def get_product_by_upc(self, upc, limit):
        ...
        response = self.DB.find({ "upc": upc }, {'Response':0,'_id':0}).sort("timestamp", -1).limit(limit)
    ...
    def get_product_by_title_search(self, search, limit):
        ...
        if not limit:
            ...
            response = self.DB.find({'$text': {'$search': f'/{search}/'}}, {'Response': 0, '_id': 0}).sort("timestamp", -1).limit(limit)
