...
def compute_metrics(*,
    ...
    validate_compute_metrics(kwargs, task_type, y_pred, y_pred_proba, y_test)

    if task_type is None:
        computed_metrics = compute_supported_metrics(kwargs, y_pred, y_pred_proba, y_test)
    else:
        computed_metrics = compute_metrics_on_task_type(kwargs, task_type, y_pred, y_pred_proba, y_test)

    return computed_metrics


def compute_supported_metrics(kwargs, y_pred, y_pred_proba, y_test):
    ...
    for task_type, required_metrics in required_task_type_metrics_map.items():
        if len(required_metrics) > 0:
            ...
            try:
                caluculated_metrics = compute_metrics_on_task_type(kwargs, task_type, y_pred, y_pred_proba, y_test)
...
def validate_compute_metrics(kwargs, task_type, y_pred, y_pred_proba, y_test):
    if y_test is None:
...
def compute_metrics_on_task_type(kwargs, task_type, y_pred, y_pred_proba, y_test):
    ...
    if task_type in classification_task_types:
        computed_metrics = compute_metrics_function(kwargs, task_type, y_pred, y_pred_proba, y_test)

    else:
        computed_metrics = compute_metrics_function(kwargs, task_type, y_pred, y_test)
