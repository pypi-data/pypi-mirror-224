...
def bond_split(file,n,out) :
    ...
    #df = pd.read_csv(file, header = None)
    df = split(file,n)
    bonds=pd.read_csv(nf_path + "/../Data/bonds.csv")
    for i in range(0,len(df)) :
        tot = 0
        h = 0
        S = 0
...
def p_aa(prop,a):
    if ((a.upper()=='A') or (a.upper()=='C') or (a.upper()=='D') or (a.upper()=='E') or (a.upper()=='F') or (a.upper()=='G') or (a.upper()=='H') or (a.upper()=='I') or (a.upper()=='K') or (a.upper()=='L') or (a.upper()=='M') or (a.upper()=='N') or (a.upper()=='P') or (a.upper()=='Q') or (a.upper()=='R') or (a.upper()=='S') or (a.upper()=='T') or (a.upper()=='V') or (a.upper()=='W') or (a.upper()=='Y')):
        data=pd.read_table(nf_path + '/../Data/z_aaindex.csv',sep=',',index_col='INDEX' )
...
def autocorr_full_aa(filename,d,out):
    if (d<=30):
        ...
        prop=list((pd.read_csv(nf_path + '/../Data/aaindices.csv',sep=',',header=None)).iloc[0,:])
        output=[[]]
...
def main():
    ...
    AAindices = nf_path + '/../Data/aaind.txt'
    AAIndex = pd.read_csv(nf_path + '/../Data/aaindex.csv',index_col='INDEX');
    AAIndexNames = pd.read_csv(nf_path + '/../Data/AAIndexNames.csv',header=None);
