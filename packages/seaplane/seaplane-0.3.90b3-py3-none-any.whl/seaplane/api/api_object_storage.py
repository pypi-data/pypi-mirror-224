...
class ObjectStorageAPI:
    ...
    def download(self, bucket_name: str, path: str) -> bytes:
        ...
        return unwrap(
            self.req(
                lambda access_token: requests.get(url, params=params, headers=headers(access_token, "application/octet-stream"))
            )
        )
        ...
        return unwrap(
            self.req(
                lambda access_token: requests.put(
                    url, params=params, data=object, headers=headers(access_token, "application/octet-stream")
                )
            )
        )
    ...
    def upload_file(self, bucket_name: str, path: str, object_path: str) -> str:
        ...
        return unwrap(
            self.req(
                lambda access_token: requests.put(
                    url, params=params, data=file_data, headers=headers(access_token, "application/octet-stream")
                )
            )
        )
