...
def multithread(func,input,progressbar=None,threads=None,status=None):
        ...
        if threads is None:
                ...
                memory_buffer = 1024 # specify this in MB
                available_memory = (virtual_memory().available)//(1024*1024)
                # available_memory = 2056         # Just to test the calculation
                max_threads_mem = (available_memory-memory_buffer)//minimum_memory_per_thread
                cprint("INFO: Memory buffer - {} MB".format(memory_buffer))
