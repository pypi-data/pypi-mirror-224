...
def runtests(ctx, path):
    ...
    for block in block_json:
        ...
        print(block_response.json()["run"]["traces"])
        ...
        for value_group in values:
            print(value_group)
            for value_data in value_group[1]:
                print(value_data)
                for value_in in value_data:
                    print(value_in)
                    """ val = value_data.get('value', {}).get('input', '')
