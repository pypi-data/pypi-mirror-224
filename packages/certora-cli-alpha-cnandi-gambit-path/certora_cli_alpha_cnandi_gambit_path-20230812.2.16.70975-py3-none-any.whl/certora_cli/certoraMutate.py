...
def submit(args: argparse.Namespace) -> None:
    ...
    # call gambit
    if args.gambit_conf is not None:
        gambit_conf = load_gambit_conf(args.gambit_conf)
        manual_mutants = []
        if Constants.MANUAL_MUTANTS in gambit_conf:
            manual_mutants = parse_manual_mutations(args.gambit_conf)
            if args.debug:
                print(f"successfully parsed manual mutants from {args.gambit_conf}")

        generated_mutants = []
        if get_num_mutants(args) > 0:
...
def run_gambit(args: argparse.Namespace) -> List[GambitMutant]:
    gambit_conf = load_gambit_conf(args.gambit_conf)
    # By default, we should just send the gambit_conf straight to gambit.
    conf_to_send_to_gambit: Path = args.gambit_conf
    ...
    new_gambit_conf = args.gambit_conf.parent / Constants.TMP_GAMBIT
    if Constants.MANUAL_MUTANTS in gambit_conf:
        gambit_conf.pop(Constants.MANUAL_MUTANTS)
        with open(new_gambit_conf, 'w') as g_conf:
            json.dump(gambit_conf, g_conf)
            conf_to_send_to_gambit = new_gambit_conf
    ...
    gambit_args = [gambit_exec, "mutate", "--json", str(conf_to_send_to_gambit),
                   "-o", str(get_gambit_out_dir(args)), "--skip_validate"]
    ...
    if os.path.exists(new_gambit_conf):
        os.remove(new_gambit_conf)
    if args.debug:
...
def parse_manual_mutations(gambit_conf: Path) -> List[GambitMutant]:
    print(f"Parsing manual mutations from {gambit_conf} file.")
    ret_mutants = []
    manual_id = 0
    with open(gambit_conf, "r") as conf:
        ...
        for orig_file in gambit_cfg[Constants.MANUAL_MUTANTS]:
            if not Path(orig_file).exists():
                ...
                continue
            manual_mutant_dir = gambit_conf.resolve().parent.joinpath(gambit_cfg[Constants.MANUAL_MUTANTS][orig_file])
...
def load_gambit_conf(gambit_conf: str) -> Any:
    if gambit_conf is not None:
        with open(Path(gambit_conf), 'r') as g_conf:
            return json.load(g_conf)
    else:
        print(f"{gambit_conf} is None.")
        sys.exit(1)


def get_num_mutants(args: argparse.Namespace) -> int:
    conf = load_gambit_conf(args.gambit_conf)
    if Constants.NUM_MUTANTS not in conf:
        return DEFAULT_NUM_MUTANTS
    else:
        return conf[Constants.NUM_MUTANTS]
...
def get_gambit_out_dir(args: argparse.Namespace) -> Path:
    ...
    conf = load_gambit_conf(args.gambit_conf)
    if Constants.GAMBIT_OUTDIR not in conf:
        return Path(Constants.GAMBIT_OUT)
    else:
        return conf[Constants.GAMBIT_OUTDIR]
