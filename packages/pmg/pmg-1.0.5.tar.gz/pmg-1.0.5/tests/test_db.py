...
def init_test_db(db):
    db.execute("CREATE TABLE IF NOT EXISTS TestTable (IntValue INTEGER NOT NULL, TextValue TEXT NULL, FloatValue REAL NULL);")
    db.executemany("INSERT INTO TestTable VALUES (?, ?, ?)", ((random.randint(-sys.maxsize + 1, sys.maxsize), str(random.random()), random.random()) for i in range(100)))
    db.commit()
