...
from .settings import REDIS_CHECKPOINT
...
class Sync(Base, metaclass=Singleton):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.checkpoint_memory = REDIS_CHECKPOINT
    ...
    def analyze(self) -> None:
        for node in self.tree.traverse_breadth_first():
            ...
            for index in self.indices(node.table, node.schema):
                ...
                logger.info(f'Create one with: "\033[4m{query}\033[0m"\n')
                logger.info("-" * 80)
                logger.info("\n")
                sys.stdout.flush()
    ...
    def teardown(self, drop_view: bool = True) -> None:
        ...
        try:
            self.redis.unset(self._checkpoint_file)
    ...
    def sync(
        ...
        logger.warning(
            f'Synchronizing {count} records for node with table {node.schema}.{node.table} '
            f'containing {len(node.children)} children for index: {self.index}'
        )
    ...
    def checkpoint(self) -> int:
        ...
        if self.checkpoint_memory:
            if self.redis.exists(self._checkpoint_file):
                value = self.redis.get(self._checkpoint_file).decode() or 0
                self._checkpoint: int = int(value)
        else:
    ...
    def checkpoint(self, value: Optional[str] = None) -> None:
        ...
        # Set checkpoint in memory
        if self.checkpoint_memory:
            self.redis.set(self._checkpoint_file, value)
        else:
    ...
    def _status(self, label: str) -> None:
        logger.warning(
            f"{label} {self.database}:{self.index} Xlog: [{self.count['xlog']:,}] => Db: [{self.count['db']:,}] => "
            f"Redis: [total = {self.count['redis']:,} pending = {self.redis.qsize:,}] => {self.search_client.name}: "
            f"[{self.search_client.doc_count:,}]"
        )
...
@click.option("--schema", "-s", help="PG_SCHEMA override")
@click.option(
    "--verbose",
    "-v",
    is_flag=True,
