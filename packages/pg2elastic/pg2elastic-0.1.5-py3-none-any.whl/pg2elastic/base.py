...
class Base(object):
    def __init__(
        ...
        self.__schema: str = PG_SCHEMA or ''
    ...
    def schemas(self) -> dict:
        ...
        # Default schema provided, filter out other schemas
        if self.__schema:
            for schema in self.__schemas:
                if schema != self.__schema:
                    self.__schemas.remove(schema)
            logger.debug(f'Applying & using only predefined schema: {self.__schema}')
    ...
    def drop_triggers(
        ...
        """Drop all pg2elastic defined triggers in database."""
        queries: List[str] = []
        for table in self.tables(schema):
    ...
    def disable_triggers(self, schema: str) -> None:
        """Disable all pg2elastic defined triggers in database."""
        for table in self.tables(schema):
    ...
    def enable_triggers(self, schema: str) -> None:
        """Enable all pg2elastic defined triggers in database."""
        for table in self.tables(schema):
