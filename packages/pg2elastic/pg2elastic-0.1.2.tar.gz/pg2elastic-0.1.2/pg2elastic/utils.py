...
def timeit(func: Callable):
    def timed(*args, **kwargs):
        ...
        fn = func(*args, **kwargs)
        until: float = time()
        logger.info(f"{func.__name__}: {until - since} secs\n")
        return fn
...
def compiled_query(
    ...
    if label:
        ...
        logger.info(f"\033[4m{label}:\033[0m\n{query}\n")
    else:
