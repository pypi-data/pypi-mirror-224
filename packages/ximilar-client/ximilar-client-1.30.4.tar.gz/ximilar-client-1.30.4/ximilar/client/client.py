...
class RestClient(object):
    ...
    def cv2img_to_base64(self, image, image_space="RGB", resize=True):
        ...
        retval, buffer = cv2.imencode(".jpg", image, params=[cv2.IMWRITE_JPEG_QUALITY, 99])
    ...
    def base64_to_cv2img(self, base64image):
        ...
        try:
            ...
            image = numpy.frombuffer(img_array, dtype=numpy.uint8)
    ...
    def load_url_image(self, path, resize=True, timeout=30):
        ...
        img_bin = r.content
        image = numpy.asarray(bytearray(img_bin), dtype="uint8")
    ...
    def preprocess_records(self, records):
        ...
        for i in range(len(records)):
            if (
                ...
                and "_text_data" not in records[i]
    ...
    def parallel_records_processing(self, records, method, max_workers=3, batch_size=1, output=False):
        ...
        def catch_exception_method(method_to_process, records):
            try:
                return method_to_process(records)
            except Exception as e:
                return None

        executor = concurrent.futures.ThreadPoolExecutor(max_workers=max_workers)
        futures = [
            {"future": executor.submit(catch_exception_method, method, records_to_proc), "size": len(records_to_proc)}
