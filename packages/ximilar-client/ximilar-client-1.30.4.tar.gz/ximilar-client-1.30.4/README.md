...
This Python 3.9+ Client library is lightweight wrapper for `ximilar.com` and `vize.ai`. 
...
This will install also urllib3, requests, tqdm and pytest library. This will not install python-opencv which is required if you want to upload local images to the Ximilar system. For more information about installing opencv on different systems we have a small page in our [docs](https://docs.ximilar.com/quickstart/credits/).
