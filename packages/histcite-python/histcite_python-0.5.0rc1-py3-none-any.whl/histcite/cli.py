...
def main():
    ...
    cm = ComputeMetrics(docs_df, citation_relationship, refs_df, args.source_type)
    cm.write2excel(os.path.join(output_path, 'descriptive_statistics.xlsx'))
    
    doc_indices = citation_relationship[citation_relationship['LCS']>0].sort_values('LCS', ascending=False).index[:args.node_num].tolist()
