...
class GraphViz:
    def __init__(self, docs_df: pd.DataFrame, 
        ...
        self.merged_docs_df = docs_df.merge(citation_relationship, left_index=True, right_index=True, suffixes=(None, '_y')).drop(columns=['doc_index_y'])
        self.source_type = source_type
    
    @staticmethod
    def __generate_edge(doc_index: int, 
                        related_doc_index_list: Union[str, list[int]], 
                        citation_type: Literal['cited', 'citing']) -> set[tuple[int, int]]:
    ...
    def __generate_edge_set_from_specific_doc(self, doc_index: int, edge_type: Literal['cited', 'citing']):
        def pipeline(doc_index: int):
            ...
            if isinstance(cell, str):
                ...
                if edge_type == 'cited':
                    edge_set.update(self.__generate_edge(doc_index, related_doc_index, 'cited'))
                else:
                    edge_set.update(self.__generate_edge(doc_index, related_doc_index, 'citing'))
    ...
    def __generate_edge_set_from_multi_doc(self, doc_indices: list[int]):
        edge_set: set[tuple[int, int]] = set()
        for idx in doc_indices:
            ...
            if isinstance(cited_doc_index, str):
                edge_set.update(self.__generate_edge(idx, cited_doc_index, 'cited'))
            if isinstance(citing_doc_index, str):
                edge_set.update(self.__generate_edge(idx, citing_doc_index, 'citing'))
        edge_set = {(edge[0], edge[1]) for edge in edge_set if edge[0] in doc_indices and edge[1] in doc_indices}
        return edge_set
    
    def __generate_edge_set(self):
        if len(self.doc_indices)>1:
            edge_set = self.__generate_edge_set_from_multi_doc(self.doc_indices)
        else:
            initial_doc_index = self.doc_indices[0]
            if self.edge_type == 'cited':
                edge_set = self.__generate_edge_set_from_specific_doc(initial_doc_index, 'cited')
            elif self.edge_type == 'citing':
                edge_set = self.__generate_edge_set_from_specific_doc(initial_doc_index, 'citing')
            elif self.edge_type is None:
                edge_set = self.__generate_edge_set_from_specific_doc(initial_doc_index, 'cited')
                edge_set.update(self.__generate_edge_set_from_specific_doc(initial_doc_index, 'citing'))
            else:
                raise ValueError('Parameter edge_type must be one of "cited", "citing" or None')
        
        # drop nodes without PY info
        if len(self.empty_year_index) > 0:
            edge_set = {(edge[0], edge[1]) for edge in edge_set if edge[0] not in self.empty_year_index and edge[1] not in self.empty_year_index}

        # generate node_list according to edges
    ...
    def generate_dot_file(self, doc_indices: Union[list[int], int], 
                          edge_type: Optional[Literal['cited', 'citing']] = None,
                          show_timeline: bool = True):
        if isinstance(doc_indices, list) and len(doc_indices) > 1:
            assert edge_type is None, "Parameter edge_type should be None when doc_indices contains >1 index"
            self.doc_indices = doc_indices
        elif isinstance(doc_indices, int):
            assert doc_indices in self.merged_docs_df.index, "Don't select doc_index not in docs_df"
            assert doc_indices not in self.empty_year_index, "Don't select doc_index without <PY> info"
            self.doc_indices = [doc_indices]
        self.edge_type = edge_type
        self.show_timeline = show_timeline

        edge_dict = self.__generate_edge_set()
        grouped_doc_index, year_list = self.__obtain_groups()
        ...
        if self.show_timeline is True:
            ...
            year_edge_list = [(year, reversed_year_list[idx+1]) for idx, year in enumerate(reversed_year_list) if idx < len(reversed_year_list)-1]
