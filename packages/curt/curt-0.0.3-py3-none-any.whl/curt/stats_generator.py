...
def generate_errors_stats(df: 'pd.DataFrame'):
    ...
    -------
    pandas.core.frame.DataFrame
        Global and per-method error stats DataFrame
    """
...
def generate_errors_stats_dataframe(df: 'pd.DataFrame'):
    ...
    -------
    pandas.core.frame.DataFrame
        Error stats DataFrame
    """
    errors_df = df.query('Result == 1')
    total_errors = len(errors_df)
    bd_df = pd.DataFrame(errors_df.StatusCode.value_counts().reset_index().rename(
        columns={'index': 'Error', 'StatusCode': 'Incidencias'}))
