...
def convert_json_to_data_frame(json_file_path_model_1, json_file_path_model_2, json_file_path_model_3, embeddings_file):
    ...
    for main_index, item in enumerate(model_2):
        ...
        for index, detection in enumerate(item["outputs"][0]["detections"]):
            id = index+1
            AnnotationsV1.add(ObjectDetection(Id=id, ClassId=0, ClassName=detection['class'], Confidence=round(random.uniform(0, 1), 1), BBox= [], Format="xywh_normalized"))
        ...
        for index, detection in enumerate(merged_item["outputs"][0]["detections"]):
            id = index+1
            AnnotationsV3.add(ObjectDetection(Id=id, ClassId=0, ClassName=detection['class'], Confidence=random.randint(0, 1), BBox= [], Format="xywh_normalized"))
            if detection['roi_embedding']:
                print('ROI1')
...
# convert_json_to_data_frame("ModelA_faster_rcnn_r50_fpn_nuscene_US_rainy_clear_engg_embedding.json", "ModelB_faster_rcnn_swin-t_nuscene_US_rainy_clear_engg_embedding.json", "GT_cascade_rcnn_swin-t_nuscene_US_rainy_clear_engg_embedding.json", "nuscene_US_rainy_clear_engg_embedding.json")

#Convert JSON dataset to pandas Data Frame

# pd_data_frame = pd.DataFrame(convert_json_to_data_frame("ModelA_faster_rcnn_r50_fpn_nuscene_US_rainy_clear_engg_embedding.json", "ModelB_faster_rcnn_swin-t_nuscene_US_rainy_clear_engg_embedding.json", "GT_cascade_rcnn_swin-t_nuscene_US_rainy_clear_engg_embedding.json", "nuscene_US_rainy_clear_engg_embedding.json")).to_pickle('50_images_no_bbox.pkl')

# data_frame_extractor(pd_data_frame).to_csv("TestingDataFrame.csv", index=False)

# pd_data_frame = data_frame_extractor(pd.read_pickle("50_images_no_bbox.pkl")).to_csv("50_images_no_bbox.csv", index=False)
# print(pd_data_frame)
pd_data_frame = pd.read_pickle("50_images_no_bbox.pkl")
schema = RagaSchema()
...
test_session = TestSession(project_name="testingProject",run_name= "run-9-aug-data-50-v1")

# # #create test_ds object of Dataset instance
test_ds = Dataset(test_session=test_session, name="dataset-50-images-aug-9-v1")
