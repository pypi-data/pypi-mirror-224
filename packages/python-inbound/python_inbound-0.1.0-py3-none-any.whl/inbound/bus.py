...
from inbound.utils import gather_with_concurrency, lazy_load_map
...
class EventBus:
    ...
    async def _execute_callback(self, callback: EventCallback, event: Event):
        # Execute a single callback
        # If the event is not the correct type, parse it
        try:
            await callback(event)
        except BaseException as e:
            logger.error(f"Encountered error in Event callback: {e}", exc_info=e)

    async def _fire_callbacks(self, callbacks: list[EventCallback], event: Event):
        return await gather_with_concurrency(
            self._concurrency_limit,
            *(self._execute_callback(callback, event) for callback in callbacks)
        )
    ...
    async def watch_for_event(
        ...
        async with self.subscribe(channel) as stream:
            async for event in stream:
                ...
                has_headers = all(
                    [
                        True if event.headers.get(key) == value else False
                        for key, value in conditions.items()
                    ]
                )
                correct_type = event_type == "*" or event.type == event_type
...
async def create_event_bus(
    ...
    **kwargs
    ...
    """
    Create an EventBus instance based on the URL scheme.

    :param url: The URL to connect to
    :type url: str
    :param serializer: The serializer to use
    :type serializer: str | Serializer
    ...
    _AVAILABLE_BROKERS = {
        ...
        "amqp": "inbound.brokers.rabbitmq.RabbitMQBroker"
    }
    ...
    parsed = urlparse(url)
    broker = lazy_load_map(_AVAILABLE_BROKERS, parsed.scheme)

    if isinstance(serializer, Serializer):
        _serializer = serializer
    else:
        _serializer = lazy_load_map(_AVAILABLE_SERIALIZERS, serializer)

    kwargs.update({"_serializer": _serializer, "url": url})

    return EventBus(broker=broker, **kwargs)
