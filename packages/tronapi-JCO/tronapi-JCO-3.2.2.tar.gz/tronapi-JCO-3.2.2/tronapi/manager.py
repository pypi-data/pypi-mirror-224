...
class TronManager(object):
    ...
    def request(self, url, params=None, method=None):
        ...
        if self.tron.api_key:
            headers = {
                'Content-Type': "application/json",
                'TRON-PRO-API-KEY': self.tron.api_key,
                'User-Agent': self.tron.BaseProvider.format_user_agent()
            }
        else:
            headers = {
                'Content-Type': "application/json",
        ...
        if split[0] in ('walletsolidity', 'walletextension',):
            return self.solidity_node.request(url, json=params, method=method, headers=headers)
        elif split[0] in ('wallet',):
            return self.full_node.request(url, json=params, method=method, headers=headers)
        elif split[0] in ('event', 'healthcheck',):
            return self.event_server.request(url, json=params, method=method, headers=headers)
