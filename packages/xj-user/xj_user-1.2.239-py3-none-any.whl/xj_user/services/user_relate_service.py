...
from ..models import UserRelateType, UserRelateToUser, BaseInfo, DetailInfo
...
class UserRelateToUserService():
    ...
    def list(params=None, filter_fields=None, only_first=False, **kwargs):
        ...
        if params.get("real_name"):
            user_id_list = list(DetailInfo.objects.filter(real_name__contains=params.get("real_name")).values("user_id"))
            params.setdefault("user_id_list", [i["user_id"] for i in user_id_list])

        if params.get("with_real_name"):
            user_id_list = list(DetailInfo.objects.filter(real_name__contains=params.get("with_real_name")).values("user_id"))
            params.setdefault("with_user_id_list", [i["user_id"] for i in user_id_list])

        filter_params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "user_id|int", "user_id_list", "with_user_id_list", "with_user_id|int", "user_relate_type_id|int", "relate_key",
                "user_name", "full_name", "user_phone", "with_user_phone", "nickname", "with_user_name", "with_full_name", "with_nickname",
        ...
        ).filter(user__is_delete=0, with_user__is_delete=0).order_by(sort).filter(**filter_params).values(*filter_fields)
        # ------------------------- section 构建ORM end ------------------------------------
    ...
    def distribution_relate(*args, params: dict = None, **kwargs):
        ...
        params = format_params_handle(
            param_dict=params,
            filter_filed_list=[
                "relate_key", "relate_type_name", "user_name", "user_phone", "full_name", "nickname",
                "with_user_name", "with_user_phone", "with_full_name", "with_nickname", "user_id_list"
            ],
    ...
    def bxtx_relate_user(*args, params: dict = None, **kwargs):
        ...
        # ------------------------- section 分页查询 end   ------------------------------------
        data_map = {}
        EnrollStatisticsServices, err = dynamic_load_class(import_path="xj_enroll.service.enroll_statistics_services", class_name="EnrollStatisticsServices")
        if not err:
            data, err = EnrollStatisticsServices.every_one_total(params={"user_id_list": [i["user_id"] for i in page_list]}, need_Pagination=False)
            data_map = {i["user_id"]: i for i in data}

        user_detail_list, err = DetailInfoService.get_list_detail(user_id_list=[i["user_id"] for i in page_list], filter_fields=["user_id", "avatar", "real_name"])
        user_detail_list = user_detail_list if user_detail_list else []
        user_detail_map = {i["user_id"]: i for i in user_detail_list}
        for j in page_list:
