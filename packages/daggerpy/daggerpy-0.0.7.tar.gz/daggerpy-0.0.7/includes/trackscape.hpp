...
template <class fT, class Graph_t, class Connector_t> class trackscape {
public:
	// ###############################################
	// ###############################################
...
public:
	...
	void init_random(int nx, int ny, fT dx, fT dy,
	...
	// -> nx/ny are the number of nodes in the x/y dir
	// -> dx dy are the related spacing in [L]
	void init_perlin(int nx, int ny, fT dx, fT dy,
									 std::string boundary_conditions, fT frequency, int octaves,
									 fT Zmax, std::uint32_t seed, bool noise_on_top) {
	...
	template <class in_t> void set_variable_Kr(in_t &arr) {
	...
	template <class in_t> void set_variable_Ks(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_precipitations(in_t &prec) {
	...
	template <class in_t> void set_variable_depcoeff(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_kappa_s(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_kappa_r(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_Sc(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_Sc_M(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_Ke(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_lambda(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void set_variable_sea_level(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	template <class in_t> void feed_topo(in_t &arr) {
		auto tarr = DAGGER::format_input(arr);
	...
	void determine_functors() {
		...
		if (this->hillslopes == TSC_HILLSLOPE::CIDRE)
			...
			this->downstreamfuncs.emplace_back(
					&trackscape<fT, Graph_t,
											Connector_t>::hillslopes_cidre_dep_only_for_highlands);
		...
		if (this->secondary_fluvial == TSC_FLUVIAL::LATERALDAVY) {
			if (this->full_stochastic == false)
				this->downstreamfuncs.emplace_back(
						&trackscape<fT, Graph_t,
												Connector_t>::fluvial_lateral_erosion_deposition_davy);
			else
				this->downstreamfuncs.emplace_back(
...
*/
	...
	void _fluvial_lateral_erosion_deposition_davy(int onode) {
		...
		if (dZ > 0) {
			fT tlatdep =
					dZ / this->tdy * this->Kld(this->tnode) * this->tfDs; // * this->tdt;
			...
			fT tlater = std::abs(dZ) / this->tdy * this->Kle(this->tnode) *
									(this->tfEs + this->tfEr);
			this->add_to_dhsdt(onode, -1 * tlater * this->tdt,
	...
	*/
	...
	void _hylands_trigger_single_landslide(
			int node, fT tslope, std::stack<int, std::vector<int>> &stackhelper,
...
`- - ~ ~ -._|      /_ - ~ ~ ^|      /- _      `.
	...
	*/
	...
	void Standalone_hylands_single_landslide(int node) {
		...
		this->_hylands_trigger_single_landslide(node, slope, stackhelper,
																						neighbours_nodes, neighbours_links);
...
`- - ~ ~ -._|      /_ - ~ ~ ^|      /- _      `.
	...
	*/
	...
	void run_SFD(fT dt) {
		...
		for (int i = this->graph.nnodes - 1; i >= 0; --i) {
			...
			if (isNodeContinental) {
				...
				if (this->hillslopes == TSC_HILLSLOPE::CIDRE) {
					this->_compute_SFD_hillslopes(node, rec, S, hEs, hEr, hDs, dt, dx,
																				cellarea);
				}

				// Fluvial processes
				if (this->fluvial != TSC_FLUVIAL::NONE) {
					this->_compute_SFD_fluvial(node, rec, S, fEs, fEr, fDs, dt, dx,
																		 cellarea);
				}
			} else {
				this->_compute_SFD_marine(node, rec, S, mEs, mEr, mDs, dt, dx,
																	cellarea);
			}
		}
	}
	...
	void _compute_SFD_hillslopes(int node, int rec, fT &S, fT &hEs, fT &hEr,
															 fT &hDs, fT &dt, fT &dx, fT &cellarea) {
		// Storing the proportion of bedrock-power used
		fT propused = 0;
	...
	void _compute_SFD_fluvial(int node, int rec, fT &S, fT &fEs, fT &fEr, fT &fDs,
														fT &dt, fT &dx, fT &cellarea) {
		// Erosion
		fT stream_power =
	...
	void _compute_SFD_marine(int node, int rec, fT &S, fT &mEs, fT &mEr, fT &mDs,
													 fT &dt, fT &dx, fT &cellarea) {
		// Storing the proportion of bedrock-power used
		// fT propused = 0;
		...
		if (S <= Sc_M(node) - 1e-9) {
			...
			fT L = (this->connector.get_travers_dy_from_dx(dx) * this->lambda(node)) /
	...
	template <class in_t> void init_TSP_module(fT dz, in_t &tconcent) {
	...
	template <class in_t> void update_TSP_source(in_t &tconcent) {
		...
		if (!this->connector.flow_out_or_pit(ir)) {
			if (thillslopes)
				BasePropStorer<fT>::mix(this->Qs_hs[ir] /
																...
																this->TSP_Qsh[ir], zfQ, tpropr);
			else
				BasePropStorer<fT>::mix(this->Qs_fluvial[ir] /
																		this->connector.get_area_at_node(i) * dt,
																this->TSP_Qsf[ir], zfQ, tpropr);
		}
	}

	template <class out_t> out_t get_TSP_surface_concentrations() {
		if (this->TSP_module == false)
	...
	template <class out_t> out_t sample_carrot_TSP(int row, int col) {
	...
	void apply_Ch_MTSI_SFD(int i, int ir, fT Es, fT Er, fT Ds, fT dt,
												 bool thillslopes) {
		...
		if (!this->connector.flow_out_or_pit(ir)) {
			if (thillslopes)
				BasePropStorer<fT>::mix(this->Qs_hs[ir] /
																		this->connector.get_area_at_node(i) * dt,
																this->Ch_MTSI_Qsh[ir], zfQ, tpropr);
			else
				BasePropStorer<fT>::mix(this->Qs_fluvial[ir] /
																		this->connector.get_area_at_node(i) * dt,
																this->Ch_MTSI_Qsf[ir], zfQ, tpropr);
		}
	}
	...
	template <class out_t> out_t get_Ch_MTIS_surface_age() {
	...
	template <class out_t> out_t sample_carrot_Ch_MTSI(int row, int col) {
		int i = this->connector.nodeid_from_row_col(row, col);
	...
	*/
	...
	void standalone_implicit_SPL(){
		...
		if(this->flowtopo == TSC_FLOW_TOPOLOGY::SFD){
			if(this->variable_precipitations){
				this->Qw = this->graph._accumulate_variable_downstream_area_SFD(this->_precipitations);
			} else{
				this->Qw = this->graph._accumulate_constant_downstream_area_SFD(this->_precipitations[0]);
			}
		} else {
			...
			if(this->variable_precipitations){
				this->Qw = this->graph._accumulate_variable_downstream_area_MFD(weights, this->_precipitations);
			} else{
				this->Qw = this->graph._accumulate_constant_downstream_area_MFD(weights, this->_precipitations[0]);
			}
