...
#         ).check().payload_raw    # type: ignore
    ...
    '''used in services: Iot Account Application Backend'''
    @internal_api_error_handler
    def get_events_to_sync_api_user_data(
        self,
        device_state_sync_offset_id: UUID4,
        limit: Optional[int],
        offset: Optional[int],
    ) -> InternalApiResponse[List[JsonApiResponsePayload]]:
        q = {}
        if limit is not None:
            q['limit'] = limit
        if offset is not None:
            q['offset'] = offset
        return self._api_device.request_get(f'/device-state-sync-offsets/{device_state_sync_offset_id}/events', q=q).typed(List[DeviceEventsResponse]).check()

    @internal_api_error_handler
    def create_device_state_sync_offset_event(self) -> DeviceStateSyncDataResponse:
        return self._api_device.request_post('/device-state-sync-offsets/events').typed(DeviceStateSyncDataResponse).check().payload
