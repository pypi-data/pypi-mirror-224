...
def engine_proxy_mysql(table_name,hosts = u.all_hosts('bi_data_warehouse')):
    engine = f'''MySQL('rm-uf698x9pde1ytqxe890130.mysql.rds.aliyuncs.com:3306', '{hosts['db']}','{table_name}', '{hosts['name']}', '{hosts['code']}')'''
    return engine
...
def df_vm_create(origin_table_name,origin_hosts,vm_table_name,target_hosts):
    ...
    # 获取字段结构与表注释
    df_schema = act.auto_schema(origin_table_name, origin_hosts, vm_table_name, target_hosts)
...
def df_create(vm_table_name,target_table_name,target_hosts):
    ...
    # 获取字段结构与表注释
    df_schema = act.auto_schema(vm_table_name, target_hosts, target_table_name, target_hosts)
...
# 2.4 插入实体表
def df_insert(vm_table_name,target_table_name,target_hosts):
...
def schema_check(origin_table_name, origin_hosts, target_table_name, target_hosts):
    try:
        ...
        origin_list_ = [(origin_df_schema['col_name'][i],origin_df_schema['col_type_name'][i],origin_df_schema['col_comment'][i]) for i in range(len(origin_df_schema))]
        target_list = [(target_df_schema['col_name'][i],target_df_schema['col_type_name'][i],target_df_schema['col_comment'][i]) for i in range(len(target_df_schema))]
...
def auto_proxy_table(origin_table_name, origin_hosts,vm_table_name, target_table_name, target_hosts):
    ...
    if vm_table_check==True and vm_schema_check==True:
        ...
        while is_drop==0:
            try:
                ...
                logger.info("代理表副本删除还在删除中，代理表创建失败 ！480秒后进行重试 ")
                time.sleep(480)
    ...
    # 插入表数据
    logger.info("开始插入实体表……")
    df_insert(vm_table_name,target_table_name,target_hosts)
    logger.info("实体表插入完成 ！")
