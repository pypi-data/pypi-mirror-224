...
def mysql_etl(m_id,mission_name,name,comment,sql,hosts_read,hosts_write,hosts_records,robot_key,index_columns_list=[],wtype='r',astype_dic={}): # 
    ...
    print(f'\n开始执行：{plan}\n')
    try:
        u.mysql_write(df,name,update = True,host_dic = hosts_write,wtype=wtype)
    ...
    try:
        u.mysql_ddl(alter_sql,host_dic = hosts_write)
    ...
    try:
        if len(m_list)-f_c==0:
            ...
            r='fail'
        u.py_mission_records(mission_name,m_id,m_name,r,ee,usetime,rows,host_dic=hosts_records,sql = sql)
...
def same_mysql_etl(m_id,mission_name,name,comment,sql,hosts_read,hosts_write,hosts_records,robot_key,index_columns_list=[],wtype='r',astype_dic={},division=False): # 
    ...
    try:
        ...
        del_rows = u.mysql_ddl(drop_sql,host_dic = hosts_write)
        print('数据量为：',del_rows)
    ...
    try:
        try:
            ...
            row = u.mysql_ddl(sql = create_sql,host_dic = hosts_write,division=division)
        except:
            print('表已存在，直接尝试插入数据')
            insert_sql = f'''insert into {hosts_write['db']}.{name} \n {sql}'''
            row = u.mysql_ddl(sql = insert_sql,host_dic = hosts_write,division=division)
        # df.info()
        print('写入成功')
        print('数据量为：',row)
...
def excel_to_mysql(m_id,mission_name,name,comment,file,sheet,skiprows,hosts_write,hosts_record,robot_key,index_columns_list=[],wtype='r'):
    ...
    print(f'\n开始执行：{plan}\n')
    try:
        u.mysql_write(df,name,update = True,host_dic = hosts_write)
    ...
    try:
        if len(m_list)-f_c==0:
            ...
            r='fail'
        u.py_mission_records(mission_name,m_id,m_name,r,ee,usetime,rows,host_dic=hosts_record)
...
def mysql_to_clickhouse(m_id,mission_name,name,comment,sql,hosts_read,hosts_write,hosts_record,robot_key):
    ...
    try:
        if len(m_list)-f_c==0:
            ...
            r='fail'
        u.py_mission_records(mission_name,m_id,m_name,r,ee,usetime,rows,host_dic=hosts_record,sql = sql)
...
def clickhouse_del_insert_i(m_id,mission_name,name,comment,del_sql,sql_insert,hosts_read,hosts_write,hosts_records,robot_key,index_columns_list=[],wtype='r'):
    ...
    try:
        if len(m_list)-f_c==0:
            ...
            r='fail'
        u.py_mission_records(mission_name,m_id,m_name,r,ee,usetime,rows,host_dic=hosts_records,sql = f'{del_sql}\n-----\n{sql_insert}\n-----\nselect count(*) from {name}')
