...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task):
        ...
        # print('do_pr_metric:', self.do_pr_metric)
        if self.do_pr_metric:
            stats, ap = [], []
            seen = 0
            iouv = torch.linspace(0.5, 0.95, 10)  # iou vector for mAP@0.5:0.95
            niou = iouv.numel()
            # print('plot_confusion_matrix:', self.plot_confusion_matrix)
            if self.plot_confusion_matrix:
                # print('plotting confusion_matrix')
                from ..utils.metrics import ConfusionMatrix
                confusion_matrix = ConfusionMatrix(nc=model.nc)
                # print('plotted confusion_matrix')
