...
from pitrix.config import Config, fixture_paths

# allure源文件临时目录，那一堆json文件，生成HTML报告会删除
allure_source_path = ".allure.source.temp"


def _pitrix_reports(config):
    """
    --pitrix-reports命令行参数不能和allure命令行参数同时使用，否则可能出错
    """
    if config.getoption("--pitrix-reports") and not config.getoption("allure_report_dir"):
        return True
    return False
...
class Plugin:
    ...
    def pytest_addoption(parser):
        ...
        parser.addoption(
            ...
            action="store_const",
            const=True,
            help="Create pitrix allure HTML reports."
        )
    ...
    def pytest_sessionfinish(session):
        ...
        if _pitrix_reports(session.config):
