from PySide6.QtWidgets import QWidget, QTableWidgetItem, QLabel, QMessageBox, QLineEdit, QCheckBox, QGridLayout, QComboBox, QTextEdit, QSpacerItem, QSizePolicy, QPushButton, QDialog
...
class Datbroker_Exporter(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.file_box = path_button_edit.Path_Button_Edit(self,
        ...
        self.textedit_description.setTextInteractionFlags(Qt.TextSelectableByKeyboard | Qt.TextSelectableByMouse)
        self.textedit_overview.setTextInteractionFlags(Qt.TextSelectableByKeyboard | Qt.TextSelectableByMouse)
        ...
        layout.addItem(QSpacerItem(0, 0, hData=QSizePolicy.Policy.Minimum, vData=QSizePolicy.Policy.Expanding), 6, 3)
    ...
    def export_data(self):
        ...
        databroker_export.export_run(filename=filename, run_number=run,
                                     session_name=session,
                                     export_to_csv=exp_csv,
                                     export_to_json=exp_json,
                                     catalog_name=catalog)
        self.setCursor(Qt.CursorShape.ArrowCursor)
        warn_popup.WarnPopup(self, 'Succesfully exported data!',
                             'Data exported', True)
        self.accept()
    ...
    def change_catalog(self):
        ...
        for run in sorted(run_time_names, reverse=True):
            ...
            self.run_box.addItem(f'{run_time} - {run_name}',
                                 userData=run_name)
