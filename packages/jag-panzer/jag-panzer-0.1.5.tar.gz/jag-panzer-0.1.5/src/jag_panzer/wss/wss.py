...
The payload header contains:
  - Flags
      4 bits each acting as an array of booleans.
  - Opcode
      Another 4 bits acting as a single code.
  - Payload len
      The size of the payload, not including the frame.
  - Masking-key
      By default WSS payloads are XORed.
...
Browsers only recognize the first one, indicating whether this frame
...
- 0x0: continuation.
- 0x1: The payload consists of text decodable with UTF-8.
- 0x2: The payload consists of binary data.
- 0x8: The payload indicates that the connection should be closed.
...
A complex system was introduced to save 3 bytes per payload.
...
Masking key consists of 4 bytes which comes immediately after payload length.
...
class wssFrame:
	...
	def construct_header(self):
		"""
		Construct header bytes from the info that was written down. 
		"""
		length = self.length
		struct = self.session.struct
