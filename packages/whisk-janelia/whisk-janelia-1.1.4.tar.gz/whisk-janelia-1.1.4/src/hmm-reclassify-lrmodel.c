...
{ int N = 2 * nwhisk + 1;
  ...
  { real *row = E + i*nobs;
    ...
    // prev
    //
    if( st > -1 && (p = prev->whiskers[ st ]) ) 
    { for(j=0;j<nobs;j++) // iterate over segments
      { vel = Eval_Velocity_Likelihood_Log2( vel_dists,
                                              p->data,
                                              obs[j].data,
                                              state );
