...
from sempy.fabric._token_provider import _create_default_token_provider, _get_token_seconds_remaining, TokenProvider
...
class WorkspaceClient:
    ...
    def __init__(self, workspace: Optional[Union[str, UUID]] = None, token_provider: Optional[TokenProvider] = None):
        ...
        if workspace is None:
            ...
            workspace_name = self._rest_api._try_get_workspace_name_from_id(current_workspace_id)
            # We are assuming that not finding a workspace name is only possible for "My workspace".
            # How I wish we could be a little more self-assured with a more solid API.
            if workspace_name:
                self._workspace_id = current_workspace_id
                self._workspace_name = workspace_name
            else:
                # For "My workspace" we set the ID to None, to be consistent with other branch
                # of the code, where we cannot obtain it:
    ...
    def get_workspace_id(self) -> Optional[str]:
        ...
        Null, if executed for "My workspace".
