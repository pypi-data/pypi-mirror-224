...
if TYPE_CHECKING:
    import Microsoft.AnalysisServices.Tabular.Database
...
@log
def get_dataset(dataset: Union[str, UUID], workspace: Optional[Union[str, UUID]] = None) -> "Microsoft.AnalysisServices.Tabular.Database":
    """
    Get `PowerBI TOM model <https://learn.microsoft.com/en-us/dotnet/api/microsoft.analysisservices.core.database?view=analysisservices-dotnet>`_ for a given `dataset <https://learn.microsoft.com/en-us/power-bi/connect-data/service-datasets-understand>`_.
    ...
    Microsoft.AnalysisServices.Tabular.Database
        TOM database object for PowerBI dataset.
    """ # noqa E501
    return _get_or_create_workspace_client(workspace).get_dataset(dataset)
...
def list_tables(
    ...
    def dotnet_datetime_to_python(dt):
        from System import DateTime

        epoch = (dt - DateTime(1970, 1, 1)).TotalSeconds
    ...
    for table in tabular_database.Model.Tables:
        ...
        if include_partitions:
            row["Partitions_Name"] = [p.Name for p in table.Partitions]
            row["Partitions_RefreshedTime"] = [
                dotnet_datetime_to_python(p.RefreshedTime)
    ...
    if include_partitions:
        df = df.explode(["Partitions_Name", "Partitions_RefreshedTime"])
...
def plot_relationships(
    ...
    *,
    ...
    Parameters
    ----------
    tables : dict[str, FabricDataFrame] or list[FabricDataFrame]
