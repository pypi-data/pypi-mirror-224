...
def _get_trident_config(key):
    if _get_environment() == "local":
        return "local"
    from pyspark import SparkContext
    sc = SparkContext.getOrCreate()
    return sc._jsc.hadoopConfiguration().get(key)


def _get_synapse_endpoint():
    # Environment is already set since user has to _set_backend before using SemPy
    return f"https://{SUPPORTED_ENVIRONMENTS[environment]}"


def _get_pbi_uri():
    # Environment is already set since user has to _set_backend before using SemPy
    return f"powerbi://{SUPPORTED_ENVIRONMENTS[environment]}"
