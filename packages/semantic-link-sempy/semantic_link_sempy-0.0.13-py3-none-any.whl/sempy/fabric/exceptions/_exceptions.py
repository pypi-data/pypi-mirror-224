...
class FabricHTTPException(SemPyException, requests.HTTPError):
    ...
    def __init__(self, response: requests.Response):
        ...
        if 400 <= response.status_code < 500:
            self.error_type = "Client"

        elif 500 <= response.status_code < 600:
            self.error_type = "Server"

        self.error_text = f"\nError: {response.text}" if response.text else ""
        self.status_code = response.status_code

        msg = f"{self.status_code} {self.error_type} Error: {self.error_reason} for url: {response.url}{self.error_text}"

        requests.HTTPError.__init__(self, msg, response=response)
