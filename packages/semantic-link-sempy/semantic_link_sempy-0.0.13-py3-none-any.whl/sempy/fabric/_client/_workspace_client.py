...
class WorkspaceClient:
    ...
    def get_workspace_id(self) -> str:
        ...
        String
            Workspace ID.
        """
        # we run retrieval once when needes, then store the result in _workspace_id
        if self._workspace_id is not None:
            return self._workspace_id
        if self._workspace_name is None:
            workspace_id = get_workspace_id()
        else:
            workspace_id = self._rest_api.get_workspace_id_from_name(self._workspace_name)
    ...
    def get_workspace_name(self) -> str:
        ...
        String
            Workspace name.
        """
        # if workspace wasn't specified, we run retrieval once when needed and store the result in _workspace
        if self._workspace_name is not None:
            return self._workspace_name

        workspace_id = self.get_workspace_id()
        workspace = self._rest_api.get_workspace_name_from_id(workspace_id)
        self._workspace_name = workspace
        return workspace
