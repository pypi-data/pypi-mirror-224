...
def annovar_run(filename1, humandb):
    ...
    for j in tqdm(filename1):
        if not glob.glob(j + '/*.new.vcf4'):
            ...
            os.system(f'perl {convertor} ' + j + '/*.vcf -format vcf4old > ' + j + '/' + k + '.new.vcf4')
            os.system(f'perl {annotate} -geneanno ' + j + '/' + k + '.new.vcf4 -buildver hg19 ' + humandb)
...
def main():
    ...
    if args.database == None:
        db_file = nf_path + "/../humandb"
    else:
        db_file = args.database
    if args.clinical == None:
        cl_file = nf_path + "/../clincal_data.tsv"
    else:
