...
from MobileHelperService.service_api import MobileHelperService as Service, is_developer
...
def checker(telephone_number: str) -> bool:
    if not A.D_C.empty(A.SRV.get_support_host_list(A.CT_SR.MOBILE_HELPER)):
        ...
        if is_developer() or A.D.contains(host, A.CT_H.DEVELOPER.NAME):
            return am_i_tester
        return not am_i_tester
    return True

Service(None, checker).start()