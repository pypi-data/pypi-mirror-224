...
class MobileHelper:
        ...
        with self.output.personalized():
            ...
            if A.D_C.empty(action):
                ...
                if not action_description.confirm or self.input.yes_no(f"Выполнить действие {action_description.description}" if A.D_C.empty(action_description.question) else action_description.question):
                    if A.A_ACT.was_done(action, self.session.login):
                        if not action_description.silence:
                            self.output.good("Действие зарегистрировано, спасибо!")
    ...
    def get_polibase_person_card_position_label(self, card_folder_name: str | None, card_position: int | None = None, card_length: int | None = None, display_only_card_folder: bool = False) -> str:
        ...
        if not A.D_C.empty(card_folder_name):
            ...
            result_label_list.append(j((
                b(A.CT_FC.POSITION.caption), if_else(display_only_card_folder, lambda: j((" карты ", b(card_folder_name))), ''), ":")))
            card_folder_first_letter: str | None = card_folder_name[0]
            if card_folder_first_letter in A.CT.CARD_REGISTRY.PLACE_NAME:
                result_label_list.append(f" {A.CT_V.BULLET} Место: {A.CT.CARD_REGISTRY.PLACE_NAME[card_folder_first_letter]}")
            ...
            if not A.D_C.empty(card_registry_folder_was_registered_event):
                ...
                result_label_list.append(if_else(A.D_C.empty(card_position), A.CT_FC.POSITION.default_value, lambda: f" {A.CT_V.BULLET} Карта в папке: {b(card_position)} из {b(card_length)}"))
            return nl(j(result_label_list, nl()), reversed=True)
        return ""
    ...
    def get_language_index(self, value: str, allow_in_group: bool = False) -> bool:
        if allow_in_group:
    ...
    def do_pih(self, line: str = PIH.NAME, sender_user: User | None = None, income_flags: int = 0, allow_in_group: bool = False) -> bool:
        result: bool = True
        self.line = line
        if self.get_language_index(line, allow_in_group):
            self.level += 1
