...
class MobileSession(SessionBase):
    ...
    def say_hello(self, telephone_number: str | None = None) -> None:
        try:
            ...
            if not BM.has(self.flags, Flags.ONLY_RESULT):
...
class MobileHelper:
    ...
    def polibase_status() -> str:
        return A.D_F.yes_no(A.R_R.get_resource_status(A.CT_R_D.POLIBASE).accessable, True)
        ...
        if non_search_action:
            if is_all:
                if not self.input.yes_no(("Перезагрузить" if action_index == 0 else "Выключить") + " все компьютеры, которые помечены как разрешенные", False, b("Да") + " (Введите слово \"Workstation\")", yes_checker=(lambda item: item == "Workstation")):
                    return
        try:
            workstations: Result[list[Workstation]] | None = None
            if non_search_action:
                workstations = A.R_WS.all_with_prooperty(
                ...
                if A.R.is_empty(workstations):
                    ...
                    A.R.every(workstations, every_function)
            else:
                workstations = A.R_WS.all() if is_all else A.R_WS.by_any(search_value)
                try:
                    def data_label_function(index: int, field: FieldItem, data: Any, item_data: Any) -> tuple[bool, str]:
                        ...
                        return False, None
                    self.output.write_result(workstations, False,
