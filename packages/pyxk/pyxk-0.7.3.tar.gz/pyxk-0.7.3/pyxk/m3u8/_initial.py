...
class _M3U8Init:
    ...
    def _norm_store(value) -> Optional[str]:
        ...
        if not isinstance(value, str) or not value:
            raise TypeError(f"'store' type must be a str, got: {type(value)}")
    ...
    def _norm_output(self, value, suffix="mp4") -> Optional[str]:
        ...
        if not isinstance(value, str):
            raise TypeError(f"'output' type must be a str, got: {type(value)}")
    ...
    def user_agent(self, value):
        if not isinstance(value, str):
            raise TypeError(f"'user_agent' type must be a str, got: {type(value)}")
    ...
    def limit(self, value):
        if not isinstance(value, int) or value <= 0:
            raise ValueError(f"'limit' initial value must be > 0, got: {value}")
        self._limit = value
