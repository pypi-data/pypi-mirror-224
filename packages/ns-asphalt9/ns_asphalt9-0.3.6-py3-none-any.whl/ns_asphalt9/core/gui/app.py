...
class App(customtkinter.CTk):
    ...
    def _build_settings(self):
        ...
        self._build_mp1()
        # 多人二配置
        self._build_common(row=3, key="多人二")
        # 寻车配置
        self._build_common(row=4, key="寻车", type="寻车")
        # 传奇寻车配置
        self._build_common(row=5, key="传奇寻车", type="寻车")
        # 多人三配置
    ...
    def _build_navi_box(self, frame, key):
        ...
        if self.settings_data and "自动选路" in self.settings_data[key]:
    ...
    def _build_common(self, row=6, key="多人三", type="多人"):
        """多人和寻车配置"""
        ...
        base_row = 1
        if type == "寻车":
            car_hunt_position = customtkinter.CTkLabel(master=frame, text="寻车位置:")
            ...
            if self.settings_data and key in self.settings_data:
                position_option.set(self.settings_data[key]["寻车位置"])
            self.setting_modules[key]["寻车位置"] = position_option
            base_row = 2

        self._build_car_position(
            frame,
            self.setting_modules[key],
            self.settings_data[key] if self.settings_data else {},
            base_row,
        )
        return frame
