...
from .exceptions import (
    ArticleLoginException, UserLoginException, UserPrivateException,
    UserLoseException, UserException, UserSuspendedException,
    ArticleException
)
from easy_twitter_crawler.twitter_utils.twitter_utils import (
    generating_page_link, jsonpath, regx_match,
    generating_page_links, quote, extract_error_info,
    extract_time, extract_cursor
)
...
class BaseExtractor:
    ...
    def extract_embed_url(self) -> PartialType:
        ...
        return {'embed_url': list(set(short_links))}
...
class UserExtractor(BaseExtractor):
    ...
    def extract_user(self) -> PartialType:
        ...
        join_date = jsonpath(self.element, '$.legacy.created_at', first=True, default='')

        user_name = jsonpath(self.element, '$.legacy.screen_name', first=True, default='')
        return {
            'user_id': jsonpath(self.element, '$.rest_id', first=True, default=''),
            'user_name': user_name,
            'display_name': jsonpath(self.element, '$.legacy.name', first=True, default=''),
            'user_url': USER_BASE_URL.format(user_name=user_name) if user_name else '',
            'profile_picture': jsonpath(self.element, '$.legacy.profile_image_url_https', first=True, default=''),
            'header_photo': jsonpath(self.element, '$.legacy.profile_banner_url', first=True, default=''),
            'bio': jsonpath(self.element, '$.legacy.description', first=True, default=''),
            'location': jsonpath(self.element, '$.legacy.location', first=True, default=''),
            'website_link': {
                'display_url': jsonpath(self.element, '$.legacy.entities.url.urls[*].display_url', first=True,
                                        default=''),
                'expanded_url': jsonpath(self.element, '$.legacy.entities.url.urls[*].expanded_url', first=True,
                                         default=''),
            },
            'birthday': {
            ...
            'followers_count': jsonpath(self.element, '$.legacy.followers_count', first=True, default=0),
            'following_count': jsonpath(self.element, '$.legacy.friends_count', first=True, default=0),
            'tweets_count': jsonpath(self.element, '$.legacy.statuses_count', first=True, default=0),
            'likes_count': jsonpath(self.element, '$.legacy.favourites_count', first=True, default=0),
            'media_count': jsonpath(self.element, '$.legacy.media_count', first=True, default=0),
            'listed_count': jsonpath(self.element, '$.legacy.listed_count', first=True, default=0),
            'join_date': extract_time(join_date),
            'auth': {
                'is_verified': jsonpath(self.element, '$.legacy.verified', first=True, default=False),
                'verified_type': jsonpath(self.element, '$.legacy.verified_type', first=True, default=''),
            ...
            'protected': jsonpath(self.element, '$.legacy.protected', first=True, default=False),
        }
    ...
    def extract_meta_data(self, start_url, type_name, **kwargs) -> PartialType:
        ...
        for page_url in page_urls:
            page_url = generating_page_link(base_url, cursor=next_cursor, user_id=user_id)
            ...
            if response and response.status_code == 200:
                ...
                if err_info['name'] == 'AuthorizationError':
                    # 需要登录
                    raise UserLoginException(user_id, err_info['msg'])
                ...
                for element in elements:
                    ...
                    if type_name == FOLLOWERS:
                        extractor = UserFollowersExtractor(element, self.options, self.request_fn, full_html=None)
...
class UserArticleExtractor(BaseExtractor):
    """用户文章"""
    pass


class UserReplyExtractor(CommentExtractor):
    """用户回复"""


class UserFollowersExtractor(UserExtractor):
    """用户粉丝"""
