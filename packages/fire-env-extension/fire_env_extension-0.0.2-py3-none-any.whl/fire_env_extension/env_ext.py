...
class ParameterEnvValidator():
    ...
    def str_validate(self, value: str, value_name: str, default_value, env_name: str) -> str:
        ...
        if default_value is None:
            ...
            self.info_messages.append(
                f"{value_name} or env.{env_name}  -- default: '{default_value}'")
        if retval is None:
            self.err_messages.append(
                f"{value_name} or env.{env_name} is not set")
        return str(retval)
...
def to_bool(value, value_name: str) -> bool:
    ...
    if not value:
        msg = f"{value_name}: {value} is False"
        logger.info(msg)
        return False
    ...
    if isinstance(value, str):
        if value.upper() in TRUE_SET:
            msg = f"{value_name}: {value} is True"
            logger.info(msg)
            return True
