...
from locker import ROOT_PATH, util, error
...
class BinaryAdapter(object):
    ...
    def get_binary_file(self):
        ...
        if self.system_platform == "darwin":
            return os.path.join(ROOT_PATH, "bin", "locker_secret_mac")
        # Windows
        elif self.system_platform == "win32":
            return os.path.join(ROOT_PATH, "bin", "locker_secret.exe")
        # Default is linux
        else:
            return os.path.join(ROOT_PATH, "bin", "locker_secret_linux")
