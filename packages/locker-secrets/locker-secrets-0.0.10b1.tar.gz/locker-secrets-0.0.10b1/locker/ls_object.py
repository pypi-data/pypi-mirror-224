...
import locker
from locker import util, binary_adapter
...
class LSObject(dict):
    ...
    def __init__(self, id=None, access_key=None, api_base=None, api_version=None, last_response=None, **params):
        super().__init__()
    ...
    @classmethod
    def construct_from(cls, values, access_key, api_base=None, api_version=None, last_response=None):
        instance = cls(
            values.get("id"),
    ...
    def refresh_from(self, values, access_key=None, partial=False, api_base=None, api_version=None, last_response=None):
        self.access_key = access_key or getattr(values, "access_key", None)
        ...
        for k, v in six.iteritems(values):
            super().__setitem__(
                k,
                util.convert_to_ls_object(v, access_key, api_base, api_version),
            )
    ...
    def call(self, cli, asjson=True, load=False, shell=True, timeout=30, params=None):
        return LSObject._call(
            self, cli, asjson=asjson, load=load, shell=shell, timeout=timeout, params=params
        )

    # The 'cli_' is suffixed with an underscore to avoid conflicting with request parameters in `params`
    def _call(self, cli_, access_key=None, api_base=None, api_version=None, asjson=True, load=False, shell=True,
    ...
    def serialize(self, previous):
        ...
        for k, v in six.iteritems(self):
            if k == "id" or (isinstance(k, str) and k.startswith("_")):
                continue
            elif isinstance(v, locker.ls_resources.abstract.APIResource):
                continue
