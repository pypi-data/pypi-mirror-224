...
class SlackClient:
    ...
    def get_conversations_list(self):
        ...
        page = requests.get(url=base_url, cookies=self.cookies, data=payload, headers=self.headers).json()
        result = page.copy()
        if page["ok"]:
            while page.get("response_metadata", {}).get("next_cursor"):
                url = base_url + f'&cursor={page.get("response_metadata", {}).get("next_cursor")}'
                page = requests.get(url=url, cookies=self.cookies, data=payload, headers=self.headers).json()
                if page['ok']:
