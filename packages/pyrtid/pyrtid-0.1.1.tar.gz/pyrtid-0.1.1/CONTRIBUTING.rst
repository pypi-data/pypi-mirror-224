...
Please report defects to the `the GitLab Tracker <https://github.com/antoinecollet5/pyrtid/issues>`_
...
If you wish to propose a feature, please file an issue on `the GitLab Tracker <https://github.com/antoinecollet5/pyrtid/issues>`_ using the **Feature** description template. Community members will help refine and design your idea until it is ready for implementation.
...
After an issue is created, the progress of the issues is tracked on the `GitHub issue board <https://github.com/antoinecollet5/pyrtid/pulse>`_.
The maintainers will update the state using `labels <https://github.com/antoinecollet5/pyrtidlabels>`_ .
...
Setting up PyRTID for local development
---------------------------------------

Ready to contribute? Here's how to set up `Python Reactive Transport Inversion Demonstrator` for local development.

1. Fork the `pyrtid` repo on GitLab.
2. Clone your fork locally::

    $ git clone git@github.com:your_name_here/pyrtid.git
...
5. When you're done making changes, check that your changes pass flake8, the tests and have test coverage::
  ...
  The docs will be located in `pyrtid/docs/_build`.
...
.. tip:: Running parts of the test suite
    ...
    .. code-block:: shell
        ...
        $ py.test tests/measurement --cov pyrtid/measurement

    will look for tests located in the tests/measurement directory and report test coverage of the pyrtid/measurement module.
