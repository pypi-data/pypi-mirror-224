...
def distance(tau, sigma):
    ...
    for k in np.arange(tau.dim):
        dist += (
            np.max((0.0, tau.alpha[k] - sigma.beta[k])) ** 2.0
            + np.max((0.0, sigma.alpha[k] - tau.beta[k])) ** 2.0
        )
...
class BlockCluster(Cluster):
    ...
    def construct_block_tree(self, tau, sigma):
        ...
        if (
            not admissible(tau, sigma)
            and len(tau.children) != 0
            and len(sigma.children) != 0
        ):
    ...
    def construct_low_rank_representation(self, pts, kernel, rkmax, eps):
        if len(self.children) != 0:
            ...
            if self.admissible:
                self.A, self.B = ACApp(
                    pts, self.tau.indices, self.sigma.indices, kernel, rkmax, eps
                )
            else:
                self.mat = GenerateDenseMatrix(
                    pts, self.tau.indices, self.sigma.indices, kernel
                )
    ...
    def mult(self, x, y, pts, kernel):
        ...
        if len(self.children) != 0:
            ...
            if not self.admissible:  # Direct interactions
                ...
                y[self.tau.indices] += np.dot(
                    self.A, np.dot(self.B.T, x[self.sigma.indices])
                )
        return
