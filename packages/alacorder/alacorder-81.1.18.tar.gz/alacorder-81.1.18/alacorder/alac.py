...
class Cases:
    ...
    def filing_charges(self, log=True, debug=False):
        """
        Make filing charges table.
        """

        def make_fc(archive, debug=False):
                ...
                [
                    ...
                    .then(
                        pl.col("Charge").str.extract(
                            r"\d+ [A-Z0-9]+ (.+?) (TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|TRAFFIC|VIOLATION)$",
                            group_index=1,
                        )
                    )
            ...
            df = df.with_columns(
                ...
                .alias("Cite"),
                pl.when(pl.col("Cite").is_null())
                .then(pl.col("Description").str.replace(r"(\d+\.\d+)", "").str.strip())
                .otherwise(pl.col("Description"))
                .alias("Description"),
            )
            # fix PRE-CONV HABEAS CORPUS cites and descriptions
            df = df.with_columns(
                pl.when(pl.col("Description").str.contains("PRE-CONV HABEAS CORPUS"))
            ...
            df = df.with_columns(
                pl.when(
                    pl.col("Cite").is_null() & pl.col("Charge").str.contains(" MUN-")
                )
                .then(
                    pl.col("Charge")
            ...
            df = df.with_columns(
                pl.when(
                    pl.col("Cite").is_null() & pl.col("Charge").str.contains(" HSV-")
                )
                .then(
                    pl.col("Charge")
                    .str.extract(r"(HSV-.+?) (MISDEMEANOR|VIOLATION)$", group_index=1)
                    .str.replace_all(" ", "")
                )
                .otherwise(pl.col("Cite"))
                .alias("Cite"),
                ...
                .then(pl.col("Description").str.replace(r"(HSV-.+)", "").str.strip())
                .otherwise(pl.col("Description"))
                .alias("Description"),
            )
            # fix ORD-AM cites
            df = df.with_columns(
                pl.when(
                    pl.col("Cite").is_null() & pl.col("Charge").str.contains("ORD-AM")
                )
                .then(
                    pl.col("Charge").str.extract(r"(ORD-AM.+?) (MISDEMEANOR|VIOLATION)")
                )
                .otherwise(pl.col("Cite"))
                .alias("Cite"),
                pl.when(
                ...
                .then(pl.col("Description").str.replace(r"(ORD-AM.+)", "").str.strip())
                .otherwise(pl.col("Description"))
                .alias("Description"),
            )
            ...
            df = df.with_columns(
                pl.when(
                    pl.col("Charge").str.contains("RESERVED") & pl.col("Cite").is_null()
                )
            ...
            df = df.with_columns(
                [
                    pl.col("Description").str.extract(r"^([ASC]) ").alias("ID"),
                    pl.col("Description")
                    .str.replace(r"^([ASC]) ", "")
                    .alias("Description"),
                ]
                ...
                [
                    ...
                    (
                        pl.col("Code").str.contains(
                            r"(OSUA|EGUA|MAN1|MAN2|MANS|ASS1|ASS2|KID1|KID2|HUT1|HUT2|BUR1|BUR2|TOP1|TOP2|TP2D|TP2G|TPCS|TPCD|TPC1|TET2|TOD2|ROB1|ROB2|ROB3|FOR1|FOR2|FR2D|MIOB|TRAK|TRAG|VDRU|VDRY|TRAO|TRFT|TRMA|TROP|CHAB|WABC|ACHA|ACAL|TER1|TFT2|TLP1|TLP2|BIGA)"
                        )
                        | pl.col("Cite").str.contains("026-015-003")
                    ).alias("CERVCode"),
                    pl.col("Code")
                    .str.contains(
        ...
        if isinstance(self._filing_charges, pl.DataFrame):
            ...
            if log:
                with console.status("Parsing filing charges…"):
                    self._filing_charges = make_fc(self.archive, debug=debug)
            else:
                self._filing_charges = make_fc(self.archive, debug=debug)
            return self._filing_charges
            ...
            with console.status("Parsing disposition charges…"):
                ...
                df = df.with_columns(
                    [
                        ...
                        pl.col("Row")
                        .str.extract(
                            r"(BOUND|GUILTY PLEA|NOT GUILTY/INSAN E|WAIVED TO GJ|DISMISSED W/CONDITION S|DISMISSED/N OL PROS W/CONDITION S|TIME LAPSED PRELIM\. FORWARDED TO GJ|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|TRANSFERED ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? ?\(MUST MAKE OCS ENTRY TO EXPLAIN \)|OTHER \(MUST ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? MAKE OCS ENTRY\)|FINAL BOND FORF\.|FORFEITURE SET ASIDE \(\.50 CASE\)|FINAL FORFEITURE \(\.50 CASE\)|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|ANCTION|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\)|PROBATION REVOKED \(\.70 CASE\)|PRETRIAL DIVERSION)"
                        )
                        .str.replace(r" \)", ")")
                        ...
                        pl.col("Row")
                        .str.extract(
                            r"(WAIVED TO GJ \d\d/\d\d/\d\d\d\d|GJ|GUILTY PLEA|NOT GUILTY/INSAN E|WAIVED TO GJ|DISMISSED W/CONDITION S|DISMISSED/N OL PROS W/CONDITION S|TIME LAPSED PRELIM\. FORWARDED TO GJ|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|DISMISSED|TRANSFERED ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? ?\(MUST MAKE OCS ENTRY TO EXPLAIN \)|OTHER \(MUST ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? MAKE OCS ENTRY\)|FINAL BOND FORF\.|FORFEITURE SET ASIDE \(\.50 CASE\)|FINAL FORFEITURE \(\.50 CASE\)|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\)|PROBATION REVOKED \(\.70 CASE\)|PRETRIAL DIVERSION)\s+(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET/MISC|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL ORDINANCE|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|BUSINESS|JUVENILE)\s+(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)(.+?)([A-Z0-9]{3}-\s*[A-Z0-9]{3}-\s*[^\s]+)",
                            group_index=4,
                        )
                        .str.strip()
                        ...
                        pl.col("Row")
                        .str.extract(
                            r"(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET/MISC|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL ORDINANCE|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|BUSINESS|JUVENILE)\s*(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)",
                            group_index=2,
                        )
                        .alias("TypeDescription"),
                        pl.col("Row")
                ...
                df = df.with_columns(
                    ...
                    .then(
                        pl.col("Row")
                        .str.extract(
                            r"(WAIVED TO GJ \d\d/\d\d/\d\d\d\d|GJ|GUILTY PLEA|NOT GUILTY/INSAN E|WAIVED TO GJ|DISMISSED W/CONDITION S|DISMISSED/N OL PROS W/CONDITION S|TIME LAPSED PRELIM\. FORWARDED TO GJ|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|TRANSFERED ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? ?\(MUST MAKE OCS ENTRY TO EXPLAIN \)|OTHER \(MUST ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? MAKE OCS ENTRY\)|FINAL BOND FORF\.|FORFEITURE SET ASIDE \(\.50 CASE\)|FINAL FORFEITURE \(\.50 CASE\)|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\)|PROBATION REVOKED \(\.70 CASE\)|PRETRIAL DIVERSION)\s+(ALCOHOL|BOND FORFEITURE|CONSERVATION|DOCKET/MISC|DRUG|GOVERNMENT PUBLIC|HEALTH|MUNICIPAL ORDINANCE|MUNICIPAL|OTHER|PERSONAL|PROPERTY|SEX OFFENSE|TRAFFIC|DOCKET|REVENUE – PSC \(PUBLIC SERVICE COMMISSION\)|BUSINESS|JUVENILE)\s+(.+?)([A-Z0-9]{3}-\s*[A-Z0-9]{3}-\s*[^\s]+)",
                            group_index=3,
                        )
                        .str.strip()
                    )
                ...
                df = df.with_columns(
                    ...
                    .then(
                        pl.col("Row")
                        .str.extract(
                            r"(WAIVED TO GJ \d\d/\d\d/\d\d\d\d|GJ|GUILTY PLEA|NOT GUILTY/INSAN E|WAIVED TO GJ|DISMISSED W/CONDITION S|DISMISSED/N OL PROS W/CONDITION S|TRANSFERED ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? ?\(MUST MAKE OCS ENTRY TO EXPLAIN \)|OTHER \(MUST ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? MAKE OCS ENTRY\)|FINAL BOND FORF\.|FORFEITURE SET ASIDE \(\.50 CASE\)|FINAL FORFEITURE \(\.50 CASE\)|TIME LAPSED PRELIM\. FORWARDED TO GJ|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|DISMISSED|FORFEITURE|TRANSFER|REMANDED|ACQUITTED|WITHDRAWN|PETITION DENIED|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|ANCTION|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\)|PROBATION REVOKED \(\.70 CASE\)|PRETRIAL DIVERSION)\s+(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)(.+?)([A-Z0-9]{3}-\s*[A-Z0-9]{3}-\s*[^\s]+)",
                            group_index=3,
                        )
                        .str.strip()
                    )
                ...
                df = df.with_columns(
                    ...
                    .then(
                        pl.col("Row")
                        .str.extract(
                            r"(WAIVED TO GJ \d\d/\d\d/\d\d\d\d|WAIVED TO GJ|GUILTY PLEA|NOT GUILTY/INSAN E|GJ|DISMISSED W/CONDITION S|DISMISSED/N OL PROS W/CONDITION S|TIME LAPSED PRELIM\. FORWARDED TO GJ|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|TRANSFERED ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? ?\(MUST MAKE OCS ENTRY TO EXPLAIN \)|OTHER \(MUST ?\d?\d?/?\d?\d?/?\d?\d?\d?\d? MAKE OCS ENTRY\)|FINAL BOND FORF\.|FORFEITURE SET ASIDE \(\.50 CASE\)|FINAL FORFEITURE \(\.50 CASE\)|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|ANCTION|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\)|PROBATION REVOKED \(\.70 CASE\)|PRETRIAL DIVERSION)\s+(.+?)([A-Z0-9]{3}-\s*[A-Z0-9]{3}-\s*[^\s]+)",
                            group_index=2,
                        )
                        .str.strip()
                    )
                ...
                df = df.with_columns(
                    ...
                    .str.replace(r"\\", "\\\\")
                    .str.replace(
                        r"^(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|TRAFFIC|VIOLATION|\(PUBLIC SERVICE COMMISSION\))",
                        "",
                    )
                    .str.strip()
                )
                ...
                df = df.with_columns(
                    ...
                    .then(
                        pl.col("Row").str.extract(
                            r"(TRAFFIC MISDEMEANOR|BOND|FELONY|MISDEMEANOR|OTHER|VIOLATION|TRAFFIC)"
                        )
                    )
                ...
                df = df.with_columns(
                    pl.col("Row")
                    .str.extract(r"\d\d/\d\d/\d\d\d\d ([ASC]) ")
                    .alias("ID")
                )
            ...
            with console.status("Parsing settings…"):
                ...
                df = df.with_columns(
                    [
                        ...
                        pl.col("Settings")
                        .str.extract(
                            r"\d\d?/\d\d?/\d\d\d\d \d\d\d (\d\d?\:\d\d [A|P]?M?)"
                        )
                        .alias("Time"),
                        pl.col("Settings")
                        .str.extract(
        ...
        if isinstance(self._financial_history, pl.DataFrame):
            ...
            with console.status("Parsing financial history…"):
                ...
                df = df.select(
                    ...
                    .str.replace(r"(Money Type\s*)", "")
                    .str.replace(r"(Reason Disbursement Accoun\s*)", "")
                    .str.replace(r"(Transaction Batch\s*)", "")
                    .str.replace(r"(Operator\s*)", "")
                    .alias("FinancialHistory"),
                )
                df = df.with_columns(
                    pl.col("FinancialHistory").apply(
                        lambda x: re.split(r"(\s\w{3}\s*\n)", x)
                    )
                )
                df = df.select(
                    [
                ...
                df = df.with_columns(
                    [
                        ...
                        pl.col("Row")
                        .str.extract(r"^\d\d/\d\d/\d\d\d\d (.+?)\s*\n?\s*\$")
                        .str.replace("REMITTANC E", "REMITTANCE")
                        .alias("Description"),
                        pl.col("Row")
                        .str.extract(
                            r"\$[0-9,]+\. ?\d\d [A-Z0-9]{4} *[A-Z0-9]+ ?\w *([A-Z0-9]{4})"
                        )
                        .alias("DisbursementAccount"),
                        pl.col("Row")
                        .str.extract(
                            r"\$[0-9,]+\. ?\d\d [A-Z0-9]{4} *[A-Z0-9]+ ?\w *[A-Z0-9]{4} *\d+ (\d+)"
                        )
                        .alias("TransactionBatch"),
                        pl.col("Row")
                        .str.extract(
                            r"\$[0-9,]+\. ?\d\d [A-Z0-9]{4} *[A-Z0-9]+ ?\w *[A-Z0-9]{4} *(\d+)"
                        )
                        .alias("ReceiptNumber"),
                        pl.col("Row")
                        .str.extract(r"\$([0-9,]+\. ?\d\d)")
                        .str.replace(",", "")
                        .cast(pl.Float64, strict=False)
                        .alias("Amount"),
                        pl.col("Row")
                        .str.extract(r"\$[0-9,]+\. ?\d\d ([A-Z0-9]{4})")
                        .alias("FromParty"),
                        pl.col("Row")
                        .str.extract(
                            r"\$[0-9,]+\. ?\d\d [A-Z0-9]{4} *([A-Z0-9]{3,4})? "
                        )
                        .alias("ToParty"),
                        pl.col("Row")
                        .str.extract(r" (\w) [A-Z0-9]{4} *\d+ \d+")
                        .alias("AdminFee"),
                        pl.col("Operator"),
                    ]
                )
                df = df.drop_nulls("Operator")
                df = df.filter(pl.col("Operator") != "Fee")
                if not debug:
                    df = df.select(
                        [
                            "CaseNumber",
...
def getCourtAction(text):
    try:
        return re.sub(
            ...
            "DOCKETED BY MISTAKE",
            re.search(
                r"Court Action: (BOUND|GUILTY PLEA|WAIVED TO GJ|DISMISSED|TIME LAPSED|NOL PROSS|CONVICTED|INDICTED PRIOR TO ADJUDICATIO N|DISMISSED|FORFEITURE|TRANSFER|REMANDED|WAIVED|ACQUITTED|WITHDRAWN|PETITION DENIED|PRETRIAL|COND\. FORF\. SET ASIDE|COND\. FORF\.|PROBATION REVOKED|OTHER|PROBATION NT REVOKE|PROBATION/S|NO PROBABLE CAUSE|PETITION GRANTED|PROBATION TERMINATED|FINAL FORF\. SET ASIDE|DOCKETED|PROBATION NOT REVOKED \(\.70 CASE\))",
                str(text),
            ).group(1),
        )
    except:
