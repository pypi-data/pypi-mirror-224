...
def broadcast_tensors(tensors: Iterable[Tensor]) -> None:
    if torch.distributed.is_initialized():
        handles = [torch.distributed.broadcast(tensor) for tensor in tensors]
...
class ResidualVectorQuantization(nn.Module):
    ...
    Input:
        x: Tensor of shape ``(batch_size, dim, seq_len)``.

    Output:
        quantized: Tensor of shape ``(batch_size, dim, seq_len)``.
