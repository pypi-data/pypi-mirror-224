...
class StorableModel(BaseModel):
    ...
    async def _cache_get(cls: Type[TModel],
        ...
        if await getctx().cache_l1.has(cache_key):
            data = await getctx().cache_l1.get(cache_key)
            td = time() - t1
            getctx().log.debug(
                "%s L1 hit %s %.3f secs", getctx().cache_l1.__class__.__name__, cache_key, td
            )
            return ctor(data)

        if await getctx().cache_l2.has(cache_key):
            data = await getctx().cache_l2.get(cache_key)
            await getctx().cache_l1.set(cache_key, data)
            td = time() - t1
            getctx().log.debug(
        ...
        if obj:
            ...
            await getctx().cache_l2.set(cache_key, data)
            await getctx().cache_l1.set(cache_key, data)
    ...
    async def _invalidate(cache_key: str) -> None:
        if await getctx().cache_l1.delete(cache_key):
            getctx().log.debug("%s delete %s", getctx().cache_l1.NAME, cache_key)
        if await getctx().cache_l2.delete(cache_key):
            getctx().log.debug("%s delete %s", getctx().cache_l2.NAME, cache_key)
