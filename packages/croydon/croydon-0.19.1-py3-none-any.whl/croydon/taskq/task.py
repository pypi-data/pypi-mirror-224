...
class BaseTask(Generic[TTaskData]):
    ...
    async def publish(self) -> bool:
        return await getctx().queue.enqueue(self)

    async def done(self, status_message: str = "ok") -> None:
        return await getctx().queue.set_task_done(self, status_message)

    @classmethod
    def from_message(cls, msg: Message) -> TBaseTask:
        if "data" not in msg:
            getctx().log.error("malformed message, no data field: %s", msg)
        task_id = msg["id"]
        task_type = msg["type"]
        ...
        getctx().log.debug(f"processing queue message with task type {task_type}")
        if task_type in cls.REGISTRY:
            task_class = cls.REGISTRY[task_type]
            getctx().log.debug(f"task type {task_type} is in registry, task class is {task_class}")
        else:
            task_class = cls
            getctx().log.debug(f"task type {task_type} is NOT in registry, task class is {task_class}")
    ...
    @classmethod
    def register(cls):
        getctx().log.info("Registering task type %s", cls.TYPE)
