...
async def auth_token(authorization: Optional[str] = Header(default=None)) -> Optional[Token]:
    ...
    if token:
        if token.is_expired():
            getctx().log.debug(f"token {token.token} is expired")
            return None
...
def authenticated_user(required: bool = True) -> Callable[[Optional[Token]], Awaitable[Optional[User]]]:
    async def inner(token: Optional[Token] = Depends(auth_token),
        ...
        if token is not None:
            ...
            if user:
                getctx().log.debug("authenticated_user found by token")
                return user

        if session is not None:
            user = await session.user()
            if user:
                getctx().log.debug("authenticated_user found by session")
                return user

        if required:
            getctx().log.debug("authenticated_user was not found but auth is required")
            raise AuthenticationError()

        getctx().log.debug("authenticated_user was not found")
        return None
