...
class MongoMockTest(IsolatedAsyncioTestCase):
    ...
    def setUpClass(cls) -> None:
        super().setUpClass()
        if not getctx().initialised:
            from ..config import BaseConfig
            cfg = BaseConfig()
            getctx().from_config(cfg, ".")
        getctx().db.reconfigure(DatabaseConfig(), mock=True)
        ...
        getctx()._cache_l1 = TraceCache()
        getctx()._cache_l2 = RequestLocalCache(custom_cache_ctxvar)
