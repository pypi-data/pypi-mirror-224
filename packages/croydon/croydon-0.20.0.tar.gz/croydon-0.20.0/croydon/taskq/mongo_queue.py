...
class MongoQueue(BaseQueue):
    ...
    async def ensure_indexes(self):
        getctx().log.info("setting up indexes for task queue")
        coll = self.tasks_collection
        await coll.create_index("state")
        await coll.create_index("id")

    async def cleanup(self):
        getctx().log.info("cleaning up task queue")
        ...
        getctx().log.info("cleaning up completed, %d tasks removed", res.deleted_count)

    @property
    def tasks_collection(self) -> AsyncIOMotorCollection:
        return getctx().db.meta.conn[self._cfg.tasks_collection]
    ...
    async def tasks(self, no_block: bool = False) -> AsyncGenerator[TBaseTask, None]:
        ...
        getctx().log.info(f"entering tasks queue loop, registry={BaseTask.REGISTRY}")
        if not self._initialised:
            await self.initialise()
        ...
        while True:
            ...
            if doc:
                ...
                getctx().log.debug(f"yielding task {task}")
                yield task
                continue
