...
class Index(Command):
    ...
    async def run_async(self) -> None:
        from app import app
        app.initialise(config_filename=self.config)

        models_dir = os.path.join(getctx().project_dir, "app/models")
        ...
        for directory, _, files in os.walk(models_dir):
            if "__pycache__" in directory:
                continue
            rel_directory = directory[len(getctx().project_dir) + 1:]
        ...
        for cls_name, cls in classes.items():
            getctx().log.debug(f"Creating index for model {cls_name}")
