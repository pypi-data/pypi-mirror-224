...
class SessionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        session_key = getctx().cfg.session.cookie
        ...
        if request.state.session.is_modified():
            try:
                ...
                getctx().log.error(f"can't save session: {e}")

        if need_cookie:
            response.set_cookie(getctx().cfg.session.cookie, session.key)
...
class TimingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        getctx().log.debug("[TIME %.3fs %s %s] ", t2-t1, request.method, path)
