...
class DataMigrationTask(Task):
    ...
    def __init__(self, name: str, task_type: TaskType, destination: Destination, source: Source,
    ...
    def run(self) -> Task:
        try:
            ...
            self.destination.execute_sql(f"drop table if exists {self.name}")
            file_paths = self.source.unload_data(self)
            for file_path in file_paths:
                self.destination.load_external_data(file_path=file_path, schema_name=self.name.split(".")[0],
                                                    table_name=self.name.split(".")[1])
