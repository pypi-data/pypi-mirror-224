...
def dotDiag(A, diag, minVal=None, symmetric=False, exponent=1.0):
    ...
    result = A * NP.lib.stride_tricks.as_strided(
        diagcorr, (diagcorr.size, A.shape[1]), (diagcorr.itemsize, 0)
    )
    if symmetric:
        result = dotDiag(
            A=A.T,
...
class lmm2k(object):
    ...
    def findGamma1givenGamma0(
        self, gamma0=0.0, nGridGamma1=10, minGamma1=0.0, maxGamma1=10000.0, **kwargs
    ):
        """
