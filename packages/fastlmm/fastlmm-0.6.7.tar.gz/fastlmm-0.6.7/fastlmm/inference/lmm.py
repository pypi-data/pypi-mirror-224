...
class LMM(object):
    ...
    def innerLoopTwoKernel(
        self, a2=0.5, nGridH2=10, minH2=0.0, maxH2=0.99999, **kwargs
    ):
        """
    ...
    def findA2(
        ...
        **kwargs
        ...
        def f(x, resmin=resmin, nGridH2=nGridH2, minH2=minH2, maxH2=maxH2, **kwargs):
            self.numcalls += 1
            t0 = time.time()
            res = self.innerLoopTwoKernel(
                a2=x, nGridH2=nGridH2, minH2=minH2, maxH2=maxH2, **kwargs
            )
            if (resmin[0] is None) or (res["nLL"] < resmin[0]["nLL"]):
                resmin[0] = res
            t1 = time.time()
            logging.info(
                "x={0}. one objective function call took {1} seconds elapsed ".format(
                    x, t1 - t0
                )
            )
        ...
        if dof is None:  # Use the Multivariate Gaussian
            if REML:
                ...
                variance_beta = (
                    h2
                    * sigma2
                    * (UxKx[:, i_pos] / SxKx[i_pos] * UxKx[:, i_pos]).sum(-1)
                )
            result = {
                "nLL": nLL,
                "sigma2": sigma2,
    ...
    def setTestData(self, Xstar, K0star=None, K1star=None, G0star=None, G1star=None):
        ...
        if G1star is None:
            ...
            if self.a2 == 0.0:
                ...
                self.Gstar = np.concatenate(
                    (np.sqrt(1.0 - self.a2) * G0star, np.sqrt(self.a2) * G1star), 1
                )
    ...
    def predict_mean_and_variance(lmm, beta, sigma2, h2, Kstar_star):
        ...
        if lmm.G is not None:
            K = np.dot(
                lmm.G, lmm.G.T
            )  #!!!later this is very inefficient in memory and computation
        else:
            K = np.dot(
                np.dot(lmm.U, np.eye(len(lmm.U)) * lmm.S), lmm.U.T
            )  # Re-compose the Eigen value decomposition #!!!later do this more efficiently
        V = varg * K + vare * np.eye(len(K))
        Vinv = LA.inv(V)

        a = np.dot(varg * lmm.Kstar, Vinv)

        y_star = np.dot(lmm.Xstar, beta) + np.dot(
            a, lmm.y - np.dot(lmm.X, beta)
        )  #!!!later shouldn't the 2nd dot be precomputed?
        y_star = y_star.reshape(-1, 1)  # Make 2-d

        var_star = (
            varg * Kstar_star
            + vare * np.eye(len(Kstar_star))
            - np.dot(a, (varg * lmm.Kstar.T))
        )
    ...
    def predictVariance(
        self, h2=0.0, logdelta=None, delta=None, sigma2=1.0, Kstar_star=None
    ):
        """
    ...
    def nLLeval_test(
        ...
        sigma = self.predictVariance(
            h2=h2, logdelta=logdelta, delta=delta, sigma2=sigma2, Kstar_star=Kstar_star
        )
        ...
        if term2 < -10000:
            ...
            logging.warning(
                ["delta:", delta, "log det", logdetK, "term 2", term2, "term 3:", term3]
            )
            logging.warning(
                [
                    "largest eigv:",
