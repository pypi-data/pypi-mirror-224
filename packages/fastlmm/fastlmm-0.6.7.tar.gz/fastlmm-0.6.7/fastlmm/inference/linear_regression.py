    ...
    def predict(
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            assert K0_whole_test is None or isinstance(
                K0_whole_test, KernelIdentity
            )  # could also accept no snps
            assert K1_whole_test is None or isinstance(
                K1_whole_test, KernelIdentity
            )  # could also accept no snps
            ...
            ret1 = KernelData(
                iid=X.iid, val=np.eye(X.iid_count) * self.ssres / self.iid_count
            )
            return ret0, ret1
...
def f_regression_block(fun, X, y, blocksize=None, **args):
    ...
    while idx_start < X.shape[1]:
        stats[idx_start:idx_stop], pval[idx_start:idx_stop] = fun(
            X[:, idx_start:idx_stop], y, **args
        )
