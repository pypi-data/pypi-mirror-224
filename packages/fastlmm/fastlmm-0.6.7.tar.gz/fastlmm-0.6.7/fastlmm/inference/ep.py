...
class EPGLMM_N1K3(GLMM_N1K3, EPGLMM):
    ...
    def _regular_marginal_loglikelihood(self):
        ...
        r = (
            0.5 * NP.sum(NP.log(1.0 + ttau / tau_))
            - 0.5 * NP.sum(NP.log(A))
            - NP.sum(NP.log(NP.diag(Lk)))
    ...
    def _rmll_gradient(self, optSig02=True, optSig12=True, optSign2=True, optBeta=True):
        ...
        if optSig02:
            r = 0.5 * (
                dot(b, dot(G0, dot(G0.T, b)))
                - trace2(ddot(V, G0, left=True), G0.T)
        ...
        if optSig12:
            r = 0.5 * (
                dot(b, dot(G1, dot(G1.T, b)))
                - trace2(ddot(V, G1, left=True), G1.T)
        ...
        if optSign2:
            r = 0.5 * (dot(b, b) - NP.sum(V) + trace2(H.T, H))
    ...
    def _calculateSig2Mu(self, ttau, tnu, mean):
        ...
        sig2 = (
            self._dKn()
            - sign2**2 * V
            - dotd(G01, dot(dot(G01tV, G01), G01.T))
            - 2.0 * sign2 * dotd(G01, G01tV)
            + dotd(HK.T, HK)
        )
...
class EPGLMM_N3K1(GLMM_N3K1, EPGLMM):
    ...
    def _regular_marginal_loglikelihood(self):
        ...
        r = (
            0.5 * NP.sum(NP.log(1.0 + ttau / tau_))
            - NP.sum(NP.log(NP.diag(Ln)))
            + NP.sum(ST.norm._logcdf((self._y * nu_) / NP.sqrt(tau_**2 + tau_)))
            + 0.5
            * (dot(tnu, dot(K, tnu)) - dot(LnSsqKtnu, LnSsqKtnu) - dot(tnu * ts_1, tnu))
            + dot(m, tnu)
            - dot(LnSsqm.T, LnSsqKtnu)
            - 0.5 * dot(LnSsqm.T, LnSsqm)
            + 0.5 * dot(nu_ * ts_1, (ttau / tau_) * nu_ - 2.0 * tnu)
        )
    ...
    def _predict(self, meanstar, kstar, kstarstar, prob):
        ...
        mustar = (
            meanstar
            + dot(kstar.T, tnu)
            - dot(LnSsqkstar.T, dot(LnSsq, m) + dot(LnSsqK, tnu))
        )
