...
class GLMM(object):
    ...
    def _optimize(
        self, lowerBound, optSig02=True, optSig12=True, optSign2=True, optBeta=True
    ):
        def func(x):
    ...
    def marginal_loglikelihood(self):
        ...
        if self._penalty == "l1":
            ...
            return (
                margll
                - self._lambdaBeta * sum(abs(beta))
                - self._lambdaSign2 * abs(self.sign2)
            )
            ...
            r = (
                margll
                - self._lambdaBeta * NP.dot(beta, beta)
                - self._lambdaSign2 * self.sign2**2
            )
            if self._isK0Set:
    ...
    def _betaNoBias(self):
        ...
        if self._hasBias:
            assert self._hasBias, "You set to penalize bias but there isn't one."
            beta = NP.concatenate(
                (beta[: self._biasIndex], beta[self._biasIndex + 1 :])
            )
        return beta
...
class GLMM_N3K1(GLMM):
    ...
    def setK(self, K0, K1=None):
        ...
        if K1 is not None:
            ...
            assert check_definite_positiveness(
                K1
            ), "K1 must be a definite positive matrix."
            self._K1 = K1
    ...
    def _updateConstants(self):
        ...
        if self._G0 is not None and self._K0 is not None:
            assert abs(self._K0 - self._G0.dot(self._G0.T)).max() < NP.sqrt(
                NP.finfo(NP.float).eps
            ), "You have set both G0 and K0, but K0!=G0*G0^T."

        if self._G1 is not None and self._K1 is not None:
            assert abs(self._K1 - self._G1.dot(self._G1.T)).max() < NP.sqrt(
                NP.finfo(NP.float).eps
            ), "You have set both G1 and K1, but K1!=G1*G1^T."
        ...
        assert (
            self._K0.shape[0] == self._X.shape[0]
        ), "K0 (or G0) and X must have the same number of rows."
        assert (
            self._K1.shape[0] == self._X.shape[0]
        ), "K1 (or G1) and X must have the same number of rows."
...
class GLMM_N1K3(GLMM):
    ...
    def _updateConstants(self):
        ...
        assert (
            self._G0.shape[0] == self._X.shape[0]
        ), "G0 and X must have the same number of rows."
        assert (
            self._G1.shape[0] == self._X.shape[0]
        ), "G1 and X must have the same number of rows."
    ...
    def _calculateG01(self):
        G01 = NP.concatenate(
            (NP.sqrt(self._sig02) * self._G0, NP.sqrt(self._sig12) * self._G1), axis=1
        )
        return G01
