...
def minimize1D(
    ...
    if verbose:
        print("evaluating target function on a grid")
    ...
    if resultgrid is None:
        [evalgrid, resultgrid] = evalgrid1D(
            f, evalgrid=evalgrid, nGrid=nGrid, minval=minval, maxval=maxval
        )
    ...
    if brent:  # do Brent search in addition to rest?
        ...
        for i in range(
            ...
            if (resultgrid[i + 1] < resultgrid[i + 2]) and (
                ...
                minlocal = opt.brent(
                    f,
                    brack=(evalgrid[i], evalgrid[i + 1], evalgrid[i + 2]),
                    full_output=True,
                )
                if minlocal[1] < minglobal[1]:
                    minglobal = minlocal[0:2]
                    if verbose:
                        print("found new minimum from brent search")
    if return_grid:
