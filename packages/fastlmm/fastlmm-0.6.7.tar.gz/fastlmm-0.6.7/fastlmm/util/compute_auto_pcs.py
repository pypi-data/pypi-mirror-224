...
def compute_auto_pcs(
    ...
    with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
        ...
        all_std_snpdata = (
            snpreader.read().standardize()
        )  #!!could doing C or F be better?
        ...
        remove_set = set(
            vc.VertexCut().work(rrm, cutoff)
        )  # These are the indexes of the IIDs to remove
        logging.info(
            "removing {0} of {1} iids".format(len(remove_set), snpreader.iid_count)
        )
        ...
        if (
            max(k_values) >= nofam_snpreader.iid_count
            or max(k_values) >= nofam_snpreader.sid_count
        ):
            raise Exception(
                "The number of PCs search should be less than the # of rows and also the # of cols in the matrix after near relatives are removed"
            )
        randomstate = 1
        ...
        n_folds = 10
        folds = KFold(n_splits=n_folds, shuffle=True, random_state=randomstate).split(
            list(range(nofam_snpreader.sid_count))
        )
        ...
        for i_fold, [train_idx, test_idx] in enumerate(folds):
            ...
            t0 = time.time()
            X_train = (
                nofam_snpreader[:, train_idx].read(order="F").val.T
            )  #!!would order='C' be faster?
        ...
        pca._fit(
            nofam_snpdata.val, Utr, Str, Vtr, mean
        )  # This will zero-center nofam_snpdata.val, so don't use after this point.
        t1 = time.time()
