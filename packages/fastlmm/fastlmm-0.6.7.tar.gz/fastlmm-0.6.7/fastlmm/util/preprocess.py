...
def mean_impute(X, imissX=None, maxval=2.0):
    ...
    one_over_sqrt_pi = 1.0 / np.sqrt(one_over_sqrt_pi * (1.0 - one_over_sqrt_pi))
...
def standardize(
    snps, blocksize=None, standardizer=stdizer.Unit(), force_python_only=False
):
    ...
    if (
        blocksize is not None and blocksize >= snps.shape[1]
    ):  # If blocksize is larger than the # of snps, set it to None
        blocksize = None
