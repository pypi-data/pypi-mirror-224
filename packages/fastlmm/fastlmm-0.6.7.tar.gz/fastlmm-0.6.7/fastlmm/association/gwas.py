...
class GWAS(object):
    def __init__(
        ...
        if self.covariates is not None:
            self.lmm = lmm_cov.LMM(
                X=self.covariates.values, Y=None, G=G, K=self.K, inplace=True
            )
        else:
    ...
    def compute_association(self, blocksize=10000, temp_dir=None):
        ...
        self._generate_intervals(
            blocksize=blocksize, total_snps=self.snps_test.sid.shape[0]
        )
        for stop in range(1, len(self.intervals)):
            res = self.snps_test_block(
                block_start=self.intervals[stop - 1], block_end=self.intervals[stop]
            )
            if temp_dir is not None:
                self.save_results_block(
                    dir_name=temp_dir,
                    res=res,
    ...
    def snps_test_block(self, block_start, block_end):
        snps = (
            self.snps_test[:, block_start:block_end]
            .read()
        ...
        for i, p in enumerate(self.phenotype.columns):
            ...
            res = self.lmm.nLLeval(
                h2=h2, dof=None, scale=1.0, penalty=0.0, snps=snps.val
            )
    ...
    def _format_gwas_results(self, res, snps, h2):
        ...
        p_values = st.f.sf(
            chi2stats, 1, self.lmm.U.shape[0] - (self.covariates.shape[0] + 1)
        )[
            :, 0
    ...
    def check_pheno_format(phenotype):
        if type(phenotype) == np.ndarray:
            ...
            pheno_new = pd.DataFrame(
                data=phenotype["vals"],
                columns=phenotype["header"],
                index=phenotype["iid"],
            )
            return pheno_new
            ...
            raise NotImplementedError(
                "phenotype is assumed to be a numpy.ndarray or pandas.DataFrame, found %s instead"
                % (str(type(phenotype)))
            )
    ...
    def _pheno_fixup(pheno_input, iid_source_if_none=None):
        if isinstance(pheno_input, str):
            return pysnptools.util.pheno.loadPhen(
                pheno_input
            )  #!!what about missing=-9?
...
if __name__ == "__main__":
    ...
    t00 = time.time()
    snp_intersect, pheno_intersect = pysnptools.util.intersect_apply(
        [snp_reader, pheno], sort_by_dataset=True
    )
    t1 = time.time()
    ...
    if 0:
        ...
        t1 = time.time()
        print(
            "done computing eigenvalue decomposition of kernel after %.4fs" % (t1 - t0)
        )
    ...
    if 1:
        ...
        if 1:
            result = mygwas.compute_association(
                blocksize=blocksize, temp_dir=None
            )  #'./temp_dir_testdata/')
        else:
