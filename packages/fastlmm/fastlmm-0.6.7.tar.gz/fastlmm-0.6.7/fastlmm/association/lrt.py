...
class lrt(association.varcomp_test):
    ...
    def __init__(
        ...
        if nullModel["effect"] == "fixed":
            ...
            assert (
                "approx" not in nullModel or nullModel["approx"] is None
            ), "Cannot use approx with fixed effect"

        elif nullModel["effect"] == "mixed":
            if nullModel["link"] == "linear":
                self._nullModelMixedEffectLinear(G0=G0, K0=K0)
            else:
                self._nullModelMixedEffectNonLinear(
                    G0, nullModel["approx"], nullModel["link"], nullModel["penalty"]
                )
        else:
    ...
    def testGupdate(self, y, X, type=None):
        ...
        assert (
            self._testGcalled
        ), "must have called testG before updateTestG which assumes only a change in y"
        origX = self.X
        origY = self.Y
        self._updateYX(y, X)
    ...
    def testG(self, G1, type=None, i_exclude=None, G_exclude=None):
        ...
        if self.altModel["effect"] == "fixed":
            ...
            assert (
                "approx" not in altModel or altModel["approx"] is None
            ), "Cannot use approx with fixed effect"
        elif self.altModel["effect"] == "mixed":
            if self.altModel["link"] == "linear":
                (lik1, stat, alteqnull) = self._altModelMixedEffectLinear(G1)
            else:
                (lik1, stat, alteqnull) = self._altModelMixedEffectNonLinear(
                    G1,
                    self.altModel["approx"],
                    self.altModel["link"],
    ...
    def _altModelMixedEffectLinearUpdate(self, lmm1, tol=0.0):
        ...
        if self.G0 is not None:
            ...
            lik1 = (
                lmm1.findH2()
            )  # The alternative model has one kernel and needs to find only h2
    ...
    def _altModelMixedEffectNonLinear(self, G1, approx, link, penalty):
        if self.G0 is None:
            (lik1, stat) = self._altModelMixedEffectNonLinear1Kernel(
                G1, approx, link, penalty
            )
        else:
            (lik1, stat) = self._altModelMixedEffectNonLinear2Kernel(
                G1, approx, link, penalty
            )
    ...
    def _altModelMixedEffectNonLinear2Kernel(self, G1, approx, link, penalty):
        if self.forcefullrank:
            ...
            glmm1 = inference.getGLMM(
                approx, link, self.Y, self.G0, G1, penalty=penalty
            )
