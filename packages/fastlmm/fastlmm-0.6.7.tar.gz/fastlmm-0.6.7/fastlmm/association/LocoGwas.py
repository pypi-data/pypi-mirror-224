...
class LocoGwas(object):  # implements IDistributable
    ...
    def work_sequence(self):
        ...
        for i, (train_snp_idx, test_snp_idx) in enumerate(loco):
            if i == 0:
                ...
                result = None
            yield lambda i=i, train_snp_idx=train_snp_idx, test_snp_idx=test_snp_idx, result=result, G=G, y=y: self.dowork(
                i, train_snp_idx, test_snp_idx, result, G, y
    ...
    def __str__(self):
        return "{0}({1},'{2}','{3},...')".format(
            self.__class__.__name__, self.chrom_count, self.snp_reader, self.pheno_fn
        )
    ...
    def dowork(self, i, train_snp_idx, test_snp_idx, result, G, y):
        ...
        if not self.selected_snps is None:
            ...
            int_snp_idx = argintersect_left(
                self.snp_reader.rs[train_snp_idx], self.selected_snps
            )
...
class FastGwas(object):
    ...
    def __init__(
        ...
        if delta is None:
            ...
            self.delta = (
                delta * train_snps.shape[1]
            )  #!!should also add in the number of train_pcs
    ...
    def run_gwas(self):
        ...
        with patch.dict("os.environ", {"ARRAY_MODULE": "numpy"}) as _:
            ...
            if self.train_pcs is None and self.train_snps is not None:
                ...
                G = np.concatenate(
                    (
                        ...
                        np.sqrt(self.mixing) * self.train_pcs,
                    ),
                    1,
                )
            ...
            self.p_values_F = stats.f.sf(chi2stats, 1, G.shape[0] - (lmm.linreg.D + 1))[
                :, 0
