...
class testCV(association.varcomp_test):
    ...
    def __init__(
        ...
        nullModel=None,
        altModel=None,
    ):  # Note that this code will break as we don't know what it does
        ...
        if data_permutation is None:
            data_permutation = util.generate_permutation(
                self.Y.shape[0], 93828231
            )  # permute with an arbitrary seed
        ...
        self.scores_null, self.params_null = self.score_nestedCV(
            None, model, param_grid, self.nullModel["effect"], nested
        )
    ...
    def _getModel(self, modelDesc, param_grid):
        if modelDesc["effect"] == "fixed":
            return self._getFixedEffectModel(
                modelDesc["penalty"], modelDesc["link"], param_grid
            )
        elif modelDesc["effect"] == "mixed":
            return self._getMixedEffectModel(
                modelDesc["link"], modelDesc["approx"], param_grid
            )
        else:
            assert False
        ...
        if link == "linear":
            if penalty == "l2":
                model = linear_model.Ridge(
                    alpha=param_grid["alpha"][0], fit_intercept=True, solver="auto"
                )
            elif penalty == "l1":
                model = linear_model.LassoCV(
                    alphas=param_grid["alpha"],
                    cv=self.n_folds_params,
                    precompute="auto",
                    fit_intercept=True,
                )
    ...
    def _isNested(self, modelDesc):
        if (
            modelDesc["effect"] == "fixed"
            and modelDesc["link"] == "linear"
    ...
    def testG(self, G1, type="", i_exclude=None):
        ...
        scores, params = self.score_nestedCV(
            G1, model, param_grid, self.altModel["effect"], nested
        )
    ...
    def score_nestedCV(self, G1, model, param_grid, effect, nested):
        ...
        for train, test in k_fold:
            ...
            if nested:
                ...
                scores[i_fold] = clf.score(
                    testData, testY.flatten(), method_scorer=False
                )
            else:
