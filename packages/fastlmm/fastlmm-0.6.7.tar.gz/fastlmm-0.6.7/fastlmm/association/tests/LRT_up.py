...
class lrt(association.varcomp_test):
    ...
    def testG(self, G1, type=None, i_exclude=None, G_exclude=None):
        ...
        (lik1, stat, alteqnull) = self._altModelMixedEffectLinear(
            G1, i_exclude=i_exclude, G_exclude=G_exclude
        )
    ...
    def _altModelMixedEffectLinear(self, G1, tol=0.0, i_exclude=None, G_exclude=None):
        ...
        if n_exclude:
            if UUGup is not None:
                ...
                UUGup_ = None
            lik0 = self.lmm.findH2_2K(
                nGridH2=100,
                minH2=0.0,
                maxH2=0.99999,
                i_up=i_up[0:n_exclude],
                i_G1=i_G1[0:n_exclude],
                UW=UGup[:, 0:n_exclude],
                UUW=UUGup_,
...
class LRT_up(object):
    ...
    def pv_adj_and_ind(
        ...
        return Cv.pv_adj_and_ind(
            ...
            nullfitfile,
            nlocalperm,
        )  # call the shared version of this method
    ...
    def write(self, fp, ind, result_dict, pv_adj, detailed_table, signal_ratio=True):
        ...
        if detailed_table:
            ...
            nullIndices = sorted(
                list(range(len(nullNames))), key=lambda k: nullNames[k]
            )
            nullNames.sort()
        ...
        for i in range(len(ind)):
            ...
            if "pv-local-aUD" in result_dict[0].test:
                rowvals = [
                    result.setname,
                    str(-result.test["lik1"]["nLL"][0]),
                    ll0,
                    str(pv_adj[ii]),
                    str(result.test["pv-local"]),
                    str(result.pv),
                    str(result.setsize),
                    str(result.nexclude),
                    result.ichrm,
