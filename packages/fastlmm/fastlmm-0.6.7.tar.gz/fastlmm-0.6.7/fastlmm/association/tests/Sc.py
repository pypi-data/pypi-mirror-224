...
class Sc(object):
    ...
    def construct(
        ...
        """
        assert (
            nullModel["effect"] == "mixed" and altModel["effect"] == "mixed"
        ...
        if self.score == "mom":
            assert nullModel["link"] == "linear" and altModel["link"] == "linear", (
                "You are allowed to use " "only the linear link for sc mom test."
            )
        G0, K0 = tu.set_snps0(
            SNPs0=SNPs0,
    ...
    def pv_davies_eig(squaredform, eigvals):
        ...
        result = qf.qf(
            squaredform, eigvals, acc=1e-07
        )  # decided on 1e-7 after experimentation between -4 and -12. Thresh on exp in QFC.C seems to have no effect
        return result[0]
