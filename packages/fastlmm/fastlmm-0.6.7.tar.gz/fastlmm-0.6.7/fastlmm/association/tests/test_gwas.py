...
class TestGwas(unittest.TestCase):
    ...
    def test_results_identical_with_fastlmmc(self):
        ...
        bed_fn = os.path.join(
            currentFolder, "../../feature_selection/examples/toydata.5chrom"
        )
        pheno_fn = os.path.join(
            currentFolder, "../../feature_selection/examples/toydata.phe"
        )
        ...
        np.testing.assert_allclose(
            gwas_c_reml.p_values, p_vals_by_genomic_pos, rtol=0.1
        )
        np.testing.assert_allclose(gwas_c_reml.p_values, gwas_f.p_values_F, rtol=0.1)
        np.testing.assert_allclose(
        ...
        _, pvalue_list_search = (
            frame_search["SNP"].values,
            frame_search["PValue"].values,
        )

        p_vals_by_genomic_pos = frame_search.sort_values(["Chr", "ChrPos"])[
            "PValue"
        ].tolist()
        np.testing.assert_allclose(
            gwas_c_reml_search.p_values, p_vals_by_genomic_pos, rtol=0.001
        )
        np.testing.assert_allclose(
            gwas_c_reml_search.sorted_p_values, pvalue_list_search, rtol=0.001
        )
...
class GwasTest(object):
    ...
    def run_gwas(self):
        ...
        run_fastlmmc.run(
            bfile=self.bedFile,
            pheno=self.phenoFile,
            bfileSim=self.bedFileSim,
            optLogdelta=self.optLogdelta,
            extractSim=self.extractSim,
            fastlmm_path=self.fastlmm_path,
            out=self.out_file,
            extract=self.extract,
            REML=self.REML,
            excludeByPosition=self.excludeByPosition,
        )
