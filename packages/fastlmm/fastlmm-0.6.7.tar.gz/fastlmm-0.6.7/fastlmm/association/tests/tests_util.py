...
def set_snps0(SNPs0, sample_size, i_exclude=None, forcefullrank=False, blocksize=10000):
    ...
    if "K" in SNPs0:
        ...
        if len(SNPs0["snp_set"]) > sample_size or forcefullrank:  # N = Y.shape[0]
            SNPs0["K"] = psd.build_kernel_blocked(
                snpreader=SNPs0["reader"],
                snp_idx=SNPs0["snp_set"].to_index,
                blocksize=blocksize,
                allowlowrank=forcefullrank,
            )
            K0 = SNPs0["K"]
            G0 = None
        else:
    ...
    if i_exclude is not None:
        if K0 is not None:
            ...
            K_up = psd.build_kernel_blocked(
                snpreader=SNPs0["reader"],
                snp_idx=np.array(SNPs0["snp_set"].to_index)[i_exclude],
                blocksize=blocksize,
                allowlowrank=forcefullrank,
            )
            K0 = K0 - K_up
    ...
    if K0 is not None:
        ...
        G0 = G0 / np.sqrt(
            num_snps
        )  # (G0*G0).mean() ) # computes the sqrt of the mean of the diagonal of K=GG^T; *  means pointwise multiplication
    return G0, K0
