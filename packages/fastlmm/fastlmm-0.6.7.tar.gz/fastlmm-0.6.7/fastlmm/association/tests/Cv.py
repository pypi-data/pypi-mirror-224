...
class Cv(object):
    ...
    def pv_adj_and_ind(
        ...
        if nullfit == "ml":
            ...
            logging.info(
                "using qq fit for parameterized null distrib, without using any permutations"
            )
            pv_adj, mixture, scale, dof = Cv.lrtpvals_qqfit(
                nperm,
                lrt,
    ...
    def lrtpvals_qqfit(
        ...
        if nperm > 0:
            ...
            dat = ut.extractcols(
                nullfitfile,
                colnameset=colnames,
                dtypeset={"2*(LL(alt)-LL(null))": np.float64},
            )
            ...
            logging.info(
                "estimating mixture parameters for non-permuted data\nusing quantile regression of log-pvalues with qmax="
                + str(qmax)
                + "..."
            )
            mix = c2.chi2mixture(
    ...
    def lrtpvals_mlfit(nperm, a2, a2perm, lrt, lrtperm):
        ...
        if nperm > 0:
            logging.info(
                "estimating mixture parameters for permuted data using maximum likelihood..."
            )
            # estimating mixture parameters for permuted data, using
            [pvperm, mixture, scale, dof, i0] = c2.computePVmixtureChi2_old(
                lrtperm, a2perm, tol=0.0, mixture=None, scale=None, dof=None
            )
            logging.info("        Done")
            logging.info("adjusting the observed p-values ...")
            # adjust the observed p-values with estimated null mixture, scale, dof parameters
            [pv_adj, mixture, scale, dof, i0] = c2.computePVmixtureChi2_old(
                lrt, a2, tol=0.0, mixture=mixture, scale=scale, dof=dof
            )
            logging.info("        Done")
        else:
            logging.info(
                "nperm = " + str(nperm) + " : No permutations were performed..."
            )
            logging.info("adjusting the observed p-values ...")
            # adjust the observed p-values with estimated null mixture, scale, dof parameters
            [pv_adj, mixture, scale, dof, i0] = c2.computePVmixtureChi2_old(
                lrt, a2, tol=0.0, mixture=None, scale=None, dof=None
            )
            logging.info("        Done")
