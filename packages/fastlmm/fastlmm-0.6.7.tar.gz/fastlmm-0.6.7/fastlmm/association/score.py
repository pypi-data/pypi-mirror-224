...
class scoretest(association.varcomp_test):
    ...
    def _score(self, G1):
        ...
        (
            self.squaredform,
            self.expectationsqform,
            self.varsqform,
            self.GPG,
            self.GPY,
        ) = scoreNoK(
            Y=self.Y, X=self.X, Xdagger=None, G=G1, sigma2=None, Bartletcorrection=True
        )
    ...
    def testG(self, G1, type, altModel=None, i_exclude=None, G_exclude=None):
        ...
        test = {"pv": pv, "stat": self.squaredform}
        return test
...
class scoretest2K(scoretest):
    ...
    def __init__(
        self, Y, X=None, K=None, G0=None, appendbias=False, forcefullrank=False
    ):
        scoretest.__init__(self, Y=Y, X=X, appendbias=appendbias)
        self.Xdagger = None
        self.G0 = G0
        ...
        if self.lowrank:
            self.expectedinfo[0, 0] += ((self.Neff - self.S.shape[0])) / (
                (1.0 - self.optparams["h2"]) * (1.0 - self.optparams["h2"])
            )
        # tr(PKPI)
        Sd *= self.S
        self.expectedinfo[1, 0] = (
            Sd
        ).sum()  # /(self.optparams['sigma2']*self.optparams['sigma2'])
        self.expectedinfo[0, 1] = self.expectedinfo[1, 0]
        # tr(PKPK)
        Sd *= self.S
        self.expectedinfo[1, 1] = (
            Sd
        ).sum()  # /(self.optparams['sigma2']*self.optparams['sigma2'])
        self.expectedinfo *= (
            0.5 * P / (self.optparams["sigma2"] * self.optparams["sigma2"])
        )
    ...
    def _score(self, G1):
        ...
        varsqform = 1.0 / (
            trPGGPGG
            - expectedInfoCross.dot(LA.inv(self.expectedinfo).dot(expectedInfoCross))
        )
        self.squaredform = squaredform
...
def scoreNoK(Y, X=None, Xdagger=None, G=None, sigma2=None, Bartletcorrection=True):
    ...
    if X is None:
        ...
        D = X.shape[1]
    RxY, Xdagger = linreg(
        Y=Y, X=X, Xdagger=Xdagger
    )  # residual of y regressed on X, which here, is equivalent to sigma2*Py (P is the projection matrix, which is idempotent)

    if (
        sigma2 is None
    ):  #     note: Xdagger is pseudo inverse of X, or (X^T*X)^1*X^T such that Xdagger*y=beta
    ...
    RxG, Xdagger = linreg(
        Y=G, X=X, Xdagger=Xdagger
    )  # residual of G regressed on X, which here, is equivalent to PG (P is the projection matrix, and in this one kernel case, is idempotent)
...
if __name__ == "__main__":
    if 1:  # example p-value computation for sample data
        ...
        mindist = (
            10  # minimum distance to alternative SNPs to be included in null model
        )
