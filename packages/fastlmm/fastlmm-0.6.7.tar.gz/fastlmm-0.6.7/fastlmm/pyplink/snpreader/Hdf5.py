...
class Hdf5(object):
    ...
    def run_once(self):
        ...
        if "SNP-major" not in self.snpsInFile.attrs:
            raise Exception(
                "In Hdf5 the 'snps' matrix must have a Boolean 'SNP-major' attribute"
            )
        ...
        if self.is_snp_major:
            if not self.snpsInFile.shape == (S_original, N_original):
                raise Exception(
                    "In Hdf5, snps matrix dimensions don't match those of 'rs' and 'iid'"
                )
        else:
            if not self.snpsInFile.shape == (N_original, S_original):
                raise Exception(
                    "In Hdf5, snps matrix dimensions don't match those of 'rs' and 'iid'"
                )
    ...
    def read_with_specification(
        ...
        is_simple = (
            ...
            and snps_are_sorted
            and matches_order
        )  # If 'is_simple' may be able to use a faster reader
        ...
        ret = {"rs": rs, "pos": pos, "snps": SNPs, "iid": iids}
        return ret
    ...
    def write(snpMatrix, hdf5file, dtype="f8", snp_major=True, compression=None):
        if not isinstance(dtype, str) or len(dtype) != 2 or dtype[0] != "f":
            raise Exception(
                "Expect dtype to start with 'f', e.g. 'f4' for single, 'f8' for double"
            )
        data = (snpMatrix["snps"].T) if snp_major else snpMatrix["snps"]
        with h5py.File(hdf5file, "w") as h5:
            h5.create_dataset(
                "snps", data=data, dtype=dtype, compression=compression, shuffle=True
            )
