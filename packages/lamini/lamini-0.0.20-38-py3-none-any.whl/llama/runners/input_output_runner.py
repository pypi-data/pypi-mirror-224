...
class InputOutputRunner:
    ...
    def load_data(self, data: list, verbose: bool = False):
        if not (isinstance(data, list) and isinstance(data[0], list) and isinstance(data[0][0], self.input_type) and isinstance(data[0][1], self.output_type)):
            raise ValueError(f"Data must be [[input_object, output_object], [input_object, output_object], ...], that is a list of lists (with inner input and output objects), where inner objects have keys matching input keys: {self.input_type_keys}; and, output keys: {self.output_type_keys} respectively.")
    ...
    def load_data_from_paired_dicts(self, data, verbose: bool = False):
        ...
        if not (isinstance(data, list) and isinstance(data[0], dict) and "input" in data[0] and "output" in data[0] and isinstance(data[0]["input"], dict) and isinstance(data[0]["output"], dict)):
            raise ValueError(f"Data must be [{'input': input_dict, 'output': output_dict}, {'input': input_dict, 'output': output_dict}, ...], that is a list of dicts (with inner input and output dicts), where inner dicts have keys matching input keys: {self.input_type_keys}; and, output keys: {self.output_type_keys} respectively.")
        ...
        if input_keys != self.input_type_keys:
            raise ValueError(f"Keys have to match {self.input_type}. But got {input_keys} instead.")

        output_datum = data[0]["output"]
        output_keys = output_datum.keys()
        if output_keys != self.output_type_keys:
            raise ValueError(f"Keys have to match {self.output_type}. But got {output_keys} instead.")
        ...
        for datum in data:
            try:
                ...
                print(f"Error converting input-output pair to Input and Output objects; skipping this pair {datum}")
                continue
    ...
    def load_data_from_paired_lists(self, data, verbose: bool = False):
        ...
        if not (isinstance(data, list) and isinstance(data[0], list) and isinstance(data[0][0], dict) and isinstance(data[0][1], dict)):
            raise ValueError(f"Data must be [[input_dict, output_dict], [input_dict, output_dict], ...], that is a list of lists (inner list is an input-output pair), where input and output are dicts with keys matching input keys: {self.input_type_keys}; and, output keys: {self.output_type_keys} respectively.")
        ...
        for pair in data:
            try:
                ...
                print(f"Error converting input-output pair to Input and Output objects; skipping this pair {pair}")
                continue
            input_output_objects.append([input_object, output_object])
    ...
    def load_data_from_paired_jsonlines(self, input_file_path: str, output_file_path: str, verbose: bool = False):
        """
        ...
        if len(input_data) != len(output_data):
            print(f"Input and output data are not the same length, so going with the smaller length: {min_length}")

        data = [[input_data[i], output_data[i]] for i in range(min_length)]
        self.load_data_from_paired_lists(data, verbose=verbose)


    def load_data_from_dataframe(self, df: pd.DataFrame, verbose: bool = False, input_prefix: str = "input-", output_prefix: str = "output-"):
        """
        ...
        if not len(input_keys) > 0:
            raise ValueError(f"Input keys must start with {input_prefix}. But got {input_keys} instead.")
        if not len(output_keys) > 0:
            raise ValueError(f"Output keys must start with {output_prefix}. But got {output_keys} instead.")
        ...
        if set(input_df.columns) != set(self.input_type_keys):
            raise ValueError(f"Keys have to match " + str(self.input_type) + ". But got " + str(input_df.columns) + " instead.")
        ...
        if set(output_df.columns) != set(self.output_type_keys):
            raise ValueError(f"Keys have to match " + str(self.output_type) + ". But got " + str(output_df.columns) + " instead.")
        ...
        for i in range(len(input_df)):
            try:
                ...
                print(f"Error converting input-output pair to Input and Output objects; skipping this pair {input_df.iloc[i], output_df.iloc[i]}")
                continue
            input_output_objects.append([input_object, output_object])
    ...
    def load_data_from_paired_dataframes(self, input_df: pd.DataFrame, output_df: pd.DataFrame, verbose: bool = False):
        """
    ...
    def load_data_from_paired_csvs(self, input_file_path: str, output_file_path: str, verbose: bool = False):
        """
    ...
    def train(
        ...
        final_status = self.llm.train(
            verbose=verbose, finetune_args=finetune_args, is_public=is_public
        )
        try:
